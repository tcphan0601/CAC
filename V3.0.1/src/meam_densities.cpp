...
void MEAM::meam_dens_setup(int atom_nmax, int elem_nmax, int atom_nall, int elem_nall, int n_neigh)
{
  int i, j, k, l;
    ...
    int numneigh_full, int *firstneigh_full, int *firstneighindex_full)
{
  //if (iindex < 0)
  //if (atom->tag[i] == TEST)
  //printf("Start compute sij for iID = %d fnoffset = %d\n",atom->tag[i],fnoffset);
    ...
    int numneigh_full, int *firstneigh_full, int *firstneighindex_full)
{
  ...
  for (jn = 0; jn < numneigh; jn++) {
    ...
    //if (atom->tag[i] == TEST  && atom->tag[j] == TEST2)
    //  printf("Start compute sij for iID = %d jID = %d rij2 = %g cutforcesq = %g\n",
    //      atom->tag[i],atom->tag[j],rij2,this->cutforcesq);
    ...
    //if (iindex < 0 && jindex < 0)
    //  if (atom->tag[i] == TEST  && atom->tag[j] == TEST2)
    //    printf("   rbound = %g rnorm = %g sij = %g numneigh_full = %d xi = %g %g %g xj = %g %g %g\n",
    //        rbound,rnorm,sij,numneigh_full,xitmp,yitmp,zitmp,xjtmp,yjtmp,zjtmp);
    ...
    for (kn = 0; kn < numneigh_full; kn++) {
      ...
      // skip if j and k are the same atom

      if (j == k && jindex == kindex) continue;
      ...
      //if (iindex < 0 && jindex < 0)
      //  if (atom->tag[i] == TEST  && atom->tag[j] == TEST2)
      //    printf("     kn = %d kID = %d sij = %g xk = %g %g %g\n",kn,atom->tag[k],sij,xktmp,yktmp,zktmp);
      ...
      //if (iindex < 0 && jindex < 0 && kindex < 0)
      //  if (atom->tag[i] == TEST  && atom->tag[j] == TEST2)
      //    printf("              kID = %d rik = %g rjk = %g a = %g cikj = %g Cmax = %g Cmin = %g elti j k = %d %d %d\n"
      //        ,atom->tag[k],sqrt(rik2),sqrt(rjk2),a,cikj,Cmax,Cmin,elti,eltj,eltk);
      ...
      //if (iindex < 0 && jindex < 0 && kindex < 0)
      //  if (atom->tag[i] == TEST  && atom->tag[j] == TEST2)
      //    printf("              kID = %d sij = %g fcut(cikj) = %g rik = %g rjk = %g cikj = %g Cmax = %g Cmin = %g delc = %g\n"
      //        ,atom->tag[k],sij,fcut(cikj),sqrt(rik2),sqrt(rjk2),cikj,Cmax,Cmin,delc);
    ...
    //if (iindex < 0 && jindex < 0 && kindex < 0)
    //  if (atom->tag[i] == TEST  && atom->tag[j] == TEST2)
    //    printf("Finish compute sij for iID = %d jID = %d sij = %g scrfcn[%d] = %g\n"
    //        ,atom->tag[i],atom->tag[j],sij,jn,scrfcn[jn]);
