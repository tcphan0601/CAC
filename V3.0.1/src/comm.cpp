...
void Comm::init()
{
  ...
  // set per-atom/element sizes for forward/reverse/border comm
  // augment by velocity and fix quantities if needed

  atom_size_forward = atom->avec->size_forward;
  atom_size_reverse = atom->avec->size_reverse;
  atom_size_border = atom->avec->size_border;

  elem_size_forward = element->evec->size_forward;
  elem_size_reverse = element->evec->size_reverse;
  elem_size_border = element->evec->size_border;  

  if (ghost_velocity) atom_size_forward += atom->avec->size_velocity;
  if (ghost_velocity) atom_size_border += atom->avec->size_velocity;

  if (atom->ngrains)
    atom_size_border++;

  for (int i = 0; i < modify->nfix; i++) {
    atom_size_border += modify->fix[i]->comm_atom_border; 
    elem_size_border += modify->fix[i]->comm_elem_border; 
  }

  // per-atom/element limits for communication
  ...
  atom_maxforward = MAX(atom_size_forward, atom_size_border);
  atom_maxreverse = atom_size_reverse;
  elem_maxforward = MAX(elem_size_forward, elem_size_border);
  elem_maxreverse = elem_size_reverse;

  if (force->pair) {
    atom_maxforward = MAX(atom_maxforward, force->pair->comm_atom_forward);
    atom_maxreverse = MAX(atom_maxreverse, force->pair->comm_atom_reverse);
    elem_maxforward = MAX(elem_maxforward, force->pair->comm_elem_forward);
    elem_maxreverse = MAX(elem_maxreverse, force->pair->comm_elem_reverse);
  }

  for (int i = 0; i < modify->nfix; i++) {
    atom_maxforward = MAX(atom_maxforward, modify->fix[i]->comm_atom_forward);
    atom_maxreverse = MAX(atom_maxreverse, modify->fix[i]->comm_atom_reverse);
    elem_maxforward = MAX(elem_maxforward, modify->fix[i]->comm_elem_forward);
    elem_maxreverse = MAX(elem_maxreverse, modify->fix[i]->comm_elem_reverse);
  }

  for (int i = 0; i < modify->ncompute; i++) {
    atom_maxforward = MAX(atom_maxforward, modify->compute[i]->comm_atom_forward);
    atom_maxreverse = MAX(atom_maxreverse, modify->compute[i]->comm_atom_reverse);
    elem_maxforward = MAX(elem_maxforward, modify->compute[i]->comm_elem_forward);
    elem_maxreverse = MAX(elem_maxreverse, modify->compute[i]->comm_elem_reverse);
  }

  for (int i = 0; i < output->ndump; i++) {
    atom_maxforward = MAX(atom_maxforward, output->dump[i]->comm_atom_forward);
    atom_maxreverse = MAX(atom_maxreverse, output->dump[i]->comm_atom_reverse);
    elem_maxforward = MAX(elem_maxforward, output->dump[i]->comm_elem_forward);
    elem_maxreverse = MAX(elem_maxreverse, output->dump[i]->comm_elem_reverse);
  }
  ...
  if (force->pair) {
    atom_maxreverse = MAX(atom_maxreverse, force->pair->comm_atom_reverse_off);
    elem_maxreverse = MAX(elem_maxreverse, force->pair->comm_elem_reverse_off);
  }
}
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "mode") == 0) {
      ...
      if (strcmp(arg[iarg+1], "single") == 0) {
        ...
        if (mode == MULTI) cutghostuser[0] = cutghostuser[1] = cutghostuser[2] = 0.0;
        memory->destroy(cutusermulti);
        cutusermulti = NULL;
        mode = SINGLE;
      } else if (strcmp(arg[iarg+1], "multi") == 0) {
        // need to reset cutghostuser when switching comm mode
        if (mode == SINGLE) cutghostuser [0] = cutghostuser[1] = cutghostuser[2 ]= 0.0;
        mode = MULTI;
      ...
      if (iarg+4 > narg) error->all(FLERR, "Illegal comm_modify command");
      if (mode == MULTI)
        error->all(FLERR, 
            "Use cutoff/multi keyword to set cutoff in multi mode");
      cutghostuser[0] = universe->numeric(FLERR, arg[iarg+1]);
      cutghostuser[1] = universe->numeric(FLERR, arg[iarg+2]);
      cutghostuser[2] = universe->numeric(FLERR, arg[iarg+3]);
      if (cutghostuser[0] < 0.0 || 
          cutghostuser[1] < 0.0 ||
          cutghostuser[2] < 0.0)
    ...
    //      cutusermulti[i] = -1.0;
    //  }
    //  universe->bounds(FLERR, arg[iarg+1], ntypes, nlo, nhi, 1);
    //  cut = universe->numeric(FLERR, arg[iarg+2]);
...
void Comm::set_processors(int narg, char **arg)
{
  ...
  else user_procgrid[0] = universe->inumeric(FLERR, arg[0]);
  if (strcmp(arg[1], "*") == 0) user_procgrid[1] = 0;
  else user_procgrid[1] = universe->inumeric(FLERR, arg[1]);
  if (strcmp(arg[2], "*") == 0) user_procgrid[2] = 0;
  else user_procgrid[2] = universe->inumeric(FLERR, arg[2]);
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "grid") == 0) {
      ...
      if (strcmp(arg[iarg+1], "onelevel") == 0) {
        ...
        ncores = universe->inumeric(FLERR, arg[iarg+2]);
        if (strcmp(arg[iarg+3], "*") == 0) user_coregrid[0] = 0;
        else user_coregrid[0] = universe->inumeric(FLERR, arg[iarg+3]);
        if (strcmp(arg[iarg+4], "*") == 0) user_coregrid[1] = 0;
        else user_coregrid[1] = universe->inumeric(FLERR, arg[iarg+4]);
        if (strcmp(arg[iarg+5], "*") == 0) user_coregrid[2] = 0;
        else user_coregrid[2] = universe->inumeric(FLERR, arg[iarg+5]);
      ...
      int isend = universe->inumeric(FLERR, arg[iarg+1]);
      int irecv = universe->inumeric(FLERR, arg[iarg+2]);
...
void Comm::set_proc_grid(int outflag)
{
  ...
  if (outflag && me == 0) {
    for (int i = 0; i < 2; i++) {
      ...
      if (out) {
        fprintf(out, "  %d by %d by %d MPI processor grid\n", 
            procgrid[0], procgrid[1], procgrid[2]);
        if (gridflag == NUMA || gridflag == TWOLEVEL)
          fprintf(out, "  %d by %d by %d core grid within node\n", 
