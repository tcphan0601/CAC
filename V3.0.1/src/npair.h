...
class NPair : protected Pointers {
 ...
 protected:
  ...
  double **myinnercutneighsq;    // per-type cutoffs when user specified
  double maxcutneigh;
  ...
  double *rbucket;
  int maxbucket;
  void sort_neigh(int, int, int *, int *, double *, int *, int *);

  // temporary element neighbor list
  int *elemneighlist;
  int maxelemneigh;

  void grow_elemneighlist();
