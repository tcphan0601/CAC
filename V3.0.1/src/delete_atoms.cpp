...
void DeleteAtoms::list_delete_overlap(int narg, char **arg)
{
  ...
  int i, j, ii, jj, inum, jnum, jindex, jbasis, japc, jucell;
  double xtmp, ytmp, ztmp, delx, dely, delz, rsq;
  int *ilist, *iindexlist, *jlist, *jindexlist;
  int **firstneigh, **firstneighindex, *numneigh;
  ...
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    if (!(amask[i] & (group1bit | group2bit))) continue;
    xtmp = x[i][0];
    ytmp = x[i][1];
    ztmp = x[i][2];
    ...
    jnum = numneigh[i];
    jlist = firstneigh[i];
    jindexlist = firstneighindex[i];
    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      jindex = jindexlist[jj];
      if (jindex < 0) {
        if (!(amask[j] & (group1bit | group2bit))) continue;
        ...
        if (!(emask[j] & (group1bit | group2bit))) continue;
        japc = apc[etype[j]];
        jucell = jindex / japc;
        jbasis = jindex % japc;
        delx = xtmp - element->evec->interpolate(nodex, j, jbasis, jucell, 0);
        dely = ytmp - element->evec->interpolate(nodex, j, jbasis, jucell, 1);
        delz = ztmp - element->evec->interpolate(nodex, j, jbasis, jucell, 2);
        rsq = delx * delx + dely * dely + delz * delz;
        if (rsq >= cutsq) continue;
        dlist[i] = 1;
        break;
