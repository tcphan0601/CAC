...
NeighRequest::NeighRequest(CAC *cac) : Pointers(cac)
{
  ...
  // which list to include: defaults are atom, element, and gaussian cell neighbor lists
...
int NeighRequest::identical(NeighRequest *other)
{
  ...
  if (nodelist != other->nodelist) same = 0;
  if (gausslist != other->gausslist) same = 0;
  if (vatomlist != other->vatomlist) same = 0;
...
//  if (granonesided != other->granonesided) same = 0;
  ...
  if (group != other->group) same = 0;
  else if (groupbit != other->groupbit) same = 0;

  if (noinner != other->noinner) same = 0;
...
void NeighRequest::copy_request(NeighRequest *other)
{
  ...
  gausslist = other->gausslist;
  vatomlist = other->vatomlist;
...
//  granonesided = other->granonesided;
  ...
  group = other->group;
  groupbit = other->groupbit;
  noinner = other->noinner;
