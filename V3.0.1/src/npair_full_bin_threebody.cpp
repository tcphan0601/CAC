...
void NPairFullBinThreebody::build(NeighList *list)
{
  ...
  for (i = 0; i < neall; i++)
    for (j = 0; j < element->maxapc; j++)
      for (k = 0; k < element->max_nucell; k++)
        va2nvalist[i][j][k] = -1;
              ...
              for (l = 0; l < nucell_subelem[jetype][isub]; l++) {
                ...
                for (jbasis = 0; jbasis < japc; jbasis++) {
                  ...
                  if (rsq <= cutneighsq[ictype][jctype]) {
                    ...
                    if (u2g[jetype][jucell] < 0 && va2nvalist[j][jbasis][jucell] < 0) {

                      va2nvalist[j][jbasis][jucell] = nvainum;
                      nvailist[nvainum] = j;
                      nvaindexlist[nvainum] = jucell * japc + jbasis;
                      nvainum++;
                    }
                  }
                }
