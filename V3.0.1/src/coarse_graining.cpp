...
enum{CNA_UNKNOWN, CNA_OTHER, FCC, HCP, BCC, ICOS};
enum{IDS_UNKNOWN, IDS_OTHER, CUBIC, HEX};
...
void CoarseGraining::command(int narg, char **arg)
{
  ...
  if (element->maxapc > 1) 
    error->all(FLERR, "Coarse graining command only works for 1 atom per unit cell");
  ...
  memory->create(nodecoord, 1, 8, 3, "coarse_graining:nodecoord");
  ...
  if (grain_tag_enable) {
    ...
    for (int igrain = 1; igrain <= ngrains; igrain++) {
      ...
      if (centroid_flag) {
        ...
        if (centroid_count == 0) error->one(FLERR,"No perfect crystal atoms");
  ...
  int *npe = element->npe;
  int *apc = element->apc;
  for (i = 0; i < element->nlocal; i++) domain->remap(ex[i], nodex[i], apc[etype[i]], npe[etype[i]]);
    ...
    double *a2, double *a3, int iex, int iey, int iez, int first_flag)
{
  ...
  for (ii = 0; ii < 8; ii++) 
    copy3(x[nodelist[ii]], nodecoord[0][ii]);
  ...
  for (j = 0; j < 3; j++) {
    pa1[j] = nodecoord[0][1][j] - nodecoord[0][0][j] + 
      nodecoord[0][2][j] - nodecoord[0][3][j] +
      nodecoord[0][5][j] - nodecoord[0][4][j] +
      nodecoord[0][6][j] - nodecoord[0][7][j];
    pa2[j] = nodecoord[0][3][j] - nodecoord[0][0][j] +
      nodecoord[0][2][j] - nodecoord[0][1][j] +
      nodecoord[0][7][j] - nodecoord[0][4][j] +
      nodecoord[0][6][j] - nodecoord[0][5][j];
    pa3[j] = nodecoord[0][4][j] - nodecoord[0][0][j] +
      nodecoord[0][7][j] - nodecoord[0][3][j] +
      nodecoord[0][5][j] - nodecoord[0][1][j] +
      nodecoord[0][6][j] - nodecoord[0][2][j];
    na1[j] = -pa1[j];
    na2[j] = -pa2[j];
    na3[j] = -pa3[j];
  }
    ...
    double *old_a3, int iax, int iay, int iaz)
{
  ...
  if (pattern[i] != structure_type || grain_tag[i] != grain_id) return 0;
