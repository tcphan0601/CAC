...
class FixStore : public Fix {
 public:
  ...
  double ***a3store;     // 3D array storage for PERATOM (nodal values or virtual atom values)
  double ****a4store;    // 4D array storage for PERATOM (nodal values or virtual atom values)
 ...
 private:
  ...
  int elem_flavor;              // ELEMENT, NODE or VATOM
  int ncell, nbasis;
