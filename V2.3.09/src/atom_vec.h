...
class AtomVec : protected Pointers {
 public:
  ...
  int size_dat_atom;                   // number of values in Discrete Atom line in .dat file
  ...
  virtual void pack_dat(double **) = 0;
  virtual void pack_vel(double **);
  virtual void write_data(FILE *, int, double **) = 0;
  virtual void write_dat(FILE *, int, double **) = 0;
  virtual void write_vel(FILE *, int, double **);
  virtual void create_atom(double *, int, tagint) = 0;
