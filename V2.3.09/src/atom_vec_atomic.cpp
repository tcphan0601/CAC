...
void AtomVecAtomic::write_dat(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
/* ----------------------------------------------------------------------
   pack atom info for dat file including 3 image flags
------------------------------------------------------------------------- */

void AtomVecAtomic::pack_dat(double **buf)
{
  for (int i = 0; i < atom->nlocal; i++) {
    buf[i][0] = x[i][0];
    buf[i][1] = x[i][1];
...
/* ----------------------------------------------------------------------
   add a new atom
   called by element2atom function to 
      discretize element to atoms
   called by create_atoms command
   ------------------------------------------------------------------------- */

void AtomVecAtomic::create_atom(double *coord, int itype, tagint itag)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  x[nlocal][0] = coord[0];
