...
void Domain::reset_box()
{
  ...
  if (nonperiodic == 2) {
    ...
    if (triclinic) {
      ...
      lamda2x(element->nlocal, element->npe, element->nodex);
    }
  ...
  */
  ...
  if (nonperiodic == 2 && triclinic) {
    ...
    x2lamda(element->nlocal, element->npe, element->nodex);
    pbc();
  }
...
void Domain::unmap(double *x, double **nodex, imageint image)
{
  int xbox = (image & IMGMASK) - IMGMAX;
  int ybox = (image >> IMGBITS & IMGMASK) - IMGMAX;
  int zbox = (image >> IMG2BITS) - IMGMAX;
...
void Domain::lamda2x(int n1, int n2, double ***x)
{
  for (int i = 0; i < n1; i++) 
    for (int j = 0; j < n2; j++) {
...
void Domain::x2lamda(int n1, int n2, double ***x)
{
  double delta[3];
