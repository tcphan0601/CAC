...
Element::Element(CAC *cac) : Pointers(cac)
{
  ...
  max_size = 0.0;
  max_size_array[0] = max_size_array[1] = max_size_array[2] = 0.0;
  max_diag_size = 0.0;
  max_same = 0;
...
void Element::data_nodes(int n, char *buf, tagint id_offset, int shiftflag, double *shift) {
  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    if ((m = map(tagdata)) >= 0) evec->data_node(m,&values[1],shiftflag,shift);
...
void Element::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"esplit") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"node_weight") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      if (domain->box_exist) error->all(FLERR,"Cannot change apc after box exist");
      node_weight = universe->numeric(FLERR,arg[iarg+1]);
      if (node_weight < 0) error->all(FLERR,"Illegal element_modify command");
      // reset all integration points' weight for all etypes
      for (int i = 1; i <= netypes; i++)
        evec->setup_integration_point(i);
      iarg += 2;
...
void Element::add_etype(int narg, char **arg)
{
  ...
  if (itype <= netypes) {
    fprintf(screen,"itype = %d netype = %d\n",itype,netypes);
