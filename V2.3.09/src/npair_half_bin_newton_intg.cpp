            ...
            for (isub = 0; isub < nsubelem; isub++) {
              ...
              if (rsq < cut_subelemsq) {
                ...
                for (l = 0; l < natom_subelem[jetype][isub]; l++) {
                  ...
                  delx = xtmp; dely = ytmp; delz = ztmp;
                  for (inode = 0; inode < npe; inode++) {
                    delx -= shape_array[jetype][jintpl][inode]*nodex[j][inode][0];
                    dely -= shape_array[jetype][jintpl][inode]*nodex[j][inode][1];
                    delz -= shape_array[jetype][jintpl][inode]*nodex[j][inode][2];
                  }
                  rsq = delx*delx + dely*dely + delz*delz;
                  if (rsq < cutneighsq[ictype][jctype]) {
