...
void ReadData::command(int narg, char **arg)
{
  ...
  int interpolateflag,integrationflag;

  elementflag = atomflag = 0;
  interpolateflag = integrationflag = 0;

  // values in this data file

  natoms = nelements = nnodes;
  netypes = natypes = 0;
  ...
  // error check for required sections
  
  if (elementflag && !integrationflag)
    error->one(FLERR,"Needed integration information not in data file");
  if (elementflag && !interpolateflag)
    error->one(FLERR,"Needed interpolate information not in data file");
...
void ReadData::header()
{
  ...
  {"Atoms","Velocity","Ellipsoids","Lines","Triangles","Bodies",
...
void ReadData::interpolate()
{
  ...
  for (int i = 0; i < netypes; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    element->evec->set_interpolate(buf,eoffset);
    buf = next + 1; 
  }
  delete [] original;
}
...
void ReadData::integration()
{
  ...
  for (int i = 0; i < netypes; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    element->evec->set_integration(buf,eoffset);
    buf = next + 1; 
  }
