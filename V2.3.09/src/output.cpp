...
void Output::add_dump(int narg, char **arg)
{
  ...
  if (dump_map->find(arg[2]) != dump_map->end()) {
    ...
    dump[ndump] = dump_creator(cac, narg, arg);
  }
...
/* ----------------------------------------------------------------------
  ...
  template <typename T>
Dump *Output::dump_creator(CAC *cac, int narg, char ** arg)
{
...
/*
   ...
   else every = universe->inumeric(FLERR,arg[0]);
