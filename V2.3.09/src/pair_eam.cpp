...
void PairEAM::file2array()
{
  ...
  for (i = 0; i < nfuncfl; i++) {
    ...
    for (m = 1; m <= nrho; m++) {
      ...
      p -= k;
      p = MIN(p,2.0);
      cof1 = -sxtmpth*p*(p-1.0)*(p-2.0);
      cof2 = 0.5*(p*p-1.0)*(p-2.0);
      cof3 = -0.5*p*(p+1.0)*(p-2.0);
      cof4 = sxtmpth*p*(p*p-1.0);
  ...
  // set of z2r arrays only fill lower triangular Nelement matrxtmp
  // value = n = sum over rows of lower-triangular matrxtmp until reach irow,icol
...
int PairEAM::pack_elem_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  ...
  for (i = 0; i < n; i++) {
    j = list[i];
    for (k = 0; k < npe; k++) buf[m++] = noderho[j][k];
  }
  return m;
}
...
void PairEAM::unpack_elem_forward_comm(int n, int first, double *buf)
{
  ...
  for (i = first; i < last; i++)
    for (j = 0; j < npe; j++) noderho[i][j] = buf[m++];
}
