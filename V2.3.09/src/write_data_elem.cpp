...
void WriteDataElem::write(char *file)
{
  ...
  if (nelements) {
    ...
    if (nodeflag) {
      ...
      nodes(max_atom_tag_all+max_elem_tag_all);
    }
  }
  ...
  if (nelements) {
    ...
    if (nodeflag) 
      node_velocities(max_atom_tag_all+max_elem_tag_all);
  }
...
void WriteDataElem::header()
{
  ...
  fprintf(fp,"\n");
  fprintf(fp,BIGINT_FORMAT " atoms\n",atom->natoms + element->nelements*(element->npe*nodeflag+1));
...
void WriteDataElem::elements(tagint tag_offset)
{
  ...
  element->evec->pack_element_data_elem(buf,tag_offset);
...
void WriteDataElem::nodes(tagint tag_offset)
{
  ...
  element->evec->pack_node_data_elem(buf,tag_offset);
...
void WriteDataElem::node_velocities(tagint tag_offset)
{
  // communication buffer for all my Node info
  // max_size = largest buffer needed by any proc
