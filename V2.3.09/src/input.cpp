...
int Input::execute_command()
{
  ...
  else if (!strcmp(command,"terminate")) terminate();
  ...
  else if (!strcmp(command,"modify_elements")) modify_elements();
...
{
  if (narg > 0) error->all(FLERR,"Illegal terminate command");
  if (me == 0) {
    if (screen) fprintf(screen,"Simulation terminated\n");
    if (logfile) fprintf(logfile,"Simulation terminated\n");
  }
