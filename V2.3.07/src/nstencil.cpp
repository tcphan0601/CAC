...
void NStencil::copy_neighbor_info()
{
  ...
  cutx = cutneighmaxatom + element->max_size_array[0];
  cuty = cutneighmaxatom + element->max_size_array[1];
  cutz = cutneighmaxatom + element->max_size_array[2];
  cutneighmaxelem = cutneighmaxatom + element->max_diag_size;
  cutneighmaxelemsq = cutneighmaxelem * cutneighmaxelem;
}
...
void NStencil::create_setup()
{
  ...
  if (element->nelements) {
    elemsx = static_cast<int> (cutx*elembininvx);
    if (elemsx*elembinsizex < cutx) elemsx++;
    elemsy = static_cast<int> (cuty*elembininvy);
    if (elemsy*elembinsizey < cuty) elemsy++;

    if (dimension == 3) {
      elemsz = static_cast<int> (cutz*elembininvz);
      if (elemsz*elembinsizez < cutz) elemsz++;
    } else elemsz = 0;
...
double NStencil::elembin_distance(int i, int j, int k)
{
  double delx,dely,delz;

  if (i > 0) delx = (i-1)*elembinsizex;
  else if (i == 0) delx = 0.0;
  else delx = (i+1)*elembinsizex;

  if (j > 0) dely = (j-1)*elembinsizey;
  else if (j == 0) dely = 0.0;
  else dely = (j+1)*elembinsizey;

  if (k > 0) delz = (k-1)*elembinsizez;
  else if (k == 0) delz = 0.0;
  else delz = (k+1)*elembinsizez;
