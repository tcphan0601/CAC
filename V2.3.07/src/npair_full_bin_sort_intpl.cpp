...
NPairFullBinSortIntpl::~NPairFullBinSortIntpl() 
{
          ...
          if (rsq < cut_allsq) {
            ...
            for (jsub = 0; jsub < nsubelem; jsub++) {
              ...
              for (jnode = 0; jnode < npe; jnode++) {
                delx -= shape_array_center_subelem[jsub][jnode]*nodex[j][jnode][0];
                dely -= shape_array_center_subelem[jsub][jnode]*nodex[j][jnode][1];
                delz -= shape_array_center_subelem[jsub][jnode]*nodex[j][jnode][2];
              }
              rsq = delx*delx + dely*dely + delz*delz;
