          ...
          if (rsq < cut_elemsq) {
            ...
            for (isub = 0; isub < nsubelem; isub++) {
              ...
              for (node = 0; node < npe; node++) {
                delx -= shape_array_center_subelem[isub][node]*nodex[j][node][0];
                dely -= shape_array_center_subelem[isub][node]*nodex[j][node][1];
                delz -= shape_array_center_subelem[isub][node]*nodex[j][node][2];
              }
              rsq = delx*delx + dely*dely + delz*delz;
              if (rsq < cut_subelemsq) {
                ...
                for (l = 0; l < natom_subelem[jetype][isub]; l++) {
                  ...
                  if (rsq < cutneighsq[ictype][jctype]) {
                    ...
                    // mark j as nia j is not an integration 
            ...
            if (rsq < cut_subelemsq)
              ...
              for (l = 0; l < natom_subelem[jetype][isub]; l++) {
                ...
                if (rsq < cutneighsq[ictype][jctype]) {
                  ...
                  // mark j as nia if j is not an integration 
                  // point and was not marked as nia already
        ...
        if (rsq < cut_subelemsq)
          ...
          for (l = 0; l < natom_subelem[jetype][isub]; l++) {
            ...
            if (rsq < cutneighsq[ictype][jctype]) {
              ...
              // mark j as nia if not an integration 
              // point and was not marked as nia already
