...
enum{NONE,SC,BCC,FCC,PRIFCC,PRIBCC,HCP,DIAMOND,PRIDIAMOND,SQ,SQ2,HEX,CUSTOM};
...
Lattice::Lattice(CAC *cac, int narg, char **arg) : Pointers(cac)
{
  ...
  else if (strcmp(arg[0],"prifcc") == 0) style = PRIFCC;
  else if (strcmp(arg[0],"pribcc") == 0) style = PRIBCC;
  else if (strcmp(arg[0],"pridiamond") == 0) style = PRIDIAMOND;
  ...
  if (style == PRIFCC || style == PRIDIAMOND) {
  ...
  if (spaceflag == 0) {
    if (style == PRIFCC || style == PRIDIAMOND || style == PRIBCC) {
      xlattice = a1[0]*scale;
