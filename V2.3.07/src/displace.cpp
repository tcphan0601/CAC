...
enum{MOVE,RAMP,RANDOM,ROTATE,DISLOCATION,MULTIDISL};
enum{NODE,ELEMENT};
...
void Displace::rotate(char **arg)
{
  ...
  for (int i = 0; i < nelocal; i++) {
    if (emask[i] & groupbit) {
      // unwrap coordinate and reset image flags accordingly
      domain->unmap(ex[i],nodex[i],eimage[i]);
...
void Displace::multi_dislocations(char **arg)
{
  ...
  offset = universe->numeric(FLERR,arg[4]);     // offset distance between dislocation
  ratio = universe->numeric(FLERR,arg[5]);      // ratio between each offset
  b = universe->numeric(FLERR,arg[7]);
  disl_angle = universe->numeric(FLERR,arg[8])*MY_PI/180;   // Burger Vector directional angle (from dislocation line) 
                                                            // input as degree and converted to rad
  ...
  double bedge = b*sin(disl_angle);
  double bscrew = b*cos(disl_angle);
  ...
  if (nalocal) 
    for (i = 0; i < nalocal; i++) {
      ...
      for (int idisl = 0; idisl < ndisl; idisl++) {
        x1 = ax[i][dim1] - list_center1[idisl];
        edge_dislocation_field(u1,u2,x1,x2,bedge,v);
        screw_dislocation_field(u,x1,x2,bscrew);
      }
      ax[i][dim1] += u1;
  ...
  if (nelocal) {
    for (i = 0; i < nelocal; i++)
      for (j = 0; j < npe; j++) {
        ...
        x2 = nodex[i][j][dim2] - center2;
        for (int idisl = 0; idisl < ndisl; idisl++) {
          x1 = nodex[i][j][dim1] - list_center1[idisl];
          edge_dislocation_field(u1,u2,x1,x2,bedge,v);
          screw_dislocation_field(u,x1,x2,bscrew);
        }
        nodex[i][j][dim1] += u1;
