...
void Atom::data_pass_on_fix_compute_dump_variable(int i, int i_old, int iintpl)
{
  for (int m = 0; m < modify->nfix; m++) {
    Fix *fix = modify->fix[m];
    if (fix->create_attribute)
      fix->pass_on_atom_arrays(i,i_old,iintpl);
  }

  for (int m = 0; m < modify->ncompute; m++) {
    Compute *compute = modify->compute[m];
    if (compute->create_attribute)
      compute->pass_on_atom_arrays(i,i_old,iintpl);
  }

  for (int m = 0; m < output->ndump; m++) {
    Dump *dump = output->dump[m];
    if (dump->create_attribute)
      dump->pass_on_atom_arrays(i,i_old,iintpl);
  }

  input->variable->pass_on_atom_arrays(i,i_old,iintpl);
}
