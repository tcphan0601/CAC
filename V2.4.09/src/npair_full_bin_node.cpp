...
{
  int inode,i,j,jj,jnum,k,l,n,ictype,jctype,ietype,jetype,ibin,isub,nnode;
  int *a2aneighptr,*a2ianeighptr,*a2ia_indexneighptr;
  a2aneighptr = a2ianeighptr = a2ia_indexneighptr = NULL;
  int *n2aneighptr,*e2eneighptr,*n2ianeighptr,*n2ia_indexneighptr;
  n2aneighptr = e2eneighptr = n2ianeighptr = n2ia_indexneighptr = NULL;
  int *jlist;
  int na2a,na2ia,nn2a,ne2e,nn2ia;
  ...
  MyPage<int> *n2apage = list->n2apage;
  MyPage<int> *e2epage = list->e2epage;
  MyPage<int> *n2iapage = list->n2iapage;
  MyPage<int> *n2ia_indexpage = list->n2ia_indexpage;
  ...
  /*------------------------------------------------------------------- 
   * NODE NEIGHBORS
   * loop over each node and store neighbors 
   *-----------------------------------------------------------------*/ 
  ...
  for (i = 0; i < einum; i++) {
    ...
    for (inode = 0; inode < nnode; inode++) {
      ...
      xtmp = nodex[i][inode][0];
      ytmp = nodex[i][inode][1];
      ztmp = nodex[i][inode][2];
      
      iintpl = i2ia[ietype][n2i[ietype][inode]];

      // loop over all atoms in all atom bins in stencil including self

      if (naall) {
        nn2a = 0;
        n2aneighptr = n2apage->vget();
        ...
        firstneighn2a[ninum] = n2aneighptr;
        numneighn2a[ninum] = nn2a;
        n2apage->vgot(nn2a);
        if (n2apage->status())
          error->one(FLERR,"Neighbor list overflow, boost neigh_modify one");
      } else { 
      ...
      nn2ia = 0;
      n2ianeighptr = n2iapage->vget();
      n2ia_indexneighptr = n2ia_indexpage->vget();
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        jctype = ctype[j];
        jetype = etype[j];
        ...
        for (isub = 0; isub < nsubelem[jetype]; isub++) {
          ...
          if (rsq < cutoffsq)
            ...
            for (l = 0; l < natom_subelem[jetype][isub]; l++) {
              ...
              rsq = delx*delx + dely*dely + delz*delz;
              if (rsq < cutneighsq[ictype][jctype]) {
                n2ianeighptr[nn2ia] = j;
                n2ia_indexneighptr[nn2ia++] = jintpl;
              }
            }
      ...
      firstneighn2ia[ninum] = n2ianeighptr;
      firstneighn2ia_index[ninum] = n2ia_indexneighptr;
      numneighn2ia[ninum] = nn2ia;
      n2iapage->vgot(nn2ia);
      n2ia_indexpage->vgot(nn2ia);
      if (n2iapage->status() ||
          n2ia_indexpage->status())
        error->one(FLERR,"Neighbor list overflow, boost neigh_modify one");
      ninum++;
    }
  }
  ...
  // finish e2nlist for ghost elements if needed
