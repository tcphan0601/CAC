...
#define NSHAPES       10     // increase NSHAPES after adding new element shapes
#define DELTA         1
...
Element::Element(CAC *cac) : Pointers(cac)
{
  ...
  strcpy(element_shape_list[TRAPEPRISM],"TrapePrism");          // prism with trapezoidal cross-section
  strcpy(element_shape_list[PYRAMIDFRUSTUM],"PyrFrustum");      // a frustum made by chopping the top of a pyramid
  strcpy(element_shape_list[TRIFRUSTUM],"TriFrustum");          // a frustum made by chopping the top of a tetrahedral
  ...
  max_outer_intpl = 2400;
  max_surface_intpl = 400;
  max_surface = 8;
  max_edge_intpl = 20;
  max_edge = 12;
  maxintpl = 8000;  
  ...
  // gauss integration points and weights for each element shape
...
#undef ELEMENT_CLASS
  ...
  debug_mode = 0;
  debug_intg_type = NULL;
  debug_intg_inactive = NULL;
}
...
Element::~Element()
{
  ...
  if (max_apc > 1 && element_cluster_flag) memory->destroy(element_clusters);
...
void Element::data_elements(int n, char *buf, tagint id_offset, int ctype_offset, 
  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (coord[0] >= sublo[0] && coord[0] < subhi[0] &&
      ...
      if (ctype_offset) {
        ctype[nlocal-1] += ctype_offset;
        if (ctype[nlocal-1] > atom->ntypes)
...
/* ----------------------------------------------------------------------
   unpack N lines from Element clusters section of data file
   check that element IDs are > 0 and <= map_tag_max
   ------------------------------------------------------------------------- */

void Element::data_element_clusters(int n, char *buf, tagint id_offset)
{
  if (element_cluster_flag == 0 || max_apc == 1) return;
  ...
  *next = '\n';
  ...
  if (nwords != evec->size_data_cluster)
    error->all(FLERR,"Incorrect Element Cluster format in data file");

  char **values = new char*[nwords];
  tagint tagdata[nwords];

  // loop over lines of Element Cluster
  // tokenize the line into values
  // if I own element tag, unpack its values
  ...
  for (i = 0; i < n; i++) {
    ...
    // extract tag values and number of elements in this clusters
    // check for errors and duplications

    int iapc = -1;
    for (j = 0; j < nwords; j++) {
      tagdata[j] = ATOTAGINT(values[j]);
      if (tagdata[j] < 0) {
        error->one(FLERR,"Invalid element ID in Element Clusters section of data file");
      } else if (tagdata[j] > 0) {
        if (iapc > 0) 
          error->one(FLERR,"Invalid element ID in Element Clusters section of data file");
        tagdata[j] += id_offset;
    ...
    if (iapc < 0) iapc = nwords;
    if (iapc == 0)
      error->one(FLERR,"Invalid element ID in Element Clusters section of data file");
    for (j = 0; j < iapc; j++) {
      if (tagdata[j] > map_tag_max)
        error->one(FLERR,"Invalid element ID in Element Clusters section of data file");
      if ((m = map(tagdata[j])) >= 0) {
        if (iapc != apc[etype[m]])
          error->one(FLERR,"Element cluster does not match its apc value");
        if (element_clusters[m][0] != 0) 
          error->one(FLERR,"Duplicate element IDs in Element Clusters section of data file");
        for (k = 0; k < iapc; k++)
          element_clusters[m][k] = tagdata[k];
      }
    }
    buf = next + 1; 
  }
...
void Element::data_nodes(int n, char *buf, tagint id_offset, int shiftflag, double *shift) 
{
  int j,m;
  tagint tagdata;
  char *next;
  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    if ((m = map(tagdata)) >= 0)
      evec->data_node(m,&values[1],shiftflag,shift);
...
void Element::data_nodes_reference(int n, char *buf, tagint id_offset, int shiftflag, double *shift) 
{
  int j,m;
  tagint tagdata;
  char *next;
  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (tagdata <= id_offset || tagdata > map_tag_max)
      error->one(FLERR,"Invalid element ID in Nodes section of data file");
    if ((m = map(tagdata)) >= 0)
      evec->data_node_reference(m,&values[1],shiftflag,shift);
...
bigint Element::memory_usage()
{
  ...
  bytes += memory->usage(is_outer,n,maxintpl);
  bytes += memory->usage(n2i,n,max_npe);
  bytes += memory->usage(n2ia,n,max_npe);
  bytes += memory->usage(nintpl,n);
  bytes += memory->usage(surface_intpl,n,max_surface,max_surface_intpl);
  bytes += memory->usage(nsurface_intpl,n,max_surface);
  bytes += memory->usage(edge_intpl,n,max_edge,max_edge_intpl);
  bytes += memory->usage(nedge_intpl,n,max_edge);
  bytes += memory->usage(shape_array,n,maxintpl,max_npe);
  bytes += memory->usage(weighted_shape_array,n,maxintg,max_npe);
  bytes += memory->usage(shape_array_center_subelem,n,maxsubelem,max_npe);
  bytes += memory->usage(nsubelem,n);
  bytes += memory->usage(subsplit,n);
  bytes += memory->usage(natom_subelem,n,maxsubelem);
  bytes += memory->usage(ias2ia,n,maxsubelem,maxsubintpl);
...
void Element::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"max/surface/intpl") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      max_surface_intpl = universe->inumeric(FLERR,arg[iarg+1]);
      if (max_surface_intpl <= 0) error->all(FLERR,"Illegal element_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"max/outer/intpl") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      max_outer_intpl = universe->inumeric(FLERR,arg[iarg+1]);
      if (max_outer_intpl <= 0) error->all(FLERR,"Illegal element_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"max/edge/intpl") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      max_edge_intpl = universe->inumeric(FLERR,arg[iarg+1]);
      if (max_edge_intpl <= 0) error->all(FLERR,"Illegal element_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"mass/style") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      if (strcmp(arg[iarg+1],"lumped") == 0) mass_style = LUMPED;
      else if (strcmp(arg[iarg+1],"consistent") == 0) mass_style = CONSISTENT;
      else error->all(FLERR,"Illegal element_modify command, invalid mass matrix type");
      iarg += 2;
    } else if (strcmp(arg[iarg],"cluster") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      if (strcmp(arg[iarg+1],"yes") == 0) element_cluster_flag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) element_cluster_flag = 0;
      else error->all(FLERR,"Illegal element_modify command, invalid cluster option");
      iarg += 2;
...
/* ----------------------------------------------------------------------
   reset node tags for node connectivity in dump tecplot command
   called during dump tecplot
   if cluster_flag = 1 && element_cluster_flag = 1, only assign node tags to clusters owned by me
   ------------------------------------------------------------------------- */

void Element::reset_node_tags(int igroup, int cluster_flag)
{
  ...
  tagint maxtag = 0;
  if (cluster_flag && element_cluster_flag) {
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit && tag[i] == element_clusters[i][0]) 
        maxtag += npe[etype[i]]; 
  } else {
    for (int i = 0; i < nlocal; i++)
...
/* ----------------------------------------------------------------------
   count number of element clusters
   ------------------------------------------------------------------------- */

int Element::count_element_clusters()
{
  int n = 0;
  for (int i = 0; i < nlocal; i++) 
...
void Element::data_pass_on_fix_compute_dump_variable(int i, int i_old, int *iintpl_list)
{
  for (int m = 0; m < modify->nfix; m++) {
    Fix *fix = modify->fix[m];
    if (fix->create_attribute)
      fix->pass_on_elem_arrays(i,i_old,iintpl_list);
  }

  for (int m = 0; m < modify->ncompute; m++) {
    Compute *compute = modify->compute[m];
    if (compute->create_attribute)
      compute->pass_on_elem_arrays(i,i_old,iintpl_list);
  }

  for (int m = 0; m < output->ndump; m++) {
    Dump *dump = output->dump[m];
    if (dump->create_attribute)
      dump->pass_on_elem_arrays(i,i_old,iintpl_list);
  }

  input->variable->pass_on_elem_arrays(i,i_old,iintpl_list);
}

/* ----------------------------------------------------------------------
   elements in a cluster must have the same type (can have different ctype)
   must be called after performing comm->borders()
   ------------------------------------------------------------------------- */

void Element::check_element_clusters()
{
  int mapflag = 0;
  if (map_style == 0) {
    mapflag = 1;
  ...
  for (int i = 0; i < nlocal; i++) {
    for (int j = 0; j < apc[etype[i]]; j++) {
      m = map(element_clusters[i][j]);
      if (m >= 0) {
        if (etype[m] != etype[i])
          error->one(FLERR,"Elements in the same cluster must have the same etype!");
      } else {
        printf("m = %d i = %d apc = %d j - %d element_clusters = %d\n",m,i,apc[etype[i]],j,element_clusters[i][j]);
        error->warning(FLERR,"Incorrect element ghost to check clusters");
      }
    }
  }
  if (mapflag) {
...
triclinic: bound box in lamda coords
...
void Element::update_element_bound_box(int check)
{
  ...
  double *prd;
  if (domain->triclinic) prd = domain->prd_lamda;
  else prd = domain->prd;
...
void Element::update_subelem_size()
{
  ...
  for (int i = 0; i < nlocal + nghost; i++) {
    ...
    if (element_shape_ids[itype] == Element::QUADRILATERAL) {
      for (int j = 0; j < 2; j++) {
        delx = inodex[j][0] - inodex[j+2][0];
        dely = inodex[j][1] - inodex[j+2][1];
      ...
      delx = inodex[1][0] + inodex[2][0] - 3.0*inodex[0][0];
      dely = inodex[1][1] + inodex[2][1] - 3.0*inodex[0][1];
      rsq = delx*delx + dely*dely;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[1][0] - inodex[2][0];
      dely = inodex[1][1] - inodex[2][1];
      rsq = delx*delx + dely*dely;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      subelem_size[i] *= 1.21;
      ...
      for (int j = 0; j < 4; j++) {
        if (j < 2) {
          delx = inodex[j][0] - inodex[j+6][0];
          dely = inodex[j][1] - inodex[j+6][1];
          delz = inodex[j][2] - inodex[j+6][2];
        } else {
          delx = inodex[j][0] - inodex[j+2][0];
          dely = inodex[j][1] - inodex[j+2][1];
          delz = inodex[j][2] - inodex[j+2][2];
        }
      ...
      // form a regular hex element to find max diag
      
      delx = inodex[2][0] + inodex[4][0] - 2*inodex[0][0];
      dely = inodex[2][1] + inodex[4][1] - 2*inodex[0][1];
      delz = inodex[2][2] + inodex[4][2] - 2*inodex[0][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      delx = inodex[3][0] + inodex[5][0] - 2*inodex[1][0];
      dely = inodex[3][1] + inodex[5][1] - 2*inodex[1][1];
      delz = inodex[3][2] + inodex[5][2] - 2*inodex[1][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      delx = inodex[0][0] + inodex[6][0] - 2*inodex[2][0];
      dely = inodex[0][1] + inodex[6][1] - 2*inodex[2][1];
      delz = inodex[0][2] + inodex[6][2] - 2*inodex[2][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      delx = inodex[1][0] + inodex[7][0] - 2*inodex[3][0];
      dely = inodex[1][1] + inodex[7][1] - 2*inodex[3][1];
      delz = inodex[1][2] + inodex[7][2] - 2*inodex[3][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      subelem_size[i] *= 1.21;
      ...
      delx = inodex[1][0] + inodex[2][0] + inodex[3][0] - 4.0*inodex[0][0];
      dely = inodex[1][1] + inodex[2][1] + inodex[3][1] - 4.0*inodex[0][1];
      delz = inodex[1][2] + inodex[2][2] + inodex[3][2] - 4.0*inodex[0][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[1][0] + inodex[2][0] - inodex[3][0] - inodex[0][0];
      dely = inodex[1][1] + inodex[2][1] - inodex[3][1] - inodex[0][1];
      delz = inodex[1][2] + inodex[2][2] - inodex[3][2] - inodex[0][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      ...
      delx = inodex[4][0] + 0.75*inodex[0][0] - 0.25*inodex[1][0] - 1.25*inodex[2][0] - 0.25*inodex[3][0];
      dely = inodex[4][1] + 0.75*inodex[0][1] - 0.25*inodex[1][1] - 1.25*inodex[2][1] - 0.25*inodex[3][1];
      delz = inodex[4][2] + 0.75*inodex[0][2] - 0.25*inodex[1][2] - 1.25*inodex[2][2] - 0.25*inodex[3][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[4][0] - 0.25*inodex[0][0] + 0.75*inodex[1][0] - 1.25*inodex[2][0] - 1.25*inodex[3][0];
      dely = inodex[4][1] - 0.25*inodex[0][1] + 0.75*inodex[1][1] - 1.25*inodex[2][1] - 1.25*inodex[3][1];
      delz = inodex[4][2] - 0.25*inodex[0][2] + 0.75*inodex[1][2] - 1.25*inodex[2][2] - 1.25*inodex[3][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[4][0] - 1.25*inodex[0][0] - 0.25*inodex[1][0] + 0.75*inodex[2][0] - 0.25*inodex[3][0];
      dely = inodex[4][1] - 1.25*inodex[0][1] - 0.25*inodex[1][1] + 0.75*inodex[2][1] - 0.25*inodex[3][1];
      delz = inodex[4][2] - 1.25*inodex[0][2] - 0.25*inodex[1][2] + 0.75*inodex[2][2] - 0.25*inodex[3][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[4][0] - 0.25*inodex[0][0] - 1.25*inodex[1][0] - 0.25*inodex[2][0] + 0.75*inodex[3][0];
      dely = inodex[4][1] - 0.25*inodex[0][1] - 1.25*inodex[1][1] - 0.25*inodex[2][1] + 0.75*inodex[3][1];
      delz = inodex[4][2] - 0.25*inodex[0][2] - 1.25*inodex[1][2] - 0.25*inodex[2][2] + 0.75*inodex[3][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      subelem_size[i] *= 1.21;
      ...
      delx = inodex[1][0] - inodex[5][0];
      dely = inodex[1][1] - inodex[5][1];
      delz = inodex[1][2] - inodex[5][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[2][0] - inodex[4][0];
      dely = inodex[2][1] - inodex[4][1];
      delz = inodex[2][2] - inodex[4][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[1][0] + inodex[2][0] - 2.0*inodex[0][0] - inodex[3][0];
      delx = inodex[1][1] + inodex[2][1] - 2.0*inodex[0][1] - inodex[3][1];
      delx = inodex[1][2] + inodex[2][2] - 2.0*inodex[0][2] - inodex[3][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 

      delx = inodex[4][0] + inodex[5][0] - 2.0*inodex[3][0] - inodex[0][0];
      delx = inodex[4][1] + inodex[5][1] - 2.0*inodex[3][1] - inodex[0][1];
      delx = inodex[4][2] + inodex[5][2] - 2.0*inodex[3][2] - inodex[0][2];
      rsq = delx*delx + dely*dely + delz*delz;
      subelem_size[i] = MAX(subelem_size[i],rsq); 
      subelem_size[i] *= 1.21;
    }

    subelem_size[i] = sqrt(subelem_size[i]) / subsplit[itype] / 2.0 * subelem_size_factor;
  }
}
...
void Element::define_inversed_mass_matrices()
{
  ...
  inversed_mass_matrices[TRAPEPRISM][0][0] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][0][1] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][0][2] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][0][3] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][0][4] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][0][5] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][0][6] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][0][7] = 16.0;

  inversed_mass_matrices[TRAPEPRISM][1][0] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][1][1] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][1][2] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][1][3] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][1][4] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][1][5] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][1][6] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][1][7] = -8.0;

  inversed_mass_matrices[TRAPEPRISM][2][0] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][2][1] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][2][2] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][2][3] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][2][4] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][2][5] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][2][6] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][2][7] = 16.0;

  inversed_mass_matrices[TRAPEPRISM][3][0] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][3][1] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][3][2] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][3][3] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][3][4] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][3][5] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][3][6] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][3][7] = -32.0;

  inversed_mass_matrices[TRAPEPRISM][4][0] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][4][1] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][4][2] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][4][3] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][4][4] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][4][5] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][4][6] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][4][7] = -32.0;

  inversed_mass_matrices[TRAPEPRISM][5][0] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][5][1] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][5][2] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][5][3] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][5][4] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][5][5] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][5][6] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][5][7] = 16.0;

  inversed_mass_matrices[TRAPEPRISM][6][0] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][6][1] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][6][2] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][6][3] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][6][4] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][6][5] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][6][6] = 64.0;
  inversed_mass_matrices[TRAPEPRISM][6][7] = -32.0;

  inversed_mass_matrices[TRAPEPRISM][7][0] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][7][1] = -8.0;
  inversed_mass_matrices[TRAPEPRISM][7][2] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][7][3] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][7][4] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][7][5] = 16.0;
  inversed_mass_matrices[TRAPEPRISM][7][6] = -32.0;
  inversed_mass_matrices[TRAPEPRISM][7][7] = 64.0;
...
void Element::define_mass_matrices()
{
  ...
  mass_matrices[TRAPEPRISM][0][0] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][0][1] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][0][2] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][0][3] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][0][4] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][0][5] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][0][6] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][0][7] = 1.0 / 108.0;

  mass_matrices[TRAPEPRISM][1][0] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][1][1] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][1][2] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][1][3] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][1][4] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][1][5] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][1][6] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][1][7] = 1.0 / 216.0;

  mass_matrices[TRAPEPRISM][2][0] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][2][1] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][2][2] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][2][3] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][2][4] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][2][5] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][2][6] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][2][7] = 1.0 / 108.0;

  mass_matrices[TRAPEPRISM][3][0] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][3][1] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][3][2] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][3][3] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][3][4] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][3][5] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][3][6] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][3][7] = 1.0 / 54.0;

  mass_matrices[TRAPEPRISM][4][0] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][4][1] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][4][2] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][4][3] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][4][4] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][4][5] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][4][6] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][4][7] = 1.0 / 54.0;

  mass_matrices[TRAPEPRISM][5][0] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][5][1] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][5][2] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][5][3] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][5][4] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][5][5] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][5][6] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][5][7] = 1.0 / 108.0;

  mass_matrices[TRAPEPRISM][6][0] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][6][1] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][6][2] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][6][3] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][6][4] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][6][5] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][6][6] = 1.0 / 27.0;
  mass_matrices[TRAPEPRISM][6][7] = 1.0 / 54.0;

  mass_matrices[TRAPEPRISM][7][0] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][7][1] = 1.0 / 216.0;
  mass_matrices[TRAPEPRISM][7][2] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][7][3] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][7][4] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][7][5] = 1.0 / 108.0;
  mass_matrices[TRAPEPRISM][7][6] = 1.0 / 54.0;
  mass_matrices[TRAPEPRISM][7][7] = 1.0 / 27.0;
...
/* ---------------------------------------------
   ...
   In consistent with the index of surface in surface_intpl in evec->set_interpolate_element() function
...
void Element::define_surface_node_list()
{
  ...
  surface_node_list[TRAPEPRISM][0][0] = 0;
  surface_node_list[TRAPEPRISM][0][1] = 4;
  surface_node_list[TRAPEPRISM][0][2] = 7;
  surface_node_list[TRAPEPRISM][0][3] = 3;
  surface_num_node[TRAPEPRISM][0] = 4;

  // +x 

  surface_node_list[TRAPEPRISM][1][0] = 1;
  surface_node_list[TRAPEPRISM][1][1] = 2;
  surface_node_list[TRAPEPRISM][1][2] = 6;
  surface_node_list[TRAPEPRISM][1][3] = 5;
  surface_num_node[TRAPEPRISM][1] = 4;

  // -y 

  surface_node_list[TRAPEPRISM][2][0] = 0;
  surface_node_list[TRAPEPRISM][2][1] = 1;
  surface_node_list[TRAPEPRISM][2][2] = 5;
  surface_node_list[TRAPEPRISM][2][3] = 4;
  surface_num_node[TRAPEPRISM][2] = 4;

  // +y 

  surface_node_list[TRAPEPRISM][3][0] = 3;
  surface_node_list[TRAPEPRISM][3][1] = 7;
  surface_node_list[TRAPEPRISM][3][2] = 6;
  surface_node_list[TRAPEPRISM][3][3] = 2;
  surface_num_node[TRAPEPRISM][3] = 4;

  // -z 

  surface_node_list[TRAPEPRISM][4][0] = 0;
  surface_node_list[TRAPEPRISM][4][1] = 3;
  surface_node_list[TRAPEPRISM][4][2] = 2;
  surface_node_list[TRAPEPRISM][4][3] = 1;
  surface_num_node[TRAPEPRISM][4] = 4;
...
/* ---------------------------------------------
   Called from command compress_etype
   Clear unused etype
   ---------------------------------------------*/
...
void Element::compress_etype()
{
  ...
  for (int i = 1; i <= netypes; i++) {
    ...
    old_type_info[i][3] = evec->nintgs[i][0];
    old_type_info[i][4] = evec->nintgs[i][1];
    old_type_info[i][5] = evec->nintgs[i][2];
    ...
    if (screen) {
      ...
      for (int i = 1; i <= netypes; i++)
        fprintf(screen,"  %d %s %d %d %d %d %d %d %d\n",i
            ...
            ,evec->nintgs[i][0],evec->nintgs[i][1],evec->nintgs[i][2]);
    }
    if (logfile) {
...
void Element::destroy_etype_arrays()
{
  ...
  memory->destroy(surface_intpl);
  memory->destroy(nsurface_intpl);
  memory->destroy(edge_intpl);
  memory->destroy(nedge_intpl);
  ...
  memory->destroy(debug_intg_type);
  memory->destroy(debug_intg_inactive);

}

// debug command
void Element::debug_element_intg_off(int narg, char **arg)
{
  if (domain->box_exist == 0)
    error->all(FLERR,"debug_element_intg command before simulation box is defined");
  if (narg < 2) error->all(FLERR,"Invalid debug_element_intg command");
  debug_mode = 1;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"inner") == 0) {
      if (ietype)
        debug_intg_inactive[ietype][0] = 1;
      else 
        for (int itype = 1; itype <= netypes; itype++)
          debug_intg_inactive[itype][0] = 1;
    } else if (strcmp(arg[iarg],"surface") == 0) {
      if (ietype)
        debug_intg_inactive[ietype][1] = 1;
      else 
        for (int itype = 1; itype <= netypes; itype++)
          debug_intg_inactive[itype][1] = 1;
    } else if (strcmp(arg[iarg],"edge") == 0) {
      if (ietype)
        debug_intg_inactive[ietype][2] = 1;
      else 
        for (int itype = 1; itype <= netypes; itype++)
          debug_intg_inactive[itype][2] = 1;
    } else if (strcmp(arg[iarg],"node") == 0) {
      if (ietype)
        debug_intg_inactive[ietype][3] = 1;
      else 
        for (int itype = 1; itype <= netypes; itype++)
          debug_intg_inactive[itype][3] = 1;
    } else error->all(FLERR,"Invalid debug_element_intg command");
    iarg++;
  }
}
