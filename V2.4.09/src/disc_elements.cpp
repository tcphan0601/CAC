...
enum{ATOM,ELEMENT};
enum{HEX2HEX,HEX2WEDGE,HEX2PYRTET,WEDGE2WEDGE,WEDGE2PYRTET,WEDGE2TET,WEDGE2TRAPE,PYR2TET,HEX2HEXQUAD};
    ...
    for (int i = 0; i < nlocal_previous; i++)
      if (disc_flag_list[i]) {
        if (split_style == HEX2HEX)
          ...
          evec->wedge2wedge(i,split_cell,1);
        else if (split_style == WEDGE2TRAPE)
          evec->wedge2trapeprism(i,split_cell,split_axis,1);
    ...
    for (int i = 0; i < nlocal_previous; i++)
      if (disc_flag_list[i]) {
        ...
        if (split_style == HEX2HEX)
          ...
          evec->wedge2wedge(i,split_cell,0);
        else if (split_style == WEDGE2TRAPE)
          evec->wedge2trapeprism(i,split_cell,split_axis,0);
        else if (split_style == WEDGE2PYRTET)
      ...
      if (strcmp(arg[iarg+1],"atom") == 0) {
        ...
        if (origin_shape_id == Element::HEXAHEDRON) {
          ...
          if (strcmp(arg[iarg+3],"Wedge") == 0) {
            ...
            iarg += 6;
          } else if (strcmp(arg[iarg+3],"TrapePrism") == 0) {
            split_style = WEDGE2TRAPE;
            if (iarg+6 > narg) error->all(FLERR,"Illegal disc_elements command");

            if (strcmp(arg[iarg+4],"yz") == 0) split_axis = 0;
            else if (strcmp(arg[iarg+4],"y") == 0) split_axis = 1;
            else if (strcmp(arg[iarg+4],"z") == 0) split_axis = 2;
            else error->all(FLERR,"Illegal disc_elements command");
