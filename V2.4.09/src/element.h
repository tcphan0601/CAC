...
class Element : protected Pointers {
 public: 
  ...
  enum{QUADRILATERAL,TRIANGLE,HEXAHEDRON,TETRAHEDRON,OCTAHEDRON,PYRAMID,WEDGE,TRAPEPRISM,PYRAMIDFRUSTUM,TRIFRUSTUM};
  ...
  int max_outer_intpl;            // max number of interpolated atoms on the element boundary
  ...
  double weight_scale[4];     // weight scale of each integration point type (node, edge, surface, inner)
  int weight_scale_flag;    // if weight scale of each integration point type (node, edge, surface, inner) is set
  ...
  void data_nodes_reference(int, char *, tagint, int, double *);
 ...
 public:
  int **debug_intg_type; // integration point type 
                         // 0 = inner
                         // 1 = surface
                         // 2 = edge
                         // 3 = node
  ...
  int **debug_intg_inactive;

  void debug_element_intg_off(int, char **);
};
};
