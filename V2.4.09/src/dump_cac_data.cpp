...
void DumpCACData::init_style()
{
  ...
  if (reference_flag) {
    ...
    delete [] format_elem;
    str = (char *) TAGINT_FORMAT " %d %d %g %g %g\n";
    n = strlen(str) + 2;
    format_elem = new char[n];
    strcpy(format_elem,str);

    delete [] format_node;
    str = (char *) TAGINT_FORMAT " %d %g %g %g %d %g %g %g\n";
    n = strlen(str) + 2;
    ...
    if (precision_flag) {
      ...
      delete [] format_elem;
      str = (char *) TAGINT_FORMAT " %d %d %-1.16e %-1.16e %-1.16e\n";
      n = strlen(str) + 2;
      format_elem = new char[n];
      strcpy(format_elem,str);

      delete [] format_node;
      str = (char *) TAGINT_FORMAT " %d %-1.16e %-1.16e %-1.16e %d\n";
      n = strlen(str) + 2;
      format_node = new char[n];
      strcpy(format_node,str);
    } else {
      ...
      delete [] format_node;
      str = (char *) TAGINT_FORMAT " %d %g %g %g %d\n";
      n = strlen(str) + 2;
      format_node = new char[n];
      strcpy(format_node,str);
    }
...
void DumpCACData::header_item(bigint ndump)
{
  ...
  fprintf(fp,"%d atom types",atom->ntypes); 
  if (nelem_total) 
    fprintf(fp,"%d element types",element->netypes); 
...
void DumpCACData::pack_elem_noscale_reference(tagint *ids)
{
  ...
  for (i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      inpe = npe[etype[i]]; 
      for (j = 0; j < inpe; j++) {
        xtmp += nodexoriginal[i][j][0]; 
        ytmp += nodexoriginal[i][j][1]; 
        ztmp += nodexoriginal[i][j][2]; 
      }
      buf[m++] = xtmp/inpe;
      buf[m++] = ytmp/inpe;
      buf[m++] = ztmp/inpe;
  ...
  for (i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) 
      for (j = 0; j < npe[etype[i]]; j++) {
        buf[m++] = tag[i];
        buf[m++] = j+1;
        buf[m++] = nodexoriginal[i][j][0];
        buf[m++] = nodexoriginal[i][j][1];
        buf[m++] = nodexoriginal[i][j][2];
        buf[m++] = nodetag[i][j];
        coord[0] = nodex[i][j][0];
        coord[1] = nodex[i][j][1];
        coord[2] = nodex[i][j][2];
...
void DumpCACData::pass_on_atom_arrays(int i, int i_old, int iintpl)
{
  if (reference_flag) {
    double **xoriginal = fix->astore;
    double ***nodexoriginal = fix->a3store;
...
void DumpCACData::pass_on_elem_arrays(int i, int i_old, int *iintpl_list)
{
  if (reference_flag) {
    double ***nodexoriginal = fix->a3store;
    int *npe = element->npe;
