...
PairHybrid::PairHybrid(CAC *cac) : Pair(cac),
...
void PairHybrid::compute(int eflag, int vflag)
{
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    if (eflag_atom) {
      ...
      for (i = 0; i < nelocal; i++) 
        for (j = 0; j < npe[etype[i]]; j++) enode[i][j] += enode_substyle[i][j];
    ...
    if (vflag_atom) {
      ...
      for (i = 0; i < nelocal; i++) 
        for (j = 0; j < npe[etype[i]]; j++) 
          for (l = 0; l < 6; l++) vnode[i][j][l] += vnode_substyle[i][j][l];
    }
  }
...
void PairHybrid::flags()
{
  ...
  // set comm_atom_forward, comm_atom_reverse, comm_atom_reverse_off,
  // comm_elem_forward, comm_elem_reverse, comm_elem_reverse_off to max of any sub-style

  for (m = 0; m < nstyles; m++) {
    if (styles[m]) comm_atom_forward = MAX(comm_atom_forward,styles[m]->comm_atom_forward);
    if (styles[m]) comm_elem_forward = MAX(comm_elem_forward,styles[m]->comm_elem_forward);
    if (styles[m]) comm_atom_reverse = MAX(comm_atom_reverse,styles[m]->comm_atom_reverse);
    if (styles[m]) comm_elem_reverse = MAX(comm_elem_reverse,styles[m]->comm_elem_reverse);
    if (styles[m]) comm_atom_reverse_off = MAX(comm_atom_reverse_off,
        styles[m]->comm_atom_reverse_off);
    if (styles[m]) comm_elem_reverse_off = MAX(comm_elem_reverse_off,
        styles[m]->comm_elem_reverse_off);
  }
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    if (strcmp(arg[2],keywords[m]) == 0) {
      if (multiple[m]) {
        ...
        int index = universe->inumeric(FLERR,arg[3]);
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
/* restart file not supported in CAC yet
...
int multiflag = universe->inumeric(FLERR,arg[2]);
for (m = 0; m < nstyles; m++)
...
void PairHybrid::modify_special(int m, int /*narg*/, char **arg)
{
  ...
  special[0] = 1.0;
  special[1] = universe->numeric(FLERR,arg[1]);
  special[2] = universe->numeric(FLERR,arg[2]);
  special[3] = universe->numeric(FLERR,arg[3]);
