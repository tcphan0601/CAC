...
Domain::Domain(CAC *cac) : Pointers(cac)
{
  ...
  prd_lambda[0] = prd_lambda[1] = prd_lambda[2] = 1.0;
  prd_half_lambda[0] = prd_half_lambda[1] = prd_half_lambda[2] = 0.5;
  boxlo_lambda[0] = boxlo_lambda[1] = boxlo_lambda[2] = 0.0;
  boxhi_lambda[0] = boxhi_lambda[1] = boxhi_lambda[2] = 1.0;
...
void Domain::set_lambda_box()
{
  if (comm->layout != LAYOUT_TILED) {
    ...
    sublo_lambda[0] = xsplit[myloc[0]];
    subhi_lambda[0] = xsplit[myloc[0]+1];
    sublo_lambda[1] = ysplit[myloc[1]];
    subhi_lambda[1] = ysplit[myloc[1]+1];
    sublo_lambda[2] = zsplit[myloc[2]];
    subhi_lambda[2] = zsplit[myloc[2]+1];

  } else {
    double (*mysplit)[2] = comm->mysplit;

    sublo_lambda[0] = mysplit[0][0];
    subhi_lambda[0] = mysplit[0][1];
    sublo_lambda[1] = mysplit[1][0];
    subhi_lambda[1] = mysplit[1][1];
    sublo_lambda[2] = mysplit[2][0];
    subhi_lambda[2] = mysplit[2][1];
  }
}
...
/* ----------------------------------------------------------------------
   ...
   for triclinic, atoms must be in lambda coords (0-1) before reset_box is called
   ------------------------------------------------------------------------- */
...
void Domain::reset_box()
{
  ...
  // for triclinic, this is done in lambda space
  ...
  if (nonperiodic == 2) {
    ...
    if (triclinic) {
      lambda2x(atom->nlocal,atom->x);
      lambda2x(element->nlocal,element->x);
      lambda2x(element->nlocal, element->npe, element->nodex);
    }

    // in shrink-wrapped dims, set box by atom extent
    // if minimum set, enforce min box size settings
    // for triclinic, convert lambda extent to box coords, then set box lo/hi
  ...
  */
  // if shrink-wrapped & triclinic, re-convert to lambda coords for new box
  // re-invoke pbc() b/c x2lambda result can be outside [0,1] due to roundoff

  if (nonperiodic == 2 && triclinic) {
    x2lambda(atom->nlocal, atom->x);
    x2lambda(element->nlocal, element->x);
    x2lambda(element->nlocal, element->npe, element->nodex);
    pbc();
  }
...
/* ----------------------------------------------------------------------
   ...
   for triclinic, atoms must be in lambda coords (0-1) before pbc is called
...
void Domain::pbc()
{
  ...
  for (i = 0; i < n3; i++)
    if (!ISFINITE(*coord++)) flag = 1;
  if (flag) error->one(FLERR,"Non-numeric element coords - simulation unstable");

  n3 = 3*nlocal*npe;
  coord = &nodex[0][0][0];  // note: nodex is always initialized to at least one element.
  flag = 0;
  for (i = 0; i < n3; i++)
    if (!ISFINITE(*coord++)) flag = 1;
  if (flag) error->one(FLERR,"Non-numeric node coords - simulation unstable");
...
int Domain::inside(double* x)
{
  ...
  if (triclinic == 0) {
    ...
    if (lambda[0] < lo[0] || lambda[0] >= hi[0] ||
        lambda[1] < lo[1] || lambda[1] >= hi[1] ||
        lambda[2] < lo[2] || lambda[2] >= hi[2]) return 0;
    else return 1;
...
int Domain::inside_nonperiodic(double* x)
{
  ...
  if (triclinic == 0) {
    ...
    if (!xperiodic && (lambda[0] < lo[0] || lambda[0] >= hi[0])) return 0;
    if (!yperiodic && (lambda[1] < lo[1] || lambda[1] >= hi[1])) return 0;
    if (!zperiodic && (lambda[2] < lo[2] || lambda[2] >= hi[2])) return 0;
    return 1;
  }
...
void Domain::subbox_too_small_check(double thresh)
{
  ...
  if (!triclinic) {
    ...
    double delta = subhi_lambda[0] - sublo_lambda[0];
    if (delta*prd[0] < thresh) flag = 1;
    delta = subhi_lambda[1] - sublo_lambda[1];
    if (delta*prd[1] < thresh) flag = 1;
    if (dimension == 3) {
      delta = subhi_lambda[2] - sublo_lambda[2];
...
/* ----------------------------------------------------------------------
   ...
   resulting coord must satisfy lo <= coord < hi
   MAX is important since coord - prd < lo can happen when coord = hi
   for triclinic, point is converted to lambda coords (0-1) before doing remap
...
/* ----------------------------------------------------------------------
   ...
   resulting coord must satisfy lo <= coord < hi
   MAX is important since coord - prd < lo can happen when coord = hi
   for triclinic, point is converted to lambda coords (0-1) before remap
   ------------------------------------------------------------------------- */
...
void Domain::remap(double *x, double **nodex, imageint &image)
{
  double *lo,*hi,*period,*coord;
  double lambda[3];
  imageint idim,otherdims;
  ...
  if (xperiodic) {
    while (coord[0] < lo[0]) {
      coord[0] += period[0];
      for (int i = 0; i < npe; i++) nodex[i][0]+= period[0];
      idim = image & IMGMASK;
      otherdims = image ^ idim;
      idim--;
      idim &= IMGMASK;
    ...
    if (coord[0] < lo[0]){
      for (int i = 0; i < npe; i++) nodex[i][0] += lo[0] - coord[0];
      coord[0] = lo[0];
    }
  }
  ...
  if (yperiodic) {
    while (coord[1] < lo[1]) {
      coord[1] += period[1];
      for (int i = 0; i < npe; i++) nodex[i][1]+= period[1];
      idim = (image >> IMGBITS) & IMGMASK;
      otherdims = image ^ (idim << IMGBITS);
      idim--;
      idim &= IMGMASK;
    ...
    if (coord[1] < lo[1]){
      for (int i = 0; i < npe; i++) nodex[i][1] += lo[1] - coord[1];
      coord[1] = lo[1];
    }
  ...
  if (zperiodic) {
    while (coord[2] < lo[2]) {
      coord[2] += period[2];
      for (int i = 0; i < npe; i++) nodex[i][2]+= period[2];
      idim = image >> IMG2BITS;
      otherdims = image ^ (idim << IMG2BITS);
      idim--;
      idim &= IMGMASK;
    ...
    if (coord[2] < lo[2]){
      for (int i = 0; i < npe; i++) nodex[i][2] += lo[2] - coord[2];
      coord[2] = lo[2];
    }
  } 
}
/* ----------------------------------------------------------------------
   ...
   for triclinic, both points are converted to lambda coords (0-1) before remap
   ------------------------------------------------------------------------- */
...
void Domain::remap_near(double *xnew, double *xold)
{
  ...
  double lambdanew[3],lambdaold[3];
  ...
  if (triclinic == 0) {
    ...
    period = prd_lambda;
    half = prd_half_lambda;
    x2lambda(xnew,lambdanew);
    coordnew = lambdanew;
    x2lambda(xold,lambdaold);
    coordold = lambdaold;
  }
  ...
  if (triclinic) lambda2x(coordnew,xnew);
}
...
/* ----------------------------------------------------------------------
   convert triclinic 0-1 lambda coords to box coords for all N atoms/elements
   x = H lambda + x0;
   ------------------------------------------------------------------------- */

void Domain::lambda2x(int n, double **x)
{
  for (int i = 0; i < n; i++) {
...
/* ----------------------------------------------------------------------
   convert box coords to triclinic 0-1 lambda coords for all N atoms/elements
   lambda = H^-1 (x - x0)
   ------------------------------------------------------------------------- */

void Domain::x2lambda(int n, double **x)
{
  double delta[3];
...
/* ----------------------------------------------------------------------
   convert triclinic 0-1 lambda coords to box coords for all N nodes
   x = H lambda + x0;
   ------------------------------------------------------------------------- */

void Domain::lambda2x(int n1, int n2, double ***x)
{
  for (int i = 0; i < n1; i++) 
    for (int j = 0; j < n2; j++) {
      x[i][j][0] = h[0]*x[i][j][0] + h[5]*x[i][j][1] + h[4]*x[i][j][2] + boxlo[0];
      x[i][j][1] = h[1]*x[i][j][1] + h[3]*x[i][j][2] + boxlo[1];
      x[i][j][2] = h[2]*x[i][j][2] + boxlo[2];
    }
}

/* ----------------------------------------------------------------------
   convert box coords to triclinic 0-1 lambda coords for all N nodes
   lambda = H^-1 (x - x0)
   ------------------------------------------------------------------------- */

void Domain::x2lambda(int n1, int n2, double ***x)
{
  double delta[3];

  for (int i = 0; i < n1; i++) 
    for (int j = 0; j < n2; j++) {
      delta[0] = x[i][j][0] - boxlo[0];
      delta[1] = x[i][j][1] - boxlo[1];
      delta[2] = x[i][j][2] - boxlo[2];

      x[i][j][0] = h_inv[0]*delta[0] + h_inv[5]*delta[1] + h_inv[4]*delta[2];
      x[i][j][1] = h_inv[1]*delta[1] + h_inv[3]*delta[2];
      x[i][j][2] = h_inv[2]*delta[2];
    }
}


/* ----------------------------------------------------------------------
   convert triclinic 0-1 lambda coords to box coords for one atom
   x = H lambda + x0;
   lambda and x can point to same 3-vector
   ------------------------------------------------------------------------- */

void Domain::lambda2x(double *lambda, double *x)
{
  x[0] = h[0]*lambda[0] + h[5]*lambda[1] + h[4]*lambda[2] + boxlo[0];
  x[1] = h[1]*lambda[1] + h[3]*lambda[2] + boxlo[1];
  x[2] = h[2]*lambda[2] + boxlo[2];
}

/* ----------------------------------------------------------------------
   convert box coords to triclinic 0-1 lambda coords for one atom
   lambda = H^-1 (x - x0)
   x and lambda can point to same 3-vector
   ------------------------------------------------------------------------- */

void Domain::x2lambda(double *x, double *lambda)
{
  double delta[3];
  delta[0] = x[0] - boxlo[0];
  delta[1] = x[1] - boxlo[1];
    ...
    double *my_boxlo, double *my_h_inv)
{
  ...
  lambda[0] = my_h_inv[0]*delta[0] + my_h_inv[5]*delta[1] + my_h_inv[4]*delta[2];
  lambda[1] = my_h_inv[1]*delta[1] + my_h_inv[3]*delta[2];
  lambda[2] = my_h_inv[2]*delta[2];
}

/* ----------------------------------------------------------------------
   convert 8 lambda corner pts of lo/hi box to box coords
...
/* ----------------------------------------------------------------------
   ...
   output is in corners, see ordering in lambda_box_corners
   ------------------------------------------------------------------------- */

void Domain::box_corners()
{
  lambda_box_corners(boxlo_lambda,boxhi_lambda);
}
...
{
  lambda_box_corners(sublo_lambda,subhi_lambda);
}

/* ----------------------------------------------------------------------
   compute 8 corner pts of any triclinic box with lo/hi in lambda coords
