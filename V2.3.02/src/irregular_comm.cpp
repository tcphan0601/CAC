...
/* ----------------------------------------------------------------------
   ...
   for triclinic: atoms/elements must be in lambda coords (0-1) before this is called
------------------------------------------------------------------------- */
...
void IrregularComm::migrate(int sortflag, int preassign, int *procassign)
{
  ...
  if (triclinic == 0) {
    ...
    sublo = domain->sublo_lambda;
    subhi = domain->subhi_lambda;
  }
  ...
  //   in triclinic x2lambda(), then keep atom and don't send
