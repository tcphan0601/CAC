...
void ElementVecRHB::grow_etype_arrays(int n)
{
  ...
  ias2ia = memory->grow(element->ias2ia,n+1,nsubelem,element->maxsubintpl,"element:i2ia");
  ...
  for (int itype = old_netypes + 1; itype <= n; itype++) {
    ...
    nintg[itype] = 0; 
    nintpl[itype] = 0; 
    for (int iintg = 0; iintg < element->maxintg; iintg++) {
      i2ia[itype][iintg] = -1;
      i2n[itype][iintg] = -1;
    }
...
void ElementVecRHB::set_intg_user(char *str, int type_offset)
{
  ...
  if (i2ia[itype][iintg] < 0 )
    i2ia[itype][iintg] = nx*niay*niaz + ny*niaz + nz; 
  else error->all(FLERR,"Duplicate integration points in data file");
  ...
  if (i2ia[itype][iintg] < 0) i2ia[itype][iintg] = iintpl;
    ...
    for (j = 0; j < niy; j++){
      ...
      if (domain->dimension == 3) {
        for (k = 0; k < niz; k++){
          ...
          if (w == 3) weight[itype][n] = 1.0;

          // integration point is on an edge

          else if (w == 2) {
            // integration point is on edge in x direction
            if (!x) weight[itype][n] = w_edge_x*wgauss[nix-2][i];
            // integration point is on edge in y direction
            else if (!y) weight[itype][n] = w_edge_y*wgauss[niy-2][j];
            // integration point is on edge in z direction
            else if (!z) weight[itype][n] = w_edge_z*wgauss[niy-2][k];

            // integration point is on a face

          } else if (w == 1) {
            // integration point is on yz face 
            if (x) weight[itype][n] = w_face_yz*wgauss[niy-2][j]*wgauss[niz-2][k];
            // integration point is on xz face 
            else if (y) weight[itype][n] = w_face_xz*wgauss[nix-2][i]*wgauss[niz-2][k];
            // integration point is on xy face 
            else if (z) weight[itype][n] = w_face_xy*wgauss[nix-2][i]*wgauss[niy-2][j];

            // integration point is inside element

          } else weight[itype][n] = w_inner*wgauss[nix-2][i]*wgauss[niy-2][j]*wgauss[niz-2][k];
          ...
          i2ia[itype][n] = nx*niay*niaz + ny*niaz + nz;

          // list to convert node index to integration point index

          for (l = 0; l < npe; l++)
            if (fabs(shape_array[itype][i2ia[itype][n]][l]-1.0) < EPSILON) {
              if (n2i[itype][l] < 0) {
                n2i[itype][l] = n;
                i2n[itype][n] = l;
              } else error->all(FLERR,"Node to integration point mapping is not correct");
        ...
        if (w == 2) weight[itype][n] = 1.0;

        // integration point is on an edge

        else if (w == 1) {
          // integration point is on edge in x direction
          if (!x) weight[itype][n] = w_edge_x;
          // integration point is on edge in y direction
          else if (!y) weight[itype][n] = w_edge_y;

          // integration point is inside element

        } else weight[itype][n] = w_inner;
