...
void FixPressBerendsen::remap()
{
  ...
  // convert pertinent atoms/elements and rigid bodies to lambda coords

  if (allremap) {
    domain->x2lambda(nalocal,ax);
    domain->x2lambda(nelocal,npe,nodex);
  } else {
    for (i = 0; i < nalocal; i++)
      if (amask[i] & groupbit)
        domain->x2lambda(ax[i],ax[i]);
    for (i = 0; i < nelocal; i++)
      if (emask[i] & groupbit)
        for (j = 0; j < npe; j++)
          domain->x2lambda(nodex[i][j],nodex[i][j]);
  }
  ...
  if (allremap) {
    domain->lambda2x(nalocal,ax);
    domain->lambda2x(nelocal,npe,nodex);
  } else {
    for (i = 0; i < nalocal; i++)
      if (amask[i] & groupbit)
        domain->lambda2x(ax[i],ax[i]);
    for (i = 0; i < nelocal; i++)
      if (emask[i] & groupbit)
        for (j = 0; j < npe; j++)
          domain->lambda2x(nodex[i][j],nodex[i][j]);
  }
...
int FixPressBerendsen::modify_param(int narg, char **arg)
{
  if (strcmp(arg[0],"temp") == 0) {
    error->all(FLERR,"No temperature compute yet");
