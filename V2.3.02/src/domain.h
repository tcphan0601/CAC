...
class Domain : protected Pointers {
 public:
  ...
  double prd_half[3];                    // array form of half dimensions
                                         ...
                                         // same as if untilted
  double prd_lambda[3];                   // lambda box = (1,1,1)
  double prd_half_lambda[3];              // lambda half box = (0.5,0.5,0.5)

  double boxlo[3],boxhi[3];              // orthogonal box global bounds

                                         // triclinic box
                                         // boxlo/hi = same as if untilted
  double boxlo_lambda[3],boxhi_lambda[3];  // lambda box = (0,1)
  ...
  double sublo[3],subhi[3];              // sub-box bounds on this proc
                                         ...
                                         // triclinic box
                                         // sublo/hi = undefined
  double sublo_lambda[3],subhi_lambda[3];  // bounds of subbox in lambda
...
//  class Lattice *lattice;                  // user-defined lattice
  ...
  void closest_image(const double * const, const double * const,
                     double * const);
  void remap(double *, double **, imageint &);
...
//  void set_lattice(int, char **);
  ...
  virtual void lambda2x(int, double **);
  virtual void x2lambda(int, double **);
  virtual void lambda2x(int, int, double ***);
  virtual void x2lambda(int, int, double ***);
  virtual void lambda2x(double *, double *);
  virtual void x2lambda(double *, double *);
  int inside(double *);
  int inside_nonperiodic(double *);
  void x2lambda(double *, double *, double *, double *);
  void bbox(double *, double *, double *, double *);
  void box_corners();
  void subbox_corners();
  void lambda_box_corners(double *, double *);
