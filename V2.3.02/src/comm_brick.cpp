...
void CommBrick::setup()
{
  ...
  if (triclinic == 0) {
    ...
    domain->x2lambda(element_size_x, element_size_lambda);

    length0 = sqrt(h_inv[0]*h_inv[0] + h_inv[5]*h_inv[5] + h_inv[4]*h_inv[4]);
    cutghost[0] = cut * length0 + element_size_lambda[0];
    length1 = sqrt(h_inv[1]*h_inv[1] + h_inv[3]*h_inv[3]);
    cutghost[1] = cut * length1 + element_size_lambda[1];
    length2 = h_inv[2];
    cutghost[2] = cut * length2 + element_size_lambda[2];
...
/* ----------------------------------------------------------------------
   ...
   for triclinic, atoms/elements must be in lambda coords (0-1) before borders is called
   ------------------------------------------------------------------------- */
