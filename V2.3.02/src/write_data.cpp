...
void WriteData::header()
{
  ...
  if (element->nelements) fprintf(fp,BIGINT_FORMAT " elements\n",element->nelements);
  fprintf(fp,"%d atom types\n",atom->ntypes);
  if (element->nelements) fprintf(fp,"%d element types\n",element->netypes);
...
void WriteData::type_arrays()
{
  ...
  if (element->nelements) {
    ...
    // write out Interpolate section
    
    fprintf(fp,"\nInterpolate\n\n");
    element->evec->write_interpolate(fp);

    // write out Integration section

    if (element->intg_input_style == 2) fprintf(fp,"\nIntegration List\n\n");
    else fprintf(fp,"\nIntegration\n\n");
    element->evec->write_integration(fp);
  } 
}
...
void WriteData::nodes()
{
  ...
  bigint sendrow = element->nlocal*element->npe;
  bigint maxrow;
...
void WriteData::node_velocities()
{
  // communication buffer for all my Atom info
  // max_size = largest buffer needed by any proc
  ...
  int sendrow = element->nlocal*element->npe;
  int maxrow;
  MPI_Allreduce(&sendrow,&maxrow,1,MPI_INT,MPI_MAX,world);
  ...
  if (me == 0) {
    ...
    for (int iproc = 0; iproc < nprocs; iproc++) {
      ...
      element->evec->write_vel(fp,recvrow,buf);
    }
