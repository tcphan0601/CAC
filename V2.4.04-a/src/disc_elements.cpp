...
void DiscElements::command(int narg, char **arg)
{
  ...
  if (disc_style == ATOM) {
    ...
    for (int i = 0; i < nlocal_previous; i++)
      if (disc_flag_list[i]) {
        if (split_style == HEX2HEX)
          element->evec->hex2hex(i,split_cell,split_dim,1);
        else if (split_style == HEX2WEDGE)
          element->evec->hex2wedge(i,split_axis,split_dir,larger_side,1);
      }
    ...
    // update new global nelements, global nnodes
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"compress") == 0) {
      ...
      if (strcmp(arg[iarg+1],"atom") == 0) {
        ...
        target_shape_id = element->find_element_shape_id(arg[iarg+3]);
        if (origin_shape_id < 0)
            error->all(FLERR,"Illegal origin element shape in disc_elements command");
        if (target_shape_id < 0)
            error->all(FLERR,"Illegal target element shape in disc_elements command");
        if (origin_shape_id == Element::HEXAHEDRON) {
          if (target_shape_id == Element::HEXAHEDRON) {
            split_style = HEX2HEX;
            ...
            iarg += 6;
          } else if (target_shape_id == Element::WEDGE) {
