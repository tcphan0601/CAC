...
CreatePolycrystal::CreatePolycrystal(CAC *cac) : Pointers(cac) {}

/* ---------------------------------------------------------------------- */

void CreatePolycrystal::command(int narg, char **arg)
{
  ...
  if (narg < 2) error->all(FLERR,"Illegal create_polycrystal command");
  ...
  if (strcmp(arg[2],"box") == 0) {
    ...
    style = REGION;
    if (narg < 4) error->all(FLERR,"Illegal create_polycrystal command");
    ...
    iarg = 4;
  } else error->all(FLERR,"Illegal create_polycrystal command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"basis") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal create_polycrystal command");
      ...
      if (ibasis <= 0 || ibasis > nbasis || ictype <= 0 || ictype > atom->ntypes)
        error->all(FLERR,"Invalid basis setting in create_polycrystal command");
      basisctype[ibasis-1] = ictype;
      iarg += 3;
    } else if (strcmp(arg[iarg],"remap") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal create_polycrystal command");
    ...
    //  if (style != SINGLE)
    //    error->all(FLERR,"Cannot use create_polycrystal rotate unless single style");
    //  if (iarg+5 > narg) error->all(FLERR,"Illegal create_polycrystal command");
    //  double thetaone;
    //  double axisone[3];
    ...
    //    error->all(FLERR,"Illegal create_polycrystal command");
    //  if (domain->dimension == 2 && (axisone[0] != 0.0 || axisone[1] != 0.0))
    //    error->all(FLERR,"Invalid create_polycrystal rotation vector for 2d model");
  ...
  if (nctype <= 0 || nctype > atom->ntypes)
    error->all(FLERR,"Invalid atom type in create_polycrystal command");
  if (netype <= 0 || netype > element->netypes)
    error->all(FLERR,"Invalid element type in create_polycrystal command");
  ...
  //if (style == RANDOM) {
  //  if (nrandom < 0) error->all(FLERR,"Illegal create_polycrystal command");
  //  if (seed <= 0) error->all(FLERR,"Illegal create_polycrystal command");
  //}

  // error check and further setup for variable test

  if (!vstr && (xstr || ystr || zstr))
    error->all(FLERR,"Incomplete use of variables in create_polycrystal command");
  if (vstr && (!xstr && !ystr && !zstr))
    error->all(FLERR,"Incomplete use of variables in create_polycrystal command");
  ...
  if (varflag) {
    ...
    if (vvar < 0)
      error->all(FLERR,"Variable name for create_polycrystal does not exist");
    if (!input->variable->equalstyle(vvar))
      error->all(FLERR,"Variable for create_polycrystal is invalid style");
  ...
  fprintf(screen,"me = %d nlocal = %d nelements = %d\n",comm->me,element->nlocal,element->nelements);
  ...
  //else add_lattice();
  add_lattice();
  fprintf(screen,"after adding elements, me = %d nlocal = %d nelements = %d elementx = %g %g %g\n",
      comm->me,element->nlocal,element->nelements,
      element->x[0][0],element->x[0][1],element->x[0][2] 
      );
...
void CreatePolycrystal::add_single()
{
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
/*
   void CreatePolycrystal::add_random()
   {
...
error->all(FLERR,"No overlap of box and region for create_polycrystal");
...
void CreatePolycrystal::add_lattice()
{
  ...
  int nx = element->evec->ncells[netype][0];
  int ny = element->evec->ncells[netype][1];
  int nz = element->evec->ncells[netype][2];
...
int CreatePolycrystal::vartest(double *x)
{
