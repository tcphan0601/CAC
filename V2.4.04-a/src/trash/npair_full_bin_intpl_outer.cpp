#include "npair_full_bin_intpl_outer.h"
#include "neighbor.h"
...
NPairFullBinIntplOuter::NPairFullBinIntplOuter(CAC *cac) : NPair(cac) {}

/* ----------------------------------------------------------------------
   binned neighbor list construction for all neighbors
   every neighbor pair appears in list of both atoms i and j
   build list for atoms, elements, and outer interpolated atoms
------------------------------------------------------------------------- */

void NPairFullBinIntplOuter::build(NeighList *list)
{
  ...
  int iintpl,jintpl,iouter,inode,jnode,jnpe;
  ...
  int *nouter_intpl = element->nouter_intpl;
  int **outer_intpl = element->outer_intpl;
  ...
  /*------------------------------------------------------------------- 
   * OUTER INTERPOLATED ATOM NEIGHBORS
   * loop over each outer interpolated atom and store neighbors 
   * for outer interpolated atoms
   *-----------------------------------------------------------------*/ 
  ...
  for (ii = 0; ii < einum; ii++) {
    ...
    // store list of mapping between element index and its first outer interpolated atom index

    e2ialist[i] = iainum; 

    // loop over outer integration points in element i

    for (iouter = 0; iouter < nouter_intpl[ietype]; iouter++) {

      // index of outer interpolated atom
      
      iintpl = outer_intpl[ietype][iouter];

      // store list of mapping between outer interpolated atom index and element index

      ia2elist[iainum] = i;

      // interpolating positions for outer interpolated atom I
