...
Fix::Fix(CAC *cac, int narg, char **arg) : 
  ...
  vector_elem(NULL), array_elem(NULL), vector_node(NULL), array_node(NULL),
  vector_local(NULL), array_local(NULL), eatom(NULL), vatom(NULL), 
  eintpl(NULL), vintpl(NULL)
{
...
Fix::~Fix()
{
  ...
  memory->destroy(eintpl);
  memory->destroy(vintpl);
...
void Fix::ev_setup(int eflag, int vflag)
{
  ...
  if (eflag_atom) {
    ...
    if (element->nlocal > maxeelem) {
      ...
      memory->destroy(eintpl);
      memory->create(eintpl,maxeelem,element->maxintpl,"fix:eintpl");
    }
  ...
  if (vflag_atom) {
    ...
    if (element->nlocal > maxvelem) {
      ...
      memory->destroy(vintpl);
      memory->create(vintpl,maxvelem,element->maxintpl,"fix:vintpl");
    }
