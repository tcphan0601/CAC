      ...
      for (j = 0; j < apc[etype[i]]; j++) 
        for (k = 0; k < npe[etype[i]]; k++) {
          for (int idisl = 0; idisl < ndisl; idisl++) {
            ...
            double bscrew = b[idisl] * cos(disl_angle[idisl]/180 * MY_PI);
            x1 = direction[idisl] *
              (nodex[i][j][k][dim1[idisl]] - center1[idisl]);
            x2 = direction[idisl] *
              (nodex[i][j][k][dim2[idisl]] - center2[idisl]);
            u[idisl] = u1[idisl] = u2[idisl] = 0.0;
            edge_dislocation_field(u1[idisl], u2[idisl], x1, x2, bedge, v[idisl], theta[idisl]);
            screw_dislocation_field(u[idisl], x1, x2, bscrew, theta[idisl]);
          }
          for (int idisl = 0; idisl < ndisl; idisl++) {
            nodex[i][j][k][dim1[idisl]] += u1[idisl];
            nodex[i][j][k][dim2[idisl]] += u2[idisl];
            nodex[i][j][k][d_dim[idisl]] += u[idisl];
          }
        }
    element->evec->update_center_coord();
  }
