...
enum{ATOMIC, MESHGRID}

/*  ----------------------------------------------------------------------  */

ComputeStressMech::ComputeStressMech(CAC *cac, int narg, char **arg) :
  Compute(cac, narg, arg), 
  atom_stress(NULL), node_stress(NULL)
{
  if (narg < 4) error->all(FLERR, "Illegal compute stress/mech command");

  peratom_flag = 1;
  size_peratom_cols = 18;
  mechatomflag = 1;
  timeflag = 1;
  comm_atom_reverse = 18;
  comm_elem_reverse = 18 * element->maxnpe * element->maxapc;

  a = universe->numeric(FLERR, arg[3]);
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "style") == 0) {
      if (strcmp(arg[iarg], "atomic") == 0) style = ATOMIC;
      else if (strcmp(arg[iarg], "mesh/grid") == 0) style = MESHGRID;
      else error->all(FLERR, "Illegal compute stress/mech command");
    } else error->all(FLERR, "Illegal compute stress/mech command");
    iarg++;
  }
...
void ComputeStressMech::compute_peratom()
{
  int i, j, k;
  double onemass;
  ...
  if (atom->nmax > atom_nmax) {
    memory->destroy(atom_stress);
    atom_nmax = atom->nmax;
    array_atom = memory->create(atom_stress, atom_nmax, 6, "stress/mech:atom_stress");
  }

  if (element->nmax > elem_nmax || maxnpe < element->maxnpe) {
    memory->destroy(node_stress);
    elem_nmax = element->nmax;
    array_node = memory->create(node_stress, elem_nmax, element->maxapc, 
        element->maxnpe, 6, "stress/mech:atom_stress");
  }
  ...
  memset(&atom_stress[0][0], 0, atom_ntotal * 6 * sizeof(double));
  memset(&node_stress[0][0][0][0], 0, elem_ntotal * element->maxapc * 
      element->maxnpe * 6 * sizeof(double));
    ...
    for (i = 0; i < elem_ntotal; i++)
      for (j = 0; j < apc[etype[i]]; j++) 
        for (k = 0; k < npe[etype[i]]; k++) {
          node_stress[i][j][k][0] += vnode[i][j][k][0];
          node_stress[i][j][k][1] += vnode[i][j][k][1];
          node_stress[i][j][k][2] += vnode[i][j][k][2];
          node_stress[i][j][k][3] += vnode[i][j][k][3];
          node_stress[i][j][k][4] += vnode[i][j][k][4];
          node_stress[i][j][k][5] += vnode[i][j][k][5];
        }
  }
  ...
  for (i = 0; i < elem_nlocal; i++) 
    if (!(elem_mask[i] & groupbit)) 
      for (j = 0; j < apc[etype[i]]; j++) 
        for (k = 0; k < npe[etype[i]]; k++) {
          node_stress[i][j][k][0] = 0.0;
          node_stress[i][j][k][1] = 0.0;
          node_stress[i][j][k][2] = 0.0;
          node_stress[i][j][k][3] = 0.0;
          node_stress[i][j][k][4] = 0.0;
          node_stress[i][j][k][5] = 0.0;
        } 
  ...
  for (i = 0; i < elem_nlocal; i++)
    if (elem_mask[i] & groupbit) 
      for (j = 0; j < apc[etype[i]]; j++) 
        for (k = 0; k < npe[etype[i]]; k++) {
          node_stress[i][j][k][0] *= nktv2p;
          node_stress[i][j][k][1] *= nktv2p;
          node_stress[i][j][k][2] *= nktv2p;
          node_stress[i][j][k][3] *= nktv2p;
          node_stress[i][j][k][4] *= nktv2p;
          node_stress[i][j][k][5] *= nktv2p;
        }
}

/*  ----------------------------------------------------------------------  */

int ComputeStressMech::pack_atom_reverse_comm(int n, int first, double *buf)
{
  int i, m, last;
...
void ComputeStressMech::unpack_atom_reverse_comm(int n, int *list, double *buf)
{
  int i, j, m;
...
int ComputeStressMech::pack_elem_reverse_comm(int n, int first, double *buf)
{
  int i, j, k, m, last;
  int *etype = element->etype;
  int *npe = element->npe;
...
void ComputeStressMech::unpack_elem_reverse_comm(int n, int *list, double *buf)
{
  int i, j, k, l, m;
  int *etype = element->etype;
  int *npe = element->npe;
...
double ComputeStressMech::memory_usage()
{
  ...
  bytes += elem_nmax * 6 * element->maxapc * element->maxnpe * sizeof(double);
  return bytes;
}
