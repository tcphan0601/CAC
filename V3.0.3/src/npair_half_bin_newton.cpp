...
NPairHalfBinNewton::NPairHalfBinNewton(CAC *cac) : NPair(cac) {}
...
{
  int i, j, jj, jnum, k, l, m, n, iindex;
  ...
  *loop over each atom, storing neighbors for atoms
  ...
  for (i = 0; i < nalocal; i++) {
    ...
    for (j = atombins[i]; j >= 0; j = atombins[j]) {
      if (j >= nalocal) {
    ...
    // loop over all atoms in other atom bins in stencil, store every pair
              ...
              for (l = 0; l < nucell_subelem[jetype][isub]; l++) {
                ...
                for (jbasis = 0; jbasis < japc; jbasis++) {
                  ...
                  if (jgcell >= 0) {
                    if (j < nelocal) {
                      if (i > j) continue;
                      else if (i == j && iindex >= jgcell * japc + jbasis) continue;
                    } else {
                      if (ex[j][2] < ex[i][2]) continue;
                      if (ex[j][2] == ex[i][2]) {
                        if (ex[j][1] < ex[i][1]) continue;
                        if (ex[j][1] == ex[i][1] && ex[j][0] < ex[i][0]) continue;
                      }
                    }
                  }
