/*  - * - c++ - * - ----------------------------------------------------------
   CAC - Large-scale Atomic/Molecular Massively Parallel Simulator
 ...
 */
  ...
  typedef Dump *( * DumpCreator)(CAC *, int, char **);
...
//  void create_restart(int, char **); // create Restart and restart files
 ...
 private:
  template <typename T> static Dump *dump_creator(CAC *, int, char **);
};
