...
class Pair : protected Pointers {
 ...
 public:
  ...
  double *eatom, **vatom, **matom;             // accumulated per-atom energy/virial/mech
  double ***enode, ****vnode, ****mnode;       // accumulated per-node energy/virial/mech
  ...
  virtual double init_one_inner(int, int) {return 0.0;}
  ...
  virtual void pair_u2ga(int, int, double *, int, int, double *, double *) {}
  void compute_dummy(int, int, int);
 ...
 protected:
  void ev_init(int eflag, int vflag, int mflag, int alloc = 1) {
    if (eflag || vflag || mflag) ev_setup(eflag, vflag, mflag, alloc);
    else ev_unset();
  }
  virtual void ev_setup(int, int, int, int alloc = 1);
  void ev_unset();
  void ev_tally(double, double, double *, double *, double *, double *, 
      double, double, double, double, double, double);
  void ev_tally3(double, double, double *, double *, 
