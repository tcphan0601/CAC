...
enum{ATOM, ELEMENT};
enum{HEX2HEX, HEX2MULTIHEX, HEX2WEDGE, HEX2PYRTET, WEDGE2WEDGE, WEDGE2PYRTET, WEDGE2TET, PYR2TET, HEX2HEXQUAD};
  ...
  if (disc_style == ATOM) {
    ...
    for (int i = 0; i < nlocal_previous; i++)
      if (disc_flag_list[i]) {
        ...
        else if (split_style == HEX2MULTIHEX)
          evec->hex2multihex(i, split_x, split_y, split_z, 1);
    ...
    for (int i = 0; i < nlocal_previous; i++)
      if (disc_flag_list[i]) {
        ...
        if (split_style == HEX2HEX)
          ...
          evec->hex2multihex(i, split_x, split_y, split_z, 0);
        else if (split_style == HEX2HEXQUAD)
      ...
      if (strcmp(arg[iarg+1], "atom") == 0) {
        ...
        if (origin_shape_id == Element::HEXAHEDRON) {
          ...
          if (strcmp(arg[iarg+3], "Hex") == 0) {
            ...
            iarg += 6;
          } else if (strcmp(arg[iarg+3], "MultiHex") == 0) {
            split_style = HEX2MULTIHEX;
            if (iarg+7 > narg) error->all(FLERR, "Illegal disc_elements command");
            split_x = universe->inumeric(FLERR, arg[iarg+4]);
            split_y = universe->inumeric(FLERR, arg[iarg+5]);
            split_z = universe->inumeric(FLERR, arg[iarg+6]);
            iarg += 7;
