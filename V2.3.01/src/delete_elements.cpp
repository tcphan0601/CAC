...
if (domain->triclinic) domain->x2lambda(element->nlocal);
domain->pbc();
domain->reset_box();
comm->setup();
...
comm->exchange();
comm->borders();
if (domain->triclinic) domain->lambda2x(element->nlocal+element->nghost);
neighbor->build();
...
/*
   ...
   RanMars *random = new RanMars(cac,seed + comm->me);
