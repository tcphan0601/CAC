...
double Pair::memory_usage()
{
  ...
  bytes += maxeelem*element->npe*sizeof(double);
  bytes += maxvelem*element->npe*6*sizeof(double);
  return bytes;
}
/* ----------------------------------------------------------------------
...
------------------------------------------------------------------------- */
void Pair::atom_v_tally(int i,double fpair, double delx, double dely, double delz)
{
  double v[6];
  v[0] = delx*delx*fpair;
  v[1] = dely*dely*fpair;
...
/* ----------------------------------------------------------------------
   tally virial into per-node accumulators
   called by EAM potential
   ------------------------------------------------------------------------- */
void Pair::node_v_tally(int i, int inode, double fpair, double delx, double dely, double delz, double scale)
{
  double v[6];
  v[0] = delx*delx*fpair;
  v[1] = dely*dely*fpair;
  ...
  v[5] = dely*delz*fpair;
  if (vflag_global) {
    virial[0] += 0.5*v[0]/scale;
    virial[1] += 0.5*v[1]/scale;
    virial[2] += 0.5*v[2]/scale;
    virial[3] += 0.5*v[3]/scale;
    virial[4] += 0.5*v[4]/scale;
    virial[5] += 0.5*v[5]/scale;
  }
  if (vflag_atom) {
    vnode[i][inode][0] += 0.5*v[0];
    vnode[i][inode][1] += 0.5*v[1];
    vnode[i][inode][2] += 0.5*v[2];
    vnode[i][inode][3] += 0.5*v[3];
    vnode[i][inode][4] += 0.5*v[4];
    vnode[i][inode][5] += 0.5*v[5];
  }
}
