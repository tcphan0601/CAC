...
void NBinStandard::setup_bins(int style)
{
  ...
  //   subdomain is in lambda coords
  //   include dimension-dependent extension via comm->cutghost
  //   domain->bbox() converts lambda extent to box coords and computes bbox
  ...
  if (triclinic == 0) {
    ...
    double lo[3],hi[3];
    lo[0] = domain->sublo_lambda[0] - cutghost[0];
    lo[1] = domain->sublo_lambda[1] - cutghost[1];
    lo[2] = domain->sublo_lambda[2] - cutghost[2];
    hi[0] = domain->subhi_lambda[0] + cutghost[0];
    hi[1] = domain->subhi_lambda[1] + cutghost[1];
    hi[2] = domain->subhi_lambda[2] + cutghost[2];
  ...
  double elem_cut = 0.0;
  if (element->nelements) elem_cut = element->max_size;
