...
class RunTab(ttk.Frame):
    ...
    def _build(self):
        cfg = tk.Frame(self, bg=P["bg2"], padx=12, pady=10)
        cfg.pack(fill="x")

        def row(label, var, r, browse=None):
            tk.Label(cfg, text=label, fg=P["text_dim"], bg=P["bg2"],
                     font=FU, anchor="w", width=20
                     ).grid(row=r, column=0, sticky="w", pady=3)
            tk.Entry(cfg, textvariable=var,
                     bg=P["bg3"], fg=P["text"],
                     insertbackground=P["accent"],
                     relief="flat", font=FMS, bd=4
                     ).grid(row=r, column=1, sticky="ew", padx=4, pady=3)
            if browse:
                tk.Button(cfg, text="Browse", command=browse,
                          bg=P["bg3"], fg=P["accent"], relief="flat",
                          font=FU, bd=0, padx=8, cursor="hand2"
                          ).grid(row=r, column=2, padx=4)
            cfg.columnconfigure(1, weight=1)

        row("CAC Executable",    self.app.cac_exe_var, 0, self._br_exe)
        tk.Checkbutton(cfg,
            text="Set as default executable",
        ...
        row("Working Dir",       self.app.workdir_var, 2, self._br_dir)
        row("Input Script",      self.app.script_var,  3, self._br_script)
        row("MPI ranks (0=off)", self.app.mpi_var,     4)
        row("module load",       self.app.module_var,  5)
        tk.Label(cfg,
            text="(space-separated, e.g.  CAC  intel/2021  openmpi)",
            fg=P["text_dim"], bg=P["bg2"],
            font=("Arial", 9), anchor="w"
            ).grid(row=6, column=1, sticky="w", pady=(0,4))

        bf = tk.Frame(self, bg=P["bg"], pady=8)
        bf.pack(fill="x", padx=12)

        self.run_btn = tk.Button(bf, text="RUN CAC",
            command=self._run,
            bg=P["accent2"], fg=P["bg"],
            activebackground="#2fa870",
            font=(FUB[0], 11, "bold"),
        ...
        tk.Button(lh, text="+",
            command=lambda: self._resize_log(+1),
    ...
    def _run(self):
        ...
        if (script == self.app.editor_tab.get_current_file()
                and self.app.editor_tab._unsaved):
        ...
        self._log_append("[CAC] dir:     %s\n" % wdir, "info")
        if modules:
            self._log_append("[CAC] modules: %s\n" % modules, "info")
        self._log_append("[CAC] command: %s\n" % cac_cmd, "info")
        self._log_append("-" * 72 + "\n", "info")
        ...
        threading.Thread(target=self._worker,
                         args=(shell, wdir), daemon=True).start()

    def _worker(self, shell, wdir):
        try:
    ...
    def _on_done(self, rc):
        ...
        if rc == 0:
            ...
            self._log_append("\n[CAC] Run completed successfully.\n", "ok")
            self.app.tecplot_tab.on_run_complete()
        else:
            self.status_lbl.config(text="Exit (%d)" % rc, fg=P["danger"])
            self._log_append("\n[CAC] Process exited with code %d.\n" % rc, "err")

    def _log_append(self, text, tag=None):
        self.log.config(state="normal")
        if tag: self.log.insert("end", text, tag)
        else:   self.log.insert("end", text)
        self.log.see("end")
...
class TecplotTab(ttk.Frame):
    ...
    def _build(self):
        ...
        ab("Load Now",  self._load_now, P["accent2"]).pack(side="left")
    ...
    def _scatter_on(self, fmap):
        """Configure fieldmap as scatter point cloud."""
        fmap.show = True
    ...
    def _zone_cg(self):
        """CG preset: elements only — all zones as boundary surface."""
        ...
        try:
            ...
            plot.show_mesh    = False
            for z in frame.dataset.zones():
                self._surface_on(plot.fieldmap(z))
            self._log("  -> CG applied: %d zone(s) as boundary surface.\n" % frame.dataset.num_zones, "ok")
        except Exception as e:
            self._log("Zone preset error: %s\n" % e, "err")

    def _zone_cac(self):
        """CAC preset: element zones as surface, atom zones as scatter."""
        if not self._require_tp(): return
        self._log("Zone preset: CAC\n")
        try:
            atom_kws = ["atom","discrete","point","node","scatter"]
            frame, plot = self._get_plot_3d()
            plot.show_scatter = True
            plot.show_mesh    = False
            s_count = a_count = 0
            for z in frame.dataset.zones():
                fm = plot.fieldmap(z)
                if any(k in z.name.lower() for k in atom_kws):
                    self._scatter_on(fm); a_count += 1
                else:
                    self._surface_on(fm); s_count += 1
            self._log("  -> CAC applied: %d surface + %d scatter.\n" % (s_count, a_count), "ok")
        except Exception as e:
            self._log("Zone preset error: %s\n" % e, "err")
    ...
    def _apply_axis_ratio(self):
        ...
        try:
            ...
            if self.axis_equal_var.get():
                ...
                self._log("Axis mode -> XYZDependent (1:1:1 ratio).\n", "ok")
            else:
                plot.axes.axis_mode = tp.constant.AxisMode.Independent
                self._log("Axis mode -> Independent.\n", "ok")
        except Exception as e:
            ...
            with open(self._config_path()) as f:
                for line in f:
                    ...
                    if "=" in line and not line.startswith("#"):
                        ...
                        cfg[k.strip()] = v.strip()
            cfg["save_exe"] = (cfg.get("save_exe") == "true")
        except (IOError, OSError):
            pass
        return cfg

    def _save_default_exe(self):
        try:
            with open(self._config_path(), "w") as f:
                if self.save_exe_var.get():
                    exe = self.cac_exe_var.get().strip()
                    f.write("default_exe=%s\n" % exe)
                    f.write("save_exe=true\n")
                else:
                    f.write("save_exe=false\n")
        except (IOError, OSError):
            pass

    def _close(self):
        if self.save_exe_var.get():
            self._save_default_exe()
        if self.run_tab._running:
            if not messagebox.askyesno("Job running","Kill and exit?"):
                return
            self.run_tab._kill()
        self.destroy()
