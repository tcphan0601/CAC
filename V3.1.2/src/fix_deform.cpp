...
FixDeform::FixDeform(CAC *cac, int narg, char **arg) : Fix(cac, narg, arg), 
irregular(nullptr), set(nullptr)
{
  ...
  int nskip;
  if (strcmp(style, "deform/pressure") == 0) {
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "x") == 0 ||
      ...
      if (strcmp(arg[iarg+1], "final") == 0) {
        ...
        set[index].flo = universe->numeric(FLERR, arg[iarg+2]);
        set[index].fhi = universe->numeric(FLERR, arg[iarg+3]);
        iarg += 4;
      } else if (strcmp(arg[iarg+1], "delta") == 0) {
        if (iarg+4 > narg) error->all(FLERR, "Illegal fix deform command");
        set[index].style = DELTA;
        set[index].dlo = universe->numeric(FLERR, arg[iarg+2]);
        set[index].dhi = universe->numeric(FLERR, arg[iarg+3]);
        iarg += 4;
      } else if (strcmp(arg[iarg+1], "scale") == 0) {
        if (iarg+3 > narg) error->all(FLERR, "Illegal fix deform command");
        set[index].style = SCALE;
        set[index].scale = universe->numeric(FLERR, arg[iarg+2]);
        iarg += 3;
      } else if (strcmp(arg[iarg+1], "vel") == 0) {
        if (iarg+3 > narg) error->all(FLERR, "Illegal fix deform command");
        set[index].style = VEL;
        set[index].vel = universe->numeric(FLERR, arg[iarg+2]);
        iarg += 3;
        ...
        set[index].amplitude = universe->numeric(FLERR, arg[iarg+2]);
        set[index].tperiod = universe->numeric(FLERR, arg[iarg+3]);
        ...
        if (iarg+nskip > narg) error->all(FLERR, "Illegal fix deform/pressure command");
      ...
      if (strcmp(arg[iarg+1], "final") == 0) {
        if (iarg+3 > narg) error->all(FLERR, "Illegal fix deform command");
        set[index].style = FINAL;
        set[index].ftilt = universe->numeric(FLERR, arg[iarg+2]);
        iarg += 3;
      } else if (strcmp(arg[iarg+1], "delta") == 0) {
        if (iarg+3 > narg) error->all(FLERR, "Illegal fix deform command");
        set[index].style = DELTA;
        set[index].dtilt = universe->numeric(FLERR, arg[iarg+2]);
        iarg += 3;
      } else if (strcmp(arg[iarg+1], "vel") == 0) {
        if (iarg+3 > narg) error->all(FLERR, "Illegal fix deform command");
        set[index].style = VEL;
      ...
      if (child_styles.find(arg[iarg + 1]) != child_styles.end()) {
        nskip = child_styles[arg[iarg + 1]];
        if (iarg + nskip > narg)
          error->all(FLERR, "Illegal fix deform/pressure command");
  ...
  if (force_reneighbor) irregular = new IrregularComm(cac);
...
void FixDeform::pre_exchange()
{
  ...
  for (int i = 0; i < nalocal; i++) domain->remap(x[i], image[i]);

  x = element->x;
  image = element->image;
  int nelocal = element->nlocal;
  for (int i = 0; i < nelocal; i++) domain->remap(x[i], image[i]);
  element->evec->update_node_coord();

  domain->x2lamda(nalocal, atom->x);
  domain->x2lamda(nelocal, element->x);
  domain->nodex2lamda(nelocal, element->nodex);
  irregular->migrate();
  domain->lamda2x(nalocal, atom->x);
  domain->lamda2x(nelocal, element->x);
  domain->lamda2nodex(nelocal, element->nodex);
...
/*
  ...
  // convert atoms/elements and rigid bodies to lamda coords
  ...
  // convert atoms/elements and rigid bodies back to box coords
...
void FixDeform::update_domain()
{
  ...
  // convert atoms/element nodes and rigid bodies to lamda coords

  if (remapflag == Domain::X_REMAP) {
    domain->x2lamda(atom->nlocal, atom->x, atom->mask, groupbit);
    domain->nodex2lamda(element->nlocal, element->nodex, groupbit);
  ...
  // convert atoms/element nodes and rigid bodies back to box coords

  if (remapflag == Domain::X_REMAP) {
    domain->lamda2x(atom->nlocal, atom->x, atom->mask, groupbit);
    domain->lamda2nodex(element->nlocal, element->nodex, groupbit);
    element->evec->update_center_coord();
...
void FixDeform::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "remap") == 0) {
      ...
      for (int i = 0; i < nskip; i++) leftover_iarg.push_back(iarg + i);
      iarg += nskip;
    } else error->all(FLERR, "Illegal fix deform command: unknown keyword");
  }
}

/*  ----------------------------------------------------------------------
   memory usage of IrregularComm
   -------------------------------------------------------------------------  */
