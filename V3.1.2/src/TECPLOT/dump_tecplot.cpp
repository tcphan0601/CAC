...
DumpTecplot::DumpTecplot(CAC *cac, int narg, char **arg) : Dump(cac, narg, arg)
{
  ...
  if (average_flag) {
    if (stress_flag == 1) comm_elem_forward += (3 + 3 * stress_flag) * element->maxnpe;
...
void DumpTecplot::init_style() 
{
  atom_size_one = node_size_one = 5 + 
    3 * (velocity_flag + force_flag) + (15 * stress_flag + 3 * stress_flag * stress_flag)/2 + 4 *displace_flag + pe_flag + owner_proc_flag;
  ...
  if (fileformat == ASCII) {
    ...
    if (stress_flag == 2)
      strcat(values, " sxx syy szz sxy sxz syz syx szx szy");
    if (displace_flag)
...
void DumpTecplot::write()
{
  ...
  if (fileformat == ASCII) {
    ...
    if (natom_total) {
      ...
      if (stress_flag) {
        valueflag = STRESS;
        for (ival = 0; ival < 3 + 3 * stress_flag; ival++) {
          pack_atom(nullptr);
          comm_buf_tecio(natom_me);
        }
      }
...
void DumpTecplot::write_box_zone()
{
  ...
  // 8 corners in TecPlot FEBRICK IJK order:
  // node 1 (I=1,J=1,K=1) ... node 8 (I=2,J=2,K=2)
  double cx[8] = {
    xlo,          xlo+lx,
    xlo+lx+xy,    xlo+xy,
    xlo+xz,       xlo+lx+xz,
  ...
  if (fileformat == ASCII) {
    ...
    // write 8 node rows: x y z, then zeros for every extra variable
    int nextra = atom_size_one - 3;
    for (int i = 0; i < 8; i++) {
      fprintf(fp, "%g %g %g", cx[i], cy[i], cz[i]);
      for (int j = 0; j < nextra; j++) fprintf(fp, " 0");
      fprintf(fp, "\n");
    }
    // connectivity: single brick element
    fprintf(fp, "1 2 3 4 5 6 7 8\n");

  } else {

    // mark all variables except x(0), y(1), z(2) as passive
    int nvars = node_size_one;
    int *passive = new int[nvars];
    for (int i = 0; i < nvars; i++) passive[i] = (i < 3) ? 0 : 1;

    int nnodes = 8, nelems = 1;
    int zt = (domain->dimension == 3) ? 5 : 3;   // 5=FEBRICK, 3=FEQUADRILATERAL
    int success = TECZNE142((char *) "Simulation Cell",
        ...
        &dummy, &dummy,
        0, 0, 0,
        passive, nullptr, nullptr,
        &dummy);
    delete [] passive;
    if (success == -1) error->one(FLERR, "Cannot write Simulation Cell zone");

    // write x, y, z (3 variables, 8 doubles each)
    int nvals = 8;
    TECDAT142(&nvals, cx, &dummy1);
    TECDAT142(&nvals, cy, &dummy1);
    TECDAT142(&nvals, cz, &dummy1);

    // connectivity: 1 element, nodes 1–8
    int connect[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    int nconnect = 8;
    success = TECNODE142(&nconnect, connect);
    if (success == -1) error->one(FLERR, "Cannot write Simulation Cell connectivity");
  }
}
