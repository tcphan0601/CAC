...
#define PGDELTA 1    // same as in neighbor.cpp
      ...
      deep_base_model_devi(deep_model_devi)
{
  ...
  //if (cac->citeme) {
  //  cac->citeme->add(cite_user_package);
  //}
  ...
  //comm_atom_reverse = comm_elem_reverse = 1;

  // reconstructed neighbor list for deepmd from CAC neighbor list
  ...
  // reconstructed comm data for deepmd from the reconstructed neighbor list
...
void PairDeepBaseModel::print_summary(const string pre) const {
  if (comm->me == 0) {
    ...
    cout << "Summary of CAC deepmd module ..." << endl;
    cout << pre << ">>> Info of deepmd-kit:" << endl;
    deep_base->print_summary(pre);
    cout << pre << ">>> Info of CAC module:" << endl;
    ...
    std::cout.rdbuf(sbuf);
    universe->logmesg(cac, buffer.str());
  }
}
...
void PairDeepBaseModel::make_ttm_fparam(vector<double> &fparam) {
  assert(do_ttm);
  // get ttm_fix
  const FixTTMDP *ttm_fix = nullptr;
...
void PairDeepBaseModel::init_style() 
{
  ...
  // setup page for reconstructed neighbor list

  delete [] ipage;
  int nmypage = comm->nthreads;
  ipage = new MyPage<int>[nmypage];
  for (int i = 0; i < nmypage; i++) 
    ipage[i].init(neighbor->oneatom, neighbor->pgsize, PGDELTA);
}
