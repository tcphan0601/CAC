...
void Minimize::command(int narg, char **arg)
{
  ...
  double old_nodal_energy_weight = force->pair->nodal_energy_weight;
  ...
  force->pair->nodal_energy_weight = 1.0;
  int iarg = 4;
  while (iarg < narg) {
    if (strcmp(arg[iarg], "nodal/weight") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal minimize command");      
      force->pair->nodal_energy_weight = universe->numeric(FLERR, arg[iarg+1]);
      iarg += 2;
  ...
  force->pair->nodal_energy_weight = old_nodal_energy_weight;
}
