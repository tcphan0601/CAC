...
void Modify::add_compute(int narg, char **arg, int trysuffix)
{
  ...
  if (compute[ncompute] == nullptr) error->all(FLERR, "Unknown compute style");
...
void Modify::add_fix(int narg, char **arg, int trysuffix)
{
  ...
  if (fix[ifix] == nullptr) error->all(FLERR, "Unknown fix style");
...
void Modify::init()
{
  ...
  if (element->nlocal) {
    ...
    int ***eflag;
    memory->create(eflag, nelocal, element->maxapc, element->maxnpe, "modify:eflag");
    for (i = 0; i < nelocal; i++)
      ...
      for (j = 0; j < nelocal; j++) 
        for (k = 0; k < apc[etype[j]]; k++)
          for (l = 0; l < npe[etype[j]]; l++)
            if (nodemask[j][k][l] & groupbit) eflag[j][k][l]++;
    }
