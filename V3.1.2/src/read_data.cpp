...
#define NSECTIONS 7       // change when add to header::section_keywords
#define EPSILON 1e-6

enum{NONE, APPEND, VALUE, MERGE};
enum{OFF, REFERENCE, CURRENT};
enum{LATEST, V2};
...
void ReadData::command(int narg, char **arg)
{
  ...
  versionflag = LATEST;
  toffset = eoffset = 0;
  extra_atom_types = extra_element_types = extra_grains = 0;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "add") == 0) {
      ...
      if (iarg+3 > narg) error->all(FLERR, "Illegal read_data command");
      offsetflag = 1;
    ...
    //  iarg ++;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "extra/grains") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal read_data command");
      extra_grains = universe->inumeric(FLERR, arg[iarg+1]);
      if (extra_grains < 0) error->all(FLERR, "Illegal read_data command");
      iarg += 2;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "reference") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal read_data command");
      if (strcmp(arg[iarg+1], "cur/frame") == 0) referenceflag = CURRENT;
      else if (strcmp(arg[iarg+1], "ref/frame") == 0) referenceflag = REFERENCE;
      else if (strcmp(arg[iarg+1], "off") == 0) referenceflag = OFF;
    ...
    //  iarg += 4;
      ...
      if (iarg+2 > narg) error->all(FLERR, "Illegal read_data command");
      if (strcmp(arg[iarg+1], "V2") == 0) versionflag = V2;
      else if (strcmp(arg[iarg+1], "current") == 0) versionflag = LATEST;
      else error->all(FLERR, "Illegal read_data command");
      iarg += 2;
    } else error->all(FLERR, "Illegal read_data command");
  }
  ...
  if (netypes == 0 && extra_element_types)
    element->evec->grow_etype_arrays(netypes + extra_element_types);
  ...
  if (addflag == NONE) {
    ...
    // call irregular comm in case box changed too much and processor grids changed
    // set nlocal_previous after this 
  ...
  while (strlen(keyword)) {
    if (strcmp(keyword, "Masses") == 0) {
      ...
      if (atomflag == 0)
        error->all(FLERR, "Atoms section should be read before Velocities section");
      atom_velocities();
      ...
      if (elementflag == 0)
        error->all(FLERR, "Elements section should be read before Nodes Velocities section");
      node_velocities();
    } else {
      char str[128];
  ...
  if (natoms > 0 && atomflag == 0 && addflag == NONE)
    error->all(FLERR, "No atoms in data file");
  if (nelements > 0 && (elementflag == 0 || nodeflag == 0) && addflag == NONE)
  ...
  atom->data_fix_compute_dump_variable(nalocal_previous, atom->nlocal);
  element->data_fix_compute_dump_variable(nelocal_previous, element->nlocal);
...
void ReadData::header()
{
  ...
  while (1) {
    ...
    // Atom/Element class type and grain settings are only set by first data file
      ...
      sscanf(line, "%d", &ngrains);
      if (addflag == NONE) atom->ngrains = ngrains + extra_grains;
    }

    // local copy of box info
    // so can treat differently for first vs subsequent data files

    else if (referenceflag == CURRENT) {
      if (strstr(line, "xlocur xhicur")) {
    ...
    } else if (referenceflag == REFERENCE) {
      if (strstr(line, "xlocur xhicur")) {
      } else if (strstr(line, "ylocur yhicur")) {
      } else if (strstr(line, "zlocur zhicur")) {
      } else if (strstr(line, "xycur xzcur yzcur")) {
...
void ReadData::parse_keyword(int first)
{
  ...
  if (me == 0) {
    ...
    if (fgets(buffer, MAXLINE, fp) == nullptr) eof = 1;
  }
...
void ReadData::atoms()
{
  ...
  while (nread < natoms) {
    ...
    if (versionflag == V2) {
      if (referenceflag)
        atom->data_atoms_reference_V2(nchunk, buffer, aid_offset, toffset, shiftflag, shift);
      else 
        atom->data_atoms_V2(nchunk, buffer, aid_offset, toffset, shiftflag, shift);
    } else {
      if (referenceflag)
        atom->data_atoms_reference(nchunk, buffer, aid_offset, toffset, shiftflag, shift, referenceflag);
      else 
...
void ReadData::elements()
{
  ...
  while (nread < nelements) {
    ...
    if (eof) error->all(FLERR, "Unexpected end of data file");
    if (versionflag == V2) 
      element->data_elements_V2(nchunk, buffer, eid_offset, toffset, eoffset, shiftflag, shift);
    else 
      element->data_elements(nchunk, buffer, eid_offset, eoffset, shiftflag, shift);
    nread += nchunk;
  }
  ...
  for (int i = nelocal_previous; i < n; i++) {
    int itype = element->etype[i];
    nnode_local += npe[itype] * apc[itype];
  }
...
void ReadData::nodes()
{
  ...
  while (nread < nnodes) {
    ...
    if (versionflag == V2) {
    if (referenceflag) 
      element->data_nodes_reference_V2(nchunk, buffer, eid_offset, shiftflag, shift);
    else
      element->data_nodes_V2(nchunk, buffer, eid_offset, shiftflag, shift);

    } else {
    if (referenceflag) 
      element->data_nodes_reference(nchunk, buffer, eid_offset, toffset, shiftflag, shift, referenceflag);
    else
      element->data_nodes(nchunk, buffer, eid_offset, toffset, shiftflag, shift);
    }
    nread += nchunk;
  }  
...
void ReadData::atom_velocities()
{
  ...
  while (nread < natoms) {
    ...
    if (eof) error->all(FLERR, "Unexpected end of data file");
    if (versionflag == V2) 
      atom->data_vels_V2(nchunk, buffer, aid_offset);
    else 
...
void ReadData::node_velocities()
{
  ...
  while (nread < nnodes) {
    ...
    if (eof) error->all(FLERR, "Unexpected end of data file");
    if (versionflag == V2)
    element->data_vels_V2(nchunk, buffer, eid_offset);
    else 
