...
NeighList::NeighList(CAC *cac) : Pointers(cac)
{
  ...
  ilist = iindexlist = nullptr;
  nvailist = nvaindexlist = nullptr;

  numneigh = nullptr;
  firstneigh = firstneighindex = nullptr;
  ipage = indexpage = nullptr;
  nvanumneigh = nullptr;
  nvafirstneigh = nvafirstneighindex = nullptr;
  nvaipage = nvaindexpage = nullptr;
  ...
  va2nvalist.reserve(maxnva);

  neighneigh = 0;
  noinner = 0;
  element_firstindex = nullptr;
  // ptrs
...
void NeighList::post_constructor(NeighRequest *rq)
{
  ...
  group = rq->group;
  dnum = rq->dnum;
  neighneigh = rq->neighneigh;
  noinner = rq->noinner;

  if (noinner && neighneigh)
    error->all(FLERR, "Cannot build a list with noinner and neighneigh flag");
  if (rq->copy)
...
void NeighList::grow(int nlocal, int nall)
{
  ...
  if (vatomlist) {
    if (noinner)
    maxall += element->nmax * element->max_outer_ucell * element->maxapc;
    else
    maxall += element->nmax * element->maxucell * element->maxapc;
  }
...
void NeighList::grow_nva()
{
  ...
  memory->grow(nvanumneigh, maxnva, "neighlist:nvanumneigh");
  memory->grow(nvailist, maxnva, "neighlist:nvailist");
  memory->grow(nvaindexlist, maxnva, "neighlist:nvaindexlist");
  nvafirstneigh = (int **) 
    memory->srealloc(nvafirstneigh, maxnva * sizeof(int *), "neighlist:nvafirstneigh");
  nvafirstneighindex = (int **) 
    memory->srealloc(nvafirstneighindex, maxnva * sizeof(int *), "neighlist:nvafirstneighindex");
  va2nvalist.reserve(maxnva);
...
bigint NeighList::memory_usage()
{
  ...
  if (threebody) {
    ...
    bytes += memory->estimate_unordered_vector2d_map_memory(va2nvalist);
