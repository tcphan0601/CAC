...
#define NTYPEINFO 11  // ditto element_vec.cpp
...
void FixAdaptive::post_integrate()
{
  ...
  int **firstneigh = list->firstneigh;
  int **firstneighindex = list->firstneighindex;
  int i, j, k, l, n, ii, jj, kk, ll, iapc, ietype, jindex, jbasis, jucell, jetype, japc;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (natom >= thres) {
      ...
      if (dislocation_flag) {
        ...
        // Structure to store found planes temporarily
        struct FoundPlane {
          double n[3];
          double p[3];
          double d_proj; // Distance along the element normal for sorting
          int inlier_count;
        };
        ...
        for (dim = 0; dim < 3; dim++) {
          ...
          // ==========================================
          // FILTER DEFECT ATOMS FOR THIS DIMENSION
          // ==========================================
          int dim_natom = 0;
          int *dim_jlist = new int[natom];

          for (int jj = 0; jj < natom; jj++) {
            int idx = jindexlist[jj];
            bool match = false;

            for (int ibox = 0; ibox < 8; ibox++) {
              if (idx == boxindices[dim][ibox]) {
                match = true;
                break;
              }
            }
          ...
          // ==========================================
          // ELEMENT METRICS & DYNAMIC TOLERANCE
          // ==========================================
          int ncell = element->evec->ncells[ietype][dim];
          int planes_per_cell = element->evec->ppc[ietype][dim];

          double cell_size = element->cell_size[i][dim];
          double interplanar_spacing = cell_size / (ncell - 1.0) / planes_per_cell;

          // Set inlier threshold to exactly 10% of the calculated interplanar spacing
          double plane_dist_tol = 0.10 * interplanar_spacing;

          // ==========================================
          // INITIALIZE RANSAC PARAMETERS
          // ==========================================
          int *used_mask = new int[dim_natom];
          for (int k = 0; k < dim_natom; k++) used_mask[k] = 0;

          double *isurface = element->surface_plane[i][dim];
          int max_possible_planes = ncell;

          FoundPlane *planes = new FoundPlane[max_possible_planes];
          int num_planes = 0;

          // ==========================================
          // PHASE 1: DISCOVER ALL PLANES (RANSAC ALGORITHM)
          // ==========================================
          while (num_planes < max_possible_planes) {
            int best_inlier_count = 0;
            double best_normal[3] = {0.0, 0.0, 0.0};
            double best_P[3] = {0.0, 0.0, 0.0};
            int *best_inliers_local = new int[dim_natom];

            for (int iter = 0; iter < RANSAC_ITERS; iter++) {
              int idx1 = rand() % dim_natom;
              int idx2 = rand() % dim_natom;
              int idx3 = rand() % dim_natom;

              if (idx1 == idx2 || idx1 == idx3 || idx2 == idx3) continue;
              if (used_mask[idx1] || used_mask[idx2] || used_mask[idx3]) continue;

              double *p1 = ax[dim_jlist[idx1]];
              double *p2 = ax[dim_jlist[idx2]];
              double *p3 = ax[dim_jlist[idx3]];
              ...
              if (1.0 - fabs(n[0]*isurface[0] + n[1]*isurface[1] + n[2]*isurface[2]) > tol) continue;

              double d_val = n[0]*p1[0] + n[1]*p1[1] + n[2]*p1[2];

              int current_inlier_count = 0;
              int *current_inliers_local = new int[dim_natom];

              for (int k = 0; k < dim_natom; k++) {
                if (used_mask[k]) continue;
                double *pk = ax[dim_jlist[k]];

                double dist = fabs((n[0]*pk[0] + n[1]*pk[1] + n[2]*pk[2]) - d_val);
                if (dist < plane_dist_tol) {
                  current_inliers_local[current_inlier_count++] = k;
                }
              }
              ...
              if (current_inlier_count > best_inlier_count) {
                ...
                best_P[0] = p1[0]; best_P[1] = p1[1]; best_P[2] = p1[2];
                for (int c = 0; c < current_inlier_count; c++) 
                  best_inliers_local[c] = current_inliers_local[c];
              }
              delete [] current_inliers_local;
            }

            if (best_inlier_count >= thres) {
              planes[num_planes].n[0] = best_normal[0];
              planes[num_planes].n[1] = best_normal[1];
              planes[num_planes].n[2] = best_normal[2];
              planes[num_planes].p[0] = best_P[0];
              planes[num_planes].p[1] = best_P[1];
              planes[num_planes].p[2] = best_P[2];
              planes[num_planes].inlier_count = best_inlier_count;
              planes[num_planes].d_proj = best_P[0]*isurface[0] + best_P[1]*isurface[1] + best_P[2]*isurface[2];

              for (int c = 0; c < best_inlier_count; c++) {
                used_mask[best_inliers_local[c]] = 1;
              }
              num_planes++;
            } else {
              delete [] best_inliers_local;
              break;
            }
            delete [] best_inliers_local;
          }
          ...
          // ==========================================
          // PHASE 2: ELEMENT SPLITTING
          // ==========================================

          if (num_planes >= 2) {

            // 1. Sort the planes spatially along the normal direction
            for (int p1 = 0; p1 < num_planes - 1; p1++) {
              for (int p2 = p1 + 1; p2 < num_planes; p2++) {
                if (planes[p1].d_proj > planes[p2].d_proj) {
                  FoundPlane temp = planes[p1];
                  planes[p1] = planes[p2];
                  planes[p2] = temp;
                }
              }
            ...
            // 2. Define spacing parameters
            double spacing_tol = 0.2 * interplanar_spacing;
            double a = 2.0 / ncell;
            double edge_tol = a * 0.25;

            // Array to prevent duplicate split commands at the same boundary
            bool *cut_added = new bool[ncell];
            for (int k = 0; k < ncell; k++) cut_added[k] = false;

            // 3. Check adjacent planes for dislocation signatures
            for (int p = 0; p < num_planes - 1; p++) {
              double dist = fabs(planes[p+1].d_proj - planes[p].d_proj);

              if (fabs(dist - interplanar_spacing) < spacing_tol) {

                double mid_P[3];
                for (int k = 0; k < 3; k++) {
                  mid_P[k] = (planes[p].p[k] + planes[p+1].p[k]) / 2.0;
                }

                double tmp[3];
                element->box2natural(mid_P, tmp, i);
                double xi = tmp[dim];

                double float_cut = (xi + 1.0) / a;
                int nearest_icut = round(float_cut);
                double boundary_xi = (nearest_icut * a) - 1.0;
                bool is_near_boundary = fabs(xi - boundary_xi) < edge_tol;

                if (iapc == 1) {
                  if (is_near_boundary && nearest_icut > 0 && nearest_icut < ncell) {
                    if (!cut_added[nearest_icut]) {
                      cut_added[nearest_icut] = true;
                      if (nsplit == maxsplit) grow_splitlist();
                      splitlist[nsplit][0] = i;
                      splitlist[nsplit][1] = nearest_icut;
                  ...
                  if (is_near_boundary) {
                    if (nearest_icut > 0 && nearest_icut < ncell && !cut_added[nearest_icut]) {
                      cut_added[nearest_icut] = true;
                      if (nsplit == maxsplit) grow_splitlist();
                      splitlist[nsplit][0] = i;
                      splitlist[nsplit][1] = nearest_icut;
                    ...
                    int icut_lower = floor(float_cut);
                    int icut_upper = ceil(float_cut);

                    if (icut_lower > 0 && icut_lower < ncell && !cut_added[icut_lower]) {
                      cut_added[icut_lower] = true;
                      if (nsplit == maxsplit) grow_splitlist();
                      splitlist[nsplit][0] = i;
                      splitlist[nsplit][1] = icut_lower;
                      splitlist[nsplit][2] = dim;
                      nsplits_total++;
                      nsplit++;
                      disc_flag = 0;
                    ...
                    if (icut_upper > 0 && icut_upper < ncell && !cut_added[icut_upper]) {
                      cut_added[icut_upper] = true;
                      if (nsplit == maxsplit) grow_splitlist();
                      splitlist[nsplit][0] = i;
                      splitlist[nsplit][1] = icut_upper;
                      splitlist[nsplit][2] = dim;
                      nsplits_total++;
                      nsplit++;
                      disc_flag = 0;
                ...
                if (debug) {
                  printf("Split evaluated me = %d element %d, iapc = %d. xi = %g nearest_icut = %d is_near_boundary = %d\n",
                      comm->me, element->tag[i], iapc, xi, nearest_icut,is_near_boundary);
                }
              }
            }
            delete [] cut_added;
...
int FixAdaptive::split_element(int **splitlist, int n)
{
      ...
      for (int ix = 0; ix < nelem[0]; ix++) {
        for (int iy = 0; iy < nelem[1]; iy++) {
          for (int iz = 0; iz < nelem[2]; iz++) {
            ...
            type_info[7] = ppcy;
            type_info[8] = ppcz;
            type_info[9] = element_shape_ids[ietype];
            ...
            if (domain->dimension == 3) {
              ...
              if (type_info[0] == 1) {
                ...
                type_info[7] = ppcz;
                type_info[8] = 1;
                type_info[9] = Element::QUADRILATERAL;
              }
  ...
  ctype = element->ctype;
  apc = element->apc;
  element_shape_ids = element->element_shape_ids;
            ...
            for (int iz = 0; iz < nelem[2]; iz++) {
              ...
              if (((type_info[0] == 1) + (type_info[1] == 1) + (type_info[2] == 1)) > 1) {
                ...
                if (type_info[0] == 1) {
                  ...
                  type_info[7] = ppcy;
                  type_info[8] = ppcz;
                  type_info[9] = Element::HEXAHEDRON;
                ...
                if (myetype == 0) {
                  // Debug output 
                  printf("Error: Etype not found. Info: NCells(%d %d %d) GCells(%d %d %d) PPC(%d %d %d) Shape(%d) APC(%d)\n",
                      type_info[0], type_info[1], type_info[2],
                      type_info[3], type_info[4], type_info[5],
                      type_info[6], type_info[7], type_info[8],
                      type_info[9], type_info[10]);
                  error->one(FLERR, "Etype not defined yet in second pass");
                } 
