...
void Run::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "upto") == 0) {
      ...
      startflag = 1;
      start = universe->bnumeric(FLERR, arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg], "stop") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal run command");
      stopflag = 1;
      stop = universe->bnumeric(FLERR, arg[iarg+1]);
      iarg += 2;
    } 

    // for debugging
    
    else if (strcmp(arg[iarg], "skip/force") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal run command");
      if (strcmp(arg[iarg+1], "no") == 0) forceflag = 1;
      else if (strcmp(arg[iarg+1], "yes") == 0) forceflag = 0;
  ...
  if (nevery == 0) {
    ...
    if (preflag || update->first_update == 0) {
      cac->init();
      update->integrate->setup(forceflag);
    } else output->setup(0);

    timer->init();
    timer->barrier_start();
    update->integrate->run(nsteps, forceflag);
    timer->barrier_stop();
