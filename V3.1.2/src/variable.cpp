...
#if defined(CAC_SMALLBIG) || defined(CAC_BIGBIG)
...
Variable::Variable(CAC *cac) : Pointers(cac)
{
...
void Variable::set(int narg, char **arg)
{
  ...
  // 2 args + pad: num = N2, which = N1, data = single string
    ...
    if (narg == 3 || (narg == 4 && strcmp(arg[3], "pad") == 0)) {
      nfirst = 1;
      nlast = universe->inumeric(FLERR, arg[2]);
      ...
      nfirst = universe->inumeric(FLERR, arg[2]);
      nlast = universe->inumeric(FLERR, arg[3]);
    ...
    //    if (nvar == maxvar) grow();
    //    style[nvar] = ULOOP;
    //    num[nvar] = universe->inumeric(FLERR, arg[2]);
    //    data[nvar] = new char*[1];
    //    data[nvar][0] = nullptr;
    //    if (narg == 4) {
    //      char digits[12];
    ...
    //  reader[nvar] = new VarReader(cac, arg[0], arg[2], SCALARFILE);
    ...
    //  data[nvar] = new char*[num[nvar]];
    //  data[nvar][0] = nullptr;
    //  reader[nvar] = new VarReader(cac, arg[0], arg[2], ATOMFILE);
  ...
  // data = 1 value, string to eval
    ...
    //    error->all(FLERR, "CAC is not built with Python embedded");
    //  int ivar = find(arg[0]);
    //  if (ivar >= 0) {
    ...
    //      error->all(FLERR, "Cannot redefine variable as a different style");
    //    dvalue[nvar] = universe->numeric(FLERR, arg[2]);
    //    replaceflag = 1;
    //  } else {
    //    if (nvar == maxvar) grow();
    //    style[nvar] = INTERNAL;
...
int Variable::next(int narg, char **arg)
{
  ...
  if (istyle == STRING || istyle == EQUAL || istyle == WORLD ||
      ...
      istyle == UNIVERSE || istyle == ULOOP ||
      istyle == ATOMFILE || istyle == SCALARFILE ||
  ...
  if (istyle == INDEX || istyle == LOOP) {
    ...
    //    RanMars *random = new RanMars(cac, seed);
...
void Variable::set_atom_arrays(int i)
{
  //for (int i = 0; i < nvar; i++)
...
/* ----------------------------------------------------------------------
   initialize one element's storage values in all VarReaders via fix STORE
   called when element is created
   not used yet
   ------------------------------------------------------------------------- */

void Variable::set_elem_arrays(int i)
{
  //for (int i = 0; i < nvar; i++)
  //  if (reader[i] && style[i] == ATOMFILE)
  //    reader[i]->fixstore->vstore[i] = 0.0;
...
double Variable::evaluate(char *str, Tree **tree, int ivar)
{
  ...
  while (1) {
    ...
    // ----------------
      ...
      if (expect == OP) print_var_error(FLERR, "Invalid syntax in variable formula", ivar);
      expect = OP;
    ...
    // ----------------
      ...
      if (strncmp(word, "c_", 2) == 0 || strncmp(word, "C_", 2) == 0) {
        ...
        // c_ID[i] = scalar from per-atom vector
          ...
          peratom2global(1, nullptr, compute->vector_atom, 1, index1, -1, -1,
        ...
        // c_ID[i][j] = scalar from per-atom array
          ...
          if (compute->array_atom)
            peratom2global(1, nullptr, &compute->array_atom[0][index2-1], 
                compute->size_peratom_cols, index1, -1, -1,
                tree, treestack, ntreestack, argstack, nargstack);
          else
      ...
      // ----------------
        ...
        // f_ID[i] = scalar from per-atom vector
          ...
          peratom2global(1, nullptr, fix->vector_atom, 1, index1, -1, -1, 
              tree, treestack, ntreestack, argstack, nargstack);
        ...
        // f_ID[i][j] = scalar from per-atom array
          ...
          if (fix->array_atom)
            peratom2global(1, nullptr, &fix->array_atom[0][index2-1], 
                fix->size_peratom_cols, index1, -1, -1,
                tree, treestack, ntreestack, argstack, nargstack);
          else
            peratom2global(1, nullptr, nullptr, 
                fix->size_peratom_cols, index1, -1, -1,
      ...
      // ----------------
        ...
        // use peratom2global to extract single value from result
          ...
          peratom2global(1, nullptr, result, 1, index, -1, -1,
              tree, treestack, ntreestack, argstack, nargstack);
      ...
      // ----------------
        ...
        if (str[i] == '(') {
          ...
          if (math_function(word, contents, tree, 
                treestack, ntreestack, argstack, nargstack, ivar));
          ...
          int inode = -1;
          ptr = &str[i];
          id = int_between_brackets(ptr, 1);
          i = ptr - str + 1;
          if (str[i] == '[') {
            ptr = &str[i];
            ibasis = int_between_brackets(ptr, 1) - 1;
            i = ptr-str+1;
            if (str[i] == '[') {
              ptr = &str[i];
              inode = int_between_brackets(ptr, 1) - 1;
              i = ptr - str + 1;
            }
          }
          peratom2global(0, word, nullptr, 0, id, ibasis, inode, 
              tree, treestack, ntreestack, argstack, nargstack);
...
double Variable::collapse_tree(Tree *tree)
{
  ...
  if (tree->type == RANDOM) {
    ...
    if (randomatom == nullptr) {
      ...
      randomatom = new RanMars(cac, seed+me);
    }
    return 0.0;
  }
...
/* ----------------------------------------------------------------------
   ...
   tree was created by one-time parsing of formula string via evaluate, ivar()
   customize by adding a function:
...
double Variable::eval_tree(Tree *tree, int i)
{
  ...
  if (tree->type == RMASK) {
    if (domain->regions[tree->ivalue1]->match(atom->x[i][0], atom->x[i][1], atom->x[i][2]))
      return 1.0;
    else return 0.0;
  }
  ...
  if (tree->type == GRMASK) {
    if ((atom->mask[i] & tree->ivalue1) &&
        (domain->regions[tree->ivalue2]->match(atom->x[i][0], atom->x[i][1], atom->x[i][2])))
      return 1.0;
    else return 0.0;
  }
  ...
  if (strcmp(word, "sqrt") == 0) {
    ...
    if (tree) newtree->type = RANDOM;
      ...
      if (randomequal == nullptr) {
        ...
        if (seed <= 0)
          print_var_error(FLERR, "Invalid math function in variable formula", ivar);
        randomequal = new RanMars(cac, seed);
      }
...
/* ----------------------------------------------------------------------
   ...
   countatom(group), countnode(group), countelem(group), countvatom(group), mass(group), 
   xcm(group, dim), vcm(group, dim), bound(group, xmin), ke(group)
   angmom(group, dim), inertia(group, dim), omega(group, dim)
   ------------------------------------------------------------------------- */
    ...
    double *argstack, int &nargstack, int ivar)
{
  ...
  if (strcmp(word, "countatom") && strcmp(word, "countnode") &&
      strcmp(word, "countelem") && strcmp(word, "countelemnode") &&
      strcmp(word, "countvatom") && strcmp(word, "bound") &&
      strcmp(word, "mass") && strcmp(word, "ke") &&
      strcmp(word, "xcm") && strcmp(word, "vcm") &&
      strcmp(word, "angmom") && strcmp(word, "inertia") &&
      strcmp(word, "omega"))
    return 0;
  //&& strcmp(word, "mass") &&
  ...
  if (strcmp(word, "countatom") == 0) {
    if (narg == 1) value = group->count_atom(igroup);
    else if (narg == 2) value = group->count_atom(igroup, region_function(args[1], ivar));
    else print_var_error(FLERR, "Invalid group function in variable formula", ivar);
  } else if (strcmp(word, "countnode") == 0) {
    if (narg == 1) value = group->count_node(igroup);
    else if (narg == 2) value = group->count_node(igroup, region_function(args[1], ivar));
    else print_var_error(FLERR, "Invalid group function in variable formula", ivar);
  } else if (strcmp(word, "countelemnode") == 0) {
    if (narg == 1) value = group->count_element_node(igroup);
    else if (narg == 2) value = group->count_element_node(igroup, region_function(args[1], ivar));
    else print_var_error(FLERR, "Invalid group function in variable formula", ivar);
  } else if (strcmp(word, "countelem") == 0) {
    if (narg == 1) value = group->count_elem(igroup);
    else if (narg == 2) value = group->count_elem(igroup, region_function(args[1], ivar));
    else print_var_error(FLERR, "Invalid group function in variable formula", ivar);
  } else if (strcmp(word, "countvatom") == 0) {
    if (narg == 1) value = group->count_vatom(igroup);
    else if (narg == 2) value = group->count_vatom(igroup, region_function(args[1], ivar));
    else print_var_error(FLERR, "Invalid group function in variable formula", ivar);
...
void Variable::peratom2global(int flag, char *word, 
    double *vector, int nstride, tagint id, int ibasis, int inode, 
    ...
    double *argstack, int &nargstack)
{
  ...
  if (strcmp(word, "id") == 0 || 
    ...
    index = atom->map(id);
    if (index >= atom->nlocal) index = -1;
  } else if (strcmp(word, "eid") == 0 || 
      strcmp(word, "emass") == 0 ||
      strcmp(word, "etype") == 0 || 
      strcmp(word, "ctype") == 0 || 
      strcmp(word, "ex") == 0 ||
      ...
      strcmp(word, "nodey") == 0 ||
      strcmp(word, "nodez") == 0 ||
      strcmp(word, "nodevx") == 0 ||
      strcmp(word, "nodevy") == 0 ||
      strcmp(word, "nodevz") == 0 ||
      strcmp(word, "nodefx") == 0 ||
      strcmp(word, "nodefy") == 0 ||
      strcmp(word, "nodefz") == 0) {
    if (element->map_style == 0)
      error->all(FLERR, 
          "Indexed per-element vector in variable formula without element map");

    if (id > element->map_tag_max)
      error->all(FLERR, "Variable element ID is too large");

    // if ID does not exist, index will be -1 for all procs, 
    // and mine will be set to 0.0

    index = element->map(id);
    if (index >= element->nlocal) index = -1;
  } else error->one(FLERR, "Invalid atom/element vector in variable formula");

  double mine;
  if (index >= 0) {
    if (flag == 0) {
      if (strcmp(word, "id") == 0) mine = atom->tag[index];
      else if (strcmp(word, "mass") == 0) mine = atom->mass[atom->type[index]];
      ...
      else if (strcmp(word, "eid") == 0) mine = element->tag[index];
      else if (strcmp(word, "etype") == 0) mine = element->etype[index];
      else if (strcmp(word, "x") == 0) mine = element->x[index][0];
      else if (strcmp(word, "y") == 0) mine = element->x[index][1];
      else if (strcmp(word, "z") == 0) mine = element->x[index][2];
      else if (ibasis >= 0) {
        if (strcmp(word, "emass") == 0) mine = atom->mass[element->ctype[index][ibasis]];
        else if (strcmp(word, "ctype") == 0) mine = element->ctype[index][ibasis];
        else if (inode >= 0) {
          if (strcmp(word, "nodex") == 0) mine = element->nodex[index][ibasis][inode][0];
          else if (strcmp(word, "nodey") == 0) mine = element->nodex[index][ibasis][inode][1];
          else if (strcmp(word, "nodez") == 0) mine = element->nodex[index][ibasis][inode][2];
          else if (strcmp(word, "nodevx") == 0) mine = element->nodev[index][ibasis][inode][0];
          else if (strcmp(word, "nodevy") == 0) mine = element->nodev[index][ibasis][inode][1];
          else if (strcmp(word, "nodevz") == 0) mine = element->nodev[index][ibasis][inode][2];
          else if (strcmp(word, "nodefx") == 0) mine = element->nodef[index][ibasis][inode][0];
          else if (strcmp(word, "nodefy") == 0) mine = element->nodef[index][ibasis][inode][1];
          else if (strcmp(word, "nodefz") == 0) mine = element->nodef[index][ibasis][inode][2];
        }
      }
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
/*
   VarReader::VarReader(CAC *cac, char *name, char *file, int flag) :
   Pointers(cac)
   {
   me = comm->me;
   style = flag;
...
MPI_Bcast(str, n, MPI_CHAR, 0, world);
bigint nlines = universe->bnumeric(FLERR, str);
