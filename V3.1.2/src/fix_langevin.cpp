...
FixLangevin::FixLangevin(CAC *cac, int narg, char **arg) :
  Fix(cac, narg, arg), 
  gjfflag(0), gfactor1(nullptr), gfactor2(nullptr), ratio(nullptr), tstr(nullptr), 
  flangevin(nullptr), tforce(nullptr), franprev(nullptr), id_temp(nullptr), random(nullptr)
{
  ...
  t_period = universe->numeric(FLERR, arg[5]);
  seed = universe->inumeric(FLERR, arg[6]);
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "angmom") == 0) {
	  error->all(FLERR, "angmom not implemented fix langevin command");
	  /* 
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix langevin command");
      if (strcmp(arg[iarg+1], "no") == 0) ascale = 0.0;
      else ascale = universe->numeric(FLERR, arg[iarg+1]);
      iarg += 2;
	   */
    } else if (strcmp(arg[iarg], "gjf") == 0) {
	  error->all(FLERR, "gjf not implemented fix langevin command");
	  /* 
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix langevin command");
	  ...
	  error->all(FLERR, "omega not implemented fix langevin command");
	  /* 
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix langevin command");
	  ...
	  error->all(FLERR, "scale not implemented fix langevin command");
	  /* 
	  ...
	  error->all(FLERR, "tally not implemented fix langevin command");
	  /* 
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix langevin command");
...
void FixLangevin::setup(int vflag)
{
  if (strstr(update->integrate_style, "verlet"))
	...
	error->all(FLERR, "Respa integration has not been implemented in CAC yet");
...
void FixLangevin::post_force(int /* vflag */)
{
  ...
  post_force_untemplated(int(tstyle==ATOM), gjfflag, tallyflag, 
                         int(tbiasflag==BIAS), int(rmass!=nullptr), zeroflag);
#endif
}
...
#endif
{
  ...
  if (Tp_ZERO) {
    ...
    count = group->count_atom(igroup) + group->count_elem(igroup);
    if (count == 0)
  ...
  for (i = 0; i < nalocal; i++) {
    if (amask[i] & groupbit) {
      ...
      if (Tp_BIAS) {
		/* 
        temperature->remove_bias(i, av[i]);
        fdrag[0] = gamma1 * av[i][0];
        fdrag[1] = gamma1 * av[i][1];
        fdrag[2] = gamma1 * av[i][2];
        if (av[i][0] == 0.0) fran[0] = 0.0;
        if (av[i][1] == 0.0) fran[1] = 0.0;
        if (av[i][2] == 0.0) fran[2] = 0.0;
        temperature->restore_bias(i, av[i]);
		 */
      } else {
        fdrag[0] = gamma1 * av[i][0];
        fdrag[1] = gamma1 * av[i][1];
       ...
       */
      af[i][0] += fdrag[0] + fran[0];
      af[i][1] += fdrag[1] + fran[1];
      af[i][2] += fdrag[2] + fran[2];
      /* 
      if (Tp_TALLY) {
  ...
  double *sqrt_nodal_weight = new double[element->netypes+1];
  for (int ietype = 1; ietype <= element->netypes; ietype++)
    sqrt_nodal_weight[ietype] = sqrt(nodal_weight[ietype]);

  for (i = 0; i < nelocal; i++) {
    if (emask[i] & groupbit) {
      for (j = 0; j < apc[etype[i]]; j++) {
        gamma1 = gfactor1[ctype[i][j]];
        gamma2 = gfactor2[ctype[i][j]] * tsqrt / sqrt_nodal_weight[etype[i]];
        for (k = 0; k < npe[etype[i]]; k++) {
          ...
          fdrag[0] = gamma1 * nodev[i][j][k][0];
          fdrag[1] = gamma1 * nodev[i][j][k][1];
          fdrag[2] = gamma1 * nodev[i][j][k][2];

          nodef[i][j][k][0] += fdrag[0] + fran[0];
          nodef[i][j][k][1] += fdrag[1] + fran[1];
          nodef[i][j][k][2] += fdrag[2] + fran[2];
  ...
  if (Tp_ZERO) {
    ...
    for (i = 0; i < nelocal; i++) 
      if (emask[i] & groupbit) 
        for (j = 0; j < apc[etype[i]]; j++) 
          for (k = 0; k < npe[etype[i]]; k++) {
            nodef[i][j][k][0] -= fsumall[0];
            nodef[i][j][k][1] -= fsumall[1];
            nodef[i][j][k][2] -= fsumall[2];
          }
  }
