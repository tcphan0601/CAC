...
AtomicStrain::AtomicStrain(CAC *cac) : Pointers(cac)
{
  ...
  balance = nullptr;
  atomValidity = nullptr;
  atomVonMisesStrain = nullptr;
  atomVolStrain = nullptr;
  atomF = nullptr;
  atomQ = nullptr;
  atomW = nullptr;
  atomD = nullptr;
  atomStretchTensor = nullptr;
  atomStrainTensor = nullptr;
  atomNSD = nullptr;

  nodeValidity = nullptr;
  nodeVonMisesStrain = nullptr;
  nodeVolStrain = nullptr;
  nodeF = nullptr;
  nodeQ = nullptr;
  nodeW = nullptr;
  nodeD = nullptr;
  nodeStretchTensor = nullptr;
  nodeStrainTensor = nullptr;
  nodeNSD = nullptr;
...
void AtomicStrain::command(int narg, char **arg)
{
  ...
  for (int i = 0; i < noutputs; i++) {
    ...
    if (strcmp(arg[iarg+1], "atom") == 0) output_format_list[i] = ATOM;
...
#ifdef TECPLOT_ENABLE
    else if (strcmp(arg[iarg+1], "plt") == 0) output_format_list[i] = PLT;
...
void AtomicStrain::atoms_ref()
{
  ...
  while (nread < natoms) {
    ...
    eof = comm->read_lines_from_file(fp, nchunk, MAXLINE, buffer);
    if (eof) error->all(FLERR, "Unexpected end of data file");
    atom->data_atoms(nchunk, buffer, 0, 0, 0, nullptr);
    nread += nchunk;
  }
...
void AtomicStrain::elements()
{
  ...
  while (nread < nelements) {
    ...
    eof = comm->read_lines_from_file(fp, nchunk, MAXLINE, buffer);
    if (eof) error->all(FLERR, "Unexpected end of data file");
    element->data_elements(nchunk, buffer, 0, 0, 0, nullptr);
    nread += nchunk;
  }
...
void AtomicStrain::nodes_ref()
{
  ...
  while (nread < nnodes) {
    ...
    eof = comm->read_lines_from_file(fp, nchunk, MAXLINE, buffer);
    if (eof) error->all(FLERR, "Unexpected end of data file");
    element->data_nodes(nchunk, buffer, 0, 0, 0, nullptr);
    nread += nchunk;
  }  
  if (mapflag) {
    element->map_delete();
...
void AtomicStrain::write_output(char *filename, int output_format, int output_configuration)
{
  ...
  if (me == 0) {
    if (output_format == ASCII || output_format >= ATOM) {
      ...
      if (success == -1)
        error->one(FLERR, "Cannot close output file");
#else
      error->one(FLERR, "Need to install Tecplot Package to write Tecplot format");
#endif
    }
  }
}
