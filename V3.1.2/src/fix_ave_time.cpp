// Added from LAMMPS by Chunxu Yan
...
FixAveTime::FixAveTime(CAC *cac, int narg, char **arg) :
  Fix(cac, narg, arg), 
  ...
  column(nullptr), array(nullptr), array_total(nullptr), array_list(nullptr)
{
  ...
  nrepeat = universe->inumeric(FLERR, arg[4]);
  nfreq = universe->inumeric(FLERR, arg[5]);
  ...
  /* variable-length compute locking not yet supported in CAC;
     any_variable_length inputs with nrepeat > 1 or ave = RUNNING/WINDOW
     will proceed without row-count locking */
  ...
  if (fp && me == 0) {
    ...
    if (overwrite) {
      ...
      if ((fileend > 0) && (ftruncate(fileno(fp), fileend)))
        perror("Error while truncating output");
    }
  }
}
...
void FixAveTime::invoke_vector(bigint ntimestep)
{
  ...
  /* variable-length compute unlock not yet supported in CAC */
...
int FixAveTime::column_length(int dynamic)
{
  ...
  if (dynamic) {
    ...
    for (int i = 0; i < nvalues; i++) {
      ...
      m = value2index[i];
      if (which[i] == COMPUTE) {
        /* variable-length compute lock_length not yet supported in CAC;
           using size_vector as fallback */
        Compute *compute = modify->compute[m];
        lengthone = compute->size_vector;
      } else if (which[i] == VARIABLE) {
        double *varvec;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "file") == 0) {
      ...
      if (ave == WINDOW) {
        ...
        nwindow = universe->inumeric(FLERR, arg[iarg+2]);
      ...
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix ave/time command");
      startstep = universe->inumeric(FLERR, arg[iarg+1]);
      iarg += 2;
      ...
      offlist[noff++] = universe->inumeric(FLERR, arg[iarg+1]);
      iarg += 2;
