#ifndef CAC_IRREGULAR_COMM_H
#define CAC_IRREGULAR_COMM_H

#include "pointers.h"

namespace CAC_NS {

class IrregularComm : protected Pointers {
 public:

//#if defined(CAC_QSORT)
  // static variable across all IrregularComm objects, for qsort callback

//  static int *proc_recv_copy;
//#endif

  IrregularComm(class CAC *);
  ~IrregularComm();
  void migrate(int sortflag = 0, int preassign = 0, 
                     int *procassign = nullptr);
  int migrate_check();
  int create_comm_plan_data(int, int *, int sortflag = 0);
  void exchange_data(char *, int, char *);
  void destroy_comm_plan_data();
  bigint memory_usage();
 ...
 private:
  ...
  double *buf_send, *buf_recv;       // bufs used in migrate
  ...
  int *mproclist, *msizes;           // persistent vectors in migrate
  ...
  int create_comm_plan(int, int *, int *, int);
  void exchange(double *, int *, double *);
  void destroy_comm_plan();
