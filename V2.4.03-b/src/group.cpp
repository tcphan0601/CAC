...
/* ----------------------------------------------------------------------
   compute the max coords of group of atoms/elements
   return max coords in max[]
------------------------------------------------------------------------- */

void Group::xmax(int igroup, double *max)
{
  int groupbit = bitmask[igroup];
  ...
  int nlocal = atom->nlocal;
  double max_local[3];
  max_local[0] = max_local[1] = max_local[2] = -BIG;

 
  for (int i = 0; i < nlocal; i++) 
    if (mask[i] & groupbit) 
      for (int dim = 0; dim < 3; dim++) 
        max_local[dim] = MAX(max_local[dim],x[i][dim]);
  ...
  for (int i = 0; i < nlocal; i++) 
    if (mask[i] & groupbit)
      for (int j = 0; j < npe[etype[i]]; j++)
        for (int dim = 0; dim < 3; dim++) 
            max_local[dim] = MAX(max_local[dim],nodex[i][j][dim]);
  
  MPI_Allreduce(max_local,max,3,MPI_DOUBLE,MPI_MAX,world);
}
/* ----------------------------------------------------------------------
   compute the max coords of group of atoms/elements in region
   return max coords in max[]
   ------------------------------------------------------------------------- */

void Group::xmax(int igroup, double *max, int iregion)
{
  int groupbit = bitmask[igroup];
  Region *region = domain->regions[iregion];
  region->prematch();
  ...
  for (int i = 0; i < nlocal; i++) 
    if (mask[i] & groupbit)
      for (int j = 0; j < npe[etype[i]]; j++)
        if (region->match(nodex[i][j][0],nodex[i][j][1],nodex[i][j][2]))
          for (int dim = 0; dim < 3; dim++) 
            max_local[dim] = MAX(max_local[dim],nodex[i][j][dim]);
...
/* ----------------------------------------------------------------------
   compute the min coords of group of atoms/elements
   return min coords in min[]
------------------------------------------------------------------------- */

void Group::xmin(int igroup, double *min)
{
  int groupbit = bitmask[igroup];
  ...
  int nlocal = atom->nlocal;
  double min_local[3];
  min_local[0] = min_local[1] = min_local[2] = BIG;

  
  for (int i = 0; i < nlocal; i++) 
    if (mask[i] & groupbit) 
      for (int dim = 0; dim < 3; dim++) 
        min_local[dim] = MIN(min_local[dim],x[i][dim]);
  ...
  for (int i = 0; i < nlocal; i++) 
    if (mask[i] & groupbit)
      for (int j = 0; j < npe[etype[i]]; j++)
        for (int dim = 0; dim < 3; dim++) 
          min_local[dim] = MIN(min_local[dim],nodex[i][j][dim]);
        
  MPI_Allreduce(min_local,min,3,MPI_DOUBLE,MPI_MIN,world);
}
/* ----------------------------------------------------------------------
   compute the min coords of group of atoms/elements in region
   return min coords in min[]
   ------------------------------------------------------------------------- */

void Group::xmin(int igroup, double *min, int iregion)
{
  int groupbit = bitmask[igroup];
  Region *region = domain->regions[iregion];
  region->prematch();
