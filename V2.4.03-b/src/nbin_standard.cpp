...
void NBinStandard::setup_bins(int style)
{
  ...
  if (element->nelements) {
    ...
    if (binsizeflag) 
      ...
      binsizex_optimal = 0.5*(cutneighmax + max_bound_box[0]);
      binsizey_optimal = 0.5*(cutneighmax + max_bound_box[1]);
      binsizez_optimal = 0.5*(cutneighmax + max_bound_box[2]);
    } else {
      binsizex_optimal = 0.5*(cutneighmin + max_bound_box[0]);
      binsizey_optimal = 0.5*(cutneighmin + max_bound_box[1]);
      binsizez_optimal = 0.5*(cutneighmin + max_bound_box[2]);
    }
