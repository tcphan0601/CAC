...
void ComputeStressAtom::compute_peratom()
{
  ...
  if (element->nmax > elem_nmax) {
    ...
    array_node = memory->create(node_stress,elem_nmax,element->max_npe,6,"stress/atom:atom_stress");
  }
...
double ComputeStressAtom::memory_usage()
{
  ...
  bytes += elem_nmax * 6 * element->max_npe * sizeof(double);
  return bytes;
}
