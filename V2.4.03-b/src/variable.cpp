    ...
    double *argstack, int &nargstack, int ivar)
{
  ...
  if (strcmp(word,"countatom") && strcmp(word,"countnode") &&
      strcmp(word,"countelem") && strcmp(word,"countintpl") &&
      strcmp(word,"xmax") && strcmp(word,"xmin")) 
    //&& strcmp(word,"mass") &&
  ...
  if (strcmp(word,"countatom") == 0) {
    ...
    if (group->count_atom(igroup) + group->count_elem(igroup) == 0)
      error->all(FLERR,"Empty group in group function xmax in variable formula");

    double xmax[3];
    if (narg == 2) 
      group->xmax(igroup,xmax);
    else if (narg == 3) 
      group->xmax(igroup,xmax,region_function(args[1],ivar));
    else error->all(FLERR,"Invalid group function in variable formula");
    
    if (strcmp(args[1],"x") == 0) value = xmax[0];
    else if (strcmp(args[1],"y") == 0) value = xmax[1];
    else if (strcmp(args[1],"z") == 0) value = xmax[2];
    else error->all(FLERR,"Invalid group function in variable formula");
  } else if (strcmp(word,"xmin") == 0) {
    if (group->count_atom(igroup) + group->count_elem(igroup) == 0)
      error->all(FLERR,"Empty group in group function xmin in variable formula");
    double xmin[3];
    if (narg == 2) 
      group->xmin(igroup,xmin);
    else if (narg == 3) 
      group->xmin(igroup,xmin,region_function(args[1],ivar));
    else error->all(FLERR,"Invalid group function in variable formula");
    
    if (strcmp(args[1],"x") == 0) value = xmin[0];
    else if (strcmp(args[1],"y") == 0) value = xmin[1];
    else if (strcmp(args[1],"z") == 0) value = xmin[2];
    else error->all(FLERR,"Invalid group function in variable formula");
  }
  ...
  if (index >= 0 && index < atom->nlocal) {
    ...
    if (flag == 0) {
      if (strcmp(word,"id") == 0) mine = atom->tag[index];
        ...
        mine = atom->mass[atom->type[index]];
      }
