...
bigint Element::memory_usage()
{
  ...
  bytes += memory->usage(nsurface,n);
  bytes += memory->usage(nedge,n);
...
void Element::modify_params(int narg, char **arg)
{
  ...
  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"min/split/size") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      min_split_size = universe->inumeric(FLERR,arg[iarg+1]);
      if (min_split_size <= 0) error->all(FLERR,"Illegal element_modify command");
      iarg += 2;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"max/subintpl") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"mass_style") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal element_modify command");

      if (strcmp(arg[iarg+1],"lumped")) mass_style = LUMPED;
      else if (strcmp(arg[iarg+1],"consistent")) mass_style = CONSISTENT;
    ...
    if (fabs(xtmp-x[i][0]) > EPSILON ||
      ...
      if (update_flag)
        for (int idim = 0; idim < domain->dimension; idim++)
          evec->update_node_coord(i,idim);
      else error->one(FLERR,"Node coords does not match with element center coords");
    }
  }
}
...
void Element::update_element_bound_box()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    for (dim = 0; dim < dimension; dim++) {
      ...
      //if (periodicity[dim] && element_box_size > prd[dim]) {
      //  fprintf(screen,"element_box_size = %g prd = %g dim = %d\n",element_box_size,prd[dim],dim);
      //  error->one(FLERR,"A finite element is larger than a periodic box length; this causes material overlap");
      //}
