...
void WriteData::command(int narg, char **arg) {
  ...
  if (narg < 1)
    error->all(FLERR, "Illegal write_data command");
  ...
  if (comm->me == 0) {
    ...
    if (logfile)
      fprintf(logfile, "System init for write_data ...\n");
  }
  cac->init();
...
void WriteData::nodes(int n) {
  ...
  if (me == 0)
    memory->create(buf, (1, maxrow), ncol, "write_data:buf");
  else
    memory->create(buf, MAX(1, sendrow), ncol, "write_data:buf");
...
/* ----------------------------------------------------------------------
   write out Element Clusters section of data file
   ------------------------------------------------------------------------- */

void WriteData::element_clusters() {
  ...
  int ncol = element->evec->size_data_cluster;

  bigint sendrow = element->count_element_clusters();
  bigint maxrow;
  ...
  // pack my Element Cluster data into buf

  element->evec->pack_element_cluster_data(buf);

  // write one chunk of clusters per proc to file
  // proc 0 pings each proc, receives its chunk, writes to file
  // all other procs wait for ping, send their chunk to proc 0
  ...
  if (me == 0) {
    ...
    for (int iproc = 0; iproc < nprocs; iproc++) {
      if (iproc) {
        ...
        recvrow = sendrow;
      element->evec->write_element_cluster_data(fp, recvrow, buf);
    }
  } else {
    MPI_Recv(&tmp, 0, MPI_INT, 0, 0, world, MPI_STATUS_IGNORE);
    MPI_Rsend(&buf[0][0], sendrow * ncol, MPI_DOUBLE, 0, 0, world);
