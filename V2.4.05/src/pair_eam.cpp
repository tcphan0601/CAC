...
PairEAM::~PairEAM() {
  ...
  if (setfl) {
    for (int i = 0; i < setfl->nelements; i++)
      delete[] setfl->elements[i];
  ...
  if (fs) {
    for (int i = 0; i < fs->nelements; i++)
      delete[] fs->elements[i];
    delete[] fs->elements;
    delete[] fs->mass;
...
void PairEAM::coeff(int narg, char **arg) {
  ...
  for (ifuncfl = 0; ifuncfl < nfuncfl; ifuncfl++)
    if (strcmp(arg[2], funcfl[ifuncfl].file) == 0)
      break;

  if (ifuncfl == nfuncfl) {
    nfuncfl++;
    funcfl = (Funcfl *)memory->srealloc(funcfl, nfuncfl * sizeof(Funcfl),
                                        "pair:funcfl");
    read_file(arg[2]);
...
void PairEAM::read_file(char *filename) {
  ...
  if (me == 0) {
    ...
    (void)!fgets(line, MAXLINE, fptr);
    nwords = sscanf(line, "%d %lg %d %lg %lg", &file->nrho, &file->drho,
                    &file->nr, &file->dr, &file->cut);
  }
  ...
  if ((nwords != 5) || (file->nrho <= 0) || (file->nr <= 0) ||
      (file->dr <= 0.0))
...
void PairEAM::compute(int eflag, int vflag) {
  ...
  // fpj: derivative of embedding energy at the neighbor site of the integration
  // point
  ...
  // same as LAMMPS
  // loop over neighbors (atoms and interpolated atoms) of my atoms to calculate
  ...
  // loop over neighbors (atoms and interpolated atoms) of my element nodes to
  // calculate electron density at node k in element i
  ...
  for (ii = 0; ii < einum; ii++) {
    ...
    for (k = 0; k < npe[ietype]; k++) {
      ...
      // loop over neighboring atoms j to add electron density contribution to
      // node k in element i
      ...
      // loop over neighboring interpolated atoms jintpl in element j to add
      // electron density contribution to node k in element i
  ...
  // 3. Loop over neighboring interpolated atoms. Add force contribution to fx
  // fy fz
  ...
  for (ii = 0; ii < einum; ii++) {
    ...
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      ...
      if (eflag && inode >= 0) {
        phi = ((coeff[3] * p + coeff[4]) * p + coeff[5]) * p + coeff[6];
        if (noderho[i][inode] > rhomax)
          phi += fpi * (noderho[i][inode] - rhomax);
        phi *= scale[ictype][ictype];
        if (eflag_global)
          eng_vdwl += phi;
        if (eflag_atom)
      ...
      // if skip if rigid and intg point is inner

      iintg_local = e2ilist[ii] + iintg;
      jnum = numneighi2a[iintg_local];
      if (jnum)
        if (!(element->inner_rigid_flag && is_inner[ietype][iintg])) {
...
int PairEAM::pack_atom_forward_comm(int n, int *list, double *buf, int pbc_flag,
                                    int *pbc) {
  int i, j, m;
...
int PairEAM::pack_elem_forward_comm(int n, int *list, double *buf, int pbc_flag,
                                    int *pbc) {
  int i, j, k, m;
  int *npe = element->npe;
  int *etype = element->etype;
...
void PairEAM::pair_ia2ia(int i, int iintpl, double *xtmp, int j, int jintpl,
                         double *jx, double *f) {
  int m, node;
  double r, rsq, rhoip, rhojp, z2, z2p, recip, phi, phip, psip, fpair;
...
double PairEAM::memory_usage() {
  ...
  bytes += maxeelem * max_npe * sizeof(double);
  bytes += maxvelem * max_npe * 6 * sizeof(double);
