...
void NPairHalfBinNewtoffIntg::build(NeighList *list) {
  ...
  int i, j, jj, jnum, k, l, n, ictype, jctype, ietype, jetype, ibin, isub,
      nintg;
  ...
  int iintg, jintg, iintpl, jintpl, node, jnpe;
  ...
  for (i = 0; i < nalocal; i++) {
    ...
    // loop over all elements in other element bins in stencil including self if
    // there are elements
    ...
    if (neall) {
      ...
      ibin = nb->coord2elembin(xtmp, ytmp, xtmp);
      for (k = 0; k < nelemstencil; k++) {
        for (j = elembinhead[ibin + elemstencil[k]]; j >= 0; j = elembins[j]) {
          ...
          if (xtmp < jboxlo[0] - cutoff || xtmp > jboxhi[0] + cutoff)
            continue;
          if (ytmp < jboxlo[1] - cutoff || ytmp > jboxhi[1] + cutoff)
            continue;
          if (ztmp < jboxlo[2] - cutoff || ztmp > jboxhi[2] + cutoff)
            continue;

          // interpolate the centers of the subelements of element j and loop
          // over sub elements of element j
          ...
          for (isub = 0; isub < nsubelem[jetype]; isub++) {
            ...
            if (rsq < cutoffsq) {
              ...
              // loop over interpolate the atoms inside the subelement isub of
              // element j that is neighbor to atom i
      ...
      if (a2iapage->status() || a2ia_indexpage->status())
        error->one(FLERR, "Neighbor list overflow, boost neigh_modify one");
    } else {
  ...
  for (i = 0; i < nelocal; i++) {
    ...
    for (k = 0; k < nelemstencil; k++)
      for (j = elembinhead[ibin + elemstencil[k]]; j >= 0; j = elembins[j]) {
        ...
        if (iboxlo[0] > jboxhi[0] + cutoff || iboxhi[0] < jboxlo[0] - cutoff)
          continue;
        if (iboxlo[1] > jboxhi[1] + cutoff || iboxhi[1] < jboxlo[1] - cutoff)
          continue;
        if (iboxlo[2] > jboxhi[2] + cutoff || iboxhi[2] < jboxlo[2] - cutoff)
          continue;
  ...
  for (i = 0; i < einum; i++) {
    ...
    for (iintg = 0; iintg < nintg; iintg++) {
      ...
      if (naall) {
        ...
        for (k = 0; k < natomstencil; k++)
          for (j = atombinhead[ibin + atomstencil[k]]; j >= 0;
               j = atombins[j]) {
            jctype = atype[j];
            delx = xtmp - ax[j][0];
            dely = ytmp - ax[j][1];
      ...
      // loop over sub-element in element i and search through interpolated
      // atoms within sub-element
      ...
      if (i2iapage->status() || i2ia_indexpage->status())
        error->one(FLERR, "Neighbor list overflow, boost neigh_modify one");
      iinum++;
    }
  }
