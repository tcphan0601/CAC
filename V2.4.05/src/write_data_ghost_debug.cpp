...
void WriteDataGhostDebug::command(int narg, char **arg) {
  if (domain->box_exist == 0)
    error->all(
        FLERR,
        "Write_data_ghost_debug command before simulation box is defined");

  if (narg < 1)
    error->all(FLERR, "Illegal write_data_ghost_debug command");
  ...
  if ((ptr = strchr(arg[0], '*'))) {
    *ptr = '\0';
    sprintf(file, "%s" BIGINT_FORMAT "%s_proc_%d", arg[0], update->ntimestep,
            ptr + 1, me);
  } else
    sprintf(file, "%s_proc_%d", arg[0], me);
...
void WriteDataGhostDebug::header() {
  fprintf(fp,
          "CAC data file via write_data_ghost_debug, "
          "timestep = " BIGINT_FORMAT " from proc %d\n",
          update->ntimestep, me);

  fprintf(fp, "\n");

  long int n = atom->nlocal + element->nlocal + atom->nghost + element->nghost;
  int *npe = element->npe;
