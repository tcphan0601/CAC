...
void WriteTecplot::write(char *file)
{
  ...
  if (me == 0) {
    if (fileformat == ASCII) {
      ...
      fprintf(fp,"variables = \"x\", \"y\", \"z\", \"ID\", \"type\"\n");
    } else {
  ...
  if (nelements) {
    ...
    element->reset_node_tags(group->bitmask[group->find("all")],0);
    node_connect();
  }
  ...
  if (fileformat == ASCII) {
    ...
    if (me == 0) {
      ...
      for (int ival = 0; ival < ncol; ival++) {
        ...
        element->evec->pack_node_tecplot_binary(buf,ival);
