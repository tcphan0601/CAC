...
PairCoulWolf::~PairCoulWolf() {
  if (allocated) {
    ...
    memory->destroy(cut);
    memory->destroy(q_one_matrix);
    memory->destroy(q_two_matrix);
  }
}
...
void PairCoulWolf::allocate() {
  ...
  memory->create(q_one_matrix, n + 1, n + 1, "pair:q_one_matrix");
  memory->create(q_two_matrix, n + 1, n + 1, "pair:q_two_matrix");
...
void PairCoulWolf::settings(int narg, char **arg) {
  ...
  alpha = universe->numeric(FLERR, arg[0]);
...
void PairCoulWolf::coeff(int narg, char **arg) {
  ...
  double q_one = universe->numeric(FLERR, arg[2]);
  double q_two = universe->numeric(FLERR, arg[3]);
  double cut_one = cut_global;

  int count = 0;
  for (int i = ilo; i <= ihi; i++) {
    for (int j = MAX(jlo, i); j <= jhi; j++) {
      q_one_matrix[i][j] = q_one;
      q_two_matrix[i][j] = q_two;
      cut[i][j] = cut_one;
      setflag[i][j] = 1;
      count++;
    }
...
void PairCoulWolf::write_data_all(FILE *fp) {
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp, "%d %d %g %g %g\n", i, j, q_one_matrix[i][j],
              q_two_matrix[i][j], cut[i][j]);
}
...
void PairCoulWolf::compute(int eflag, int vflag) {
  ...
  double r, prefactor, erfcc, erfcd, v_sh, dvdrr, e_self, e_shift, f_shift,
      qisq, ecoul;
  double forcecoul, factor_coul;
  e_self = v_sh = 0.0;
  e_shift = erfc(alpha * cut_global) / cut_global;
  f_shift = -(e_shift + 2.0 * alpha / MY_PIS *
                            exp(-alpha * alpha * cut_global * cut_global)) /
            cut_global;
    ...
    if (jnum) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq[ictype][jctype]) {
          r = sqrt(rsq);
          prefactor = qqrd2e * q_one_matrix[ictype][jctype] *
                      q_two_matrix[ictype][jctype] / r;
          erfcc = erfc(alpha * r);
          erfcd = exp(-alpha * alpha * r * r);
          v_sh = (erfcc - e_shift * r) * prefactor;
          dvdrr = (erfcc / rsq + 2.0 * alpha / MY_PIS * erfcd / r) + f_shift;
          ...
          if (evflag)
            atom_ev_tally(i, j, nalocal, newton_pair, 0.0, ecoul, fpair, delx,
                          dely, delz);
        }
      }
    }
    ...
    if (jnum) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq[ictype][jctype]) {
          ...
          if (evflag)
            atom_ev_tally_full(i, 0.0, ecoul, fpair, delx, dely, delz);
        }
      }
    }
  }
      ...
      if (jnum) {
        ...
        for (jj = 0; jj < jnum; jj++) {
          ...
          if (rsq < cutsq[ictype][jctype]) {
            ...
            if (evflag)
              node_ev_tally(i, inode, 0.0, ecoul, fpair, delx, dely, delz);
          }
        }
      }
...
void PairCoulWolf::pair_a2a(int i, int j, double *f) {
  ...
  r = sqrt(rsq);
  prefactor =
      qqrd2e * q_one_matrix[ictype][jctype] * q_two_matrix[ictype][jctype] / r;
  erfcc = erfc(alpha * r);
  erfcd = exp(-alpha * alpha * r * r);
  v_sh = (erfcc - e_shift * r) * prefactor;
  dvdrr = (erfcc / rsq + 2.0 * alpha / MY_PIS * erfcd / r) + f_shift;
...
void PairCoulWolf::pair_a2ia(int i, int j, int jintpl, double *jx, double *f) {
  double delx, dely, delz;
  double rsq, r2inv, r6inv, forcelj, fpair;
  int ictype = atom->type[i];
...
void PairCoulWolf::pair_ia2ia(int i, int iintpl, double *xtmp, int j,
                              int jintpl, double *jx, double *f) {
  double delx, dely, delz;
  double rsq, r2inv, r6inv, forcelj, fpair;
