...
DumpStressMech::DumpStressMech(CAC *cac, int narg, char **arg)
    : Dump(cac, narg, arg) {

  if (narg != 6)
    error->all(FLERR, "Illegal dump stress/mech command");
  ...
  if (compute_not_exist)
    error->all(FLERR,
               "Invalid compute stress/mech id for dump stress/mech command");
...
void DumpStressMech::pack_atom(tagint *ids) { (this->*pack_atom_choice)(ids); }
...
int DumpStressMech::convert_surfaces(int n, int size_one, double *mybuf) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    offset += sprintf(&sbuf[offset], format, mybuf[m], mybuf[m + 1],
...
void DumpStressMech::write_surfaces(int n, double *mybuf) {
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp, format, mybuf[m + 1], mybuf[m + 2], mybuf[m + 3], mybuf[m + 4],
            mybuf[m + 5]);
    m += 6;
  }
}
...
int DumpStressMech::count_elements() { return 0; }

/*-------------------------------------------------------------------------------------------------*/

int DumpStressMech::count_nodes() { return 0; }
