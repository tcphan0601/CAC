...
//        && force->kspace->fftbench) fftflag = 1;
  ...
  if (timer->has_loop()) {
    ...
    // total atoms (including interpolated atoms)
    bigint natoms = atom->natoms;
    bigint nelements = element->nelements;
    bigint ntotalatoms;
    bigint nlocalintpl = 0;
    for (i = 0; i < element->nlocal; i++) nlocalintpl += element->nintpl[element->etype[i]];
    MPI_Allreduce(&nlocalintpl,&ntotalatoms,1,MPI_CAC_BIGINT,MPI_SUM,world);
    ntotalatoms += natoms;
   
    if (me == 0) {
      int ntasks = nprocs * nthreads;
      const char fmt1[] = "Loop time of %g on %d procs "
        "for %d steps with " BIGINT_FORMAT " atoms, " BIGINT_FORMAT" elements,\n  total equivalent number of atoms: " BIGINT_FORMAT"\n\n";
      if (screen) fprintf(screen,fmt1,time_loop,ntasks,update->nsteps,
                          natoms,nelements,ntotalatoms);
...
*/
  ...
  /*
    ...
    if ((ifix >= 0) && timer->has_full() && me == 0) {
      ...
      ThrData *td;
      FixOMP *fixomp = static_cast<FixOMP *>(cac->modify->fix[ifix]);
    ...
    if (cac->kokkos && cac->kokkos->ngpu > 0)
    ...
    if (histoflag) {
      ...
      if (m < neighbor->old_nrequest) {
        ...
        } else if (cac->kokkos) nneigh = cac->kokkos->neigh_count(m);
      }
      ...
      if (m < neighbor->old_nrequest) {
        if (!neighbor->lists[m]->kokkos && neighbor->lists[m]->numneigh) {
          ...
          nneighfull = cac->kokkos->neigh_count(m);
