/* ----------------------------------------------------------------------
   CAC - the concurrent atomistic-continuum (CAC) simulation environment

   See the README file in the top-level CAC directory.
------------------------------------------------------------------------- */

/* ----------------------------------------------------------------------
  Adopted by Yipeng Peng from LAMMPS
------------------------------------------------------------------------- */
...
ChangeBox::ChangeBox(CAC *cac) : Pointers(cac) {}
...
void ChangeBox::command(int narg, char **arg) {
  ...
  if (narg < 2)
    error->all(FLERR, "Illegal change_box command");
  if (modify->nfix_restart_peratom)
    error->all(FLERR, "Cannot change_box after "
                      "reading restart file with per-atom info");

  if (comm->me == 0 && screen)
    fprintf(screen, "Changing box ...\n");

  // group

  int igroup = group->find(arg[0]);
  if (igroup == -1)
    error->all(FLERR, "Could not find change_box group ID");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "x") == 0 || strcmp(arg[iarg], "y") == 0 ||
      ...
      if (strcmp(arg[iarg + 1], "final") == 0) {
        ...
        ops[nops].flo = universe->numeric(FLERR, arg[iarg + 2]);
        ops[nops].fhi = universe->numeric(FLERR, arg[iarg + 3]);
        ...
        ops[nops].dlo = universe->numeric(FLERR, arg[iarg + 2]);
        ops[nops].dhi = universe->numeric(FLERR, arg[iarg + 3]);
        ops[nops].vdim1 = ops[nops].vdim2 = -1;
        nops++;
        iarg += 4;
        ...
        ops[nops].scale = universe->numeric(FLERR, arg[iarg + 2]);
        ops[nops].vdim1 = ops[nops].vdim2 = -1;
        nops++;
        iarg += 3;
        ...
        if (ops[nops - 1].vdim2 >= 0)
          error->all(FLERR, "Change_box volume used incorrectly");
        else if (ops[nops - 1].vdim1 >= 0)
          ops[nops - 1].vdim2 = ops[nops].dim;
        else
          ops[nops - 1].vdim1 = ops[nops].dim;
        iarg += 2;
      ...
      if (strcmp(arg[iarg + 1], "final") == 0) {
        if (iarg + 3 > narg)
          error->all(FLERR, "Illegal change_box command");
        ops[nops].flavor = FINAL;
        ops[nops].ftilt = universe->numeric(FLERR, arg[iarg + 2]);
        nops++;
        iarg += 3;
      } else if (strcmp(arg[iarg + 1], "delta") == 0) {
        if (iarg + 3 > narg)
          error->all(FLERR, "Illegal change_box command");
        ops[nops].flavor = DELTA;
        ops[nops].dtilt = universe->numeric(FLERR, arg[iarg + 2]);
        nops++;
        iarg += 3;
      } else
        error->all(FLERR, "Illegal change_box command");
  ...
  for (int i = 0; i < nops; i++)
    if (ops[i].style == FINAL || ops[i].style == DELTA)
      flag = 1;
  ...
  for (int m = 0; m < nops; m++) {
    if (ops[m].style == XYZ) {
      ...
      if (ops[m].flavor == FINAL) {
        ...
        double mid =
            0.5 * (domain->boxlo[ops[m].dim] + domain->boxhi[ops[m].dim]);
      ...
      if (ops[m].flavor == FINAL) {
        if (ops[m].dim == XY)
          domain->xy = scale[X] * ops[m].ftilt;
        else if (ops[m].dim == XZ)
          domain->xz = scale[X] * ops[m].ftilt;
        else if (ops[m].dim == YZ)
          domain->yz = scale[Y] * ops[m].ftilt;
        domain->set_initial_box();
        domain->set_global_box();
        domain->set_local_box();
        domain->print_box("  ");
        ...
        if (ops[m].dim == XY)
          domain->xy += scale[X] * ops[m].dtilt;
        else if (ops[m].dim == XZ)
          domain->xz += scale[X] * ops[m].dtilt;
        else if (ops[m].dim == YZ)
          domain->yz += scale[Y] * ops[m].dtilt;
        domain->set_initial_box();
        domain->set_global_box();
        domain->set_local_box();
        domain->print_box("  ");
      ...
      if (domain->dimension == 2 && domain->zperiodic == 0)
        error->all(FLERR, "Cannot change box z boundary to "
      ...
      for (int i = 0; i < modify->nfix; i++)
        if (modify->fix[i]->no_change_box)
          error->all(FLERR, "Cannot change box ortho/triclinic with "
      ...
      for (i = 0; i < nalocal; i++)
        if (amask[i] & groupbit)
          domain->x2lamda(ax[i], ax[i], boxlo, h_inv);
      ...
      for (i = 0; i < nelocal; i++)
        if (emask[i] & groupbit)
          for (int j = 0; j < npe[etype[i]]; j++)
            domain->x2lamda(nodex[i][j], nodex[i][j], boxlo, h_inv);

      for (i = 0; i < nelocal; i++)
        if (emask[i] & groupbit)
          for (int j = 0; j < npe[etype[i]]; j++)
            domain->lamda2x(nodex[i][j], nodex[i][j]);
  ...
  for (int i = 0; i < nelocal; i++)
    domain->remap(ex[i], nodex[i], eimage[i], npe[etype[i]]);
  ...
  domain->reset_box();
  IrregularComm *irregularcomm = new IrregularComm(cac);
  irregularcomm->migrate(1);
  delete irregularcomm;
  if (domain->triclinic) {
  ...
  if (natoms != atom->natoms && comm->me == 0) {
    char str[128];
    sprintf(str,
            "Lost atoms via change_box: original " BIGINT_FORMAT
            " current " BIGINT_FORMAT,
            atom->natoms, natoms);
    error->warning(FLERR, str);
  }

  bigint nelements;
  bigint nblocale = element->nlocal;
  MPI_Allreduce(&nblocale, &nelements, 1, MPI_CAC_BIGINT, MPI_SUM, world);
