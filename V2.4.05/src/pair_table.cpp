...
void PairTable::compute(int eflag, int vflag) {
  ...
  int i, j, ii, jj, jnum, ictype, jctype, itable;
  int ietype, jetype;
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (newton_pair || j < nalocal) {
          ...
          if (itag > jtag) {
            ...
            // force contribution from the othersize will be added in
            // reverse_comm see LAMMPS documents

            if (ax[j][2] < iz)
              continue;
            if (ax[j][2] == iz && ax[j][1] < iy)
              continue;
            if (ax[j][2] == iz && ax[j][1] == iy && ax[j][0] < ix)
              continue;
          }
        }
        ...
        if (rsq < cutsq[ictype][jctype]) {
          ...
          if (tabstyle == LINEAR) {
            ...
            value = a * tb->f[itable] + b * tb->f[itable + 1] +
                    ((a * a * a - a) * tb->f2[itable] +
                     (b * b * b - b) * tb->f2[itable + 1]) *
                        tb->deltasq6;
            fpair = value;
            // fpair = factor_lj * value;
          } else {
...
void PairTable::settings(int narg, char **arg) {
  ...
  if (strcmp(arg[0], "lookup") == 0)
    ...
    error->all(FLERR, "Unknown table style in pair_style command");

  tablength = universe->inumeric(FLERR, arg[1]);
  if (tablength < 2)
    error->all(FLERR, "Illegal number of pair table entries");
...
void PairTable::coeff(int narg, char **arg) {
  ...
  tables = (Table *)memory->srealloc(tables, (ntables + 1) * sizeof(Table),
                                     "pair:tables");
  Table *tb = &tables[ntables];
  null_table(tb);
  if (me == 0)
    read_table(tb, arg[2], arg[3]);
  bcast_table(tb);

  // set table cutoff

  if (narg == 5)
    tb->cut = universe->numeric(FLERR, arg[4]);
  else if (tb->rflag)
    tb->cut = tb->rhi;
  else
  ...
  if (tb->ninput <= 1)
    error->one(FLERR, "Invalid pair table length");
  double rlo, rhi;
  if (tb->rflag == 0) {
    rlo = tb->rfile[0];
  ...
  tb->match = 0;
  if (tabstyle == LINEAR && tb->ninput == tablength && tb->rflag == RSQ &&
      tb->rhi == tb->cut)
    tb->match = 1;
...
void PairTable::read_table(Table *tb, char *file, char *keyword) {
  ...
  while (1) {
    ...
    if (strcmp(word, keyword) == 0)
      break;                         // matching keyword
    (void)!fgets(line, MAXLINE, fp); // no match, skip section
  ...
  for (int i = 0; i < tb->ninput; i++) {
    ...
    if (4 != sscanf(line, "%d %lg %lg %lg", &itmp, &rfile, &tb->efile[i],
                    &tb->ffile[i]))
      ++cerror;
    ...
    if (tb->rflag && fabs(rnew - rfile) / rfile > EPSILONR)
      rerror++;
  ...
  // bitmapped tables do not follow regular ordering, so we cannot check them
  // here
  ...
  if (ferror) {
    char str[128];
    sprintf(str,
            "%d of %d force values in table are inconsistent with -dE/dr.\n"
  ...
  if (rerror) {
    char str[128];
    sprintf(str,
            "%d of %d distance values in table with relative error\n"
  ...
  if (cerror) {
    char str[128];
    sprintf(str,
            "%d of %d lines in table were incomplete\n"
...
void PairTable::param_extract(Table *tb, char *line) {
  ...
  if (tb->ninput == 0)
    error->one(FLERR, "Pair table parameters did not set N");
}
...
void PairTable::compute_table(Table *tb) {
  ...
  if (tabstyle == SPLINE) {
    ...
    if (tb->fpflag)
      fp0 = (tb->fplo / sqrt(tb->innersq) - tb->f[0] / tb->innersq) /
            (2.0 * sqrt(tb->innersq));
    else {
    ...
    if (tb->fpflag && tb->cut == tb->rfile[tb->ninput - 1])
      fpn = (tb->fphi / tb->cut - tb->f[tlm1] / (tb->cut * tb->cut)) /
            (2.0 * tb->cut);
    else {
...
void PairTable::spline(double *x, double *y, int n, double yp1, double ypn,
                       double *y2) {
  int i, k;
  double p, qn, sig, un;
...
/*
   ...
   if (rsq < tb->innersq) error->one(FLERR,"Pair distance < table inner
cutoff");
