...
void WriteDataElem::command(int narg, char **arg) {
  if (domain->box_exist == 0)
    error->all(FLERR,
               "Write_data_elem command before simulation box is defined");

  if (narg < 1)
    error->all(FLERR, "Illegal write_data_elem command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "node") == 0) {
      ...
      if (comm->nprocs != 1)
        error->all(
            FLERR,
            "Ghost option for write_data_elem doesn't allow multiple procs");
      if (strcmp(arg[iarg + 1], "yes") == 0)
        ghostflag = 1;
      else if (strcmp(arg[iarg + 1], "no") == 0)
        ghostflag = 0;
  ...
  if (comm->me == 0) {
    ...
    if (logfile)
      fprintf(logfile, "System init for write_data_elem ...\n");
  }
...
void WriteDataElem::write(char *file) {
  ...
  if (nelements != element->nelements &&
      output->thermo->elemlostflag == Thermo::ERROR)
  ...
  if (natoms) {
    ...
    if (ghostflag) {
      ...
      MPI_Allreduce(&max_atom_tag, &max_atom_tag_all, 1, MPI_CAC_TAGINT,
                    MPI_MAX, world);
    }
  }
  ...
  if (nelements) {
    ...
    if (nodeflag) {
      if (ghostflag) {
        ...
        MPI_Allreduce(&max_elem_tag, &max_elem_tag_all, 1, MPI_CAC_TAGINT,
                      MPI_MAX, world);
...
void WriteDataElem::header() {
  fprintf(fp,
          "CAC data file via write_data_elem and should not be used as an "
          "input data, "
...
void WriteDataElem::atoms() {
  ...
  double **buf;
  if (me == 0)
    memory->create(buf, MAX(1, maxrow), ncol, "write_data_elem:buf");
  else
    memory->create(buf, MAX(1, sendrow), ncol, "write_data_elem:buf");
  ...
  if (me == 0) {
    ...
    for (int iproc = 0; iproc < nprocs; iproc++) {
      if (iproc) {
        MPI_Irecv(&buf[0][0], maxrow * ncol, MPI_DOUBLE, iproc, 0, world,
                  &request);
...
void WriteDataElem::atom_velocities() {
  ...
  double **buf;
  if (me == 0)
    memory->create(buf, MAX(1, maxrow), ncol, "write_data:buf");
  else
    memory->create(buf, MAX(1, sendrow), ncol, "write_data:buf");
