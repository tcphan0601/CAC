...
Neighbor::Neighbor(CAC *cac)
    : Pointers(cac), pairclass(NULL), pairnames(NULL), pairmasks(NULL) {
...
void Neighbor::init() {
  ...
  if (dist_check) {
    ...
    if (nodemaxhold == 0) {
      ...
      memory->create(nodexhold, nodemaxhold, element->max_npe, 3,
                     "neigh:nodexhold");
    }
  }
...
void Neighbor::morph_copy() {
  ...
  for (i = 0; i < nrequest; i++) {
    ...
    for (j = 0; j < nrequest; j++) {
      ...
      if (irq->atomlist && !jrq->atomlist)
        continue;
      if (irq->atomonlylist && !jrq->atomonlylist)
        continue;
      if (irq->elemlist && !jrq->elemlist)
        continue;
      if (irq->intglist && !jrq->intglist)
        continue;
      if (irq->intpllist && !jrq->intpllist)
        continue;
      ...
      if (irq->cutoff != jrq->cutoff)
        continue;
      if (irq->sort != jrq->sort)
        continue;
      if (irq->sortnum != jrq->sortnum)
        continue;
...
void Neighbor::print_pairwise_info() {
  ...
  if (atom->natoms) {
    if (elembinsizeflag)
      atombinsize = atombinsize_user;
    else if (style == BIN)
      atombinsize = 0.5 * cutneighmax;
    else
      atombinsize = 0.5 * cutneighmin;
    if (atombinsize == 0.0)
      atombinsize = bbox[0];
  }
  ...
  if (element->nelements) {
    if (elembinsizeflag)
      elembinsizex = elembinsizey = elembinsizez = elembinsize_user;
    else if (style == BIN) {
      elembinsizex =
          0.5 * (cutneighmax + element->max_element_bound_box_size[0]);
      elembinsizey =
          0.5 * (cutneighmax + element->max_element_bound_box_size[1]);
      elembinsizez =
          0.5 * (cutneighmax + element->max_element_bound_box_size[2]);
    } else {
      elembinsizex =
          0.5 * (cutneighmin + element->max_element_bound_box_size[0]);
      elembinsizey =
          0.5 * (cutneighmin + element->max_element_bound_box_size[1]);
      elembinsizez =
          0.5 * (cutneighmin + element->max_element_bound_box_size[2]);
    }
    if (elembinsizex == 0.0)
      elembinsizex = bbox[0];
    if (elembinsizex == 0.0)
  ...
  for (m = 0; m < 2; m++) {
    ...
    if (out) {
      ...
      if (style != NSQ) {
        if (atom->natoms)
          fprintf(out, "  atom bin: binsize = %g, bins = %g %g %g\n",
                  atombinsize, ceil(bbox[0] / atombinsize),
                  ceil(bbox[1] / atombinsize), ceil(bbox[2] / atombinsize));
        if (element->nelements)
          fprintf(out, "  elem bin: binsize = %g %g %g, bins = %g %g %g\n",
                  elembinsizex, elembinsizey, elembinsizez,
                  ceil(bbox[0] / elembinsizex), ceil(bbox[1] / elembinsizey),
      ...
      for (i = 0; i < nlist; i++) {
        ...
        if (rq->occasional)
          fprintf(out, ", occasional");
        else
          fprintf(out, ", perpetual");
        ...
        if (rq->newton == 0) {
          if (force->newton_pair)
            fprintf(out, ", newton on");
          else
            fprintf(out, ", newton off");
        } else if (rq->newton == 1)
          fprintf(out, ", newton on");
        else if (rq->newton == 2)
          fprintf(out, ", newton off");
        ...
        if (rq->atomlist)
          fprintf(out, ", atom");
        if (rq->atomonlylist)
          fprintf(out, ", atomonly");
        if (rq->elemlist)
          fprintf(out, ", element");
        if (rq->intglist)
          fprintf(out, ", integration point");
        if (rq->intpllist)
          fprintf(out, ", interpolated atom");
        if (rq->ghost)
          fprintf(out, ", ghost");
        if (rq->size)
          fprintf(out, ", size");
        if (rq->cut)
          fprintf(out, ", cut %g", rq->cutoff);
        if (rq->sort) {
          fprintf(out, ", number of sorted neighbors");
          if (rq->sortnum)
            fprintf(out, ": %d", rq->sortnum);
          else
            fprintf(out, ": all");
        }
        if (rq->off2on)
        ...
        fprintf(out, "      ");
        if (lists[i]->pair_method == 0)
          fprintf(out, "pair build: none\n");
        else
          fprintf(out, "pair build: %s\n",
                  pairnames[lists[i]->pair_method - 1]);

        fprintf(out, "      ");
        if (lists[i]->stencil_method == 0)
          fprintf(out, "stencil: none\n");
        else
          fprintf(out, "stencil: %s\n",
                  stencilnames[lists[i]->stencil_method - 1]);

        fprintf(out, "      ");
        if (lists[i]->bin_method == 0)
          fprintf(out, "bin: none\n");
        else
      ...
      for (i = 0; i < nstencil; i++)
        fprintf(out, "    (%d) %s\n", i + 1,
                stencilnames[neigh_stencil[i]->istyle - 1]);

      fprintf(out, "  %d bin methods\n", nbin);
      for (i = 0; i < nbin; i++)
        fprintf(out, "    (%d) %s\n", i + 1,
                binnames[neigh_bin[i]->istyle - 1]);
    }
  }
}
...
NeighRequest *Neighbor::find_request(void *classptr) {
  ...
  for (int i = 0; i < nrequest; i++)
    if (requests[i]->requestor == classptr)
      return requests[i];
...
int Neighbor::choose_pair(NeighRequest *rq) {
  ...
  for (int i = 0; i < npclass; i++) {
    ...
    // neighborlist for atoms, elements, integration points and/or interpolated
    // atoms must match
...
template <typename T> NBin *Neighbor::bin_creator(CAC *cac) {
  return new T(cac);
}
...
template <typename T> NStencil *Neighbor::stencil_creator(CAC *cac) {
  return new T(cac);
}
...
template <typename T> NPair *Neighbor::pair_creator(CAC *cac) {
  return new T(cac);
}
...
int Neighbor::decide() {
  if (must_check) {
    ...
    for (int i = 0; i < fix_check; i++)
      if (n == modify->fix[fixchecklist[i]]->next_reneighbor)
        return 1;
  }
...
void Neighbor::build(int topoflag) {
  ...
  if (triclinic)
    element->update_element_bound_box(0);
...
void Neighbor::build_one(class NeighList *mylist, int preflag) {
  ...
  // preflag is set by fix_adaptive, compute
...
void Neighbor::set(int narg, char **arg) {
  ...
  if (strcmp(arg[1], "nsq") == 0) {
    style = NSQ;
    error->all(FLERR, "No NSQ neighbor style yet");
