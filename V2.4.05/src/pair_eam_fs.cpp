...
PairEAMFS::PairEAMFS(CAC *cac) : PairEAM(cac) {
  one_coeff = 1;
  manybody_flag = 1;
}
...
void PairEAMFS::coeff(int narg, char **arg) {
  ...
  for (i = 3; i < narg; i++) {
    ...
    for (j = 0; j < fs->nelements; j++)
      if (strcmp(arg[i], fs->elements[j]) == 0)
        break;
    if (j < fs->nelements)
      map[i - 2] = j;
    else
  ...
  for (i = 1; i <= n; i++) {
    for (j = i; j <= n; j++) {
      if (map[i] >= 0 && map[j] >= 0) {
        setflag[i][j] = 1;
        if (i == j)
          atom->set_mass(FLERR, i, fs->mass[map[i]]);
        count++;
      }
      scale[i][j] = 1.0;
    }
...
void PairEAMFS::read_file(char *filename) {
  ...
  int nwords = universe->count_words(line);
  ...
  memory->create(file->rhor, file->nelements, file->nelements, file->nr + 1,
                 "pair:rhor");
  ...
  for (i = 0; i < file->nelements; i++) {
    ...
    if (me == 0)
      grab(fptr, file->nrho, &file->frho[i][1]);
    MPI_Bcast(&file->frho[i][1], file->nrho, MPI_DOUBLE, 0, world);

    for (j = 0; j < file->nelements; j++) {
      if (me == 0)
        grab(fptr, file->nr, &file->rhor[i][j][1]);
  ...
  for (i = 0; i < file->nelements; i++)
    for (j = 0; j <= i; j++) {
      if (me == 0)
        grab(fptr, file->nr, &file->z2r[i][j][1]);
