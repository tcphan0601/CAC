...
FixTempPress::FixTempPress(CAC *cac, int narg, char **arg)
    : Fix(cac, narg, arg), xstr(NULL), idregion(NULL) //, sforce(NULL)

{
  if (narg < 4)
    error->all(FLERR, "Illegal fix temp/press command");
  if (domain->dimension == 2)
    error->all(FLERR, "Fix temp/press command does not support 2D yet");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "random") == 0) {
      ...
      if (iarg + 2 > narg)
        error->all(FLERR, "Illegal velocity command");
  ...
  force_flag = 0;
  net_force[0] = net_force[1] = net_force[2] = net_force[3] = net_force[4] =
      net_force[5] = net_force[6] = 0.0;
...
void FixTempPress::init() {
  ...
  if (xstr) {
    ...
    if (input->variable->equalstyle(xvar))
      ...
      error->all(FLERR, "Variable for fix temp/press is invalid style");
  }
...
void FixTempPress::post_force(int vflag) {
  ...
  if (seed_time > 900000000)
    seed_time = seed_time - floor(seed_time / 900000000);
  ...
  pvalue_c = tvalue / 2.0 * 3.0 * boltz * inv_volume * 2 *
             (element->nintpls - element->nnodes);
  ...
  if (varflag == CONSTANT) {
    ...
    for (i = 0; i < nelocal; i++)
      if (emask[i] & groupbit) {
        if (region && !region->match(ex[i]))
          continue;

        ietype = etype[i];

        factor =
            0.25 / nodal_weight[ietype]; // weight for nodal force calculation
        pvalue = 0.0;
        if (const_flag)
        ...
        if (rand_flag || srand_flag)
          pvalue +=
              pvalue_c * arandom * (random->uniform() - 0.5) * 2.0 * factor;

        if (dynamic_flag == 1) {
          amp_temp = factor * adynamic * pvalue_c;
          for (k = 0; k < nfreq; k++) {
            rand_phase = random_p->uniform() * MY_2PI;
            pvalue += amp_temp * ffreq[k] *
                      sin(update->ntimestep * update->dt * vfreq[k] * MY_2PI +
                          rand_phase);
          }
        }
        ...
        for (j = 0; j < nsurface[element_shape_ids[ietype]]; j++) {
          ...
          for (k = 0; k < num_node; k++) {
            if (srand_flag) {
              if (xstyle)
                nodef[i][surface_node[k]][shear_direc - 1] +=
                    vec[shear_direc - 1];
            } else {
              if (xstyle)
                nodef[i][surface_node[k]][0] += vec[0];
              if (xstyle)
                nodef[i][surface_node[k]][1] += vec[1];
              if (xstyle)
                nodef[i][surface_node[k]][2] += vec[2];
            }
    ...
    } else error->all(FLERR,"Temperature pressure could only be added on
    element.");
    */
  }
  // clean up
