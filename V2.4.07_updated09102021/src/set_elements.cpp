...
enum{ELEMENT_SELECT,NODE_SELECT,ETYPE_SELECT,CTYPE_SELECT,GROUP_SELECT,REGION_SELECT};
...
//        else 
      ...
      } else if (strcmp(arg[iarg],"etype/fraction") == 0) {
        if (iarg+4 > narg) error->all(FLERR,"Illegal set_elements command");
        ...
        if (newtype <= 0 || newtype > element->netypes)
          error->all(FLERR,"Invalid value in set_elements command");
        if (fraction < 0.0 || fraction > 1.0)
          error->all(FLERR,"Invalid value in set_elements command");
        if (ivalue <= 0)
          error->all(FLERR,"Invalid random number seed in set_elements command");
        setrandom(ETYPE_FRACTION);
        iarg += 4;

      } else if (strcmp(arg[iarg],"etype/ratio") == 0) {
        if (iarg+4 > narg) error->all(FLERR,"Illegal set_elements command");
        newtype = universe->inumeric(FLERR,arg[iarg+1]);
        fraction = universe->numeric(FLERR,arg[iarg+2]);
        ivalue = universe->inumeric(FLERR,arg[iarg+3]);
        ...
        if (ivalue <= 0)
          error->all(FLERR,"Invalid random number seed in set_elements command");
        setrandom(ETYPE_RATIO);
        iarg += 4;

      } else if (strcmp(arg[iarg],"etype/subset") == 0) {
        if (iarg+4 > narg) error->all(FLERR,"Illegal set_elements command");
        newtype = universe->inumeric(FLERR,arg[iarg+1]);
...
//        else 
      ...
      } else if (strcmp(arg[iarg],"ctype/fraction") == 0) {
        if (iarg+4 > narg) error->all(FLERR,"Illegal set_elements command");
        newtype = universe->inumeric(FLERR,arg[iarg+1]);
        fraction = universe->numeric(FLERR,arg[iarg+2]);
        ivalue = universe->inumeric(FLERR,arg[iarg+3]);
      ...
      } else if (strcmp(arg[iarg],"ctype/ratio") == 0) {
        if (iarg+4 > narg) error->all(FLERR,"Illegal set_elements command");
        newtype = universe->inumeric(FLERR,arg[iarg+1]);
        fraction = universe->numeric(FLERR,arg[iarg+2]);
        ivalue = universe->inumeric(FLERR,arg[iarg+3]);
      ...
      } else if (strcmp(arg[iarg],"ctype/subset") == 0) {
        if (iarg+4 > narg) error->all(FLERR,"Illegal set_elements command");
        newtype = universe->inumeric(FLERR,arg[iarg+1]);
        nsubset = universe->bnumeric(FLERR,arg[iarg+2]);
        ivalue = universe->inumeric(FLERR,arg[iarg+3]);
...
void SetElements::selection(int n)
{
  ...
  if (style == ELEMENT_SELECT) {
    ...
    for (int i = 0; i < n; i++)
      if (ctype[i] >= nlo && ctype[i] <= nhi) select[i] = 1;
      else select[i] = 0;
