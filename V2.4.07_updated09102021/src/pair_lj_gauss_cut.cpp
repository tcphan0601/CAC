...
PairLJGaussCut::PairLJGaussCut(CAC *cac) : Pair(cac) {
...
void PairLJGaussCut::settings(int narg, char **arg) {
  if (narg != 1)
...
void PairLJGaussCut::coeff(int narg, char **arg) {
  if (narg < 4 || narg > 5)
...
void PairLJGaussCut::write_data_all(FILE *fp) {
...
double PairLJGaussCut::init_one(int i, int j) {
  if (setflag[i][j] == 0) {
    epsilon[i][j] =
        mix_energy(epsilon[i][i], epsilon[j][j], sigma[i][i], sigma[j][j]);
  ...
  if (tail_flag) {
    ...
    for (int k = 0; k < nelocal; k++) {
      if (ctype[k] == i)
        count[0] += nintpl[etype[k]];
      if (ctype[k] == j)
        count[1] += nintpl[etype[k]];
    }
...
void PairLJGaussCut::init_style() {
  int irequest;
...
void PairLJGaussCut::compute(int eflag, int vflag) {
  int jnum;
  ...
  double fx, fy, fz;
  double r, rexp, ugauss, gaussfpair;
    ...
    if (jnum) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq[ictype][jctype]) {
          ...
          // The extra gauss term
          /* --------------------------------------------- */
          r = sqrt(rsq);
          rexp = (r - rmh) / sigmah;

          ugauss = pgauss * exp(-0.5 * rexp * rexp);
          gaussfpair = rexp / r * ugauss / sigmah;

          /* --------------------------------------------- */
          forcelj = r6inv * (lj1[ictype][jctype] * r6inv - lj2[ictype][jctype]);
          fpair = forcelj * r2inv + gaussfpair;
...
void PairLJGaussCut::pair_a2a(int i, int j, double *f) {
...
void PairLJGaussCut::pair_a2ia(int i, int j, int jintpl, double *jx,
    double *f) {
  double delx, dely, delz;
  double rsq, r2inv, r6inv, forcelj, fpair;
  int ictype = atom->type[i];
...
void PairLJGaussCut::pair_ia2ia(int i, int iintpl, double *ix, int j,
...
double PairLJGaussCut::memory_usage() {
