...
DumpIDSAtom::DumpIDSAtom(CAC *cac, int narg, char **arg) : Dump(cac,narg,arg)
{
  ...
  int n = strlen(id) + strlen("_IDS_ATOM")+1;
  id_ids = new char[n];
  strcpy(id_ids,id);
  ...
  nkeep = 0;
  while (iarg < narg ) {
    if (strcmp(arg[iarg],"cubic") == 0) keep[nkeep++] = CUBIC;
...
void DumpIDSAtom::init_style() 
{
  ...
  if (domain->triclinic == 0)
    ...
    header_choice = &DumpIDSAtom::header_item_triclinic;
...
void DumpIDSAtom::header_item_triclinic(bigint ndump)
{
  fprintf(fp,"ITEM: TIMESTEP\n");
  fprintf(fp,BIGINT_FORMAT "\n",update->ntimestep);
  fprintf(fp,"ITEM: NUMBER OF ATOMS\n");
