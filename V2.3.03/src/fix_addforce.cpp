...
void FixAddForce::post_force(int vflag)
{
  ...
  if (varflag == CONSTANT) {
    ...
    for (i = 0; i < nalocal; i++)
      if (amask[i] & groupbit) {
        if (region && !region->match(ax[i])) continue;
      ...
      if (group_flag) {
        ...
        for (j = 0; j < npe; j++) 
          if (nodemask[i][j] & groupbit) {
            if (region && !region->match(nodex[i][j])) continue;
            foriginal[1] += nodef[i][j][0];
            foriginal[2] += nodef[i][j][1];
            foriginal[3] += nodef[i][j][2];
            if (xstyle) nodef[i][j][0] += xvalue;
    ...
    for (i = 0; i < nalocal; i++)
      if (amask[i] & groupbit) {
        ...
        else if (xstyle) af[i][0] += xvalue;
        if (ystyle == ATOM) af[i][1] += sforce[i][1];
        else if (ystyle) af[i][1] += yvalue;
        if (zstyle == ATOM) af[i][2] += sforce[i][2];
        else if (zstyle) af[i][2] += zvalue;
      }
