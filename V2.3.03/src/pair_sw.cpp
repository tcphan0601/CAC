...
void PairSW::compute(int eflag, int vflag)
{
  ...
  //size_t nbytes = sizeof(double) * (element->nlocal + element->nghost);
  //if (nbytes) memset(&intgf[0][0][0],0,3*max_nintg*nbytes);

  for (i = 0; i < neall; i++)
    for (j = 0; j < nintg[etype[i]]; j++)
      for (k = 0; k < 3; k++)
        intgf[i][j][k] = 0.0;

  // initialize shortnia_flag to 0
  ...
  //    nia (neighboring interpolated atoms), then loop through 
  ...
  // 3. Loop through neighbor of short nia from neighbor list,
  //    extract short neighborlist, loop through short neighborlist 
  //    to calculate threebody terms and accumulate forces on 
  //    atoms/integration points
  // ---------------------------------------------
  ...
  for (ii = 0; ii < ainum; ii++) {
    ...
    if (jnum) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        // if j is not integration point && j is local, mark it as short nia
    ...
    for (jj = 0; jj < numshort_a; jj++) {
      ...
      for (kk = jj+1; kk < numshort_a; kk++) {
        ...
        af[k][1] += fk[1];
        af[k][2] += fk[2];
        if (evflag) atom_ev_tally3(i,j,k,evdwl,0.0,fj,fk,delr1,delr2);
      }
      ...
      for (kk = 0; kk < numshort_ia; kk++) {
        ...
        kx = ky = kz = 0.0;
        for (node = 0; node < npe; node++) {
          kx += shape_array[ketype][kintpl][node]*nodex[k][node][0];
          ky += shape_array[ketype][kintpl][node]*nodex[k][node][1];
          kz += shape_array[ketype][kintpl][node]*nodex[k][node][2];
        }
        ...
        if (evflag) {
          atom_ev_tally3(i,j,-1,evdwl,0.0,fj,fk,delr1,delr2);
          if (kintg >= 0) {
            knode = i2n[ketype][kintg];
            node_ev_tally3(0,-1,0,-1,k,knode,evdwl,0.0,fj,fk,delr1,delr2);
          }
        }
      }
      af[j][0] += fjx;
    ...
    for (jj = 0; jj < numshort_ia-1; jj++) {
      ...
      for (kk = jj+1; kk < numshort_ia; kk++) {
        ...
        if (evflag) {
          atom_ev_tally3(i,evdwl,0.0,fj,fk,delr1,delr2);
          if (jintg >= 0) jnode = i2n[jetype][jintg];
          else jnode = -1;
          if (kintg >= 0) knode = i2n[ketype][kintg];
          else knode = -1;
          if (jnode >= 0 || knode >= 0) node_ev_tally3(0,-1,j,jnode,k,knode,evdwl,0.0,fj,fk,delr1,delr2);
    ...
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      ...
      if (jnum) {
        ...
        for (jj = 0; jj < jnum; jj++) {
          ...
          // if j is not an integration point && j is local, mark it as short nia

          if (ia2i[jetype][jintpl] < 0 && j < nelocal) {
            inia = ia2nialist[j][jintpl];  
            if (!shortnia_flag[inia]) {
              shortnia_list[numshortnia++] = inia;
              shortnia_flag[inia] = 1;
      ...
      for (jj = 0; jj < numshort_a; jj++) {
        ...
        for (kk = jj+1; kk < numshort_a; kk++) {
          ...
          af[k][2] += fk[2];
          if (evflag) {
            atom_ev_tally3(j,k,evdwl,0.0,fj,fk,delr1,delr2);
            node_ev_tally3(i,inode,0,-1,0,-1,evdwl,0.0,fj,fk,delr1,delr2);
          }
        }
        ...
        for (kk = 0; kk < numshort_ia; kk++) {
          ...
          if (evflag) {
            atom_ev_tally3(j,evdwl,0.0,fj,fk,delr1,delr2);
            if (kintg >= 0) knode = i2n[ketype][kintg];
            else knode = -1;
            node_ev_tally3(i,inode,0,-1,k,knode,evdwl,0.0,fj,fk,delr1,delr2);
          }
        }
        af[j][0] += fjx;
        af[j][1] += fjy;
      ...
      for (jj = 0; jj < numshort_ia-1; jj++) {
        ...
        for (kk = jj+1; kk < numshort_ia; kk++) {
          ...
          if (evflag) {
            ...
            node_ev_tally3(i,inode,j,jnode,k,knode,evdwl,0.0,fj,fk,delr1,delr2);
          }
        }
        if (jintg >= 0) {
          intgf[j][jintg][0] += fjx;
  ...
  for (ii = 0; ii < numshortnia; ii++) {
    ...
    for (jj = 0; jj < numshort_a; jj++) {
      ...
      for (kk = jj+1; kk < numshort_a; kk++) {
        ...
        if (evflag) atom_ev_tally3(j,k,evdwl,0.0,fj,fk,delr1,delr2);
      }
    ...
    for (jj = 0; jj < numshort_ia-1; jj++) {
      ...
      for (kk = jj+1; kk < numshort_ia; kk++) {
        ...
        if (evflag) {
          ...
          node_ev_tally3(0,-1,j,jnode,k,knode,evdwl,0.0,fj,fk,delr1,delr2);
        }
      }
      if (jintg >= 0) {
        intgf[j][jintg][0] += fjx;
  ...
  for (i = 0; i < neall; i++) {
    ...
    for (node = 0; node < npe; node++) {
      for (iintg = 0; iintg < nintg[ietype]; iintg++) {
        iintpl = i2ia[ietype][iintg];
        nodef[i][node][0] += weight[ietype][iintg]*shape_array[ietype][iintpl][node]*intgf[i][iintg][0];
        nodef[i][node][1] += weight[ietype][iintg]*shape_array[ietype][iintpl][node]*intgf[i][iintg][1];
        nodef[i][node][2] += weight[ietype][iintg]*shape_array[ietype][iintpl][node]*intgf[i][iintg][2];
      }
