...
enum{NONE,SC,BCC,FCC,PRIFCC,HCP,DIAMOND,PRIDIAMOND,SQ,SQ2,HEX,CUSTOM};
...
Lattice::Lattice(CAC *cac, int narg, char **arg) : Pointers(cac)
{
  ...
  if (dimension == 2) {
    if (style == SC || style == BCC || style == FCC || style == PRIFCC ||
        style == HCP || style == DIAMOND || style == PRIDIAMOND)
  ...
  if (spaceflag == 0) {
    if (style == PRIFCC || style == PRIDIAMOND) {
      xlattice = a1[0]*scale;
      ylattice = a2[1]*(a1[0]/a2[0])*scale;
      zlattice = a3[2]*(a1[0]/a3[0])*(a2[1]/a3[1])*scale;
    } else {
