  ...
  while (1) {
    ...
    // ----------------
      ...
      if (strncmp(word,"c_",2) == 0 || strncmp(word,"C_",2) == 0) {
        ...
        if (ivar < 0) {
          char errstr[200];
          sprintf(errstr,"Invalid variable name in variable formula: %s",word);
      ...
      // ----------------
        ...
        if (str[i] == '(') {
          ...
          if (flag) {
            char errstr[200];
            sprintf(errstr,"Invalid thermo keyword in variable formula: %s",word);
            error->all(FLERR,errstr);
          }
          if (tree) {
            Tree *newtree = new Tree();
    ...
    double *argstack, int &nargstack)
{
  ...
  if (strcmp(word,"count_atom") && strcmp(word,"count_node") &&
      strcmp(word,"count_elem")) 
    //&& strcmp(word,"mass") &&
  ...
  if (strcmp(word,"count_atom") == 0) {
    if (narg == 1) value = group->count_atom(igroup);
    else if (narg == 2) value = group->count_atom(igroup,region_function(args[1]));
    else error->all(FLERR,"Invalid group function in variable formula");
  } else if (strcmp(word,"count_node") == 0) {
    if (narg == 1) value = group->count_node(igroup);
    else if (narg == 2) value = group->count_node(igroup,region_function(args[1]));
    else error->all(FLERR,"Invalid group function in variable formula");
  } else if (strcmp(word,"count_elem") == 0) {
    if (narg == 1) value = group->count_elem(igroup);
    else if (narg == 2) value = group->count_elem(igroup,region_function(args[1]));
    else error->all(FLERR,"Invalid group function in variable formula");
