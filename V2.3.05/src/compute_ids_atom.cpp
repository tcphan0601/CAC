  ...
  ibucket(NULL), indexbucket(NULL), rbucket(NULL)
{
  ...
  comm_atom_reverse = 1;
  comm_elem_reverse = element->maxintpl;
}
...
void ComputeIDSAtom::compute_peratom()
{
  ...
  double ix,iy,iz,jx,jy,jz,kx,ky,kz,lx,ly,lz;
  ...
  // initiallize pattern to those in group = OTHER
  // do this for all atoms/interpolated atoms including ghosts
  // not in group = UNKNOWN

  for (i = 0; i < nalocal+naghost; i++) {
    if (!(amask[i] & groupbit)) apattern[i] = UNKNOWN;
    else apattern[i] = OTHER;
  }
  for (i = 0; i < nelocal+neghost; i++) {
    ietype = etype[i];
    if (!(emask[i] & groupbit)) 
  ...
  for (ii = 0; ii < ainum; ii++) {
    ...
    if (numneigha[i] < 16 || apattern[i] == UNKNOWN) continue;
    ...
    if (nfcc == 12) {
      ...
      // loop over first 4 nearest neighbor of atom I
      ...
      for (jj = 0; jj < 4; jj++) {
        ...
        jintpl = jintpllist[jj];
        if (jintpl == -1) 
          apattern[j] = MAX(apattern[j],CUBIC1);
        else 
          iapattern[j][jintpl] = MAX(iapattern[j][jintpl],CUBIC1);
      }
      
      // loop over second nearest neighbors of atom I

      for (jj = 0; jj < 12; jj++) {
        j = mynearest[jj];
        jintpl = mynearestintpl[jj];
        if (jintpl == -1)
          apattern[j] = MAX(apattern[j],CUBIC2);
        else 
          iapattern[j][jintpl] = MAX(iapattern[j][jintpl],CUBIC2);
      }
      ...
      for (jj = 0; jj < 4; jj++) {
        ...
        jintpl = jintpllist[jj];
        if (jintpl == -1) 
          apattern[j] = MAX(apattern[j],HEX1);
        else 
          iapattern[j][jintpl] = MAX(iapattern[j][jintpl],HEX1);
      }

      // loop over second nearest neighbors of I

      for (jj = 0; jj < 12; jj++) {
        j = mynearest[jj];
        jintpl = mynearestintpl[jj];
        if (jintpl == -1)
          apattern[j] = MAX(apattern[j],HEX2);
        else 
          iapattern[j][jintpl] = MAX(iapattern[j][jintpl],HEX2);
      }
    }
NEXTATOM: continue;
  }
  ...
  for (ii = 0; ii < einum; ii++) {
    ...
    // skip if element is not in group
    ...
    for (iintpl = 0; iintpl < nintpl[ietype]; iintpl++) { 
      ...
      if (nfcc == 12) {
        iapattern[i][iintpl] = CUBIC;

        // loop over first 4 nearest neighbor of I
...
int ComputeIDSAtom::pack_atom_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;

  m = 0;
  last = first + n;
  for (i = first; i < last; i++) buf[m++] = apattern[i];
  return m;
}

/* ---------------------------------------------------------------------- */

void ComputeIDSAtom::unpack_atom_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  ...
  for (i = 0; i < n; i++) {
    j = list[i];
    apattern[j] = MAX(apattern[j],buf[m]);
    m++;
  }
}
...
int ComputeIDSAtom::pack_elem_reverse_comm(int n, int first, double *buf)
{
  int i,j,m,last;
...
void ComputeIDSAtom::unpack_elem_reverse_comm(int n, int *list, double *buf)
{
  int i,j,k,m;
  ...
  for (i = 0; i < n; i++) {
    j = list[i];
    int jetype = etype[j];
    for (k = 0; k < nintpl[jetype]; k++) {
      iapattern[j][k] = MAX(iapattern[j][k],buf[m]);
      m++;
    }
