          ...
          if (rsq < cut_elemsq) {
            ...
            for (isub = 0; isub < nsubelem; isub++) {
              ...
              for (node = 0; node < npe; node++) {
                jx += shape_array_center_subelem[isub][node]*nodex[j][node][0];
                jy += shape_array_center_subelem[isub][node]*nodex[j][node][1];
                jz += shape_array_center_subelem[isub][node]*nodex[j][node][2];
              }
              delx = ix - jx; 
              dely = iy - jy; 
              delz = iz - jz; 
