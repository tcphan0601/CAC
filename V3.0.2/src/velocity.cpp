        ...
        for (int j = 0; j < apc[etype[i]]; j++) 
          for (int k = 0; k < npe[etype[i]]; k++) 
            if (nodemask[i][j][k] & groupbit) 
              if (sum_flag == 0) {
                if (xstyle) nodev[i][j][k][0] = vx;
                if (ystyle) nodev[i][j][k][1] = vy;
                if (zstyle) nodev[i][j][k][2] = vz;
              } else {
                if (xstyle) nodev[i][j][k][0] += vx;
                if (ystyle) nodev[i][j][k][1] += vy;
                if (zstyle) nodev[i][j][k][2] += vz;
  ...
  if (group_flag == Group::NODE) {
    for (int i = 0; i < nelocal; i++) 
      for (int j = 0; j < apc[etype[i]]; j++) 
        for (int k = 0; k < npe[etype[i]]; k++) 
          if (nodemask[i][j][k] & groupbit) {
            fraction = (nodex[i][j][k][coord_dim]-coord_lo) / (coord_hi - coord_lo);
            fraction = MAX(fraction, 0.0);
            fraction = MIN(fraction, 1.0);
            vramp = v_lo + fraction * (v_hi-v_lo);
            if (sum_flag) nodev[i][j][k][v_dim] += vramp;
            else nodev[i][j][k][v_dim] = vramp;
          }
...
void Velocity::create(double t_desired, int seed)
{
  ...
  if (group_flag == Group::NODE && comm->me == 0) 
    error->warning(FLERR, "Node group not supported for create style in velocity command, Element group are used instead");
      ...
      if (emask[i] & groupbit) {
        for (j = 0; j < apc[etype[i]]; j++) 
          for (k = 0; k < npe[etype[i]]; k++) {
            ...
            factor = 1.0/sqrt(mass[ctype[i][j]] / nodal_weight[etype[i]]);
            nodev[i][j][k][0] = vx * factor;
            nodev[i][j][k][1] = vy * factor;
            if (dim == 3) nodev[i][j][k][2] = vz * factor;
            else nodev[i][j][k][2] = 0.0;			
          }
      }
    }
...
/*  ----------------------------------------------------------------------
   rescale velocities of group atoms/elements to t_new from t_old
...
/*  ----------------------------------------------------------------------
   zero the linear momentum of a group of atoms/elements by adjusting v by -Vcm
   -------------------------------------------------------------------------  */

void Velocity::zero_momentum()
{
  // cannot have no atoms/elements in group
...
/*  ----------------------------------------------------------------------
   zero the angular momentum of a group of atoms/elements by adjusting v by -(w x r)
   -------------------------------------------------------------------------  */
  ...
  for (i = 0; i < nelocal; i++) {
    if (group_flag == Group::NODE) {
      for (j = 0; j < apc[etype[i]]; j++) 
        for (k = 0; k < npe[etype[i]]; k++) 
          if (nodemask[i][j][k] & groupbit) {
            domain->unmap(nodex[i][j][k], eimage[i], unwrap);
            dx = unwrap[0] - xcm[0];
            dy = unwrap[1] - xcm[1];
            dz = unwrap[2] - xcm[2];
