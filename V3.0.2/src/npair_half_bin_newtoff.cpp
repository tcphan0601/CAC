...
NPairHalfBinNewtoff::NPairHalfBinNewtoff(CAC *cac) : NPair(cac) {}
  ...
  for (i = 0; i < nelocal; i++) {
    ...
    for (igcell = 0; igcell < ngcell; igcell++) {
      ...
      for (ibasis = 0; ibasis < iapc; ibasis++) {
        ...
        // loop over atoms and store neighbor if j is ghost

        if (naghost) {
          ibin = nb->coord2atombin(xtmp, ytmp, ztmp);
          for (k = 0; k < natomstencil; k++) 
            for (j = atombinhead[ibin+atomstencil[k]]; j >= 0; j = atombins[j]) {
              if (j < nalocal) continue;
              jctype = atype[j];
            ...
            if (rsq < cutoffsq) {
              ...
              for (l = 0; l < nucell_subelem[jetype][isub]; l++) {
                ...
                for (jbasis = 0; jbasis < japc; jbasis++) {
                  ...
                  if (jgcell >= 0)
                    if (i > j) continue;
                    else if (i == j && igcell >= jgcell) continue;
