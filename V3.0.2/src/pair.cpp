    ...
    double delx, double dely, double delz)
{
  ...
  if (eflag_either) {
    ...
    if (eflag_atom) {
      ...
      if (ieptr) *ieptr += epairhalf;
      if (jeptr) *jeptr += epairhalf;
    }
  }
    ...
    double evdwl, double ecoul, double *fj, double *fk, double *drji, double *drki)
{
  ...
  if (eflag_either) {
    ...
    if (eflag_atom) {
      ...
      if (ieptr) *ieptr += epairthird;
      if (jeptr) *jeptr += epairthird;
      if (keptr) *keptr += epairthird;
    }
  }
...
void Pair::debug_open_dump()
{
  ...
  for (int i = 0; i < element->nlocal; i++)
    if (element->mask[i] & debug_groupbit) {
      if (element->debug_mode) {
        for (int igcell = 0; igcell < ngcell[etype[i]]; igcell++)
          if (!element->debug_gcell_inactive[etype[i]][element->debug_gcell_type[etype[i]][igcell]]) n++;
      } else
        n += element->ngcell[element->etype[i]];
    }
  ...
  fprintf(fp, "ITEM: ATOMS id type x y z tag igcell etype weight fx fy fz\n");
}

void Pair::debug_write_gcell_force(int i, int igcell, double x, double y, double z, double fx, double fy, double fz)
{
  int ietype = element->etype[i];
  fprintf(fp, "%d %d %g %g %g %d %d %d %g %g %g %g\n", 
      debug_gcell_counter++, element->ctype[i], 
      x, y, z, 
      element->tag[i], igcell, ietype, element->weight[ietype][igcell], 
      fx, fy, fz);
}
