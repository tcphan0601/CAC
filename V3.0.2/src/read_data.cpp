...
void ReadData::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    //  iarg ++;
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal read_data command");
      if (strcmp(arg[iarg+1],"yes") == 0) referenceflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) referenceflag = 0;
      iarg += 2;
...
void ReadData::atoms()
{
  ...
  while (nread < natoms) {
    ...
    if (referenceflag)
      atom->data_atoms_reference(nchunk, buffer, aid_offset, toffset, shiftflag, shift);
    else 
...
void ReadData::nodes()
{
  ...
  while (nread < nnodes) {
    ...
    if (eof) error->all(FLERR, "Unexpected end of data file");
    if (referenceflag) 
      element->data_nodes_reference(nchunk, buffer, eid_offset, toffset, shiftflag, shift);
    else
