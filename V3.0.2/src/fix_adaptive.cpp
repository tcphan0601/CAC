...
//    if (element->tag[i] == 508) printf("neighbor x[%d] = %g %g %g jj = %d jindex = %d\n",atom->tag[j],ax[j][0],ax[j][1],ax[j][2],jj,jindex);
      if (jindex >= 0) {
        if (dislocation_flag) {
...
//    if (element->tag[i] == 508) printf("natom = %d\n",natom);
    if (natom >= thres) {
      disc_flag = 1;
      if (dislocation_flag) {
    ...
    int *cut, int dim, int jnum, int *jlist, int *jindexlist)
{
  ...
  double plane_width = 2.0 / ncell / 10;
  double tmp[3], xi;
  double **ax = atom->x;
  for (jj = 0; jj < jnum; jj++) {
    j = jlist[jj];
      ...
      if (jindex == boxindices[dim][ibox]) {
        ...
        for (int iplane = 0; iplane < nplanes; iplane++) {
          if (fabs(xi - planexi[iplane]) < plane_width) {
            if (newbin_flag == 0) error->one(FLERR,"plane_width too large, overlapping planes exist");
            newbin_flag = 0;
