  ...
  if (element->nelements) {
    for (i = 0; i < nother; i++)
      MPI_Irecv(&buf_recv[elem_size_reverse * elem_reverse_recv_offset[i]], 
          ...
          overlap[i], 0, world, &requests[i]);
    for (i = 0; i < nother; i++) {
      n = evec->pack_reverse(elem_recvnum[i], elem_firstrecv[i], 
          buf_send);
      MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
    }
    ...
    for (i = nother; i < noverlap; i++) {
      evec->pack_reverse(elem_recvnum[i], elem_firstrecv[i], 
          buf_send);
      evec->unpack_reverse(elem_sendnum[i], elem_sendlist[i], 
          buf_send);
    }
    ...
    for (i = 0; i < nother; i++) {
      ...
      evec->unpack_reverse(elem_sendnum[irecv], elem_sendlist[irecv], 
          ...
          elem_reverse_recv_offset[irecv]]);
    }
  }
}
