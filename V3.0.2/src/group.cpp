...
void Group::assign(int narg, char **arg)
{
  ...
  // select elements (no atoms) if it has neighbor in other groups
    ...
    for (ii = 0; ii < inum; ii++) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        if (jindexlist[jj] == -1) jmask = amask[j];
        else jmask = emask[j];
  ...
  // no node added, only add to group if either atom or element matches type/id or condition
    ...
    if (strcmp(arg[2], "ctype") == 0 && !element_flag) 
      category = CTYPE;
    ...
    // args = list of values
      ...
      if (attribute2) {
        ...
        int *match_count = new int[nlocal];
        int *apc = element->apc;
        int *etype = element->etype;
        for (int i = 0; i < nlocal; i++) match_count[i] = 0;
        ...
        for (int iarg = 3; iarg < narg; iarg++) {
          ...
          // add to match count if attribute2 matches value or sequence

          for (int i = 0; i < nlocal; i++)
            for (int ibasis = 0; ibasis < apc[etype[i]]; ibasis++)
              if (attribute2[i][ibasis] >= start && attribute2[i][ibasis] <= stop &&
                  (attribute2[i][ibasis] - start) % delta == 0) mask[i] |= bit;
        }
