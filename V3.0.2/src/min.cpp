...
void Min::setup(int flag)
{
  ...
  bigint ndofme = 3 * static_cast<bigint>(atom->nlocal + element->nnodes);
  //for (int m = 0; m < nextra_atom; m++)
  //  ndofme += extra_peratom[m]*atom->nlocal;
  MPI_Allreduce(&ndofme, &ndoftotal, 1, MPI_CAC_BIGINT, MPI_SUM, world);
  ...
  // initialize locally-stored combined vectors
  ...
  if (output->thermo->normflag) {
    element->count_vatom();
    ecurrent /= atom->natoms + element->nvatoms;
  }
...
{
  // copy combined array values to avec/evec arrays
  ...
  if (update->ntimestep == 9) {
    class WriteTecplot *write = new WriteTecplot(cac);
    char **newarg = new char*[2];
    newarg[0] = (char *) "test_energy_force.plt";
    newarg[1] = (char *) "noinit";
    write->command(2, newarg);
    delete [] newarg;
  ...
  if (output->thermo->normflag) {
    element->count_vatom();
    energy /= atom->natoms + element->nvatoms;
  }
  ...
  // copy from avec/evec arrays to combined arrays
...
void Min::force_clear()
{
  ...
  if (nbytes) {
    ...
    memset(&element->gaussf[0][0][0][0], 0, 3 * element->maxapc * element->maxgcell * nbytes);
  }
}
/* ----------------------------------------------------------------------
...
void Min::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "dmax") == 0) {
      ...
      dmax = universe->numeric(FLERR, arg[iarg+1]);
      iarg += 2;
      //} else if (strcmp(arg[iarg], "delaystep") == 0) {
      //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
      //  delaystep = universe->numeric(FLERR, arg[iarg+1]);
      //  iarg += 2;
      //} else if (strcmp(arg[iarg], "dtgrow") == 0) {
      //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
      //  dtgrow = universe->numeric(FLERR, arg[iarg+1]);
      //  iarg += 2;
      //} else if (strcmp(arg[iarg], "dtshrink") == 0) {
      //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
      //  dtshrink = universe->numeric(FLERR, arg[iarg+1]);
      //  iarg += 2;
  } else if (strcmp(arg[iarg], "alpha0") == 0) {
    if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
    alpha0 = universe->numeric(FLERR, arg[iarg+1]);
    iarg += 2;
    //} else if (strcmp(arg[iarg], "alphashrink") == 0) {
    //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
    //  alphashrink = universe->numeric(FLERR, arg[iarg+1]);
    //  iarg += 2;
    //} else if (strcmp(arg[iarg], "tmax") == 0) {
    //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
    //  tmax = universe->numeric(FLERR, arg[iarg+1]);
    //  iarg += 2;
    //} else if (strcmp(arg[iarg], "tmin") == 0) {
    //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
    //  tmin = universe->numeric(FLERR, arg[iarg+1]);
    //  iarg += 2;
    ...
    //  if (iarg+2 > narg) error->all(FLERR, "Illegal min_modify command");
    //  max_vdotf_negatif = universe->numeric(FLERR, arg[iarg+1]);
    //  iarg += 2;
