...
template <typename S, typename T> static S *style_creator(CAC *cac)
{
  return new T(cac);
}
...
void Neighbor::init()
{
  ...
  if (cutneighsq == NULL) {
    ...
    memory->create(cutneigh, n+1, n+1, "neigh:cutneigh");
  ...
  if (dist_check) {
    ...
    if (nodemaxhold == 0) {
      ...
      memory->create(nodexhold, nodemaxhold, element->maxapc, element->maxnpe, 3, "neigh:nodexhold");
    }
  ...
  // depend on type, grouprsettings from neigh_modify

  n = atom->ntypes;

  if (nex_type == 0 && nex_group == 0 ) exclude = 0;
  else exclude = 1;
...
int Neighbor::init_pair()
{
  ...
  //     instead of atom2bin, this could cause error b/c atoms have
  ...
  // allow occasional request from command

  if (style == BIN) {
    for (i = 0; i < nrequest; i++)
      if (requests[i]->occasional == 1 && requests[i]->ghost)
  ...
  int maxgcell = element->nmax * element->maxgcell;
  int maxnode = element->nmax * element->maxnpe;
  int maxucell = element->nmax * element->maxucell;
  ...
  for (i = 0; i < nlist; i++)
    if (neigh_pair[i] && !lists[i]->copy) {
      if (lists[i]->atomlist || lists[i]->atomonlylist) maxall += maxatom;
      if (lists[i]->elemlist) maxall += maxelem;
      if (lists[i]->nodelist) maxall += maxnode * maxapc;
      if (lists[i]->gausslist) maxall += maxgcell * maxapc;
      if (lists[i]->vatomlist) maxall += maxucell * maxapc;
      lists[i]->grow(maxall, maxall);
      if (lists[i]->threebody)
        lists[i]->grow_nva((maxucell-maxgcell) * maxapc, maxelem);
    }
...
void Neighbor::morph_copy()
{
  ...
  for (i = 0; i < nrequest; i++) {
    ...
    for (j = 0; j < nrequest; j++) {
      ...
      if (irq->atomonlylist && !jrq->atomonlylist) continue;
      if (irq->elemlist && !jrq->elemlist) continue;
      if (irq->gausslist && !jrq->gausslist) continue;
      if (irq->vatomlist && !jrq->vatomlist) continue;
      ...
      if (irq->group != jrq->group) continue;
      if (irq->groupbit != jrq->groupbit) continue;
      if (irq->noinner != jrq->noinner) continue;
...
// if already halffull, continue

if (!irq->half || irq->halffull) continue;
...
void Neighbor::print_pairwise_info()
{
  ...
  double cut[3];
  cut[0] = MAX(cutneighmax, comm->cutghostuser[0]);
  cut[1] = MAX(cutneighmax, comm->cutghostuser[1]);
  cut[2] = MAX(cutneighmax, comm->cutghostuser[2]);

  double atombinsize, elembinsizex, elembinsizey, elembinsizez, bbox[3];
  bbox[0] =  bboxhi[0]-bboxlo[0];
  bbox[1] =  bboxhi[1]-bboxlo[1];
  bbox[2] =  bboxhi[2]-bboxlo[2];

  if (atom->natoms) {
    if (atombinsizeflag) atombinsize = atombinsize_user;
  ...
  if (element->nelements) {
    if (elembinsizeflag) elembinsizex = elembinsizey = elembinsizez = elembinsize_user;
    else if (style == BIN) {
      elembinsizex = 0.5 * (cutneighmax + element->max_element_bound_box_size[0]);
      elembinsizey = 0.5 * (cutneighmax + element->max_element_bound_box_size[1]);
      elembinsizez = 0.5 * (cutneighmax + element->max_element_bound_box_size[2]);
    } else {
      elembinsizex = 0.5 * (cutneighmin + element->max_element_bound_box_size[0]);
      elembinsizey = 0.5 * (cutneighmin + element->max_element_bound_box_size[1]);
      elembinsizez = 0.5 * (cutneighmin + element->max_element_bound_box_size[2]);
    }
    if (elembinsizex == 0.0) elembinsizex = bbox[0];
    if (elembinsizex == 0.0) elembinsizey = bbox[1];
    if (elembinsizex == 0.0) elembinsizez = bbox[2];
  }
  ...
  for (m = 0; m < 2; m++) {
    ...
    if (out) {
      ...
      fprintf(out, "  ghost atom cutoff = %g %g %g\n", cut[0], cut[1], cut[2]);
      if (style != NSQ) {
        if (atom->natoms) fprintf(out, "  atom bin: binsize = %g, bins = %g %g %g\n", atombinsize, 
            ceil(bbox[0]/atombinsize), 
            ceil(bbox[1]/atombinsize), 
            ceil(bbox[2]/atombinsize));
        if (element->nelements) fprintf(out, "  elem bin: binsize = %g %g %g, bins = %g %g %g\n", 
            elembinsizex, elembinsizey, elembinsizez, 
            ceil(bbox[0]/elembinsizex), 
            ceil(bbox[1]/elembinsizey), 
            ceil(bbox[2]/elembinsizez));
      }
      ...
      for (i = 0; i < nlist; i++) {
        ...
        if (rq->atomlist) fprintf(out, ", atom");
        if (rq->atomonlylist) fprintf(out, ", atomonly");
        if (rq->elemlist) fprintf(out, ", element");
        if (rq->gausslist) fprintf(out, ", gaussian cell");
        if (rq->vatomlist) fprintf(out, ", virtual atom");
...
int Neighbor::choose_bin(NeighRequest *rq)
{
  ...
  for (int i = 0; i < nbclass; i++) {
    ...
    if (rq->elemonlylist) {
      if (!(mask & NB_ELEMONLY)) continue;
    } else if (mask & NB_ELEMONLY) continue;
...
int Neighbor::choose_stencil(NeighRequest *rq)
{
  ...
  if (style == NSQ) return 0;
  if (rq->copy || rq->halffull) return 0;
  ...
  for (int i = 0; i < nsclass; i++) {
    ...
    //    if (!rq->ssa != !(mask & Neighbor::SSA)) continue;
    ...
    // if request elemonly list

    if (rq->elemonlylist) 
      if (!(mask & NS_ELEMONLY)) continue;
...
int Neighbor::choose_pair(NeighRequest *rq)
{
  ...
  for (int i = 0; i < npclass; i++) {
    ...
    // neighborlist for atoms, elements, gaussian cells and/or virtual atoms
    // must match

    if (!rq->atomonlylist != !(mask & NP_ATOMONLY)) continue;
    if (!rq->atomlist != !(mask & NP_ATOM)) continue;
    if (!rq->elemlist != !(mask & NP_ELEM)) continue;
    if (!rq->nodelist != !(mask & NP_NODE)) continue;
    if (!rq->gausslist != !(mask & NP_GAUSS)) continue;
    if (!rq->vatomlist != !(mask & NP_VATOM)) continue;
    ...
    if (rq->group)
      if (!(mask & NP_GROUP)) continue;

    if (rq->noinner)
      if (!(mask & NP_NOINNER)) continue;
...
int Neighbor::decide()
{
  if (must_check) {
    ...
    for (int i = 0; i < fix_check; i++) {
      if (modify->fix[fixchecklist[i]]->system_changed) {
        modify->fix[fixchecklist[i]]->system_changed = 0;
        return 1;
      }
  ...
  for (int i = 0; i < nlocal; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) {
        delx = nodex[i][j][k][0] - nodexhold[i][j][k][0];
        dely = nodex[i][j][k][1] - nodexhold[i][j][k][1];
        delz = nodex[i][j][k][2] - nodexhold[i][j][k][2];
        rsq = delx * delx + dely * dely + delz * delz;
        if (rsq > deltasq) flag = 1;
      }
...
void Neighbor::build(int topoflag)
{
  ...
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 1\n");
    }
  ago = 0;
  ncalls++;
  ...
  bigint nlocalucell = nelocal * max_nucell;
  bigint nallucell = neall * max_nucell;
  ...
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 2\n");
    }
    ...
    for (i = 0; i < nelocal; i++) 
      for (j = 0; j < apc[etype[i]]; j++) 
        for (k = 0; k < npe[etype[i]]; k++) {
          nodexhold[i][j][k][0] = nodex[i][j][k][0];
          nodexhold[i][j][k][1] = nodex[i][j][k][1];
          nodexhold[i][j][k][2] = nodex[i][j][k][2];
        }
    if (boxcheck) {
      if (triclinic == 0) {
  ...
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 3\n");
    }
  ...
  if (style != NSQ) {
    for (i = 0; i < nbin; i++) {
      ...
      //if (neigh_bin[i]->istyle == 2) continue;
      neigh_bin[i]->bin_all();
    }
  }
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 4\n");
    }

  // update subelem size before building neighbor list
  // if triclinic, update element bound box with no check
  // since element bound box currently in lamda coord from comm->border

  element->update_subelem_size();
  if (triclinic) element->update_element_bound_box(0);
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 5\n");
    }
  ...
  for (j = 0; j < npair_perpetual; j++) {
    ...
    if (!lists[m]->copy) {
      ...
      if (lists[m]->atomlist || lists[m]->atomonlylist) {
        nlocal += atom->nlocal;
      ...
      // count # of nodes, gauss points, virtual atoms 
      // for each elements instead of using max values

      if (lists[m]->nodelist) {
        for (i = 0; i < element->nlocal + element->nghost; i++) {
          if (i < element->nlocal) nlocal += npe[etype[i]] * apc[etype[i]];
          nall += npe[etype[i]] * apc[etype[i]];
        }
      }
      if (lists[m]->gausslist) {
        for (i = 0; i < element->nlocal + element->nghost; i++) {
          if (i < element->nlocal) nlocal += ngcell[etype[i]] * apc[etype[i]];
          nall += ngcell[etype[i]] * apc[etype[i]];
        }
      }
      if (lists[m]->vatomlist) {
        for (i = 0; i < element->nlocal + element->nghost; i++) {
          if (i < element->nlocal) nlocal += nucell[etype[i]] * apc[etype[i]];
          nall += nucell[etype[i]] * apc[etype[i]];
        }
      }
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 6\n");
    }

      lists[m]->grow(nlocal, nall);
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 7\n");
    }

      if (lists[m]->threebody) {
        int n = 0;
        for (i = 0; i < element->nlocal + element->nghost; i++) {
          n += (nucell[etype[i]] - ngcell[etype[i]]) * apc[etype[i]];
        }
        lists[m]->grow_nva(n, element->nlocal + element->nghost);
      }
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 8\n");
    }
  ...
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 9\n");
    }

    neigh_pair[m]->build(lists[m]);
  if (topoflag == 1000){
    MPI_Barrier(world);
    if (comm->me == 0) printf("  Neighbor Build Test 10\n");
    }
...
/*  ----------------------------------------------------------------------
    ...
    if preflag = 1, this build_one is called before main reneighbor build
    -------------------------------------------------------------------------  */
...
void Neighbor::build_one(class NeighList *mylist, int preflag)
{
  ...
  // preflag is set by fix_adaptive, compute, command
  ...
  NStencil *ns = np->ns;
  //if (comm->me == 166)
  //printf(" Build_one Step = %d before create stencil last_stencil = %d last_setup_bins = %d\n", update->ntimestep, ns->last_stencil, last_setup_bins);
  if (ns)
    if (ns->last_stencil < last_setup_bins || (preflag && ns->last_stencil <= last_setup_bins)) {
      ns->create_setup();
      ns->create();
    }
  // build the list
  ...
  if (!mylist->copy) {
    ...
    int *npe = element->npe;
    int *ngcell = element->ngcell;
    int *nucell = element->ngcell;
    if (mylist->atomlist || mylist->atomonlylist) {
      nlocal += atom->nlocal;
      nall += atom->nlocal + atom->nghost;
    }
    if (mylist->elemlist) {
    ...
    if (mylist->threebody) {
      ...
      mylist->grow_nva(n, element->nlocal + element->nghost);
    }
  }
...
void Neighbor::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "every") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "atombinsize") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal neigh_modify command");
      atombinsize_user = universe->numeric(FLERR, arg[iarg+1]);
      if (atombinsize_user <= 0.0) atombinsizeflag = 0;
      else atombinsizeflag = 1;
      iarg += 2;
    } else if (strcmp(arg[iarg], "elembinsize") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal neigh_modify command");
      elembinsize_user = universe->numeric(FLERR, arg[iarg+1]);
      if (elembinsize_user <= 0.0) elembinsizeflag = 0;
      else elembinsizeflag = 1;
      iarg += 2;
      ...
      if (strcmp(arg[iarg+1], "type") == 0) {
        ...
        ex1_type[nex_type] = universe->inumeric(FLERR, arg[iarg+2]);
        ex2_type[nex_type] = universe->inumeric(FLERR, arg[iarg+3]);
        nex_type++;
        iarg += 4;
...
bigint Neighbor::memory_usage()
{
  ...
  bytes += memory->usage(nodexhold, nodemaxhold, element->maxapc, element->maxnpe, 3);
