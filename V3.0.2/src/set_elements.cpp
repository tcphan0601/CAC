...
void SetElements::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg], "etype") == 0) {
      ...
      //        if (strstr(arg[iarg+1], "v_") == arg[iarg+1]) varparse(arg[iarg+1], 1);
      //        else 
      if (style == CTYPE_SELECT) {
        nvalues = universe->inumeric(FLERR, arg[iarg+1]);
        if (iarg+2+nvalues > narg) error->all(FLERR, "Illegal set_elements command");
        ivalues = new int[nvalues];
        for (int i = 0; i < nvalues; i++)
          ivalues[i] = universe->inumeric(FLERR, arg[iarg+2+i]);
      } else {
        ivalue = universe->inumeric(FLERR, arg[iarg+1]);
        nvalues = 0;
      }
  ...
  for (int i = 0; i < nlocal; i++) {
    if (style == CTYPE_SELECT) {
      for (int j = 0; j < apc[etype[i]]; j++) {
        ...
        if (ivalues[j] <= 0 || ivalues[j] > atom->ntypes)
          error->one(FLERR, "Invalid value in set_elements command");
        element->ctype[i][j] = ivalues[j];
        count++;
      }
    } else {
