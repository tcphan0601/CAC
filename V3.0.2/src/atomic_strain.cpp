...
#define NSECTIONS 4       // change when add to header::section_keywords
#define EPSILON 1e-6

enum{PLT = 0, SZPLT = 1, ASCII = 2, ATOM = 3, NODE = 4, FULLATOM = 5};        // output file format ASCII (.dat), binary (.plt), or SZL (.szplt)
enum{COORDINATE, ID, ETYPE, CTYPE, VALIDITY, VONMISES, VOLSTRAIN, 
  DEFGRAD, ROTATION, NSD, STRAINTENSOR, STRETCHTENSOR, NUMNEIGH};
...
void AtomicStrain::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "out/format") == 0) {
      ...
      else if (strcmp(arg[iarg+1], "node") == 0) output_format = NODE;
      else if (strcmp(arg[iarg+1], "full/atom") == 0) output_format = FULLATOM;
...
void AtomicStrain::compute()
{
  ...
  if (nsd_flag) {
    ...
    memory->grow(nodeNSD, nelocal, maxapc, maxnpe, "atomic_strain:atomNSD");  
  }

  memory->grow(atom_displacement, atom->nmax, 3, "atomic_strain:atom_displacement");
  memory->grow(node_displacement, element->nmax, maxapc, maxnpe, 3, "atomic_strain:atom_displacement");
  ...
  for (int i = 0; i < nelocal + neghost; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) 
        sub3(nodex_cur[i][j][k], nodex_ref[i][j][k], node_displacement[i][j][k]);
  ...
  double *defGradient, *Q, *stretchTensor, *strainTensor;
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    // i is node
      ...
      if (rotation_flag) Q = nodeQ[i][ibasis][inode];
      else Q = NULL;
      ...
      disp = node_displacement[i][ibasis][inode];
    }
...
int AtomicStrain::compute_strain(int numneigh, int *jlist, int *jindexlist, 
    double *coord_ref, double *idisp, double *defGradient, double *quad, 
    double *stretchTensor, double *strainTensor, double *NSD, 
    double *vonMisesStrain, double *volStrain)
{
  ...
  for (jj = 0; jj < numneigh; jj++) {
    ...
    if (jindex < 0) {
      jdisp[0] = atom_displacement[j][0]; 
      jdisp[1] = atom_displacement[j][1]; 
      jdisp[2] = atom_displacement[j][2]; 
      jx_ref[0] = x_ref[j][0];
      jx_ref[1] = x_ref[j][1];
      jx_ref[2] = x_ref[j][2];
    }
    ...
    // j is virtual atom
      ...
      jucell = jindex / japc;
      jbasis = jindex % japc;
      evec->interpolate(jdisp, node_displacement, j, jbasis, jucell, 3);
    ...
    *NSD = 0;
    ...
    for (jj = 0; jj < numneigh; jj++) {
      ...
      // j is virtual atom
        ...
        jucell = jindex / japc;
        jbasis = jindex % japc;
        evec->interpolate(jdisp, node_displacement, j, japc, jucell, 3);
...
void AtomicStrain::write_output(char *filename)
{
  ...
  if (output_format < ATOM) {
    ...
    size_one = 6 + 4 * element_info_flag + 9 * deformation_gradient_flag + 4 * rotation_flag + nsd_flag + 6 * (strain_tensor_flag + stretch_tensor_flag);
    ...
    size_one = 8 + 6 * element_info_flag + 9 * deformation_gradient_flag + 4 * rotation_flag + nsd_flag + 6 * (strain_tensor_flag + stretch_tensor_flag);
    int nalocal = atom->nlocal;
    int nelocal = element->nlocal;
    int *apc = element->apc;
    int *npe = element->npe;
...
void AtomicStrain::pack_element_connectivity()
{
  ...
  if (average_flag) {
    int **nodecell_ids;
    memory->create(nodecell_ids, nlocal, npe_connect, "atomic_strain:nocecell_ids");
    ...
    int ***node_ids;
    memory->create(node_ids, nlocal, element->maxapc, npe_connect, "atomic_strain:noce_ids");
    ...
    for (i = 0; i < nlocal; i++) {
      ...
      for (ibasis = 0; ibasis < iapc; ibasis++) {
        for (iucell = 0; iucell < nucell[ietype]; iucell++) {
          ...
          for (j = 0; j < 3; j++) 
            buf[m++] = evec->interpolate(nodex, i, ibasis, iucell, 3);
          if (wrap_flag) {
