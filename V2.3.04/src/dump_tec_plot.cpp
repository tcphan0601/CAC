...
DumpTecPlot::DumpTecPlot(CAC *cac, int narg, char **arg) : Dump(cac,narg,arg)
{
  ...
  if (link_flag && (stress_flag || force_flag)) 
    comm_elem_forward = 6*element->npe;
}
...
void DumpTecPlot::setup_pre_write() 
{
  ...
  if (link_flag == 1) {
    ...
    for (i = 0; i < nlocal; i++) {
        double **x = element->x;
      itag = tag[i];
//      if (itag == 3601) fprintf(screen,"jx = %g %g %g\n",x[i][0],x[i][1],x[i][2]);
      if ((itag-1) % apc != 0) continue;
      clusters[nclusters][0] = i;
      for (k = 1; k < apc; k++) {
        j = element->map(itag+k);

//        fprintf(screen,"me = %d i = %d, k = %d itag = %d ix = %g %g %g\n",comm->me,i,k,itag,x[i][0],x[i][1],x[i][2]);
  ...
  if (totalclusters != element->nelements/apc) {
    fprintf(screen,"me %d nclusters = %d total = %d expected = %d\n",comm->me,nclusters,totalclusters,element->nelements/apc);
