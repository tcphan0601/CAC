...
void NBinStandard::setup_bins(int style)
{
  ...
  if (element->nelements) {
    double binsize_optimal;
    double elem_cut = element->max_size;

    if (binsizeflag) binsize_optimal = binsize_user;
    else if (style == BIN) binsize_optimal = 0.5*(cutneighmax + elem_cut);
    else binsize_optimal = 0.5*(cutneighmin + elem_cut);
    if (binsize_optimal == 0.0) binsize_optimal = bbox[0];
    double binsizeinv = 1.0/binsize_optimal;
    ...
    nelembinx = static_cast<int> (bbox[0]*binsizeinv);
    nelembiny = static_cast<int> (bbox[1]*binsizeinv);
    if (dimension == 3) nelembinz = static_cast<int> (bbox[2]*binsizeinv);
    else nelembinz = 1;
    ...
    if (binsize_optimal*elembininvx > CUT2BIN_RATIO ||
        binsize_optimal*elembininvy > CUT2BIN_RATIO ||
        binsize_optimal*elembininvz > CUT2BIN_RATIO)
