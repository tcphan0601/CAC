...
void CreateElements::command(int narg, char **arg)
{
  ...
  nctype = universe->inumeric(FLERR,arg[0]);
  netype = universe->inumeric(FLERR,arg[1]);
...
void CreateElements::add_lattice()
{
  ...
  // convert 8 corners of my domain from box coords to lattice coords
  // must be whole box domain to have the same origin, otherwise elements might not align correctly between procs
  ...
  if (triclinic == 0) {
    bboxlo[0] = domain->boxlo[0]; bboxhi[0] = domain->boxhi[0];
    bboxlo[1] = domain->boxlo[1]; bboxhi[1] = domain->boxhi[1];
    bboxlo[2] = domain->boxlo[2]; bboxhi[2] = domain->boxhi[2];
  } else domain->bbox(domain->boxlo_lamda,domain->boxhi_lamda,bboxlo,bboxhi);
  ...
  bigint ntempelements;
  bigint nmax = (khi-klo+1)*(jhi-jlo+1)*(ihi-ilo+1);

  memory->create(xtemplist,nmax,3,"create_elements:xtemplist");
  memory->create(ctypelist,nmax,"create_elements:ctypelist");

  // get list of template element coords using bounds
  
  ntempelements = element->evec->create_element_template(
      klo,khi,jlo,jhi,ilo,ihi,netype,basisctype,xtemplist,ctypelist);

  // iterate through list of template elements
  // add element if it meets all criteria

  double x[3],lamda[3];
  double *coord;
  int nbasis = domain->lattice->nbasis;
  if (ntempelements % nbasis != 0) error->all(FLERR,"TEST"); 
  int nclusters = ntempelements/nbasis;

  bigint i;
  for (bigint ii = 0; ii < nclusters; ii++) {
    i = ii*nbasis;
    x[0] = xtemplist[i][0];
    x[1] = xtemplist[i][1];
    x[2] = xtemplist[i][2];
    ...
    element->evec->create_element(x,netype,ctypelist[i],0);
    
    for (int j = 1; j < nbasis; j++) 
      element->evec->create_element(xtemplist[i+j],netype,ctypelist[i+j],0);
  }
  ...
  memory->destroy(xtemplist);
  memory->destroy(ctypelist);
}
