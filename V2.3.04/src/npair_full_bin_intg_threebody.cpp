...
NPairFullBinIntgThreebody::NPairFullBinIntgThreebody(CAC *cac) : NPair(cac) {}
...
void NPairFullBinIntgThreebody::build(NeighList *list)
{
  ...
  int na2a,na2ia,ni2a,ne2e,ni2ia,nnia2a,nnia2ia;
  int iintpl,jintpl,node;
  ...
  for (int i = 0; i < list->maxelem; i++)
    for (int j = 0; j < element->max_nintpl; j++)
      ia2nialist[i][j] = -1;
          ...
          if (rsq < cut_elemsq) {
            ...
            for (isub = 0; isub < nsubelem; isub++) {
              ...
              for (node = 0; node < npe; node++) {
                jx[0] += shape_array_center_subelem[isub][node]*nodex[j][node][0];
                jx[1] += shape_array_center_subelem[isub][node]*nodex[j][node][1];
                jx[2] += shape_array_center_subelem[isub][node]*nodex[j][node][2];
              }
              delx = ix[0] - jx[0]; 
              dely = ix[1] - jx[1]; 
              delz = ix[2] - jx[2]; 
              ...
              if (rsq < cut_subelemsq) {
                ...
                for (l = 0; l < natom_subelem[jetype][isub]; l++) {
                  ...
                  for (node = 0; node < npe; node++) {
                    jx[0] += shape_array[jetype][jintpl][node]*nodex[j][node][0];
                    jx[1] += shape_array[jetype][jintpl][node]*nodex[j][node][1];
                    jx[2] += shape_array[jetype][jintpl][node]*nodex[j][node][2];
                  }
                  delx = ix[0] - jx[0]; 
                  dely = ix[1] - jx[1]; 
                  delz = ix[2] - jx[2]; 
                  ...
                  if (rsq < cutneighsq[ictype][jctype]) {
                    ...
                    // mark j as nia if j is owned, not an integration 
                    // point and was not marked as nia already
  ...
  /*------------------------------------------------------------------- 
   * ELEMENT NEIGHBORS
   * loop over each element and store neighbors for elements (including ghost neighbors, used for finding neighbor for nia)
   * need for finding neighbors of integration points && nia
   *-----------------------------------------------------------------*/ 
  ...
  for (i = 0; i < neall; i++){
    ...
    for (k = 0; k < nelemstencil; k++) {
      ...
      // skip if stencil is out of bound (this might happened for ghost elements (there is no element out there anyway)

      if (ibin+elemstencil[k] >= nb->melembins || ibin+elemstencil[k] < 0) continue;
      for (j = elembinhead[ibin+elemstencil[k]]; j >= 0; j = elembins[j]){
  ...
  for (i = 0; i < nelocal; i++){
    ...
    for (iintg = 0; iintg < nintg; iintg++){
      ...
      for (node = 0; node < npe; node++) {
        ix[0] += shape_array[ietype][iintpl][node]*nodex[i][node][0];
        ix[1] += shape_array[ietype][iintpl][node]*nodex[i][node][1];
        ix[2] += shape_array[ietype][iintpl][node]*nodex[i][node][2];
      }
          ...
          for (isub = 0; isub < nsubelem; isub++){
            ...
            if (rsq < cut_subelemsq)
              ...
              for (l = 0; l < natom_subelem[jetype][isub]; l++){
                ...
                delz = ix[2] - jx[2]; 
                rsq = delx*delx + dely*dely + delz*delz;
                //if (element->tag[i] == 10 && iintg == 63) fprintf(screen,"neighbor list jx = %g %g %g rsq = %g\n",jx[0],jx[1],jx[2],rsq);
                if (rsq < cutneighsq[ictype][jctype]) {
  ...
  /*------------------------------------------------------------------- 
   * NEIGHBORING INTERPOLATED ATOMS NEIGHBOR
   * loop over all interpolated atoms that are marked as nia and store neighbors 
   *-----------------------------------------------------------------*/ 
