...
void ReadData::command(int narg, char **arg)
{
  ...
  natoms = nelements = nnodes = nintpls = 0;
  netypes = natypes = 0;
  ...
  atom->data_fix_compute_variable(0,atom->nlocal);
  element->data_fix_compute_variable(0,element->nlocal);
...
void ReadData::elements()
{
  ...
  if (me == 0) {
    ...
    if (logfile) fprintf(logfile,"  " BIGINT_FORMAT " elementss\n",nassign);
  }
