...
void FixAveForce::init()
{
  ...
  // check if assign forceto node individually in group with atoms, might not be physically correct
  
  if (group_flag == NODE)
    if (group->count_atom(igroup) && group->count_node(igroup))
      error->all(FLERR,"Cannot use fixave on group contain both atoms and nodes for group node option");
...
void FixAveForce::post_force(int /*vflag*/)
{
  ...
  double *elemscale = force->pair->elemscale;
  double fxtmp,fytmp,fztmp;

  if (group_flag && element->nelements) {
    if (group_flag == ELEMENT) {
      for (int i = 0; i < nelocal; i++)
        if (emask[i] & groupbit) {
          ...
          foriginal[0] += fxtmp/elemscale[ietype];
          foriginal[1] += fytmp/elemscale[ietype];
          foriginal[2] += fztmp/elemscale[ietype];
          foriginal[3] += nintpl[ietype];
        }
    } else {
      for (int i = 0; i < nelocal; i++) {
        int ietype = etype[i];
        for (int j = 0; j < npe; j++) {
          if (nodemask[i][j] & groupbit) {
            ...
            foriginal[0] += nodef[i][j][0]/elemscale[ietype];
            foriginal[1] += nodef[i][j][1]/elemscale[ietype];
            foriginal[2] += nodef[i][j][2]/elemscale[ietype];
            foriginal[3] += 1.0/elemscale[ietype];
          }
        }
      }
    }
  ...
  if (group_flag == ELEMENT) {
    for (int i = 0; i < nelocal; i++) {
      ...
      if (region && !region->match(ex[i][0],ex[i][1],ex[i][2])) continue;
      for (int j = 0; j < npe; j++) {
        if (xstyle) nodef[i][j][0] = fave[0];
        if (ystyle) nodef[i][j][1] = fave[1];
        if (zstyle) nodef[i][j][2] = fave[2];
      }
    } 
  } else if (group_flag == NODE) {
    for (int i = 0; i < nelocal; i++)
      for (int j = 0; j < npe; j++) {
