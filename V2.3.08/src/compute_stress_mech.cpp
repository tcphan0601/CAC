...
void ComputeStressMech::surface_neighbor()
{
  ...
  int i,j,k,l,ibin,jetype,isub,inode,jintpl,ictype,jctype;
  ...
  double cutelemsq = force->pair->cutforce + ghostskin + element->max_size/2.0;
  double cutsubelemsq = force->pair->cutforce + ghostskin + element->max_size/2.0/element->esplit;
        ...
        if (rsq < cutelemsq) {
          for (isub = 0; isub < nsubelem; isub++) {
            ...
            for (inode = 0; inode < npe; inode++) {
              delx -= shape_array_center_subelem[isub][inode]*nodex[j][inode][0];
              dely -= shape_array_center_subelem[isub][inode]*nodex[j][inode][1];
              delz -= shape_array_center_subelem[isub][inode]*nodex[j][inode][2];
            }
            rsq = delx*delx + dely*dely + delz*delz;
