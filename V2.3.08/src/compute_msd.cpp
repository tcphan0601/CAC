  ...
  id_fix(NULL)
{
  ...
  if (fix->restart_reset) fix->restart_reset = 0;
	...
	for (i = 0; i < nelocal; i++)
	  for (j = 0; j < npe; j++)
	    if (nodemask[i][j] & groupbit) domain->unmap(nodex[i][j],image_ele[i],nodexoriginal[i][j]);
	    else nodexoriginal[i][j][0] = nodexoriginal[i][j][1] = nodexoriginal[i][j][2] = 0.0;
	  ...
	  for (i = 0; i < nelocal; i++) {
		for (j = 0; j < npe; j++) {
		  if (nodemask[i][j] & groupbit) {
		    nodexoriginal[i][j][0] -= cm[0];
            nodexoriginal[i][j][1] -= cm[1];
            nodexoriginal[i][j][2] -= cm[2];
		  }
		}
	  }
	...
	for (i = 0; i < nelocal; i++) {
	  for (j = 0; j < npe; j++) {
        if (emask[i] & groupbit) {
          ...
          ybox = (image[i] >> IMGBITS & IMGMASK) - IMGMAX;
          zbox = (image[i] >> IMG2BITS) - IMGMAX;
          xtmp = nodex[i][j][0] + xbox*xprd - cm[0];
          ytmp = nodex[i][j][1] + ybox*yprd - cm[1];
          ztmp = nodex[i][j][2] + zbox*zprd - cm[2];
		  
		  if (avflag) {
            nodexoriginal[i][j][0] = (nodexoriginal[i][j][0]*naverage + xtmp)*navfac;
            nodexoriginal[i][j][1] = (nodexoriginal[i][j][1]*naverage + ytmp)*navfac;
            nodexoriginal[i][j][2] = (nodexoriginal[i][j][2]*naverage + ztmp)*navfac;
          }

          dx = xtmp - nodexoriginal[i][j][0];
          dy = ytmp - nodexoriginal[i][j][1];
          dz = ztmp - nodexoriginal[i][j][2];
          msd[0] += dx*dx;
          msd[1] += dy*dy;
          msd[2] += dz*dz;
          msd[3] += dx*dx + dy*dy + dz*dz;
	...
	for (i = 0; i < nelocal; i++) {
	  for (j = 0; j < npe; j++) {
        if (emask[i] & groupbit) {
          ...
          ybox = (image[i] >> IMGBITS & IMGMASK) - IMGMAX;
          zbox = (image[i] >> IMG2BITS) - IMGMAX;
		  xtmp = nodex[i][j][0] + h[0]*xbox + h[5]*ybox + h[4]*zbox - cm[0];
          ytmp = nodex[i][j][1] + h[1]*ybox + h[3]*zbox - cm[1];
          ztmp = nodex[i][j][2] + h[2]*zbox - cm[2];
