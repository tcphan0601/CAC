...
void Displace::dislocation(char **arg)
{
  ...
  // for mixed style, decompose burger vectors into edge and screw components and
  // apply 2 displacement fields for each component
    ...
    double x1, double x2, double b, double v)
{
  ...
  if (rsq < EPSILON)
    error->one(FLERR,"Atoms/Nodes on edge dislocation line");
