...
void Region::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"units") == 0) {
      ...
      point[0] = universe->numeric(FLERR,arg[iarg+2]);
      point[1] = universe->numeric(FLERR,arg[iarg+3]);
      point[2] = universe->numeric(FLERR,arg[iarg+4]);
      axis[0] = universe->numeric(FLERR,arg[iarg+5]);
      axis[1] = universe->numeric(FLERR,arg[iarg+6]);
      axis[2] = universe->numeric(FLERR,arg[iarg+7]);
      rotateflag = 1;
      iarg += 8;
    } else error->all(FLERR, "Illegal region command");
  }
