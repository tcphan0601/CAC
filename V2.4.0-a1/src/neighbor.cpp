...
void Neighbor::print_pairwise_info()
{
  ...
  cut = cutghost[0] = cutghost[1] = cutghost[2] = MAX(cutneighmax,comm->cutghostuser);
  if (element->nelements) {
    cutghost[0] += element->max_bound_box_size[0];
    cutghost[1] += element->max_bound_box_size[1];
    cutghost[2] += element->max_bound_box_size[2];
  }
