  ...
  vptr(NULL), id_temp(NULL)
{
  ...
  if (strcmp(arg[3],"NULL") == 0) id_temp = NULL;
    ...
    error->all(FLERR,"Temperature compute has not been defined yet");
...
void ComputePressure::compute_vector()
{
  ...
  if (dimension == 3) {
    ...
    vector[2] = virial[2] * inv_volume * nktv2p;
    vector[3] = virial[3] * inv_volume * nktv2p;
    vector[4] = virial[4] * inv_volume * nktv2p;
    vector[5] = virial[5] * inv_volume * nktv2p;
  } else {
    inv_volume = 1.0 / (domain->xprd * domain->yprd);
    virial_compute(4,2);
    //if (keflag) {
