...
void Verlet::setup(int flag)
{
  ...
  if (element->nelements) element->evec->update_element_bound_box();
...
void Verlet::force_clear()
{
  ...
  if (nbytes) memset(&element->nodef[0][0][0],0,3*element->max_npe*nbytes);
}
...
void Verlet::run(int n)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (nflag == 0) {
      ...
      // if there are elements, force comm->setup() and neighbor->setup_bins() to 
      // update cutghost from element max_bound_box_size
