...
void CommTiled::setup()
{
  ...
  for (int idim = 0; idim < dimension; idim++) {
    for (int idir = 0; idir < 2; idir++) {
      ...
      //      = sublo to MIN(sublo+cutghost,subhi) in idim, for idir = 0
      //      = MIN(subhi-cutghost,sublo) to subhi in idim, for idir = 1
      ...
      for (i = 0; i < noverlap; i++) {
        ...
        if (idir == 0) {
          ...
          if (i < noverlap1) sbox[3+idim] = MIN(sbox[3+idim]+cutghost[idim],subhi[idim]);
  ...
  if (element->nelements) {
    ...
    if (nsize)
      for (int iswap = 0; iswap < nswap; iswap++) {
        ...
        if (sendother[iswap]) {
          for (i = 0; i < nsend; i++) {
            n = dump->pack_elem_forward_comm(elem_sendnum[iswap][i],elem_sendlist[iswap][i],
                buf_send,pbc_flag[iswap][i],pbc[iswap][i]);
            MPI_Send(buf_send,n,MPI_DOUBLE,sendproc[iswap][i],0,world);
          }
        }
        if (sendself[iswap]) {
          dump->pack_elem_forward_comm(elem_sendnum[iswap][nsend],elem_sendlist[iswap][nsend],
              buf_send,pbc_flag[iswap][nsend],pbc[iswap][nsend]);
          dump->unpack_elem_forward_comm(elem_recvnum[iswap][nrecv],
              elem_firstrecv[iswap][nrecv],buf_send);
        }
        if (recvother[iswap]) {
          for (i = 0; i < nrecv; i++) {
            MPI_Waitany(nrecv,requests,&irecv,MPI_STATUS_IGNORE);
            dump->unpack_elem_forward_comm(elem_recvnum[iswap][irecv],elem_firstrecv[iswap][irecv],
                &buf_recv[nsize*elem_forward_recv_offset[iswap][irecv]]);
          }
        }
      }
