...
void WriteDataAdrian::header()
{
  ...
  if (element->nelements) fprintf(fp,BIGINT_FORMAT " cac elements\n",element->nelements/element->apc[1]);
...
void WriteDataAdrian::type_arrays()
{
  if (atom->mass) {
...
//
//      atom->avec->write_data_adrian(fp,recvrow,buf);
//    }
//
//  } else {
...
void WriteDataAdrian::elements()
{
  ...
  int ncol = 5 + element->max_npe*element->apc[1]*3;

  int sendrow = element->nlocal/element->apc[1];
  int maxrow;
