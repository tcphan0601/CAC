...
void CommBrick::setup()
{
  ...
  if (triclinic == 0) {
    ...
    if (element->nelements) {
      cutghost[0] += element->max_bound_box_size[0]/2;
      cutghost[1] += element->max_bound_box_size[1]/2;
      cutghost[2] += element->max_bound_box_size[2]/2;
    }
    ...
    if (element->nelements) {
      ...
      element_size_x[0] = element->max_bound_box_size[0]/2 + domain->boxlo[0];
      element_size_x[1] = element->max_bound_box_size[1]/2 + domain->boxlo[1];
      element_size_x[2] = element->max_bound_box_size[2]/2 + domain->boxlo[2];
      ...
      for (ineed = 0; ineed < twoneed; ineed++) {
        ...
        if (sendflag) {
          if (!bordergroup || ineed >= 2) {
            if (mode == SINGLE) {
              for (i = nfirst; i < nlast; i++) {
                if (element_bound_box[i][dim] > hi || element_bound_box[i][dim] < lo) 
                  continue;
