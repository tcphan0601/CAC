...
void CommTiled::setup_borders() 
{
  ...
  // scaling factors for triclinic box 
  // = 1 if not triclinic 
  ...
  if (triclinic) {
    ...
    length[0] = sqrt(h_inv[0] * h_inv[0] + h_inv[5] * h_inv[5] + h_inv[4] * h_inv[4]);
    length[1] = sqrt(h_inv[1] * h_inv[1] + h_inv[3] * h_inv[3]);
    length[2] = h_inv[2];
  } else {
    prd = domain->prd;
    boxlo = domain->boxlo;
  ...
  cutghost[0] = cut * length[0];
  cutghost[1] = cut * length[1];
  cutghost[2] = cut * length[2];
  ...
  double box_grid[3][4];
  for (dim = 0; dim < dimension; dim++) {
    box_grid[dim][0] = sublo[dim] - max_cutghost[dim] + BOX_EPSILON * limit_flag[dim] * length[dim];
    box_grid[dim][1] = sublo[dim];
    box_grid[dim][2] = subhi[dim];
    box_grid[dim][3] = subhi[dim] + max_cutghost[dim] - BOX_EPSILON * limit_flag[dim] * length[dim];
  }
