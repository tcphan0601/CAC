...
enum{COORDINATE, ID, ETYPE, CTYPE, VALIDITY, VONMISES, VOLSTRAIN, DEFGRAD, ROTATION, NSD, STRAINTENSOR, STRETCHTENSOR};
...
void AtomicStrain::write_output(char *filename)
{
  ...
  if (output_format != ATOM) {
    ...
    size_one = 6 + 3 * element_info_flag + 9 * deformation_gradient_flag + 4 * rotation_flag + nsd_flag + 6 * (strain_tensor_flag + stretch_tensor_flag);
    ...
    size_one = 8 + 5 * element_info_flag + 9 * deformation_gradient_flag + 4 * rotation_flag + nsd_flag + 6 * (strain_tensor_flag + stretch_tensor_flag);
    int nalocal = atom->nlocal;
    int nelocal = element->nlocal;
    int *apc = element->apc;
    int *etype = element->etype;
...
void AtomicStrain::write_header_atom(char *file, bigint ndump) 
{
  ...
  if (me == 0) {
    ...
    if (element_info_flag)
      strcat(columns, " etype eid inode iucell ibasis");
