...
void Neighbor::print_pairwise_info()
{
  ...
  double cut = MAX(cutneighmax, comm->cutghostuser);
...
void Neighbor::build(int topoflag)
{
  ...
  for (i = 0; i < npair_perpetual; i++) {
    ...
    if (!lists[m]->copy) {
      ...
      if (lists[i]->atomlist || lists[i]->atomonlylist) {
        nlocal += atom->nlocal;
      ...
      if (lists[i]->threebody) {
        ...
        lists[i]->grow_nva(n, element->nlocal + element->nghost);
      }
    }
