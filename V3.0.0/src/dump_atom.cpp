...
DumpAtom::DumpAtom(CAC *cac, int narg, char **arg) : Dump(cac, narg, arg)
{
  ...
  if (dump_element_style == GAUSSIAN && force_flag) force->pair->save_gcell_force = 1;
...
{
  atom_size_one = 5 + 6 * stress_flag + 3 * (velocity_flag + force_flag) + 4 * displace_flag + pe_flag + 7 * element_flag;
