  ...
  id_temp(NULL), atom_stress(NULL), node_stress(NULL)
{
  ...
  timeflag = 1;
  comm_atom_reverse = 6;
  comm_elem_reverse = 6 * element->maxnpe * element->maxapc;
...
void ComputeStressAtom::compute_peratom()
{
  ...
  if (element->nmax > elem_nmax || maxnpe < element->maxnpe) {
    ...
    array_node = memory->create(node_stress, elem_nmax, element->maxapc, 
        element->maxnpe, 6, "stress/atom:atom_stress");
  }
  ...
  memset(&atom_stress[0][0],0,atom_nlocal * 6 * sizeof(double));
  memset(&node_stress[0][0][0][0],0,elem_nlocal * element->maxapc * 
