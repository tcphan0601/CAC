...
void ReadData::command(int narg, char **arg)
{
  ...
  while (strlen(keyword)) {
    if (strcmp(keyword,"Masses") == 0) {
      ...
      if (element_cluster_flag == 0 || element->max_apc == 1) {
...
void ReadData::elements()
{
  ...
  if (nclusters%product && element->element_cluster_flag && element->max_apc > 1)
...
void ReadData::nodes()
{
  ...
  while (nread < nnodes) {
    ...
    if (eof) error->all(FLERR,"Unexpected end of data file");
    if (referenceflag)
      element->data_nodes_reference(nchunk,buffer,eid_offset,shiftflag,shift);
    else 
