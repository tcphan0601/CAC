  ...
  */
  ...
  for (i = 0; i < nelocal; i++) {
    for (j = 0; j < npe[etype[i]]; j++) {
      if (nodemask[i][j] & groupbit) {
        gamma1 = gfactor1[ctype[i]];
        gamma2 = gfactor2[ctype[i]] * tsqrt / sqrt_nodal_weight[etype[i]];

        fran[0] = gamma2*(random->uniform()-0.5);
        fran[1] = gamma2*(random->uniform()-0.5);
        fran[2] = gamma2*(random->uniform()-0.5);

        fdrag[0] = gamma1*nodev[i][j][0];
        fdrag[1] = gamma1*nodev[i][j][1];
        fdrag[2] = gamma1*nodev[i][j][2];

        nodef[i][j][0] += fdrag[0] + fran[0];
        nodef[i][j][1] += fdrag[1] + fran[1];
        nodef[i][j][2] += fdrag[2] + fran[2];
  ...
  if (Tp_ZERO) {
    ...
    for (i = 0; i < nelocal; i++) {
      for (j = 0; j < npe[etype[i]]; j++) {
        if (nodemask[i][j] & groupbit) {
          nodef[i][j][0] -= fsumall[0];
          nodef[i][j][1] -= fsumall[1];
          nodef[i][j][2] -= fsumall[2];
        }
      }
    }
  }
