...
void CommTiled::setup_borders() 
{
  ...
  for (dim = 0; dim < dimension; dim++) {
    ...
    max_cutghost[dim] += cut;

    // limit max_cutghost to 3*prd
    if (max_cutghost[dim] >= 3*prd[dim]) {
      max_cutghost[dim] = MIN(3*prd[dim],max_cutghost[dim]);
      limit_flag[dim] = 1;
    } else {
      limit_flag[dim] = 0;
    }
  ...
  double box_grid[3][4];
  for (dim = 0; dim < dimension; dim++) {
    box_grid[dim][0] = sublo[dim] - max_cutghost[dim] + BOX_EPSILON*limit_flag[dim];
    box_grid[dim][1] = sublo[dim];
    box_grid[dim][2] = subhi[dim];
    box_grid[dim][3] = subhi[dim] + max_cutghost[dim] - BOX_EPSILON*limit_flag[dim];
  }
  ...
  double lo1[3],hi1[3],lo2[343][3],hi2[343][3];
  int current_pbc[3],current_pbc_flag[3];
  int pbc2[343][3]; // pbc flag for second ghost box set
  int one,two;
