...
DumpTecplot::DumpTecplot(CAC *cac, int narg, char **arg) : Dump(cac,narg,arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"stress") == 0) {
      ...
      if (average_flag && (!element->element_cluster_flag || element->max_apc == 1)) {
        average_flag = 0;
...
void DumpTecplot::pack_elem_noscale_noimage(tagint *ids)
{
  ...
  if (element->element_cluster_flag && element->max_apc > 1)
    element_clusters = element->element_clusters;
  int *element_shape_ids = element->element_shape_ids;
  int m = 0;
  int inpe,itype;

  element->reset_node_tags(igroup,average_flag);
    ...
    for (i = 0; i < nlocal; i++) {
      ...
      for (j = 0; j < inpe; j++) {
        ...
        for (k = 1; k < iapc; k++) {
          ...
          iy += nodex[ii][j][1]; 
          iz += nodex[ii][j][2];
          if (fabs(nodex[i][j][0]-nodex[ii][j][0]) > domain->prd_half[0]) {
            if (nodex[i][j][0]>nodex[ii][j][0])
              ix += domain->xprd;
            else 
              ix -= domain->xprd;
          }
          if (fabs(nodex[i][j][1]-nodex[ii][j][1]) > domain->prd_half[0]) {
            if (nodex[i][j][2]>nodex[ii][j][1])
              iy += domain->yprd;
            else 
              iy -= domain->yprd;
          }
          if (fabs(nodex[i][j][2]-nodex[ii][j][2]) > domain->prd_half[0]) {
            if (nodex[i][j][2]>nodex[ii][j][2])
              iz += domain->zprd;
            else 
              iz -= domain->zprd;
          }
      ...
      for (j = 0; j < inpe; j++) {
        ...
        for (k = 1; k < iapc; k++) {
          ...
          if (valueflag == COORDINATE) {
            ...
            if (fabs(nodex[i][j][ival]-nodex[ii][j][ival]) > domain->prd_half[ival]) {
              if (nodex[i][j][ival]>nodex[ii][j][ival])
                buf[m] += domain->prd[ival];
              else 
                buf[m] -= domain->prd[ival];
            }
          }
        ...
        if (valueflag != ID && valueflag != TYPE && valueflag != OWNERPROC)
          buf[m] /= iapc; 
        m++;
      }
    }
...
int DumpTecplot::count_nodes()
{
  ...
  for (int i = 0; i < element->nlocal; i++) {
    if (average_flag) {
      if (element->tag[i] == element->element_clusters[i][0])
        n += npe[etype[i]];
    } else {
      n += npe[etype[i]];
    }
