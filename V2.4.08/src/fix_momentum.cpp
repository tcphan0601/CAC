...
void FixMomentum::end_of_step()
{
  ...
  if (linear) {
    ...
    for (int i = 0; i < nelocal; i++)
      if (emask[i] & groupbit) 
        for (int node = 0; node < npe[etype[i]]; node++) {
          if (xflag) nodev[i][node][0] -= vcm[0];
          if (yflag) nodev[i][node][1] -= vcm[1];
          if (zflag) nodev[i][node][2] -= vcm[2];
        }
  ...
  if (angular) {
    ...
    for (int i = 0; i < nelocal; i++)
      if (emask[i] & groupbit) {
        domain->unmap(x[i],image[i],unwrap);
        for (int node = 0; node < npe[etype[i]]; node++) {
          dx = nodex[i][node][0] - x[i][0] + unwrap[0] - xcm[0];
          dy = nodex[i][node][1] - x[i][1] + unwrap[1] - xcm[1];
          dz = nodex[i][node][2] - x[i][2] + unwrap[2] - xcm[2];
          nodev[i][node][0] -= omega[1]*dz - omega[2]*dy;
          nodev[i][node][1] -= omega[2]*dx - omega[0]*dz;
          nodev[i][node][2] -= omega[0]*dy - omega[1]*dx;
        }
      } 
  }
  ...
  if (rescale) {
    ...
    for (int i = 0; i < nelocal; i++) 
      if (emask[i] & groupbit) 
        for (int node = 0; node < npe[etype[i]]; node++) {
          nodev[i][node][0] *= factor;
          nodev[i][node][1] *= factor;
          nodev[i][node][2] *= factor;
        }
