    ...
    // ----------------
      ...
      if (strncmp(word,"c_",2) == 0 || strncmp(word,"C_",2) == 0) {
        ...
        // c_ID[i] = scalar from per-atom vector
          ...
          peratom2global(1,NULL,compute->vector_atom,1,index1,-1,
        ...
        // c_ID[i][j] = scalar from per-atom array
          ...
          if (compute->array_atom)
            peratom2global(1,NULL,&compute->array_atom[0][index2-1],
                compute->size_peratom_cols,index1,-1,
                tree,treestack,ntreestack,argstack,nargstack);
          else
      ...
      // ----------------
        ...
        // f_ID[i] = scalar from per-atom vector
          ...
          peratom2global(1,NULL,fix->vector_atom,1,index1,-1,
              tree,treestack,ntreestack,argstack,nargstack);
        ...
        // f_ID[i][j] = scalar from per-atom array
          ...
          if (fix->array_atom)
            peratom2global(1,NULL,&fix->array_atom[0][index2-1],
                fix->size_peratom_cols,index1,-1,
                tree,treestack,ntreestack,argstack,nargstack);
          else
            peratom2global(1,NULL,NULL,
                fix->size_peratom_cols,index1,-1,
      ...
      // ----------------
        ...
        // use peratom2global to extract single value from result
          ...
          peratom2global(1,NULL,result,1,index,-1,
              tree,treestack,ntreestack,argstack,nargstack);
      ...
      // ----------------
        ...
        if (str[i] == '(') {
          ...
          peratom2global(0,word,NULL,0,id,inode,
              tree,treestack,ntreestack,argstack,nargstack);
...
/* ----------------------------------------------------------------------
   ...
   countatom(group),countnode(group),countelem(group),countintpl(group),mass(group),
    ...
    double *argstack, int &nargstack, int ivar)
{
  ...
  if (strcmp(word,"countatom") && strcmp(word,"countnode") &&
      ...
      strcmp(word,"countintpl") && strcmp(word,"bound") &&
  ...
  } else if (strcmp(word,"countintpl") == 0) {
    if (narg == 1) value = group->count_intpl(igroup);
    else if (narg == 2) value = group->count_intpl(igroup,region_function(args[1],ivar));
    else print_var_error(FLERR,"Invalid group function in variable formula",ivar);
...
void Variable::peratom2global(int flag, char *word,
    double *vector, int nstride, tagint id, int inode,
    ...
    double *argstack, int &nargstack)
{
  ...
  if (index >= 0) {
    ...
    if (flag == 0) {
      ...
      else if (strcmp(word,"emass") == 0) mine = atom->mass[element->ctype[index]];
      else if (strcmp(word,"etype") == 0) mine = element->etype[index];
      else if (strcmp(word,"ctype") == 0) mine = element->ctype[index];
      ...
      else if (strcmp(word,"nodex") == 0) mine = element->nodex[index][inode][0];
      else if (strcmp(word,"nodey") == 0) mine = element->nodex[index][inode][1];
      else if (strcmp(word,"nodez") == 0) mine = element->nodex[index][inode][2];
      else if (strcmp(word,"nodevx") == 0) mine = element->nodev[index][inode][0];
      else if (strcmp(word,"nodevy") == 0) mine = element->nodev[index][inode][1];
      else if (strcmp(word,"nodevz") == 0) mine = element->nodev[index][inode][2];
      else if (strcmp(word,"nodefx") == 0) mine = element->nodef[index][inode][0];
      else if (strcmp(word,"nodefy") == 0) mine = element->nodef[index][inode][1];
      else if (strcmp(word,"nodefz") == 0) mine = element->nodef[index][inode][2];
