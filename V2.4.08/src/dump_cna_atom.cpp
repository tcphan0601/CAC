...
{
  if (image_flag == 0) atom_size_one = 8;
  else atom_size_one = 11;
  ...
  delete [] format;
  char *str;
  if (image_flag == 0) str = (char *) TAGINT_FORMAT " %d %g %g %g %d %d %d";
  n = strlen(str) + 2;
  format = new char[n];
  strcpy(format,str);
  strcat(format,"\n");
  ...
  if (image_flag == 0) 
    columns = strcpy(columns,"id type x y z structure_id eid intpl");
  else
    columns = strcpy(columns,"id type x y z structure_id eid intpl ix iy iz");

  if (domain->triclinic == 0)
    header_choice = &DumpCNAAtom::header_item;
  else
    header_choice = &DumpCNAAtom::header_item_triclinic;
...
void DumpCNAAtom::header_item_triclinic(bigint ndump)
{
  fprintf(fp,"ITEM: TIMESTEP\n");
  fprintf(fp,BIGINT_FORMAT "\n",update->ntimestep);
  fprintf(fp,"ITEM: NUMBER OF ATOMS\n");
    ...
    if (mask[i] & groupbit) {
      ...
      for (int iintpl = 0; iintpl < nintpl[ietype]; iintpl++) {
        ...
        for (int k = 0; k < nkeep; k++)
          if (pattern_intpl[i][iouter] == keep[k]) {
            needtag++;
            break;
          }
      }
...
int DumpCNAAtom::count_atoms()
{
  ...
  if (compute_cna->invoked_peratom != update->ntimestep ||
      (compute_cna->invoked_peratom == update->ntimestep && compute_cna->preneighflag 
       && neighbor->lastcall == update->ntimestep)) {
    compute_cna->compute_peratom();
    compute_cna->preneighflag = 0;
  }
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit)
      for (int k = 0; k < nintpl[etype[i]]; k++) {
        int iouter = is_outer[etype[i]][k] - 1;
        if (iouter < 0) continue;
        for (int j = 0; j < nkeep; j++) 
          if (pattern_intpl[i][iouter] == keep[j]) {
            n++;
            break;
          }
      }
