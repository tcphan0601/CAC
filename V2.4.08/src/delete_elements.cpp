...
void DeleteElements::command(int narg, char **arg)
{
  ...
  if (allflag) {
    ...
    while (i < nlocal) {
      if (del_flag_list[i]) {
        ndelintpls += nintpl[etype[i]];
        element->evec->copy(nlocal-1,i,1);
  ...
  bigint sum;
  MPI_Allreduce(&ndelintpls,&sum,1,MPI_CAC_BIGINT,MPI_SUM,world);
  element->nintpls -= sum;
...
void DeleteElements::options(int narg, char **arg)
{
  ...
  if (element->element_cluster_flag && compress_flag == 1) {
    compress_flag = 0;
    if (comm->me == 0) error->warning(FLERR,"Element Cluster exists, compress flag is ignored");
  }
