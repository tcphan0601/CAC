...
RegPlane::RegPlane(CAC *cac, int narg, char **arg) :
  Region(cac, narg, arg)
{
  options(narg-8,&arg[8]);

  xp = xscale*universe->numeric(FLERR,arg[2]);
  yp = yscale*universe->numeric(FLERR,arg[3]);
  zp = zscale*universe->numeric(FLERR,arg[4]);
  normal[0] = xscale*universe->numeric(FLERR,arg[5]);
  normal[1] = yscale*universe->numeric(FLERR,arg[6]);
  normal[2] = zscale*universe->numeric(FLERR,arg[7]);
