...
Element::~Element()
{
  ...
  if (max_apc > 1) memory->destroy(element_clusters);
...
{
  if (element_cluster_flag == 0) return;
...
void Element::modify_params(int narg, char **arg)
{
  ...
  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"max/intpl") == 0) {
...
void Element::add_etype(int narg, char **arg)
{
  ...
  init();  
  evec->grow_etype_arrays(itype,0);
...
/* ----------------------------------------------------------------------
   init per-element fix/compute/variable values for a newly created element that is passed on from an old element
   called from fix_adaptive
   fixes, computes, variables may or may not exist when called
   ------------------------------------------------------------------------- */

void Element::data_pass_on_fix_compute_variable(int i, int i_old, int *iintpl_list)
{
  for (int m = 0; m < modify->nfix; m++) {
    Fix *fix = modify->fix[m];
    if (fix->create_attribute)
...
void Element::check_element_clusters()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    for (int j = 0; j < apc[etype[i]]; j++) {
      if ((m = map(element_clusters[i][j])) >= 0) {
        if (etype[m] != etype[i])
