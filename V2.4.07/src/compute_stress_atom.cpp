  ...
  id_temp(NULL), atom_stress(NULL), node_stress(NULL)
{
  ...
  timeflag = 1;
  comm_atom_reverse = 6;
  comm_elem_reverse = 6*element->max_npe;
...
void ComputeStressAtom::compute_peratom()
{
  ...
  if (element->nmax > elem_nmax || max_npe < element->max_npe) {
    ...
    comm_elem_reverse = max_npe * 6;
    array_node = memory->create(node_stress,elem_nmax,max_npe,6,"stress/atom:atom_stress");
  }
...
double ComputeStressAtom::memory_usage()
{
  ...
  bytes += elem_nmax * 6 * max_npe * sizeof(double);
  return bytes;
}
