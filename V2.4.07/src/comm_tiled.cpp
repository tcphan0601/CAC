...
void CommTiled::setup_borders() 
{
  ...
  if (dimension == 3) max = MAX(box_size[2],max);
  ...
  for (int ix = 0; ix < 3; ix ++) {
    for (int iy = 0; iy < 3; iy ++) {
      for (int iz = 0; iz < 3; iz ++) {
        ...
        for (dim = 0; dim < dimension; dim++) {
          if (periodicity[dim]) {
            if (lo1[dim] < boxlo[dim]) current_pbc[dim] = 1;
            if (hi1[dim] > boxhi[dim]) current_pbc[dim] = -1;
