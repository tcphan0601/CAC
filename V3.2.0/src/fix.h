...
class Fix : protected Pointers {
 public:
  ...
  // Where per-element data is stored (affects how dump custom accesses it).
  // NONE:            No per-element array
  // NODE:            array_node / vector_node — indexed by node; must interpolate to get vatom values
  // GAUSS:           array_varom / vector_vatom = all gauss points
  // VIRTUAL:         array_vatom / vector_vatom — all ucells (inner + outer)
  // VIRTUAL_SURFACE: array_vatom / vector_vatom — outer (surface) ucells only
  enum ElementDataStyle { NONE = 0, NODE = 1, GAUSS = 2, VIRTUAL = 3, VIRTUAL_SURFACE = 4 };
  int element_data_style;

  // clang-format on

  int group_flag;                // 2 if group by element, 1 if group by node, 0 if only select atoms
                                 // as in group.h

  int system_changed;            // 1 if Fix just changes system and will thus require immediate reneighboring, 0 if not
  ...
  double *vector_elem;           // computed per-elem vector
  double ***vector_node;         // computed per-node vector
  double ***vector_vatom;        // computed per-vatom vector
  double **array_atom;           // computed per-atom array
  double **array_elem;           // computed per-elem array
  double ****array_node;         // computed per-node array
  double ****array_vatom;        // computed per-vatom array
  ...
  int comm_atom_border;               // size of border communication (0 if none)
  ...
  int comm_elem_border;               // size of border communication (0 if none)

  double virial[6];              // accumulated virial
  double *eatom, **vatom;         // accumulated per-atom energy/virial
  double **eucell, ***vucell;     // accumulated per-ucell energy/virial
  ...
  virtual int pack_atom_border(int, int *, double *) {return 0;}
  virtual int unpack_atom_border(int, int, double *) {return 0;}
  virtual int pack_atom_exchange(int, double *) {return 0;}
  virtual int unpack_atom_exchange(int, double *) {return 0;}
  virtual int pack_elem_border(int, int *, double *) {return 0;}
  virtual int unpack_elem_border(int, int, double *) {return 0;}
  virtual int pack_elem_exchange(int, double *) {return 0;}
  virtual int unpack_elem_exchange(int, double *) {return 0;}
  ...
  virtual int pack_atom_reverse_comm_size(int, int) {return 0;}
  ...
  virtual int pack_elem_reverse_comm_size(int, int) {return 0;}
