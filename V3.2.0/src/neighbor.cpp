...
int Neighbor::init_pair()
{
  ...
  int max_outer_ucell = element->nmax * element->max_outer_ucell;
  ...
  for (i = 0; i < nlist; i++)
    if (neigh_pair[i] && !lists[i]->copy) {
      ...
      if (lists[i]->vatomlist) {
        if (lists[i]->noinner) 
          maxall += max_outer_ucell * maxapc;
        else 
          maxall += maxucell * maxapc;
      }
...
void Neighbor::morph_copy()
{
  ...
  for (i = 0; i < nrequest; i++) {
    ...
    for (j = 0; j < nrequest; j++) {
      ...
      if (irq->neighneigh != jrq->neighneigh) continue;
...
void Neighbor::print_pairwise_info()
{
  ...
  if (element->nelements) {
    ...
    if (elembinsizey == 0.0) elembinsizey = bbox[1];
    if (elembinsizez == 0.0) elembinsizez = bbox[2];
  }
  ...
  for (j = 0; j < npair_perpetual; j++) {
    ...
    if (!lists[m]->copy) {
      ...
      if (lists[m]->vatomlist) {
        if (lists[m]->noinner) {
          nlocal += element->nlocal * element->maxapc * element->max_outer_ucell;
          nall += (element->nlocal + element->nghost) * element->maxapc * element->max_outer_ucell;
        } else {
          for (i = 0; i < element->nlocal + element->nghost; i++) {
