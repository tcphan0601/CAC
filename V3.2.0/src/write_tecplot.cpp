...
#else
      else if (strcmp(arg[iarg+1], "plt") == 0 || strcmp(arg[iarg+1], "szplt") == 0) 
...
void WriteTecplot::nodes()
{
  ...
  if (fileformat == ASCII) {
    ...
    if (me == 0) memory->create(buf, MAX(1, maxrow), "write_tecplot:buf");
...
/* ----------------------------------------------------------------------
   write simulation cell as a zone, geometry coordinates only (x, y, z)
   8 corner nodes of the triclinic parallelepiped, 1 FEBRICK element
   ------------------------------------------------------------------------- */

void WriteTecplot::write_box_zone()
{
  if (me != 0) return;
  ...
  // K=1 (bottom face): nodes 1-4; K=2 (top face): nodes 5-8
  double cx[8] = {
    xlo,           xlo+lx,
    xlo+lx+xy,     xlo+xy,
    xlo+xz,        xlo+lx+xz,
  ...
  if (fileformat == ASCII) {
    ...
    // variables declared in header: x y z ID etype ctype — write zeros for non-geometric cols
    for (int i = 0; i < 8; i++)
    ...
    // 6 variables in file: x(0) y(1) z(2) ID(3) etype(4) ctype(5)
    // mark ID, etype, ctype as passive
    int passive[6] = {0, 0, 0, 1, 1, 1};
    int valueloc[6] = {1, 1, 1, 1, 1, 1};   // all nodal
