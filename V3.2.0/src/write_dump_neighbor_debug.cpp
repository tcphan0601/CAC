...
void WriteDumpNeighborDebug::command(int narg, char **arg)
{
  ...
  if (narg > 1) {
    if (strcmp(arg[1], "gauss") == 0) {
      ...
      ibasis = universe->inumeric(FLERR, arg[3]);
      igcell = universe->inumeric(FLERR, arg[4]);
    } else if (strcmp(arg[1], "atom") == 0) {
      singleflag = 2;
      if (3 != narg) error->all(FLERR, "Illegal write_dump_neighbor_debug command");
      id = universe->inumeric(FLERR, arg[2]);
    } else if (strcmp(arg[1], "nva") == 0) {
      singleflag = 3;
      if (4 != narg) error->all(FLERR, "Illegal write_dump_neighbor_debug command");
      inva = universe->inumeric(FLERR, arg[2]);
      id = universe->inumeric(FLERR, arg[3]);
...
void WriteDumpNeighborDebug::write(char *file)
{
  ...
  if (singleflag == 0) {
    ...
    if (singleflag == 1) {
      ...
      ielem = list->nvailist[inva];
      iindex = list->nvaindexlist[inva];
      ietype = etype[i];
      iapc = apc[ietype];
      iucell = iindex / iapc;
      ibasis = iindex % iapc;
...
void WriteDumpNeighborDebug::header()
{
  ...
  } else if (singleflag == 3 && inva >= 0) {
    n = list->nvanumneigh[inva] + 1;
  }
