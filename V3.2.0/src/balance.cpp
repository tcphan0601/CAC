...
void Balance::command(int narg, char **arg)
{
  ...
  if (me == 0 && logfile) fprintf(logfile, "System init for balancing work load ...\n");
  cac->init();
...
int Balance::options(int iarg, int narg, char **arg)
{
  ...
  if (outflag && me == 0) {
    ...
    if (fp == nullptr) error->one(FLERR, "Cannot open (fix) balance output file");
    if (outflag == 2) {
...
int *Balance::bisection(int sortflag)
{
  ...
  // no need to convert nodex since not needed
  ...
  // if no element, ignore per-particle weights (wt = nullptr)
  ...
  for (int i = 0; i < element->nlocal; i++) {
    ...
    if (wt) {
      ...
      else wt[i] = element->ngcell[element->etype[i]] * element->apc[element->etype[i]];
    }
  }
  ...
  double boxscale[3];
  if (triclinic) {
    boxscale[0] = domain->xprd;
    boxscale[1] = domain->yprd;
    boxscale[2] = domain->zprd;
  } else 
    boxscale[0] = boxscale[1] = boxscale[2] = 1.0;

  // invoke RCB
  // then invert() to create list of proc assignments for my atoms/elements
  // if triclinic, RCB operates on lamda coords

  rcb->compute(dim, ntotal, dots, wt, shrinklo, shrinkhi, boxscale);
  if (triclinic) {
    domain->lamda2x(atom->nlocal, atom->x);
    domain->lamda2x(element->nlocal, element->x);
  }
  ...
  for (int idim = 0; idim < ndim; idim++) {
    ...
    if (user_element_weight) 
      ...
      for (i = 0; i < element->nlocal; i++)
        mycost += ngcell[etype[i]] * apc[etype[i]];
    }
    MPI_Allreduce(&mycost, &totalcost, 1, MPI_DOUBLE, MPI_SUM, world);
...
void Balance::tally(int dim, int n, double *split)
{
  ...
  if (user_element_weight) {
    ...
    for (int i = 0; i < element->nlocal; i++) {
      index = binary_search(x[i][dim], n, split);
      onecost[index] += ngcell[etype[i]] * apc[etype[i]];
    }
  }
...
double Balance::imbalance_splits()
{
  ...
  if (user_element_weight) {
    ...
    for (int i = 0; i < element->nlocal; i++) {
      ...
      iy = binary_search(x[i][1], ny, ysplit);
      iz = binary_search(x[i][2], nz, zsplit);
      proccost[iz * nx * ny + iy * nx + ix] += ngcell[etype[i]] * apc[etype[i]];
    }
  }
