...
FixBalance::FixBalance(CAC *cac, int narg, char **arg) :
  Fix(cac, narg, arg), 
  balance(nullptr), irregular_comm(nullptr)
{
...
void FixBalance::setup_pre_exchange()
{
  ...
  //   even though pbc() will be done again in Verlet::setup()
  ...
  // if comm->layout is not tiled, force rebalance to convert to layout tiled

  imbnow = balance->imbalance_factor(maxloadperproc, minloadperproc);

  if (imbnow > thresh || comm->layout != Comm::LAYOUT_TILED) rebalance();
  else imbfinal = imbnow;
...
void FixBalance::rebalance()
{
  ...
  if (screenflag && comm->me == 0) {
    if (screen)
      fprintf(screen, "Rebalancing...\n  timestep = " BIGINT_FORMAT "\n", update->ntimestep);
    if (logfile)
      fprintf(logfile, "Rebalancing...\n  timestep = " BIGINT_FORMAT "\n", update->ntimestep);
  }
  ...
  double *sublo = domain->sublo_lamda;
  double *subhi = domain->subhi_lamda;

  // move atoms to new processors via irregular_comm() and skip comm->exchange
  ...
  if (screenflag && comm->me == 0) {
    if (screen) fprintf(screen, "  old/new imbalance factor = %g/%g\n", imbprev, imbnew);
    if (logfile) fprintf(logfile, "  old/new imbalance factor = %g/%g\n", imbprev, imbnew);
  }
}
