...
Element::Element(CAC *cac) : Pointers(cac)
{
  ...
  x = nullptr;
  image = nullptr;
  surface_plane = nullptr;
  element_box2xi = nullptr;
  element_box2xi_scale = nullptr;
  cell_size = nullptr;
  ucell_vol = nullptr;
  tag = nullptr;
  grain_tag = nullptr;
  ...
  mask = nullptr;
  nodemask = nullptr;
  subelem_size = nullptr;
  initial_box_size = nullptr;
  element_bound_box = nullptr;

  // netype-length arrays

  apc = nullptr;
  npe = nullptr;
  element_shape_ids = nullptr;
  element_shape_names = nullptr;
  nucell = nullptr;
  surface_ucell = nullptr;
  nsurface_ucell = nullptr;
  edge_ucell = nullptr;
  nedge_ucell = nullptr;
  ngcell = nullptr;
  us2u = nullptr;
  ...
  n2g = nullptr;
  n2u = nullptr;
  nucell_subelem = nullptr;
  weight = nullptr;
  weight_scale_flag = NOSCALE;
  weight_scale[0] = weight_scale[1] = -1;
  weight_scale[2] = weight_scale[3] = -1;
  shape_array = nullptr; 
  weighted_shape_array = nullptr;
  nodal_weight = nullptr;
  nodal_visual_position = 0.5;

  nsubelem = nullptr;
  shape_array_center_subelem = nullptr;
  subsplit = nullptr;
...
#undef ELEMENT_CLASS
  ...
  debug_mode = 0;
  debug_gcell_type = nullptr;
  debug_gcell_inactive = nullptr;
}
...
void Element::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "id") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "nodal/visual/position") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal element_modify command");
      nodal_visual_position = universe->numeric(FLERR, arg[iarg+1]);
      if (nodal_visual_position < 0 || nodal_visual_position > 1) 
        error->all(FLERR, "Illegal element_modify command, nodal/visual/position must be between 0 and 1");
      iarg += 2;
...
void Element::shape_function(double *shape_column, double px, double py, double pz, int shape_id)
{
  if (shape_id == QUADRILATERAL) {
    shape_column[0] = 0.25 * (1 - px) * (1 - py);
    shape_column[1] = 0.25 * (1 + px) * (1 - py);
    shape_column[2] = 0.25 * (1 + px) * (1 + py);
    shape_column[3] = 0.25 * (1 - px) * (1 + py);
  } else if (shape_id == TRIANGLE) {
    shape_column[0] = px;
    shape_column[1] = py;
    shape_column[2] = 1 - px - py;
  } else if (shape_id == HEXAHEDRON) {
    shape_column[0] = 0.125 * (1 - px) * (1 - py) * (1 - pz);
    shape_column[1] = 0.125 * (1 + px) * (1 - py) * (1 - pz);
    shape_column[2] = 0.125 * (1 + px) * (1 + py) * (1 - pz);
    shape_column[3] = 0.125 * (1 - px) * (1 + py) * (1 - pz);
    shape_column[4] = 0.125 * (1 - px) * (1 - py) * (1 + pz);
    shape_column[5] = 0.125 * (1 + px) * (1 - py) * (1 + pz);
    shape_column[6] = 0.125 * (1 + px) * (1 + py) * (1 + pz);
    shape_column[7] = 0.125 * (1 - px) * (1 + py) * (1 + pz);
  } else if (shape_id == TETRAHEDRON) {
    shape_column[0] = 1 - px - py - pz;
    shape_column[1] = px;
    shape_column[2] = py;
    shape_column[3] = pz;
  } else if (shape_id == OCTAHEDRON) {
    error->all(FLERR, "Shape function for octahedron element is not working yet");
  } else if (shape_id == PYRAMID) {
    if (fabs(pz - 1.0) < 1.0e-20) {
      shape_column[0] = 0.0;
      shape_column[1] = 0.0;
      shape_column[2] = 0.0;
      shape_column[3] = 0.0;
      ...
      shape_column[0] = 0.25 * (1.0 - px - pz) * (1.0 - py - pz)/(1 - pz);
      shape_column[1] = 0.25 * (1.0 + px - pz) * (1.0 - py - pz)/(1 - pz);
      shape_column[2] = 0.25 * (1.0 + px - pz) * (1.0 + py - pz)/(1 - pz);
      shape_column[3] = 0.25 * (1.0 - px - pz) * (1.0 + py - pz)/(1 - pz);
      shape_column[4] = pz;
    }
  } else if (shape_id == WEDGE) {
