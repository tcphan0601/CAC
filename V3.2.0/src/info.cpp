...
static const int STYLES = ATOM_STYLES | ELEMENT_STYLES 
                        | INTEGRATE_STYLES | PAIR_STYLES 
                        //| BOND_STYLES | ANGLE_STYLES | MINIMIZE_STYLES
    ...
    if (strncmp(arg[idx], "all", 3) == 0) {
      ...
      if (idx+1 < narg) {
        ...
        if (strncmp(arg[idx], "all", 3) == 0) {
          ...
          flags |= ATOM_STYLES;
          ++idx;
        } else if (strncmp(arg[idx], "element", 3) == 0) {
          flags |= ELEMENT_STYLES;
          ++idx;
...
// newer glibc versions have mallinfo2
  ...
  if (flags & COMM) {
    ...
    if (comm->mode == 0) {
      ...
      fprintf(out, "Communication cutoff = %g %g %g\n" 
              ,MAX(comm->cutghostuser[0], neighbor->cutneighmax)
              ,MAX(comm->cutghostuser[1], neighbor->cutneighmax)
              ,MAX(comm->cutghostuser[2], neighbor->cutneighmax)
              );
    }
  ...
  if (flags & SYSTEM) {
    ...
    fprintf(out, "Element style = %s\n", element->element_style);
    fprintf(out, "Atom map   = %s\n", mapstyles[atom->map_style]);
    fprintf(out, "Element map   = %s\n", mapstyles[element->map_style]);
    fprintf(out, "N_Atoms = " BIGINT_FORMAT ", N_Elements = " BIGINT_FORMAT ", chemical types = %d, element types = %d  style = %s\n", 
            atom->natoms, element->nelements, atom->ntypes, element->netypes, force->pair_style);
  ...
  if (flags & TIME) {
    double wallclock = MPI_Wtime() - cac->initclock;
    double cpuclock = 0.0;
...
void Info::available_styles(FILE * out, int flags)
{
  ...
  if(flags & ELEMENT_STYLES)   element_styles(out);
...
void Info::element_styles(FILE * out)
{
  fprintf(out, "\nElement styles:\n");

  vector<string> styles;

  for(Element::ElementVecCreatorMap::iterator it = element->evec_map->begin(); it != element->evec_map->end(); ++it) {
    styles.push_back(it->first);
  }
...
bool Info::is_active(const char *category, const char *name)
{
  ...
  } else if (strcmp(category, "element_style") == 0) {
  ...
  if (!match && cac->suffix_enable) {
    if (cac->suffix) {
      char *name_w_suffix = new char [len + 2 + strlen(cac->suffix)];
      sprintf(name_w_suffix, "%s/%s", name, cac->suffix);
      if (strcmp(style, name_w_suffix) == 0) match = 1;
      delete[] name_w_suffix;
    }
    if (!match && cac->suffix2) {
      char *name_w_suffix = new char [len + 2 + strlen(cac->suffix2)];
      sprintf(name_w_suffix, "%s/%s", name, cac->suffix2);
      if (strcmp(style, name_w_suffix) == 0) match = 1;
      delete[] name_w_suffix;
    }
  }
