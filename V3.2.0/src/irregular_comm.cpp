...
IrregularComm::IrregularComm(CAC *cac) : Pointers(cac)
{
  MPI_Comm_rank(world, &me);
  MPI_Comm_size(world, &nprocs);

  triclinic = domain->triclinic;
  atom_map_style = atom->map_style;
  elem_map_style = element->map_style;
...
/*  ----------------------------------------------------------------------
   communicate atoms/elements to new owning procs via irregular communication
   can be used in place of comm->exchange()
   unlikeh comm->exchange(), allows atoms/elements to have moved arbitrarily long distances
   sets up irregular plan, invokes it, destroys it
   sortflag = flag for sorting order of received messages by proc ID
   preassign = 1 if already know procs that atoms/elements are assigned to via RCB
   procassign = list of proc assignments for each owned atom/element (proc assignment list for atoms is after list for elements, i.e. start at index atom->nlocal)
   atoms/elemets MUST be remapped to be inside simulation box before this is called
   for triclinic: atoms/elements must be in lamda coords (0-1) before this is called
-------------------------------------------------------------------------  */

void IrregularComm::migrate(int sortflag, int preassign, int *procassign)
{

  // clear global->local map since atoms/elements move to new procs
  // clear old ghosts so map_set() at end will operate only on local atoms/elements
  ...
  // extract proc assignments for atoms and elements

  int *atom_procassign;
  int *elem_procassign;
  if (!preassign) comm->coord2proc_setup();
  else {
    atom_procassign = &procassign[element->nlocal];
    elem_procassign = procassign;
  }

  // loop over atoms/elements, flag any that are not in my sub-box
  // fill buffer with atoms/elements leaving my box, using < and >=
  ...
  // when atom/element is deleted, fill it in with last atom/element
  ...
  if (atom->natoms) {
    ...
    // prepare buf and mproclist
    
    if (preassign) {
      while (i < nlocal) {
        if (atom_procassign[i] == me) i++;
        else {
          mproclist[nsendatom] = atom_procassign[i];
          if (nsend > maxsend) grow_send(nsend, 1);
          msizes[nsendatom] = avec->pack_exchange(i, &buf_send[nsend]);
          nsend += msizes[nsendatom];
          nsendatom++;
          avec->copy(nlocal-1, i, 1);
          atom_procassign[i] = atom_procassign[nlocal-1];
          nlocal--;
        }
      }
    } else {
    ...
    int nrecv = create_comm_plan(nsendatom, msizes, mproclist, sortflag);
    if (nrecv > maxrecv) grow_recv(nrecv);
    exchange(buf_send, msizes, buf_recv);
    destroy_comm_plan();

    // unpacked received atoms
  ...
  if (element->nelements) {
    ...
    if (preassign) {
      while (i < nlocal) {
        if (elem_procassign[i] == me) i++;
        else {
          mproclist[nsendelem] = elem_procassign[i];
          if (nsend > maxsend) grow_send(nsend, 1);
          msizes[nsendelem] = evec->pack_exchange(i, &buf_send[nsend]);
          if (msizes[nsendelem] > BUFEXTRA) error->one(FLERR, "Size for one is larger than BUFEXTRA, recompile with larger BUFEXTRA");
          nsend += msizes[nsendelem];
          nsendelem++;
          evec->copy(nlocal-1, i, 1);
          elem_procassign[i] = elem_procassign[nlocal-1];
          nlocal--;
        }
      }
    } else {
      while (i < nlocal) {
        if (x[i][0] < sublo[0] || x[i][0] >= subhi[0] ||
            x[i][1] < sublo[1] || x[i][1] >= subhi[1] ||
            x[i][2] < sublo[2] || x[i][2] >= subhi[2]) {
          mproclist[nsendelem] = comm->coord2proc(x[i], igx, igy, igz);
          if (mproclist[nsendelem] == me) i++;
          else {
            if (nsend > maxsend) grow_send(nsend, 1);
            msizes[nsendelem] = evec->pack_exchange(i, &buf_send[nsend]);
            if (msizes[nsendelem] > BUFEXTRA) error->one(FLERR, "Size for one is larger than BUFEXTRA, recompile with larger BUFEXTRA");
    ...
    // create irregular communication plan, perform exchange comm, destroy plan
    // returned nrecv = size of buffer needed for incoming elements

    int nrecv = create_comm_plan(nsendelem, msizes, mproclist, sortflag);
    if (nrecv > maxrecv) grow_recv(nrecv);
    exchange(buf_send, msizes, buf_recv);
    destroy_comm_plan();
    ...
    int m = 0;
    while (m < nrecv) m += evec->unpack_exchange(&buf_recv[m]);
...
/*  ----------------------------------------------------------------------
    check if any atoms/elements need to migrate further than one proc away in any dim
    if not, caller can decide to use comm->exchange() instead
    should not be called for layout = TILED
    atoms/elements must be remapped to be inside simulation box before this is called
    for triclinic: atoms/elements must be in lamda coords (0-1) before this is called
    return 1 if migrate required, 0 if not
    -------------------------------------------------------------------------  */

int IrregularComm::migrate_check()
{
  ...
  // loop over atoms/elements, check for any that are not in my sub-box
  // assign which proc it belongs to via Comm::coord2proc()
  ...
  for (int k = 0; k < 2; k++) {
    ...
    for (int i = 0; i < nlocal; i++) {
      ...
      if (x[i][0] < sublo[0] || x[i][0] >= subhi[0] ||
        ...
        if (igx != myloc[0] && igx != glo && igx != ghi) {
          flag = 1;
          break;
        }
        ...
        if (igy != myloc[1] && igy != glo && igy != ghi) {
          flag = 1;
          break;
        }
        ...
        if (igz != myloc[2] && igz != glo && igz != ghi) {
          flag = 1;
          break;
        }
      }
...
int IrregularComm::create_comm_plan(int n, int *sizes, int *proclist, int sortflag)
{
  int i;
  ...
  // invoke by adding final arg = 1 to create_comm_plan() call in migrate()
  ...
  // else another proc could proceed to exchange() and send to me

  MPI_Barrier(world);

  // return size of atom/element data I will receive
...
//  int i = *((int *) iptr);
//  int j = *((int *) jptr);
//  int *proc_recv = IrregularComm::proc_recv_copy;
...
void IrregularComm::exchange(double *sendbuf, int *sizes, double *recvbuf)
{
  ...
  // send each message
  // pack buf with list of atoms/elements
  // m = index of atom/element in sendbuf
...
void IrregularComm::destroy_comm_plan()
{
  delete [] proc_send;
  delete [] length_send;
  delete [] num_send;
...
int IrregularComm::create_comm_plan_data(int n, int *proclist, int sortflag)
{
  int i, m;
...
void IrregularComm::exchange_data(char *sendbuf, int nbytes, char *recvbuf)
{
  int i, m, n, offset, count;
...
void IrregularComm::destroy_comm_plan_data()
{
  delete [] proc_send;
  delete [] num_send;
  delete [] index_send;
...
void IrregularComm::grow_send(int n, int flag)
{
...
void IrregularComm::grow_recv(int n)
{
...
bigint IrregularComm::memory_usage()
{
  bigint bytes = 0;
