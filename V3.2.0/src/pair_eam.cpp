...
void PairEAM::dump_rho() 
{
  char file[128];
  int me = comm->me;
  sprintf(file,"dump_edensity_atom." BIGINT_FORMAT ".atom",update->ntimestep);

  bigint nalocal = atom->nlocal;
  bigint natoms;
  MPI_Allreduce(&nalocal, &natoms, 1, MPI_CAC_BIGINT, MPI_SUM, world);
  if (natoms != atom->natoms) {
    if (comm->me == 0) printf("natoms = " BIGINT_FORMAT " atom->natoms = " BIGINT_FORMAT "\n", natoms, atom->natoms);
    error->all(FLERR, "Atom count is inconsistent, cannot write dump file");
  }
  ...
  if (nelements != element->nelements) {
    printf("nelement_count = " BIGINT_FORMAT " nelements = " BIGINT_FORMAT " \n", nelements, element->nelements);
    error->all(FLERR, "Element count is inconsistent, cannot write dump file");
  }
  ...
  if (me == 0) {
    ...
    fprintf(fp, "ITEM: ATOMS id type x y z rho fx fy fz\n");
  }
  ...
  double **buf;
  if (me == 0) memory->create(buf, MAX(1, maxrow), ncol, "pair_eam:buf");
  else memory->create(buf, MAX(1, sendrow), ncol, "pair_eam:buf");
  ...
  if (me == 0) {
    ...
    for (int iproc = 0; iproc < comm->nprocs; iproc++) {
      ...
      for (int i = 0; i < recvrow; i++)
        fprintf(fp, TAGINT_FORMAT " %d %-1.16e %-1.16e %-1.16e %-1.16e %-1.16e %-1.16e %-1.16e\n", 
            static_cast<tagint> (buf[i][0]), static_cast<int> (buf[i][1]), 
