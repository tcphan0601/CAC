    ...
    double *argstack, int &nargstack, int ivar)
{
  ...
  } else if (strcmp(word, "countnodecel") == 0) {
    if (narg == 1) value = group->count_node_cell(igroup);
    else if (narg == 2) value = group->count_node_cell(igroup, region_function(args[1], ivar));
    else print_var_error(FLERR, "Invalid group function in variable formula", ivar);
