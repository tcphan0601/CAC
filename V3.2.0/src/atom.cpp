...
Atom::~Atom()
{
  ...
  memory->destroy(x);
  memory->destroy(x_current);
  memory->destroy(v);
  memory->destroy(f);
  memory->destroy(grain_tag);
...
void Atom::allocate_type_arrays()
{
  ...
  q = new double[ntypes+1];
  q_setflag = new int[ntypes+1];
  for (int itype = 1; itype <= ntypes; itype++) q_setflag[itype] = 0;
...
{
  int m, xptr, iptr, gptr;
  int grain_id;
  imageint imagedata;
  double xdata[3], lamda[3];
  double *coord;
  char *next;
  int size_data_atom = avec->size_data_atom_V2;
  ...
  *next = '\n';
  ...
  if (ngrains) {
    if (nwords != size_data_atom && 
        nwords != size_data_atom + 1 &&
        nwords != size_data_atom + 3 && 
        nwords != size_data_atom + 4)
  ...
  // gptr = which word in line is grain id

  xptr = avec->xcol_data - 1;
  int imageflag = 0;
  int grainflag = 0;
  if (nwords == avec->size_data_atom + 3 || nwords == avec->size_data_atom + 4) imageflag = 1;
  if ((nwords == avec->size_data_atom + 1 || nwords == avec->size_data_atom + 4) && ngrains) grainflag = 1;
  if (imageflag) iptr = nwords - 3;
  if (grainflag) gptr = avec->size_data_atom;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (grainflag) {
      grain_id = atoi(values[gptr]);
      if (grain_id < 0 || grain_id > ngrains) 
        error->one(FLERR, "Invalid atom grain ID in Atoms section of data file");
    } else 
      grain_id = 0;
    ...
    if (coord[0] >= sublo[0] && coord[0] < subhi[0] &&
      ...
      avec->data_atom(xdata, imagedata, grain_id, values);
...
void Atom::data_atoms_reference(int n, char *buf, tagint id_offset, int type_offset, 
    int shiftflag, double *shift, int flag)
{
  int m, xptr;
  imageint imagedata;
  double xdata[3], lamda[3];
  ...
  *next = '\n';
  ...
  if (ngrains) {
    if (nwords != avec->size_data_atom && 
        nwords != avec->size_data_atom + 1 &&
        nwords != avec->size_data_atom + 3 && 
        nwords != avec->size_data_atom + 4)
      error->all(FLERR, "Incorrect atom format in data file");
  } else {
    if (nwords != avec->size_data_atom && 
        nwords != avec->size_data_atom + 3)
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (coord[0] >= sublo[0] && coord[0] < subhi[0] &&
        coord[1] >= sublo[1] && coord[1] < subhi[1] &&
        coord[2] >= sublo[2] && coord[2] < subhi[2]) {
      avec->data_atom(xdata, imagedata, 0, values);
      if (id_offset) tag[nlocal - 1] += id_offset;
      if (type_offset) {
        type[nlocal - 1] += type_offset;
        if (type[nlocal - 1] > ntypes)
...
void Atom::data_atoms_strain(int n, char *buf)
{
  int m, xptr, iptr, xcptr;
  int grain_id;
  imageint imagedata;
  double xdata[3], xcurrent[3], lamda[3];
  double *coord;
  char *next;
  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    xdata[1] = atof(values[xptr+1]);
    xdata[2] = atof(values[xptr+2]);
    xcurrent[0] = atof(values[xcptr]);
    xcurrent[1] = atof(values[xcptr+1]);
    xcurrent[2] = atof(values[xcptr+2]);

    domain->remap(xdata, imagedata);
    domain->unmap_reverse_current(xcurrent, imagedata);
    if (triclinic) {
      domain->x2lamda(xdata, lamda);
      coord = lamda;
    } else coord = xdata;
    if (coord[0] >= sublo[0] && coord[0] < subhi[0] &&
        coord[1] >= sublo[1] && coord[1] < subhi[1] &&
        coord[2] >= sublo[2] && coord[2] < subhi[2]) {
      avec->data_atom_strain(xdata, xcurrent, imagedata, values);
    }
...
void Atom::set_mass(const char *file, int line, int narg, char **arg)
{
  ...
  int lo, hi;
  universe->bounds(file, line, arg[0], ntypes, lo, hi);
...
/*  ----------------------------------------------------------------------
    set a charge and flag it as set
    called from reading of data file
    type_offset may be used when reading multiple data files
    -------------------------------------------------------------------------  */

void Atom::set_charge(const char *file, int line, const char *str, int type_offset)
{
  q_flag = 1;
  int itype;
  double q_one;
  int n = sscanf(str, "%d %lg", &itype, &q_one);
  if (n != 2) error->all(file, line, "Invalid charge line in data file");
  itype += type_offset;

  if (itype < 1 || itype > ntypes)
    error->all(file, line, "Invalid type for charge set");
...
void Atom::set_charge(const char *file, int line, int itype, double value)
{
  q_flag = 1;
  if (itype < 1 || itype > ntypes)
    error->all(file, line, "Invalid type for charge set");
...
/*  ----------------------------------------------------------------------
    set one or more charges and flag them as set
    called from reading of input script
    -------------------------------------------------------------------------  */

void Atom::set_charge(const char *file, int line, int narg, char **arg)
{
  q_flag = 1;
  int lo, hi;
  universe->bounds(file, line, arg[0], ntypes, lo, hi);
  if (lo < 1 || hi > ntypes) error->all(file, line, "Invalid type for charge set");
...
/*  ----------------------------------------------------------------------
    set all charges as read in from restart file
    -------------------------------------------------------------------------  */

void Atom::set_charge(double *values)
{
  q_flag = 1;
  for (int itype = 1; itype <= ntypes; itype++) {
    q[itype] = values[itype];
    q_setflag[itype] = 1;
  }
}
...
/*  ----------------------------------------------------------------------
    check that all per-atom-type charges have been set
    -------------------------------------------------------------------------  */

void Atom::check_charge(const char *file, int line)
{
  if (q_flag) {
    for (int itype = 1; itype <= ntypes; itype++)
      if (q_setflag[itype] == 0)
        error->all(file, line, "Not all per-type charges are set");
  }
}
...
void Atom::setup_sort_bins()
{
  ...
  if (natoms == 0) {
    sortfreq = 0;
    if (comm->me == 0)
      error->warning(FLERR, "No binsize set. "
          "Atom sorting therefore disabled.");
    return;
  }
...
/*  ----------------------------------------------------------------------
    init per-atom fix/compute/dump/variable values for newly created atoms
    called from create_atoms, read_data, read_dump, disc_elements
    fixes, computes, dump, variables may or may not exist when called
    -------------------------------------------------------------------------  */

void Atom::data_fix_compute_dump_variable(int nprev, int nnew)
{
  for (int m = 0; m < modify->nfix; m++) {
    Fix *fix = modify->fix[m];
    if (fix->create_attribute)
      for (int i = nprev; i < nnew; i++)
        fix->set_atom_arrays(i);
  }
  ...
  for (int m = 0; m < output->ndump; m++) {
    ...
    if (dump->create_attribute)
      for (int i = nprev; i < nnew; i++)
        dump->set_atom_arrays(i);
  }

  for (int i = nprev; i < nnew; i++)
    input->variable->set_atom_arrays(i);
}

/*  ----------------------------------------------------------------------
    init per-atom fix/compute/dump/variable values for a newly created atom that is passed on from an old element
    called from fix_adaptive
    fixes, computes, dump, variables may or may not exist when called
    -------------------------------------------------------------------------  */

void Atom::data_pass_on_fix_compute_dump_variable(int i, int i_old, int index)
{
  for (int m = 0; m < modify->nfix; m++) {
    Fix *fix = modify->fix[m];
    if (fix->create_attribute)
      fix->pass_on_atom_arrays(i, i_old, index);
  }

  for (int m = 0; m < modify->ncompute; m++) {
    Compute *compute = modify->compute[m];
    if (compute->create_attribute)
      compute->pass_on_atom_arrays(i, i_old, index);
  }

  for (int m = 0; m < output->ndump; m++) {
    Dump *dump = output->dump[m];
    if (dump->create_attribute)
      dump->pass_on_atom_arrays(i, i_old, index);
  }
  input->variable->pass_on_atom_arrays(i, i_old, index);
}
...
void Atom::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "id") == 0) {
      ...
      if (iarg+2 > narg) error->all(FLERR, "Illegal atom_modify command");
      ngrains = universe->inumeric(FLERR, arg[iarg+1]);
      if (ngrains < 0) 
    ...
    sortfreq = universe->inumeric(FLERR, arg[iarg+1]);
    userbinsize = universe->numeric(FLERR, arg[iarg+2]);
...
void Atom::data_atoms_V2(int n, char *buf, tagint id_offset, int type_offset, 
    int shiftflag, double *shift)
{
  int m, xptr, iptr, gptr;
  int grain_id;
  ...
  *next = '\n';
  ...
  if (ngrains) {
    if (nwords != avec->size_data_atom_V2 && 
        nwords != avec->size_data_atom_V2 + 1 &&
        nwords != avec->size_data_atom_V2 + 3 && 
        nwords != avec->size_data_atom_V2 + 4)
      error->all(FLERR, "Incorrect atom format in data file");
  } else {
    if (nwords != avec->size_data_atom_V2 && 
        nwords != avec->size_data_atom_V2 + 3)
  ...
  xptr = avec->xcol_data_V2 - 1;
  int imageflag = 0;
  int grainflag = 0;
  if (nwords == avec->size_data_atom_V2 + 3 || nwords == avec->size_data_atom_V2 + 4) imageflag = 1;
  if ((nwords == avec->size_data_atom_V2 + 1 || nwords == avec->size_data_atom_V2 + 4) && ngrains) grainflag = 1;
  if (imageflag) iptr = nwords - 3;
  if (grainflag) gptr = avec->size_data_atom_V2;
...
void Atom::data_atoms_reference_V2(int n, char *buf, tagint id_offset, int type_offset, 
    int shiftflag, double *shift)
{
  int m, xptr;
  imageint imagedata;
...
void Atom::data_vels_V2(int n, char *buf, tagint id_offset)
{
  int j, m;
  tagint tagdata;
  char *next;
  ...
  *next = '\n';
  ...
  if (nwords != avec->size_data_vel_V2)
    error->all(FLERR, "Incorrect velocity format in data file");
