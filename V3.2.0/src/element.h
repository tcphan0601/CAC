...
class Element : protected Pointers {
 public: 
  ...
  int nodal_force_style;      // 0 = lumped mass matrix
                              // 1 = consistent mass matrix
                              // 2 = force from nodal gaussian points only (for min style)
  ...
  enum{LUMPED, CONSISTENT, NODEONLY};
  ...
  double nodal_visual_position; // fraction position of visualized node point within the nodal unit cell
  ...
  void setup() {}
  void grow(int);
  void shape_function(double *, double, double, double, int);
  void tag_check();
  void compress_etype();
  ...
  bigint count_vatoms();
  bigint count_ucells();
  int count_nodes(int flag = 1);
  int count_polygons(int flag = 1);
  int count_node_cells(int flag = 1);
  void compute_nodef();
