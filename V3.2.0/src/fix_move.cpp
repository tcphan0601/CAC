...
FixMove::FixMove(CAC *cac, int narg, char **arg) :
  Fix(cac, narg, arg), 
  xvarstr(nullptr), yvarstr(nullptr), zvarstr(nullptr), vxvarstr(nullptr), 
  vyvarstr(nullptr), vzvarstr(nullptr), 
  xoriginal(nullptr), nodexoriginal(nullptr), /*  toriginal(nullptr), qoriginal(nullptr),  */
  displace(nullptr), nodedisplace(nullptr), 
  velocity(nullptr), nodevelocity(nullptr)
{
  ...
  if (strcmp(arg[3], "linear") == 0) {
    ...
    if (strcmp(arg[4], "NULL") == 0) vxflag = 0;
      ...
      vxflag = 1;
      vx = universe->numeric(FLERR, arg[4]);
    }
    if (strcmp(arg[5], "NULL") == 0) vyflag = 0;
    else {
      vyflag = 1;
      vy = universe->numeric(FLERR, arg[5]);
    }
    if (strcmp(arg[6], "NULL") == 0) vzflag = 0;
    else {
      vzflag = 1;
      vz = universe->numeric(FLERR, arg[6]);
    }
    ...
    if (strcmp(arg[4], "NULL") == 0) axflag = 0;
      ...
      axflag = 1;
      ax = universe->numeric(FLERR, arg[4]);
    }
    if (strcmp(arg[5], "NULL") == 0) ayflag = 0;
    else {
      ayflag = 1;
      ay = universe->numeric(FLERR, arg[5]);
    }
    if (strcmp(arg[6], "NULL") == 0) azflag = 0;
    else {
      azflag = 1;
      az = universe->numeric(FLERR, arg[6]);
    }
    period = universe->numeric(FLERR, arg[7]);
    ...
    iarg = 11;
    mstyle = ROTATE;
    point[0] = universe->numeric(FLERR, arg[4]);
    point[1] = universe->numeric(FLERR, arg[5]);
    point[2] = universe->numeric(FLERR, arg[6]);
    axis[0] = universe->numeric(FLERR, arg[7]);
    axis[1] = universe->numeric(FLERR, arg[8]);
    axis[2] = universe->numeric(FLERR, arg[9]);
    period = universe->numeric(FLERR, arg[10]);
    if (period <= 0.0) error->all(FLERR, "Illegal fix move command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "units") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "egroup/style") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix move command");
      if (strcmp(arg[iarg+1], "element") == 0) element_group_style = ELEMENT;
      else if (strcmp(arg[iarg+1], "node") == 0) element_group_style = NODE;
      else error->all(FLERR, "Illegal fix move command");
      iarg += 2;
    } else error->all(FLERR, "Illegal fix move command");
  }
  ...
  for (int i = 0; i < nelocal; i++) {
    if (element_group_style == ELEMENT) {
      if (emask[i] & groupbit) {
...
void FixMove::init()
{
  ...
  memory->destroy(nodevelocity);
  if (displaceflag) memory->create(nodedisplace, maxelem, maxapc, 8, 3, "move:nodedisplace");
  else nodedisplace = nullptr;
  if (velocityflag) memory->create(nodevelocity, maxelem, maxapc, 8, 3, "move:nodevelocity");
  else nodevelocity = nullptr;
...
void FixMove::initial_integrate(int /* vflag */)
{
  ...
  if (mstyle == LINEAR) {
      ...
      for (int j = 0; j < apc[etype[i]]; j++)
        for (int k = 0; k < npe[etype[i]]; k++) {
          if (element_group_style == ELEMENT) {
            ...
            if (!(nodemask[i][j][k] & groupbit)) continue;
          }

          xold[0] = nodex[i][j][k][0];
          xold[1] = nodex[i][j][k][1];
          xold[2] = nodex[i][j][k][2];

          if (vxflag) {
            nodev[i][j][k][0] = vx;
            nodex[i][j][k][0] = nodexoriginal[i][j][k][0] + vx * delta;
          } else {
          ...
          if (vyflag) {
            nodev[i][j][k][1] = vy;
            nodex[i][j][k][1] = nodexoriginal[i][j][k][1] + vy * delta;
          } else {
            dtfm = dtf / mass[ctype[i][j]];
          ...
          if (vzflag) {
            nodev[i][j][k][2] = vz;
            nodex[i][j][k][2] = nodexoriginal[i][j][k][2] + vz * delta;
          } else {
            dtfm = dtf / mass[ctype[i][j]];
          ...
          domain->remap_near(nodex[i][j][k], xold);
        }
    }
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < apc[etype[i]]; j++)
        for (int k = 0; k < npe[etype[i]]; k++) {
          ...
          if (axflag) {
            nodev[i][j][k][0] = ax * omega_rotate * cosine;
            nodex[i][j][k][0] = nodexoriginal[i][j][k][0] + ax * sine;
          } else {
            dtfm = dtf / mass[ctype[i][j]];
            nodev[i][j][k][0] += dtfm * nodef[i][j][k][0];
            nodex[i][j][k][0] += dtv * nodev[i][j][k][0];
          ...
          if (ayflag) {
            nodev[i][j][k][1] = ay * omega_rotate * cosine;
            nodex[i][j][k][1] = nodexoriginal[i][j][k][1] + ay * sine;
          } else {
            dtfm = dtf / mass[ctype[i][j]];
            nodev[i][j][k][1] += dtfm * nodef[i][j][k][1];
            nodex[i][j][k][1] += dtv * nodev[i][j][k][1];
          ...
          if (azflag) {
            nodev[i][j][k][2] = az * omega_rotate * cosine;
            nodex[i][j][k][2] = nodexoriginal[i][j][k][2] + az * sine;
          } else {
            dtfm = dtf / mass[ctype[i][j]];
            nodev[i][j][k][2] += dtfm * nodef[i][j][k][2];
            nodex[i][j][k][2] += dtv * nodev[i][j][k][2];
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < apc[etype[i]]; j++)
        for (int k = 0; k < npe[etype[i]]; k++) {
          ...
          d[0] = nodexoriginal[i][j][k][0] - point[0];
          d[1] = nodexoriginal[i][j][k][1] - point[1];
          d[2] = nodexoriginal[i][j][k][2] - point[2];
          ddotr = d[0] * runit[0] + d[1] * runit[1] + d[2] * runit[2];
          c[0] = ddotr * runit[0];
          c[1] = ddotr * runit[1];
          c[2] = ddotr * runit[2];
          ...
          nodex[i][j][k][0] = point[0] + c[0] + disp[0];
          nodex[i][j][k][1] = point[1] + c[1] + disp[1];
          nodex[i][j][k][2] = point[2] + c[2] + disp[2];
          nodev[i][j][k][0] = omega_rotate * (runit[1] * disp[2] - runit[2] * disp[1]);
          nodev[i][j][k][1] = omega_rotate * (runit[2] * disp[0] - runit[0] * disp[2]);
          nodev[i][j][k][2] = omega_rotate * (runit[0] * disp[1] - runit[1] * disp[0]);
          domain->remap_near(nodex[i][j][k], xold);
        }
    }
    ...
    if ((displaceflag || velocityflag)) {
      ...
      if (element->nmax > maxelem) {
        maxelem = element->nmax;
        int maxapc = element->maxapc;
        if (displaceflag) {
          memory->destroy(nodedisplace);
          memory->create(nodedisplace, maxelem, maxapc, 8, 3, "move:nodedisplace");
        }
        if (velocityflag) {
          memory->destroy(nodevelocity);
          memory->create(nodevelocity, maxelem, maxapc, 8, 3, "move:nodevelocity");
        }
      }
    }
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < apc[etype[i]]; j++)
        for (int k = 0; k < npe[etype[i]]; k++) {
          ...
          if (xvarstr && vxvarstr) {
            if (vxvarstyle == EQUAL) nodev[i][j][k][0] = vx;
            else nodev[i][j][k][0] = nodevelocity[i][j][k][0];
            if (xvarstyle == EQUAL) nodex[i][j][k][0] = nodexoriginal[i][j][k][0] + dx;
            else nodex[i][j][k][0] = nodexoriginal[i][j][k][0] + nodedisplace[i][j][k][0];
          } else if (xvarstr) {
            if (xvarstyle == EQUAL) nodex[i][j][k][0] = nodexoriginal[i][j][k][0] + dx;
            else nodex[i][j][k][0] = nodexoriginal[i][j][k][0] + nodedisplace[i][j][k][0];
          } else if (vxvarstr) {
          ...
          if (yvarstr && vyvarstr) {
            if (vyvarstyle == EQUAL) nodev[i][j][k][1] = vy;
            else nodev[i][j][k][1] = nodevelocity[i][j][k][1];
            if (yvarstyle == EQUAL) nodex[i][j][k][1] = nodexoriginal[i][j][k][1] + dy;
            else nodex[i][j][k][1] = nodexoriginal[i][j][k][1] + nodedisplace[i][j][k][1];
          } else if (yvarstr) {
            if (yvarstyle == EQUAL) nodex[i][j][k][1] = nodexoriginal[i][j][k][1] + dy;
            else nodex[i][j][k][1] = nodexoriginal[i][j][k][1] + nodedisplace[i][j][k][1];
          } else if (vyvarstr) {
          ...
          if (zvarstr && vzvarstr) {
            if (vzvarstyle == EQUAL) nodev[i][j][k][2] = vz;
            else nodev[i][j][k][2] = nodevelocity[i][j][k][2];
            if (zvarstyle == EQUAL) nodex[i][j][k][2] = nodexoriginal[i][j][k][2] + dz;
            else nodex[i][j][k][2] = nodexoriginal[i][j][k][2] + nodedisplace[i][j][k][2];
          } else if (zvarstr) {
            if (zvarstyle == EQUAL) nodex[i][j][k][2] = nodexoriginal[i][j][k][2] + dz;
            else nodex[i][j][k][2] = nodexoriginal[i][j][k][2] + nodedisplace[i][j][k][2];
          } else if (vzvarstr) {
...
double FixMove::memory_usage()
{
  double bytes = (atom->nmax * 3 * sizeof(double) 
      + element->nmax * element->maxapc * 8 * 3 * sizeof(double))
    *(1 + velocityflag + displaceflag);
...
void FixMove::grow_atom_arrays(int nmax)
{
...
void FixMove::grow_elem_arrays(int nmax)
{
  memory->grow(nodexoriginal, nmax, element->maxapc, 8, 3, "move:nodexoriginal");
  array_node = nodexoriginal;
}
...
void FixMove::copy_atom_arrays(int i, int j, int /* delflag */)
{
  ...
  //if (quat_flag) {
  //  qoriginal[j][k][0] = qoriginal[i][0];
  //  qoriginal[j][k][1] = qoriginal[i][1];
  //  qoriginal[j][k][2] = qoriginal[i][2];
  //  qoriginal[j][3] = qoriginal[i][3];
  //}
}

/*  ----------------------------------------------------------------------
    copy values within local element-based array
    -------------------------------------------------------------------------  */

void FixMove::copy_elem_arrays(int i, int j, int /* delflag */)
{
  for (int k = 0; k < element->maxapc; k++)
    for (int l = 0; l < 8; l++) {
      nodexoriginal[j][k][l][0] = nodexoriginal[i][k][l][0];
      nodexoriginal[j][k][l][1] = nodexoriginal[i][k][l][1];
      nodexoriginal[j][k][l][2] = nodexoriginal[i][k][l][2];
    }
}
...
void FixMove::set_atom_arrays(int i)
{
  //double theta;
  //double *quat;
...
/*  ----------------------------------------------------------------------
    initialize one element's array values, called when element is created
    -------------------------------------------------------------------------  */

void FixMove::set_elem_arrays(int i)
{
  double ****nodex = element->nodex;
  ...
  for (int j = 0; j < apc[etype[i]]; j++) 
    for (int k = 0; k < npe[etype[i]]; k++) {
      ...
      if (update->ntimestep == time_origin) {
        domain->unmap(nodex[i][j][k], image[i], nodexoriginal[i][j][k]);
        continue;
      }
      ...
      if (mstyle == LINEAR) {
        if (vxflag) nodexoriginal[i][j][k][0] -= vx * delta;
        if (vyflag) nodexoriginal[i][j][k][1] -= vy * delta;
        if (vzflag) nodexoriginal[i][j][k][2] -= vz * delta;
      } else if (mstyle == WIGGLE) {
        double arg = omega_rotate * delta;
        double sine = sin(arg);
        if (axflag) nodexoriginal[i][j][k][0] -= ax * sine;
        if (ayflag) nodexoriginal[i][j][k][1] -= ay * sine;
        if (azflag) nodexoriginal[i][j][k][2] -= az * sine;
      } else if (mstyle == ROTATE) {
        double a[3], b[3], c[3], d[3], disp[3], ddotr;
        double arg = - omega_rotate * delta;
        double sine = sin(arg);
        double cosine = cos(arg);
        d[0] = nodex[i][j][k][0] - point[0];
        d[1] = nodex[i][j][k][1] - point[1];
        d[2] = nodex[i][j][k][2] - point[2];
        ddotr = d[0] * runit[0] + d[1] * runit[1] + d[2] * runit[2];
        c[0] = ddotr * runit[0];
        c[1] = ddotr * runit[1];
        c[2] = ddotr * runit[2];
        ...
        nodexoriginal[i][j][k][0] = point[0] + c[0] + disp[0];
        nodexoriginal[i][j][k][1] = point[1] + c[1] + disp[1];
        nodexoriginal[i][j][k][2] = point[2] + c[2] + disp[2];
      }
    }
}
...
int FixMove::pack_atom_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
...
int FixMove::unpack_atom_exchange(int nlocal, double *buf)
{
  int n = 0;
...
/*  ----------------------------------------------------------------------
    pack values in local element-based array for exchange with another proc
    -------------------------------------------------------------------------  */

int FixMove::pack_elem_exchange(int i, double *buf)
{
  int n = 0;
  for (int j = 0; j < element->apc[element->etype[i]]; j++) 
    for (int k = 0; k < element->npe[element->etype[i]]; k++) {
      buf[n++] = nodexoriginal[i][j][k][0];
      buf[n++] = nodexoriginal[i][j][k][1];
      buf[n++] = nodexoriginal[i][j][k][2];
    }
  return n;
}

/*  ----------------------------------------------------------------------
    unpack values in local element-based array from exchange with another proc
    -------------------------------------------------------------------------  */

int FixMove::unpack_elem_exchange(int nlocal, double *buf)
{
  int n = 0;
  for (int j = 0; j < element->apc[element->etype[nlocal]]; j++) 
    for (int k = 0; k < element->npe[element->etype[nlocal]]; k++) {
      nodexoriginal[nlocal][j][k][0] = buf[n++];
      nodexoriginal[nlocal][j][k][1] = buf[n++];
      nodexoriginal[nlocal][j][k][2] = buf[n++];
    }
  return n;
}
