...
CommBrick::CommBrick(CAC *cac) : 
  Comm(cac), 
  atom_sendnum(nullptr), atom_recvnum(nullptr), elem_sendnum(nullptr), elem_recvnum(nullptr), 
  sendproc(nullptr), recvproc(nullptr), 
  atom_size_forward_recv(nullptr), elem_size_forward_recv(nullptr), 
  atom_size_reverse_send(nullptr), elem_size_reverse_send(nullptr), 
  atom_size_reverse_recv(nullptr), elem_size_reverse_recv(nullptr), 
  slablo(nullptr), slabhi(nullptr), multilo(nullptr), multihi(nullptr), 
  cutghostmulti(nullptr), pbc_flag(nullptr), pbc(nullptr), 
  atom_firstrecv(nullptr), elem_firstrecv(nullptr), 
  atom_sendlist(nullptr), atom_maxsendlist(nullptr), 
  elem_sendlist(nullptr), elem_maxsendlist(nullptr), 
...
void CommBrick::setup_borders()
{
  ...
  //   add ghostskin from element in necessary
  // for triclinic:
  ...
  double cut[3];
  cut[0] = MAX(neighbor->cutneighmax, cutghostuser[0]);
  cut[1] = MAX(neighbor->cutneighmax, cutghostuser[1]);
  cut[2] = MAX(neighbor->cutneighmax, cutghostuser[2]);
  ...
  for (i = 0; i < modify->ncompute; i++)
    if (modify->compute[i]->ghostskinflag) {
      cut[0] = MAX(modify->compute[i]->ghostskin, cut[0]);
      cut[1] = MAX(modify->compute[i]->ghostskin, cut[1]);
      cut[2] = MAX(modify->compute[i]->ghostskin, cut[2]);
    }

  // additional skin from element size
  // update element and bound box and max bound box size
  ...
  if (triclinic == 0) {
    ...
    subhi = domain->subhi;
    prd = domain->prd;
    cutghost[0] = cutghost[3] = cut[0];
    cutghost[1] = cutghost[4] = cut[1];
    cutghost[2] = cutghost[5] = cut[2];
    double extra_cut;
    for (int dim = 0; dim < domain->dimension; dim++) {
      extra_cut = sublo[dim] - element->local_element_bound_box[dim];
      if (extra_cut > 0.0) cutghost[dim] += extra_cut;
      extra_cut = element->local_element_bound_box[dim+3] - subhi[dim];
      if (extra_cut > 0.0) cutghost[dim+3] += extra_cut;
    }
    ...
    cutghost[0] = cutghost[3] = cut[0] * length0;
    cutghost[1] = cutghost[4] = cut[1] * length1;
    cutghost[2] = cutghost[5] = cut[2] * length2;
    ...
    if (mode == Comm::MULTI) {
      ...
      //     cutghostmulti[i][0] = length1 * MAX(cut, cuttype[i]);
      //     cutghostmulti[i][2] = length2 * MAX(cut, cuttype[i]);
      //   }
    }
  }

  // check if domain size is larger than element bound box
  ...
  // recvneed[idim][0/1] = # of procs away I may recv atoms from, within max cutghost
  ...
  int left, right;
  double max_cutghost_local[3];
  max_cutghost_local[0] = MAX(cutghost[0], cutghost[3]);
  max_cutghost_local[1] = MAX(cutghost[1], cutghost[4]);
  max_cutghost_local[2] = MAX(cutghost[2], cutghost[5]);
  MPI_Allreduce(max_cutghost_local, max_cutghost, 3, MPI_DOUBLE, MPI_MAX, world);

  if (layout == Comm::LAYOUT_UNIFORM) {
    maxneed[0] = static_cast<int> (max_cutghost[0] * procgrid[0] / prd[0]) + 1;
    maxneed[1] = static_cast<int> (max_cutghost[1] * procgrid[1] / prd[1]) + 1;
    maxneed[2] = static_cast<int> (max_cutghost[2] * procgrid[2] / prd[2]) + 1;
  ...
  // send/recv cutghost to/from neighbor procs
  // just copy cutghost if send/recv to/from me
  // for mode SINGLE:
  ...
  for (dim = 0; dim < 3; dim++) {
    for (ineed = 0; ineed < 2 * maxneed[dim]; ineed++) {
      ...
      if (ineed % 2 == 0) {
        ...
        if (mode == Comm::SINGLE) {
          if (me == sendproc[iswap]) 
            copy3(&cutghost[3], neigh_cutghost);
          else {
            MPI_Send(&cutghost[3], 3, MPI_DOUBLE, recvproc[iswap], 0, world);
            MPI_Recv(neigh_cutghost, 3, MPI_DOUBLE, sendproc[iswap], 0, world, MPI_STATUS_IGNORE);
          }

          if (ineed < 2) slablo[iswap] = -BIG;
          else slablo[iswap] = 0.5 * (sublo[dim] + subhi[dim]);
          slabhi[iswap] = sublo[dim] + neigh_cutghost[dim];
        } else {
          for (i = 1; i <= ntypes; i++) {
        ...
        if (mode == Comm::SINGLE) {
          if (me == sendproc[iswap]) 
            copy3(cutghost, neigh_cutghost);
          else {
            MPI_Send(cutghost, 3, MPI_DOUBLE, recvproc[iswap], 0, world);
            MPI_Recv(neigh_cutghost, 3, MPI_DOUBLE, sendproc[iswap], 0, world, MPI_STATUS_IGNORE);
          }
          slablo[iswap] = subhi[dim] - neigh_cutghost[dim];
  ...
  if (atom->natoms) {
    ...
    for (dim = 0; dim < 3; dim++) {
      ...
      for (ineed = 0; ineed < twoneed; ineed++) {
        ...
        // find send atoms according to SINGLE vs Comm::MULTI
  ...
  if (element->nelements) {
    ...
    for (dim = 0; dim < 3; dim++) {
      ...
      for (ineed = 0; ineed < twoneed; ineed++) {
        ...
        //   for first swaps in a dim, check owned and ghost
        //   for later swaps in a dim, only check newly arrived ghosts
        // store sent element indices in sendlist for use in future timesteps

        element_bound_box = element->element_bound_box;
        x = element->x;
        nlocal = element->nlocal;
        ...
        // find send elements according to SINGLE vs Comm::MULTI
        ...
        if (sendflag) {
          if (!bordergroup || ineed >= 2) {
            if (mode == Comm::SINGLE) {
              for (i = nfirst; i < nlast; i++) {
                if (element_bound_box[i][dim] > hi || element_bound_box[i][dim+3] < lo) 
                  continue;
                if ((ineed % 2 == 0 && x[i][dim] < lo) ||
                    (ineed % 2 == 1 && x[i][dim] > hi))
                  continue;
              ...
              //for (i = nfirst; i < nlast; i++) {
              //  itype = type[i];
              //  if (element_bound_box[i][dim] > mhi[itype] || element_bound_box[i][dim] < mlo[itype]) 
              //    continue;
              //  if ((ineed % 2 == 0 && x[i][dim] < mlo[itype]) ||
              //      (ineed % 2 == 1 && x[i][dim] > mhi[itype]))
              //    continue;
              //  if (nsend == elem_maxsendlist[iswap]) grow_elem_list(iswap, nsend);
              //  elem_sendlist[iswap][nsend++] = i;
              //}
...
void CommBrick::forward_comm_atom_array(int nsize, double **array)
{
  ...
  if (nsize > atom_maxforward) {
    ...
    if (atom_maxforward * smax > maxsend) grow_send(atom_maxforward * smax, 0);
    if (atom_maxforward * rmax > maxrecv) grow_recv(atom_maxforward * rmax);
  }
  ...
  for (iswap = 0; iswap < nswap; iswap++) {
    ...
    m = 0;
    for (i = 0; i < atom_sendnum[iswap]; i++) {
      j = atom_sendlist[iswap][i];
      for (k = 0; k < nsize; k++)
    ...
    if (sendproc[iswap] != me) {
      if (atom_recvnum[iswap])
        MPI_Irecv(buf_recv, nsize * atom_recvnum[iswap], MPI_DOUBLE, recvproc[iswap], 0, 
            world, &request);
      if (atom_sendnum[iswap])
        MPI_Send(buf_send, nsize * atom_sendnum[iswap], MPI_DOUBLE, 
    ...
    m = 0;
    last = atom_firstrecv[iswap] + atom_recvnum[iswap];
    for (i = atom_firstrecv[iswap]; i < last; i++)
      for (k = 0; k < nsize; k++)
        array[i][k] = buf[m++];
  }
...
/*  ----------------------------------------------------------------------
   forward communication of N values in per-elem array
   -------------------------------------------------------------------------  */

void CommBrick::forward_comm_elem_array(int nsize, double **array)
{
  ...
  if (nsize > elem_maxforward) {
    elem_maxforward = nsize;
    if (elem_maxforward * smax > maxsend) grow_send(elem_maxforward * smax, 0);
    if (elem_maxforward * rmax > maxrecv) grow_recv(elem_maxforward * rmax);
  }
  ...
  for (iswap = 0; iswap < nswap; iswap++) {
    ...
    m = 0;
    for (i = 0; i < elem_sendnum[iswap]; i++) {
      j = elem_sendlist[iswap][i];
      for (k = 0; k < nsize; k++)
        buf_send[m++] = array[j][k];
    }
    ...
    if (sendproc[iswap] != me) {
      if (elem_recvnum[iswap])
        MPI_Irecv(buf_recv, nsize * elem_recvnum[iswap], MPI_DOUBLE, recvproc[iswap], 0, 
            world, &request);
      if (elem_sendnum[iswap])
        MPI_Send(buf_send, nsize * elem_sendnum[iswap], MPI_DOUBLE, 
            sendproc[iswap], 0, world);
      if (elem_recvnum[iswap]) MPI_Wait(&request, MPI_STATUS_IGNORE);
      buf = buf_recv;
    } else buf = buf_send;
    ...
    m = 0;
    last = elem_firstrecv[iswap] + elem_recvnum[iswap];
    for (i = elem_firstrecv[iswap]; i < last; i++)
      for (k = 0; k < nsize; k++)
        array[i][k] = buf[m++];
  }
