...
Dump::Dump(CAC *cac, int narg, char **arg) : Pointers(cac)
{
  ...
  format_atom = nullptr;
  format_node = nullptr;
  format_stress = nullptr;
  format_force = nullptr;
  format_velocity = nullptr;
  format_displace = nullptr;
  format_pe = nullptr;
  format_owner_proc = nullptr;
  format_element = nullptr;
  format_user = nullptr;
    ...
    if (buffer_flag == 0 || binary) {
      if (filewriter) {
        for (int iproc = 0; iproc < nclusterprocs; iproc++) {
          ...
          write_atom_data(nlines, atom_size_one, buf);
        }
    ...
    if (buffer_flag == 0 || binary) {
      if (filewriter) {
        for (int iproc = 0; iproc < nclusterprocs; iproc++) {
          ...
          write_node_data(nlines, node_size_one, buf);
        }
        if (flush_flag && fp) fflush(fp);
    ...
    if (buffer_flag == 0 || binary) {
      if (filewriter) {
        for (int iproc = 0; iproc < nclusterprocs; iproc++) {
          ...
          write_elem_data(nlines, elem_size_one, buf);
        }
        if (flush_flag && fp) fflush(fp);
