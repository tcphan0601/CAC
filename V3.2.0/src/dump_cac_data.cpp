...
DumpCACData::DumpCACData(CAC *cac, int narg, char **arg) : Dump(cac, narg, arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "reference") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal dump cac/data command");
      ...
      else error->all(FLERR, "Illegal dump cac/data command");
      if (reference_flag == DATAFILE) {
        if (iarg+3 > narg) error->all(FLERR, "Illegal dump cac/data command");
        if (comm->me == 0) {
  ...
  if (reference_flag) {
    ...
    // store configuration from external data file as reference configuration
      ...
      if (element->nlocal) memory->create(node_read_flag, element->nlocal, 
          element->maxapc, element->maxnpe, "dump:atom_read_flag");
      memset(&atom_read_flag[0], 0, atom->nlocal * sizeof(int));
      if (element->nlocal) memset(&node_read_flag[0][0][0], 0, 
      ...
      if (element->nlocal) memory->destroy(node_read_flag);
    }
  }
}
...
void DumpCACData::write_atom_data(int n, int size_one, double *mybuf)
{
...
void DumpCACData::write_elem_data(int n, int size_one, double *mybuf)
{
...
void DumpCACData::write_node_data(int n, int size_one, double *mybuf)
{
...
void DumpCACData::write_string(int n, int size_one, double *mybuf)
{
...
void DumpCACData::write_atom_lines_noimage(int n, int size_one, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCACData::write_elem_lines_noimage(int n, int size_one, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCACData::write_node_lines_noimage(int n, int size_one, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCACData::write_atom_lines_reference(int n, int size_one, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp, format_atom, 
...
void DumpCACData::write_elem_lines_reference(int n, int size_one, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp, format_elem, 
...
void DumpCACData::write_node_lines_reference(int n, int size_one, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp, format_node, 
...
void DumpCACData::header()
{
  ...
  while (1) {
    ...
    if (me == 0) {
      if (fgets(line, MAXLINE, ref_fp) == nullptr) n = 0;
...
void DumpCACData::parse_keyword(int first)
{
  ...
  if (me == 0) {
    if (!first) {
      if (fgets(line, MAXLINE, ref_fp) == nullptr) eof = 1;
    }
    while (eof == 0 && done == 0) {
    ...
    if (fgets(buffer, MAXLINE, ref_fp) == nullptr) eof = 1;
  }
