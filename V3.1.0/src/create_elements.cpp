...
void CreateElements::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    //  iarg += 2;
    } else if (strcmp(arg[iarg], "latoffset") == 0) {
      if (iarg+4 > narg) error->all(FLERR, "Illegal create_elements command");

      if (strcmp(arg[iarg+1], "x") == 0) offsetplane = 0;
      else if (strcmp(arg[iarg+1], "y") == 0) offsetplane = 1;
      else if (strcmp(arg[iarg+1], "z") == 0) offsetplane = 2;
      else error->all(FLERR, "Illegal create_elements command");

      offset1 = universe->inumeric(FLERR, arg[iarg+2]);
      offset2 = universe->inumeric(FLERR, arg[iarg+3]);
      if (offset1 > 0) offsetdir1 = 1;
      else if (offset1 < 0) {
        offsetdir1 = -1;
        offset1 = -offset1;
      }
      if (offset2 > 0) offsetdir2 = 1;
      else if (offset2 < 0) {
        offsetdir2 = -1;
        offset2 = -offset2;
      }
  ...
  element->nucells += nelements_added * element->nucell[netype];
  element->nnodes += nelements_added * element->npe[netype] * element->apc[netype];
...
void CreateElements::add_lattice()
{
  ...
  ilo = static_cast<int> (xmin) - nx * 2;
  jlo = static_cast<int> (ymin) - ny * 2;
  klo = static_cast<int> (zmin) - nz * 2;
  ihi = static_cast<int> (xmax) + nx * 2;
  jhi = static_cast<int> (ymax) + ny * 2;
  khi = static_cast<int> (zmax) + nz * 2;
  ...
  for (k = klo; k <= khi; k += nz) {
    for (j = jlo; j <= jhi; j += ny) {
      for (i = ilo; i <= ihi; i += nx) {
        ...
        if (offsetplane == 0) {
          if (offset1) x[1] += offsetdir1 * ((int) (((i/nx)%offset1) * ((double)ny/offset1)) 
            + (((i/nx)%offset1)>0) - (((i/nx)%offset1)<0));
          if (offset2) x[2] += offsetdir2 * ((int) (((i/nx)%offset2) * ((double)nz/offset2)) 
            + (((i/nx)%offset2)>0) - (((i/nx)%offset2)<0));
        } else if (offsetplane == 1) {
          if (offset1) x[0] += offsetdir1 * ((int) (((j/ny)%offset1) * ((double)nx/offset1)) 
            + (((j/ny)%offset1)>0) - (((j/ny)%offset1)<0));
          if (offset2) x[2] += offsetdir2 * ((int) (((j/ny)%offset2) * ((double)nz/offset2)) 
            + (((j/ny)%offset2)>0) - (((j/ny)%offset2)<0));
        } else if (offsetplane == 2) {
          if (offset1) x[0] += offsetdir1 * ((int) (((k/nz)%offset1) * ((double)nx/offset1)) 
            + (((k/nz)%offset1)>0) - (((k/nz)%offset1)<0));
          if (offset2) x[1] += offsetdir2 * ((int) (((k/nz)%offset2) * ((double)ny/offset2)) 
            + (((k/nz)%offset2)>0) - (((k/nz)%offset2)<0));
//          printf("i, j, k = %d %d %d x = %d\n", i, j, k, offsetdir2 * ((int) (((k/nz)%offset2) * ((double)ny/offset2)) 
//            + (((k/nz)%offset2)>0) - (((k/nz)%offset2)<0)));
        }
        ...
        if (x[0] < sublo[0] || x[0] >= subhi[0] ||
            ...
            x[2] < sublo[2] || x[2] >= subhi[2]) continue;

        // add the element to my list of elements

//printf("x = %g %g %g ijk = %d %d %d\n",x[0],x[1],x[2],i,j,k);
        element->evec->create_element(coord, netype, basisctype, 0);
      }
    }
  }
}
