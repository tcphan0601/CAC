...
ComputeDisplaceAtom::ComputeDisplaceAtom(CAC *cac, int narg, char **arg) :
  ...
  atomdisplace(nullptr), nodedisplace(nullptr), id_fix(nullptr)
{
...
void ComputeDisplaceAtom::compute_peratom()
{
  ...
  if (element->nmax > nemax) {
    memory->destroy(nodedisplace);
    nemax = element->nmax;
    memory->create(nodedisplace, nemax, element->maxapc, element->maxnpe, 4, "displace/atom:nodedisplace");
  ...
  if (domain->triclinic == 0) {
    ...
    for (int i = 0; i < nelocal; i++)
      if (emask[i] & groupbit) {
        ...
        for (int j = 0; j < apc[etype[i]]; j++) 
          for (int k = 0; k < npe[etype[i]]; k++) {
            dx = nodex[i][j][k][0] + xbox * xprd - nodexoriginal[i][j][k][0];
            dy = nodex[i][j][k][1] + ybox * yprd - nodexoriginal[i][j][k][1];
            dz = nodex[i][j][k][2] + zbox * zprd - nodexoriginal[i][j][k][2];
            nodedisplace[i][j][k][0] = dx;
            nodedisplace[i][j][k][1] = dy;
            nodedisplace[i][j][k][2] = dz;
            nodedisplace[i][j][k][3] = sqrt(dx * dx + dy * dy + dz * dz);
          }
      } else 
        for (int j = 0; j < apc[etype[i]]; j++) 
          for (int k = 0; k < npe[etype[i]]; k++) 
            nodedisplace[i][j][k][0] = nodedisplace[i][j][k][1] =
              nodedisplace[i][j][k][2] = nodedisplace[i][j][k][3] = 0.0;
    ...
    for (int i = 0; i < nelocal; i++) 
      if (emask[i] & groupbit) {
        ...
        for (int j = 0; j < apc[etype[i]]; j++) 
          for (int k = 0; k < npe[etype[i]]; k++) {
            dx = nodex[i][j][k][0] + h[0] * xbox + h[5] * ybox + h[4] * zbox - nodexoriginal[i][j][k][0];
            dy = nodex[i][j][k][1] + h[1] * ybox + h[3] * zbox - nodexoriginal[i][j][k][1];
            dz = nodex[i][j][k][2] + h[2] * zbox - nodexoriginal[i][j][k][2];
            nodedisplace[i][j][k][0] = dx;
            nodedisplace[i][j][k][1] = dy;
            nodedisplace[i][j][k][2] = dz;
            nodedisplace[i][j][k][3] = sqrt(dx * dx + dy * dy + dz * dz);
...
void ComputeDisplaceAtom::pass_on_atom_arrays(int i, int i_old, int iindex)
{
  double **xoriginal = fix->astore;
  double ****nodexoriginal = fix->a4store;
...
void ComputeDisplaceAtom::pass_on_elem_arrays(int i, int i_old, int *iindexlist)
{
  double ****nodexoriginal = fix->a4store;
  int *etype = element->etype;
...
{
  double bytes = (namax + nemax * element->maxapc * element->maxnpe) * 4 * sizeof(double);
