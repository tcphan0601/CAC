...
Lattice::Lattice(CAC *cac, int narg, char **arg) : Pointers(cac)
{
  nbasis = 0;
  basis = nullptr;

  orientx = new double[3];
  orienty = new double[3];
  orientz = new double[3];
  ...
  else if (strcmp(arg[0], "cubic/diamond") == 0) style = DIAMOND;
  ...
  else if (strcmp(arg[0], "pri/fcc") == 0) style = PRIFCC;
  else if (strcmp(arg[0], "pri/hcp") == 0) style = PRIHCP;
  else if (strcmp(arg[0], "pri/bcc") == 0) style = PRIBCC;
  else if (strcmp(arg[0], "pri/cubic/diamond") == 0) style = PRIDIAMOND;

  // O/T interstitial sites
  
  else if (strcmp(arg[0], "bcc/t") == 0) style = BCCT;
  else if (strcmp(arg[0], "bcc/o") == 0) style = BCCO;
  else if (strcmp(arg[0], "pri/bcc/t") == 0) style = PRIBCCT;
  else if (strcmp(arg[0], "pri/bcc/o") == 0) style = PRIBCCO;
  else if (strcmp(arg[0], "fcc/t") == 0) style = FCCT;
  else if (strcmp(arg[0], "fcc/o") == 0) style = FCCO;
  else if (strcmp(arg[0], "pri/fcc/t") == 0) style = PRIFCCT;
  else if (strcmp(arg[0], "pri/fcc/o") == 0) style = PRIFCCO;
  ...
  if (style == NONE) {
    ...
    xlattice = ylattice = zlattice = universe->numeric(FLERR, arg[1]);
  ...
  if (dimension == 2) {
    if (style == SC || style == BCC || style == FCC || style == HCP || style == DIAMOND ||
        style == PRIFCC || style == PRIBCC || style == PRIDIAMOND)
  ...
  if (narg < 2) error->all(FLERR, "Illegal lattice command");
  scale = universe->numeric(FLERR, arg[1]);
  ...
  if (style == SC) {
    ...
    add_basis(0.0, 0.0, 0.5);
    add_basis(0.5, 0.5, 0.5);
  } else if (style == PRIFCC) {
    add_basis(0.0, 0.0, 0.0);
  } else if (style == PRIHCP) {
    add_basis(0, 0, 0);
    add_basis(1.0/3.0, 2.0/3.0, 0.5);
  } else if (style == PRIFCCT) {
    add_basis(0.25, 0.25, 0.25);
    add_basis(0.75, 0.75, 0.75);
  } else if (style == PRIFCCO) {
    add_basis(0.5, 0.5, 0.5);
  } else if (style == PRIBCC) {
    add_basis(0.0, 0.0, 0.0);
  } else if (style == PRIBCCT) {
    add_basis(0.25, 0.5, 0.25);
    add_basis(0.5, 0.25, 0.25);
    add_basis(0.25, 0.75, 0.5);
    add_basis(0.75, 0.25, 0.5);
    add_basis(0.75, 0.5, 0.75);
    add_basis(0.5, 0.75, 0.75);
  } else if (style == PRIBCCO) {
    add_basis(0.0, 0.5, 0.5);
    add_basis(0.5, 0.0, 0.5);
    add_basis(0.5, 0.5, 0.0);
  } else if (style == HCP) {
  ...
  if (style == PRIFCC || style == PRIFCCT || style == PRIFCCO || style == PRIDIAMOND) {
    a2[0] = 0.5; a2[1] = sqrt(3.0/4.0);
    a3[0] = 0.5; a3[1] = sqrt(1.0/12.0); a3[2] = sqrt(2.0/3.0);
  }
  if (style == PRIBCC || style == PRIBCCT || style == PRIBCCO) {
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "origin") == 0) {
      ...
      origin[0] = universe->numeric(FLERR, arg[iarg+1]);
      origin[1] = universe->numeric(FLERR, arg[iarg+2]);
      origin[2] = universe->numeric(FLERR, arg[iarg+3]);
      ...
      double *orient;     
      if (strcmp(arg[iarg+1], "x") == 0) orient = orientx;
      else if (strcmp(arg[iarg+1], "y") == 0) orient = orienty;
      else if (strcmp(arg[iarg+1], "z") == 0) orient = orientz;
      else error->all(FLERR, "Illegal lattice command");
      orient[0] = universe->numeric(FLERR, arg[iarg+2]);
      orient[1] = universe->numeric(FLERR, arg[iarg+3]);
      orient[2] = universe->numeric(FLERR, arg[iarg+4]);
      iarg += 5;

    } else if (strcmp(arg[iarg], "spacing") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal lattice command");
      if (strcmp(arg[iarg+1], "auto") == 0) {
        iarg += 2;
      } else {
        if (iarg+4 > narg) error->all(FLERR, "Illegal lattice command");
        spaceflag = 1;
        xlattice = universe->numeric(FLERR, arg[iarg+1]);
        ylattice = universe->numeric(FLERR, arg[iarg+2]);
        zlattice = universe->numeric(FLERR, arg[iarg+3]);
        iarg += 4;
      }
  ...
  basis_centroid[0] = basis_centroid[1] = basis_centroid[2] = 0.0;
  for (int ibasis = 0; ibasis < nbasis; ibasis++) {
    basis_centroid[0] += basis[ibasis][0]; 
    basis_centroid[1] += basis[ibasis][1]; 
    basis_centroid[2] += basis[ibasis][2]; 
  }
  basis_centroid[0] /= nbasis;
  basis_centroid[1] /= nbasis;
  basis_centroid[2] /= nbasis;
  ...
  if (spaceflag == 0) {
    if (style == PRIFCC || style == PRIDIAMOND || style == PRIBCC || style == PRIBCCT) {
      xlattice = a1[0] * scale;
      ylattice = a2[1] * abs(a1[0]/a2[0]) * scale;
      zlattice = a3[2] * abs(a2[1]/a3[1]) * scale;
    } else if (style == PRIHCP) {
      xlattice = a1[0] * scale;
      ylattice = a2[1] * abs(a1[0]/a2[0]) * scale;
      zlattice = a3[2] * scale;
...
{
  if (fabs(orientx[0] * orienty[0] + orientx[1] * orienty[1] +
      orientx[2] * orienty[2]) > EPSILON) return 0;
  if (fabs(orienty[0] * orientz[0] + orienty[1] * orientz[1] +
      orienty[2] * orientz[2]) > EPSILON) return 0;
  if (fabs(orientx[0] * orientz[0] + orientx[1] * orientz[1] +
      orientx[2] * orientz[2]) > EPSILON) return 0;
  return 1;
}
...
{
  double xy0 = orientx[1] * orienty[2] - orientx[2] * orienty[1];
  double xy1 = orientx[2] * orienty[0] - orientx[0] * orienty[2];
  double xy2 = orientx[0] * orienty[1] - orientx[1] * orienty[0];
...
void Lattice::setup_transform()
{
  ...
  double lensq = orientx[0] * orientx[0] + orientx[1] * orientx[1] +
    orientx[2] * orientx[2];
  length = sqrt(lensq);
