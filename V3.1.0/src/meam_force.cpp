...
void MEAM::meam_force(int i, int iindex, int eflag_either, int eflag_global, int eflag_atom, 
    int vflag_either, int vflag_global, int vflag_atom, double *virial, double *eng_vdwl, 
    double nodal_energy_weight, double *eatom, double ***enode, int *fmap, double **scale, 
    ...
    std::unordered_map<Vector2D, int, Vector2DHasher> &va2nvalist,
    int fnoffset, double **vatom, double ****vnode)
{
  ...
  // i is gauss point
    ...
    if (inode >= 0) {
      ...
      if (eflag_global) {
        if (nodal_energy_weight < 0) iescale = nodal_weight[ietype];
        else iescale = nodal_energy_weight;
      }
  ...
  for (jn = 0; jn < numneigh; jn++) {
    ...
    // j is virtual atom
      ...
      if (jnode >= 0) {
        ...
        if (eflag_global) {
          if (nodal_energy_weight < 0) jescale = nodal_weight[jetype];
          else jescale = nodal_energy_weight;
        }
    ...
    for (m = 0; m < 3; m++) {
      ...
      if (fj != nullptr) fj[m] -= fij[m];
    }

    /*
       if (iindex < 0)
       if (atom->tag[i] == TEST || atom->tag[i] == TEST2) {
       int jtag;
       if (jindex < 0) jtag = atom->tag[j];
       else jtag = element->tag[j];
       printf("ID = %d sij = %g r = %g f = %g %g %g pairforce from j = %d %d \n"
       ,atom->tag[i],sij,rij,fij[0],fij[1],fij[2],jtag,jindex);
       }
       if (jindex < 0)
       if (atom->tag[j] == TEST || atom->tag[j] == TEST2) {
       int itag;
       if (iindex < 0) itag = atom->tag[i];
       else itag = element->tag[i];
       printf("ID = %d sij = %g r = %g f = %g %g %g pairforce from i = %d %d \n"
       ,atom->tag[j],sij,rij,fij[0],fij[1],fij[2],itag,iucell*iapc+ibasis);
       }
       */
    /*
       ...
       printf("EID = %d sij = %g r = %g f = %g %g %g pairforce from j = %d %d \n"
       ,element->tag[i],sij,rij,fij[0],fij[1],fij[2],element->tag[j],jindex);
       if (jindex >= 0)
       if (element->tag[j] == TEST && jbasis == TESTBASIS && jgcell == TESTGCELL) 
       printf("EID = %d sij = %g r = %g f = %g %g %g pairforce from i = %d %d\n"
       ,element->tag[j],sij,rij,-fij[0],-fij[1],-fij[2],element->tag[i],iindex);
       if (iindex >= 0)
       if (element->tag[i] == TEST2 && ibasis == TESTBASIS2 && igcell == TESTGCELL2) 
       printf("EID = %d sij = %g r = %g f = %g %g %g pairforce from j = %d %d \n"
       ,element->tag[i],sij,rij,fij[0],fij[1],fij[2],element->tag[j],jindex);
       if (jindex >= 0)
       if (element->tag[j] == TEST2 && jbasis == TESTBASIS2 && jgcell == TESTGCELL2) 
       printf("EID = %d sij = %g r = %g f = %g %g %g pairforce from i = %d %d\n"
       ,element->tag[j],sij,rij,-fij[0],-fij[1],-fij[2],element->tag[i],iindex);
       */
    ...
    if (vflag_either && (iucell < 0 || inode >= 0 || jucell < 0 || jnode >= 0)) {
      ...
      for (m = 0; m < 6; m++) {
        if (ivptr != nullptr) ivptr[m] += v[m];
        if (jvptr != nullptr) jvptr[m] += v[m];
    ...
    for (kn = 0; kn < numneigh_full; kn++) {
      ...
      if (!iszero(dsij1) || !iszero(dsij2)) {
        ...
        for (m = 0; m < 3; m++) {
          ...
          if (fj != nullptr) fj[m] += fjk[m];
          if (fk != nullptr) fk[m] -= fik[m] + fjk[m];
        }
        /*
           if (iindex < 0)
           if (atom->tag[i] == TEST || atom->tag[i] == TEST2) {
           ...
           if (kindex < 0) ktag = atom->tag[k];
           else ktag = element->tag[k];
           printf("ID = %d f = %g %g %g threebody from j = %d %d k = %d %d\n",atom->tag[i],fik[0],fik[1],fik[2],jtag,jindex,ktag,kindex);
           }
           if (jindex < 0)
           if (atom->tag[j] == TEST || atom->tag[j] == TEST2) {
           int itag,ktag;
           ...
           printf("ID = %d f = %g %g %g threebody from i = %d %d k = %d %d\n",atom->tag[j],fjk[0],fjk[1],fjk[2],itag,iucell*iapc+ibasis,ktag,kindex);
           }
           if (kindex < 0)
           if (atom->tag[k] == TEST || atom->tag[k] == TEST2) {
           int itag,jtag;
           if (jindex < 0) jtag = atom->tag[j];
           else jtag = element->tag[j];
           if (iindex < 0) itag = atom->tag[i];
           else itag = element->tag[i];
           printf("ID = %d f = %g %g %g threebody k from i = %d %d j = %d %d\n",atom->tag[k],-fik[0]-fjk[0],-fik[1]-fjk[1],-fik[2]-fjk[2],itag,iucell*iapc+ibasis,jtag,jindex);
           }
           */
        ...
        /*
           if (iindex >= 0)
           if (element->tag[i] == TEST && ibasis == TESTBASIS && igcell == TESTGCELL) 
           printf("%g %g %g threebody from j = %d %d k = %d %d\n",fik[0],fik[1],fik[2],element->tag[j],jindex,element->tag[k],kindex);

           if (jindex >= 0)
           if (element->tag[j] == TEST && jbasis == TESTBASIS && jgcell == TESTGCELL) 
           printf("%g %g %g threebody from i = %d %d k = %d %d\n",fjk[0],fjk[1],fjk[2],element->tag[i],iindex,element->tag[k],kindex);

           if (kindex >= 0)
           if (element->tag[k] == TEST && kbasis == TESTBASIS && kgcell == TESTGCELL) 
           printf("%g %g %g threebody k from i = %d %d j = %d %d\n",-fik[0]-fjk[0],-fik[1]-fjk[1],-fik[2]-fjk[2],element->tag[i],iindex,element->tag[j],jindex);
           */
      ...
      if (va2nvalist.count({j, jindex}) == 0) error->one(FLERR,"TEST"); // sanity check
      int jnva = va2nvalist[{j, jindex}];
      ...
      /*
         ...
         printf("ID = %d f = %g %g %g threebody nva k from j = %d %d k = %d %d\n",atom->tag[i],fij[0]+fik[0],fij[1]+fik[1],fij[2]+fik[2],jtag,jindex,ktag,kindex);
         }
         */
      /*
         if (iindex >= 0)
         if (element->tag[i] == TEST && ibasis == TESTBASIS && igcell == TESTGCELL) 
         printf("%g %g %g threebody nva k from j = %d %d k = %d %d\n",fij[0]+fik[0],fij[1]+fik[1],fij[2]+fik[2],element->tag[j],jindex,element->tag[k],kindex);
