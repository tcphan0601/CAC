...
#define ONE "step cpu temp press etotal"
...
Thermo::Thermo(CAC *cac, int narg, char **arg) : Pointers(cac)
{
  ...
  normuserflag = 0;
  lineflag = ONELINE;
  atomlostflag = elemlostflag = Thermo::ERROR;
  atomlostbefore = 0;
  flushflag = 0;
...
void Thermo::init()
{
  ...
  for (i = 0; i < nfield; i++) {
    ...
    if (format_line) {
      ...
      else format_line_ptr = strtok(nullptr, " \0");
    }
...
void Thermo::compute(int flag)
{
  ...
  // turn off normflag if natoms + nucells = 0 to avoid divide by 0

  natoms = atom->natoms = lost_atom_check();
  nelements = element->nelements = lost_elem_check();
  natoms += element->nucells;
  ...
  // check for cpu time between thermo outputs
  
  if (maxcpu > 0) {
    compute_cpu();
    if (dvalue - last_cpu > maxcpu) {
      if (comm->me == 0) printf("Maxcpu = %g cpu = %g last_cpu = %g\n", maxcpu, dvalue, last_cpu);
      error->all(FLERR, "CPU time between thermo output exceeds max CPU time set by thermo_modify command");
    }
    last_cpu = dvalue;
  }
...
bigint Thermo::lost_atom_check()
{
  ...
  if (atomlostflag == Thermo::IGNORE) return ntotal;
  if (atomlostflag == Thermo::WARN && atomlostbefore == 1) {
    return ntotal;
  }
  ...
  if (atomlostflag == Thermo::ERROR) {
    int tmp;
    MPI_Status status;
    MPI_Request request;
    
    int max_nlocal;
    MPI_Allreduce(&atom->nlocal, &max_nlocal, 1, MPI_INT, MPI_MAX, world);

    if (me == 0) {
      FILE *fp = fopen("list_atom", "w");
      if (atom->nmax < max_nlocal)
        memory->grow(atom->tag, max_nlocal, "atom:tag");
      int recvrow;
      for (int iproc = 0; iproc < comm->nprocs; iproc++) {
        if (iproc) {
          MPI_Irecv(&atom->tag[0], max_nlocal, MPI_DOUBLE, iproc, 0, world, &request);
          MPI_Send(&tmp, 0, MPI_INT, iproc, 0, world);
          MPI_Wait(&request, &status);
          MPI_Get_count(&status, MPI_DOUBLE, &recvrow);
        } else recvrow = atom->nlocal;
      ...
      MPI_Rsend(&atom->tag[0], atom->nlocal, MPI_DOUBLE, 0, 0, world);
    }
    char str[99];
    sprintf(str, 
        "Lost atoms: original " BIGINT_FORMAT " current " BIGINT_FORMAT ", timestep " BIGINT_FORMAT, 
        atom->natoms, ntotal, update->ntimestep);
    error->all(FLERR, str);
  }
...
bigint Thermo::lost_elem_check()
{
  ...
  // if not checking or already warned, just return
  if (elemlostflag == Thermo::IGNORE) return ntotal;
  if (elemlostflag == Thermo::WARN && elemlostbefore == 1) {
    return ntotal;
  }

  // error message

  if (elemlostflag == Thermo::ERROR) {
    int tmp;
    MPI_Status status;
    MPI_Request request;
    
    int max_nlocal;
    MPI_Allreduce(&element->nlocal, &max_nlocal, 1, MPI_INT, MPI_MAX, world);

    if (me == 0) {
      FILE *fp = fopen("list_element", "w");
      if (element->nmax < max_nlocal)
        memory->grow(element->tag, max_nlocal, "atom:tag");
      int recvrow;
      for (int iproc = 0; iproc < comm->nprocs; iproc++) {
        if (iproc) {
          MPI_Irecv(&element->tag[0], max_nlocal, MPI_DOUBLE, iproc, 0, world, &request);
          MPI_Send(&tmp, 0, MPI_INT, iproc, 0, world);
          MPI_Wait(&request, &status);
          MPI_Get_count(&status, MPI_DOUBLE, &recvrow);
        } else recvrow = element->nlocal;
        for (int i = 0; i < recvrow; i ++) 
          fprintf(fp, "%06d\n", element->tag[i]);
      }
      ...
      MPI_Recv(&tmp, 0, MPI_INT, 0, 0, world, MPI_STATUS_IGNORE);
      MPI_Rsend(&element->tag[0], element->nlocal, MPI_DOUBLE, 0, 0, world);
    }
    char str[99];
    sprintf(str, 
        "Lost elements: original " BIGINT_FORMAT " current " BIGINT_FORMAT ", timestep " BIGINT_FORMAT, 
        element->nelements, ntotal, update->ntimestep);
    error->all(FLERR, str);
  }
...
void Thermo::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "temp") == 0) {
      ...
      if (iarg+2 > narg) error->all(FLERR, "Illegal thermo_modify command");
      if (strcmp(arg[iarg+1], "ignore") == 0) elemlostflag = atomlostflag = Thermo::IGNORE;
      else if (strcmp(arg[iarg+1], "warn") == 0) elemlostflag = atomlostflag = Thermo::WARN;
      else if (strcmp(arg[iarg+1], "error") == 0) elemlostflag = atomlostflag = Thermo::ERROR;
      else error->all(FLERR, "Illegal thermo_modify command");
      iarg += 2;
    } else if (strcmp(arg[iarg], "maxcpu") == 0) {
      if (iarg+2 > narg) error->all(FLERR, "Illegal thermo_modify command");
      maxcpu = universe->numeric(FLERR, arg[iarg+1]);
      if (maxcpu <= 0) error->all(FLERR, "Maxcpu must be positive");
      iarg += 2;
      ...
      if (strcmp(arg[iarg+1], "line") == 0) {
        ...
        *ptr = 'd';
        ...
        int i = universe->inumeric(FLERR, arg[iarg+1]) - 1;
...
void Thermo::parse_fields(char *str)
{
  ...
  while (word) {
    ...
    if (strcmp(word, "step") == 0) {
      ...
      addfield("Elements", &Thermo::compute_elements, BIGINT);
    } else if (strcmp(word, "atoms") == 0) {
      addfield("Atoms", &Thermo::compute_atoms, BIGINT);
    } else if (strcmp(word, "nodes") == 0) {
      addfield("Nodes", &Thermo::compute_nodes, BIGINT);
...
int Thermo::evaluate_keyword(char *word, double *answer)
{
  ...
  natoms = atom->natoms + element->nucells;
...
void Thermo::compute_nodes()
{
  bivalue = element->nnodes;
}
  ...
  for (int i = 0; i < nelocal; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) {
        max = MAX(max, fabs(nodef[i][j][k][0]));
        max = MAX(max, fabs(nodef[i][j][k][1]));
        max = MAX(max, fabs(nodef[i][j][k][2]));
      }
...
void Thermo::compute_imbalance()
{
  double tmp1, tmp2;
  if (fix_balance) 
    dvalue = fix_balance->compute_scalar();
  else
