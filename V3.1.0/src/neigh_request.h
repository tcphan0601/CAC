...
class NeighRequest : protected Pointers {
 public:
  ...
  int elemonlylist;      // 1 if include element neighbor list (atom/elemenet neighbors of elements, not gcell)
  int atomlist;          // 1 if include atom neighbor list (for both atom-atom pairs and atom-ucell pairs)
...
//  int granonesided;    // 1 if one-sided granular list for 
  ...
  int noinner;           // 1 if exclude neighbor of inner virtual atoms

  int dnum;              // # of extra floating point values stored in list

  int threebody;         // 1 if include neighbors of neighboring virtual atoms
