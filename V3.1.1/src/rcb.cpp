...
void RCB::compute(int dimension, int n, double **x, double *wt, 
    double *bboxlo, double *bboxhi, double *boxscale)
{
  int i, j, k;
    ...
    for (dim = 0; dim < dimension; dim++) {
      ...
      while (1) {
        ...
        if (wtlo + med.totallo < targetlo) {    // lower half TOO SMALL
          ...
          markactive = 0;
        } else break;           // Goldilocks result: both partitions just right
...
{
  // only create IrregularComm if not previously created
  // allows IrregularComm to persist for multiple RCB calls by fix balance

  if (!irregular_comm) irregular_comm = new IrregularComm(cac);
  ...
  // perform inversion via irregular_comm comm
  // nrecv = # of my dots to send to other procs

  int nrecv = irregular_comm->create_comm_plan_data(nsend, proclist, sortflag);
  Invert *rinvert =
    (Invert *) memory->smalloc(nrecv * sizeof(Invert), "RCB:rinvert");
  irregular_comm->exchange_data((char *) sinvert, sizeof(Invert), (char *) rinvert);
  irregular_comm->destroy_comm_plan_data();
...
/*  ----------------------------------------------------------------------
   memory use of IrregularComm
   -------------------------------------------------------------------------  */

bigint RCB::memory_usage()
{
  bigint bytes = 0;
  if (irregular_comm) bytes += irregular_comm->memory_usage();
  return bytes;
}
