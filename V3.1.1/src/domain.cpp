...
Domain::Domain(CAC *cac) : Pointers(cac)
{
  ...
  boxlo_current[0] = boxlo_current[1] = boxlo_current[2] = -0.5;
  boxhi_current[0] = boxhi_current[1] = boxhi_current[2] = 0.5;
  xy_current = xz_current = yz_current = 0.0;

  h_current[3] = h_current[4] = h_current[5] = 0.0;
  h_current_inv[3] = h_current_inv[4] = h_current_inv[5] = 0.0;
...
void Domain::init()
{
  ...
  for (int i = 0; i < modify->nfix; i++) {
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_SIZE) box_change_size = 1;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_SHAPE) box_change_shape = 1;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_DOMAIN) box_change_domain = 1;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_X) box_change_x++;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_Y) box_change_y++;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_Z) box_change_z++;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_YZ) box_change_yz++;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_XZ) box_change_xz++;
    if (modify->fix[i]->box_change & Fix::BOX_CHANGE_XY) box_change_xy++;
  }
...
{
  if (atom->natoms == 0 && element->nelements == 0) { 
    boxlo[0] = boxlo[1] = boxlo[2] = -1;
    boxhi[0] = boxhi[1] = boxhi[2] = 1;
  } else {
    ...
    if (nonperiodic == 2) {
      ...
      for (int i = 0; i < nlocal; i++) 
        for (int j = 0; j < apc[etype[i]]; j++) 
          for (int k = 0; k < npe[etype[i]]; k++) {
            extent[0][0] = MIN(extent[0][0], nodex[i][j][k][0]);
            extent[0][1] = MAX(extent[0][1], nodex[i][j][k][0]);
            extent[1][0] = MIN(extent[1][0], nodex[i][j][k][1]);
            extent[1][1] = MAX(extent[1][1], nodex[i][j][k][1]);
            extent[2][0] = MIN(extent[2][0], nodex[i][j][k][2]);
            extent[2][1] = MAX(extent[2][1], nodex[i][j][k][2]);
          } 
      ...
      if (triclinic) {
        lamda2x(atom->nlocal, atom->x);
        lamda2x(element->nlocal, element->x);
        lamda2nodex(element->nlocal, element->nodex);
      }
  ...
  */
  ...
  if (nonperiodic == 2 && triclinic) {
    x2lamda(atom->nlocal, atom->x);
    x2lamda(element->nlocal, element->x);
    nodex2lamda(element->nlocal, element->nodex);
    pbc();
  }
...
void Domain::pbc()
{
  ...
  // apply pbc to elements

  nlocal = element->nlocal;
  double **elemx = element->x;
  double ****nodex = element->nodex;
  int *npe = element->npe;
  int *apc = element->apc;
  int *etype = element->etype;
  ...
  n3 = 3 * nlocal;
  coord = &elemx[0][0];  // note: x is always initialized to at least one element.
  for (i = 0; i < n3; i++)
    if (!ISFINITE(*coord++)) 
      error->one(FLERR, "Non-numeric element coords - simulation unstable");

  // have to loop exactly number of nodes for each element
  // since values beyond npe are not defined

  for (i = 0; i < nlocal; i++)
    for (j = 0; j < apc[etype[i]]; j++)
      for (k = 0; k < npe[etype[i]]; k++)
        for (l = 0; l < 3; l++) 
          if (!ISFINITE(nodex[i][j][k][l])) 
            error->one(FLERR, "Non-numeric node coords - simulation unstable");
...
/*  ----------------------------------------------------------------------
    remap ATOM into the periodic box no matter how far away
...
/*  ----------------------------------------------------------------------
    remap ELEMENT into the periodic box no matter how far away
    adjust 3 image flags encoded in image accordingly
    resulting coord must satisfy lo <= coord < hi
    MAX is important since coord - prd < lo can happen when coord = hi
    for triclinic, point is converted to lamda coords (0-1) before doing remap
...
void Domain::remap(double *x, double ***nodex, imageint &image, int apc, int npe)
{
  double *lo, *hi, *period, *coord;
  double lamda[3], del[3];
  imageint idim, otherdims;
...
void Domain::remap(double *x, double ***nodex, int apc, int npe)
{
  double *lo, *hi, *period, *coord;
  double lamda[3], del[3];
  del[0] = -x[0];
...
/*  ----------------------------------------------------------------------
    unmap the ELEMENT via image flags
    x overwritten with result, don't reset image flag
    for triclinic, use h[] to add in tilt factors in other dims as needed
    -------------------------------------------------------------------------  */

void Domain::unmap(double *x, double ***nodex, imageint image, int apc, int npe)
{
  int xbox = (image & IMGMASK) - IMGMAX;
  int ybox = (image >> IMGBITS & IMGMASK) - IMGMAX;
  int zbox = (image >> IMG2BITS) - IMGMAX;
  ...
  if (triclinic == 0) {
    ...
    dx = h[0] * xbox + h[5] * ybox + h[4] * zbox;
    dy = h[1] * ybox + h[3] * zbox;
    dz = h[2] * zbox;
  }
  x[0] += dx;
  x[1] += dy;
...
void Domain::set_lattice(int narg, char **arg)
{
  ...
  lattice = nullptr;
  lattice = new Lattice(cac, narg, arg);
}
...
void Domain::add_region(int narg, char **arg)
{
  ...
  if (cac->suffix_enable) {
    if (cac->suffix) {
      char estyle[256];
      sprintf(estyle, "%s/%s", arg[1], cac->suffix);
      if (region_map->find(estyle) != region_map->end()) {
        RegionCreator region_creator = (*region_map)[estyle];
        regions[nregion] = region_creator(cac, narg, arg);
    ...
    if (cac->suffix2) {
      char estyle[256];
      sprintf(estyle, "%s/%s", arg[1], cac->suffix2);
      if (region_map->find(estyle) != region_map->end()) {
        RegionCreator region_creator = (*region_map)[estyle];
        regions[nregion] = region_creator(cac, narg, arg);
        regions[nregion]->init();
...
/*  ----------------------------------------------------------------------
  ...
  template <typename T>
Region *Domain::region_creator(CAC *cac, int narg, char ** arg)
{
...
/*  ----------------------------------------------------------------------
    convert triclinic 0-1 lamda coords to box coords for all N atoms/elements
    x = H lamda + x0;
    -------------------------------------------------------------------------  */

void Domain::lamda2x(int n, double **x)
{
  for (int i = 0; i < n; i++) {
...
void Domain::lamda2x(int n, double **x, int *mask, int groupbit)
{
  for (int i = 0; i < n; i++)
    if (mask[i] & groupbit) {
      x[i][0] = h[0]*x[i][0] + h[5]*x[i][1] + h[4]*x[i][2] + boxlo[0];
...
/*  ----------------------------------------------------------------------
    convert box coords to triclinic 0-1 lamda coords for all N atoms/elements
    lamda = H^-1 (x - x0)
    -------------------------------------------------------------------------  */

void Domain::x2lamda(int n, double **x)
{
  double delta[3];
...
void Domain::x2lamda(int n, double **x, int *mask, int groupbit)
{
  double delta[3];
...
/*  ----------------------------------------------------------------------
    convert triclinic 0-1 lamda coords to box coords for all N nodes
    x = H lamda + x0;
    -------------------------------------------------------------------------  */

void Domain::lamda2nodex(int n, double ****nodex)
{
  element->nodex_lamda_flag = 0;
  int *npe = element->npe;
  int *apc = element->apc;
  int *etype = element->etype;
  for (int i = 0; i < n; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) {
        nodex[i][j][k][0] = h[0] * nodex[i][j][k][0] + h[5] * nodex[i][j][k][1] + h[4] * nodex[i][j][k][2] + boxlo[0];
        nodex[i][j][k][1] = h[1] * nodex[i][j][k][1] + h[3] * nodex[i][j][k][2] + boxlo[1];
        nodex[i][j][k][2] = h[2] * nodex[i][j][k][2] + boxlo[2];
      }
}

void Domain::lamda2nodex(int n, double ****nodex, int groupbit)
{
  element->nodex_lamda_flag = 0;
  int *npe = element->npe;
  int *apc = element->apc;
...
/*  ----------------------------------------------------------------------
    convert box coords to triclinic 0-1 lamda coords for all N nodes
    lamda = H^-1 (x - x0)
    -------------------------------------------------------------------------  */

void Domain::nodex2lamda(int n, double ****nodex)
{
  element->nodex_lamda_flag = 1;
  double delta[3];
  int *npe = element->npe;
  int *apc = element->apc;
  int *etype = element->etype;
  ...
  for (int i = 0; i < n; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) {
        delta[0] = nodex[i][j][k][0] - boxlo[0];
        delta[1] = nodex[i][j][k][1] - boxlo[1];
        delta[2] = nodex[i][j][k][2] - boxlo[2];

        nodex[i][j][k][0] = h_inv[0] * delta[0] + h_inv[5] * delta[1] + h_inv[4] * delta[2];
        nodex[i][j][k][1] = h_inv[1] * delta[1] + h_inv[3] * delta[2];
        nodex[i][j][k][2] = h_inv[2] * delta[2];
      }
}

void Domain::nodex2lamda(int n, double ****nodex, int groupbit)
{
  element->nodex_lamda_flag = 1;
  double delta[3];
  int *npe = element->npe;
...
/*  ----------------------------------------------------------------------
    convert current box coords to triclinic 0-1 lamda coords for all N atoms/elements
    lamda = H^-1 (x - x0)
    -------------------------------------------------------------------------  */

void Domain::x2lamda_current(int n, double **x)
{
  double delta[3];

  for (int i = 0; i < n; i++) {
    delta[0] = x[i][0] - boxlo_current[0];
    delta[1] = x[i][1] - boxlo_current[1];
    delta[2] = x[i][2] - boxlo_current[2];

    x[i][0] = h_current_inv[0] * delta[0] + h_current_inv[5] * delta[1] + h_current_inv[4] * delta[2];
    x[i][1] = h_current_inv[1] * delta[1] + h_current_inv[3] * delta[2];
    x[i][2] = h_current_inv[2] * delta[2];
  }
}

/*  ----------------------------------------------------------------------
    convert current box coords to triclinic 0-1 lamda coords for all N nodes
    lamda = H^-1 (x - x0)
    -------------------------------------------------------------------------  */

void Domain::nodex2lamda_current(int n, double ****nodex)
{
  double delta[3];
  int *npe = element->npe;
  int *apc = element->apc;
  ...
  for (int i = 0; i < n; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) {
        delta[0] = nodex[i][j][k][0] - boxlo_current[0];
        delta[1] = nodex[i][j][k][1] - boxlo_current[1];
        delta[2] = nodex[i][j][k][2] - boxlo_current[2];

        nodex[i][j][k][0] = h_current_inv[0] * delta[0] + h_current_inv[5] * delta[1] + h_current_inv[4] * delta[2];
        nodex[i][j][k][1] = h_current_inv[1] * delta[1] + h_current_inv[3] * delta[2];
        nodex[i][j][k][2] = h_current_inv[2] * delta[2];
      }
}
/*  ----------------------------------------------------------------------
    convert triclinic 0-1 lamda coords to current box coords for all N atoms/elements
    x = H lamda + x0;
    -------------------------------------------------------------------------  */

void Domain::lamda2x_current(int n, double **x)
{
  for (int i = 0; i < n; i++) {
    x[i][0] = h_current[0] * x[i][0] + h_current[5] * x[i][1] + h_current[4] * x[i][2] + boxlo_current[0];
    x[i][1] = h_current[1] * x[i][1] + h_current[3] * x[i][2] + boxlo_current[1];
    x[i][2] = h_current[2] * x[i][2] + boxlo_current[2];
  }
}

/*  ----------------------------------------------------------------------
    convert triclinic 0-1 lamda coords to current box coords for all N nodes
    x = H lamda + x0;
    -------------------------------------------------------------------------  */

void Domain::lamda2nodex_current(int n, double ****nodex)
{
  int *npe = element->npe;
  int *apc = element->apc;
  int *etype = element->etype;
  for (int i = 0; i < n; i++) 
    for (int j = 0; j < apc[etype[i]]; j++) 
      for (int k = 0; k < npe[etype[i]]; k++) {
        nodex[i][j][k][0] = h_current[0] * nodex[i][j][k][0] + h_current[5] * nodex[i][j][k][1] + h_current[4] * nodex[i][j][k][2] + boxlo_current[0];
        nodex[i][j][k][1] = h_current[1] * nodex[i][j][k][1] + h_current[3] * nodex[i][j][k][2] + boxlo_current[1];
        nodex[i][j][k][2] = h_current[2] * nodex[i][j][k][2] + boxlo_current[2];
      }
}
...
void Domain::lamda2x_current(double *lamda, double *x)
{
  x[0] = h_current[0] * lamda[0] + h_current[5] * lamda[1] + h_current[4] * lamda[2] + boxlo_current[0];
  x[1] = h_current[1] * lamda[1] + h_current[3] * lamda[2] + boxlo_current[1];
  x[2] = h_current[2] * lamda[2] + boxlo_current[2];
}
...
void Domain::x2lamda_current(double *x, double *lamda)
{
  double delta[3];
  delta[0] = x[0] - boxlo_current[0];
  delta[1] = x[1] - boxlo_current[1];
  delta[2] = x[2] - boxlo_current[2];

  lamda[0] = h_current_inv[0] * delta[0] + h_current_inv[5] * delta[1] + h_current_inv[4] * delta[2];
  lamda[1] = h_current_inv[1] * delta[1] + h_current_inv[3] * delta[2];
  lamda[2] = h_current_inv[2] * delta[2];
}

/*  ----------------------------------------------------------------------
    set current global box params
    assumes boxlo/hi and triclinic tilts are already set
    -------------------------------------------------------------------------  */

void Domain::set_current_global_box()
{
  prd_current[0] = xprd_current = boxhi_current[0] - boxlo_current[0];
  prd_current[1] = yprd_current = boxhi_current[1] - boxlo_current[1];
  prd_current[2] = zprd_current = boxhi_current[2] - boxlo_current[2];

  h_current[0] = xprd_current;
  h_current[1] = yprd_current;
  h_current[2] = zprd_current;
  h_current_inv[0] = 1.0/h_current[0];
  h_current_inv[1] = 1.0/h_current[1];
  h_current_inv[2] = 1.0/h_current[2];

  if (triclinic) {
    h_current[3] = yz_current;
    h_current[4] = xz_current;
    h_current[5] = xy_current;
    h_current_inv[3] = -h_current[3] / (h_current[1] * h_current[2]);
    h_current_inv[4] = (h_current[3] * h_current[5] - h_current[1] * h_current[4]) /
      (h_current[0] * h_current[1] * h_current[2]);
    h_current_inv[5] = -h_current[5] / (h_current[0] * h_current[1]);

    boxlo_current_bound[0] = MIN(boxlo_current[0], boxlo_current[0]+xy_current);
    boxlo_current_bound[0] = MIN(boxlo_current_bound[0], boxlo_current_bound[0]+xz_current);
    boxlo_current_bound[1] = MIN(boxlo_current[1], boxlo_current[1]+yz);
    boxlo_current_bound[2] = boxlo_current[2];

    boxhi_current_bound[0] = MAX(boxhi_current[0], boxhi_current[0]+xy_current);
    boxhi_current_bound[0] = MAX(boxhi_current_bound[0], boxhi_current_bound[0]+xz_current);
    boxhi_current_bound[1] = MAX(boxhi_current[1], boxhi_current[1]+yz_current);
    boxhi_current_bound[2] = boxhi_current[2];
  }
}
...
void Domain::remap_current(double *x)
{
  double *lo, *hi, *period, *coord;
  double lamda[3];
  ...
  if (triclinic) lamda2x_current(coord, x);
}
...
void Domain::remap_current(double *x, double ***nodex, int npe, int apc)
{
  double *lo, *hi, *period, *coord;
  double lamda[3], del[3];
...
void Domain::unmap_reverse_current(double *x, imageint image)
{
  int xbox = (image & IMGMASK) - IMGMAX;
  int ybox = (image >> IMGBITS & IMGMASK) - IMGMAX;
  int zbox = (image >> IMG2BITS) - IMGMAX;
  ...
  if (triclinic == 0) {
    x[0] -= xbox * xprd_current;
    x[1] -= ybox * yprd_current;
    x[2] -= zbox * zprd_current;
  } else {
    x[0] -= h_current[0] * xbox + h_current[5] * ybox + h_current[4] * zbox;
    x[1] -= h_current[1] * ybox + h_current[3] * zbox;
    x[2] -= h_current[2] * zbox;
  }
}
