...
MinLineSearch::MinLineSearch(CAC *cac) : Min(cac)
{
  searchflag = 1;
  xvec = fvec = x0 = g = h = nullptr;
...
{
  // atomic dof
  // construct combined continuous vectors for atoms and nodes
  // pass flag = 1 to count_nodes to update element->nnodes

  nvec = 3 * (atom->nlocal + element->count_nodes(1));
  if (nvec) {
    delete [] xvec;
    delete [] fvec;
    delete [] x0;
    ...
    fvec = new double[nvec];
    x0 = new double[nvec];
    g = new double[nvec];
    h = new double[nvec];
    atomx0 = fix_minimize->request_vector(0, 0);
    atomg = fix_minimize->request_vector(1, 0);
    atomh = fix_minimize->request_vector(2, 0);
    nodex0 = fix_minimize->request_vector(0, 1);
    nodeg = fix_minimize->request_vector(1, 1);
    nodeh = fix_minimize->request_vector(2, 1);
  }

  // extra per-atom dof not yet implemented
...
int MinLineSearch::linemin_backtrack(double eoriginal, double &alpha)
{
  ...
  bigint natoms = atom->natoms + element->nnodes;
  ...
  if (output->thermo->normflag) fdothall /= natoms;
...
// the CAC forces much closer to zero.
//
...
*/
  ...
  while (1) {
    ...
    if (alpha <= 0.0 || de_ideal >= -EMACH) {
      printf("fail zeroalpha alpha = %g de_ideal = %g EMACH = %g\n",alpha,de_ideal,EMACH);
      ecurrent = alpha_step(0.0, 0);
      return ZEROALPHA;
    }
...
/* ------------------------------------------------------
   combine vectors stored by atom, element and fix minimize into 
   single continuous arrays after energy_force evaluation
   atom->x + element->nodex --> xvec
   atom->f + element->nodef --> fvec
   atomx0 + nodex0 --> x0
   atomg + nodeg --> g
   atomh + nodeh --> h
   ------------------------------------------------------ */

void MinLineSearch::copy_force() 
{
  ...
  int *apc = element->apc;
  int inpe, iapc, n, m;
  int nper_elem = element->maxapc * element->maxnpe * 3;
  ...
  for (int i = 0; i < nelocal; i++) {
    ...
    for (int j = 0; j < iapc; j++)
      for (int k = 0; k < inpe; k++) 
        for (int dim = 0; dim < 3; dim++) {
          xvec[n] = nodex[i][j][k][dim];    
          fvec[n] = nodef[i][j][k][dim];    
          x0[n] = nodex0[m];    
          g[n] = nodeg[m];    
          h[n] = nodeh[m];    
          n++; 
...
/* ------------------------------------------------------
   copy data from combined vectors to vector stored by atom, element
   and fix minimize for energy_force evaluation
   xvec --> atom->x + element->nodex
   fvec --> atom->f + element->nodef 
   x0 --> atomx0 + nodex0
   g --> atomg + nodeg
   h --> atomh + nodeh
   ------------------------------------------------------ */

void MinLineSearch::copy_vectors() 
{
  int nalocal = atom->nlocal;
  int nelocal = element->nlocal;
  double **x = atom->x;
  ...
  for (int i = 0; i < nelocal; i++) {
    ...
    for (int j = 0; j < iapc; j++)
      for (int k = 0; k < inpe; k++) 
        for (int dim = 0; dim < 3; dim++) {
          nodex[i][j][k][dim] = xvec[n];    
          nodef[i][j][k][dim] = fvec[n];    
          nodex0[m] = x0[n];    
          nodeg[m] = g[n];    
          nodeh[m] = h[n];    
          n++;
