...
//        && force->kspace->fftbench) fftflag = 1;
  ...
  if (timer->has_loop()) {
    ...
    // total atoms (including virtual atoms)
    bigint natoms = atom->natoms;
    bigint nelements = element->nelements;
    bigint ntotalatoms;
    bigint nlocalucell = 0;
    for (i = 0; i < element->nlocal; i++) 
      nlocalucell += element->nucell[element->etype[i]] 
                   * element->apc[element->etype[i]];
    MPI_Allreduce(&nlocalucell, &ntotalatoms, 1, MPI_CAC_BIGINT, MPI_SUM, world);
    ntotalatoms += natoms;
