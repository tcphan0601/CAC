  ...
  id_fix(nullptr)
{
  ...
  if (fix->restart_reset) fix->restart_reset = 0;
    ...
    double ****nodevoriginal = fix->a4store;
      ...
      if (emask[i] & groupbit) {
        for (int j = 0; j < apc[etype[i]]; j++) 
          for (int k = 0; k < npe[etype[i]]; k++) {
            nodevoriginal[i][j][k][0] = nodev[i][j][k][0];
            nodevoriginal[i][j][k][1] = nodev[i][j][k][1];
            nodevoriginal[i][j][k][2] = nodev[i][j][k][2];
          }
      }
  }
...
void ComputeVACF::init()
{
  ...
  nvacf = group->count_atom(igroup) + group->count_vatom(igroup);
}
...
void ComputeVACF::compute_vector()
{
  ...
  for (int i = 0; i < nelocal; i++)
    if (emask[i] & groupbit) {
      ...
      vxsq = vysq = vzsq = 0.0;
      if (interpolate_flag) {
        for (int j = 0; j < apc[etype[j]]; j++) 
          for (int k = 0; k < nucell[etype[i]]; k++) {
            vx = element->evec->interpolate(nodev,i,j,k,0);
            vy = element->evec->interpolate(nodev,i,j,k,1);
            vz = element->evec->interpolate(nodev,i,j,k,2);
            vxoriginal = element->evec->interpolate(nodevoriginal,i,j,k,0);
            vyoriginal = element->evec->interpolate(nodevoriginal,i,j,k,1);
            vzoriginal = element->evec->interpolate(nodevoriginal,i,j,k,2);
            vxsq += vx * vxoriginal;
            vysq += vy * vyoriginal;
            vzsq += vz * vzoriginal;
          }
        ...
        for (int j = 0; j < apc[ietype]; j++) 
          for (int k = 0; k < npe[ietype]; k++) {
            vxsq += nodev[i][j][k][0] * nodevoriginal[i][j][k][0];
            vysq += nodev[i][j][k][1] * nodevoriginal[i][j][k][1];
            vzsq += nodev[i][j][k][2] * nodevoriginal[i][j][k][2];
          }
        vxsq *= nodal_weight[ietype];
        vysq *= nodal_weight[ietype];
        vzsq *= nodal_weight[ietype];
      }
      vacf[0] += vxsq;
      vacf[1] += vysq;
      vacf[2] += vzsq;
...
void ComputeVACF::set_elem_arrays(int i)
{
  ...
  for (int j = 0; j < apc[etype[i]]; j++) 
    for (int k = 0; k < npe[etype[i]]; k++) {
      nodevoriginal[i][j][j][0] = nodev[i][j][k][0];
      nodevoriginal[i][j][j][1] = nodev[i][j][k][1];
      nodevoriginal[i][j][j][2] = nodev[i][j][k][2];
    }
}
