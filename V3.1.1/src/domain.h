...
class Domain : protected Pointers {
 public:
  ...
  typedef Region *( * RegionCreator)(CAC *, int, char **);
  ...
  void closest_image(const double * const, const double * const, 
                     double * const);
  void remap(double *, double ***, imageint &, int, int);
  void remap(double *, double ***, int, int);
  void remap(double *, imageint &);
  void remap(double *);
  void remap_near(double *, double *);
  void unmap(double *, double ***, imageint, int, int);
  void unmap(double *, imageint);
  void unmap(const double *, imageint, double *);
  void unmap_reverse(double *, imageint);
  ...
  virtual void lamda2x(int, double **);
  virtual void lamda2x(int, double **, int *, int);
  virtual void x2lamda(int, double **);
  virtual void x2lamda(int, double **, int *, int);
  virtual void lamda2nodex(int, double ****);
  virtual void lamda2nodex(int, double ****, int);
  virtual void nodex2lamda(int, double ****);
  virtual void nodex2lamda(int, double ****, int);
  ...
  // use to store current configuration box info for atomic_strain command
  
  double xprd_current, yprd_current, zprd_current; // global box dimensions
  double prd_current[3];                         // array form of dimensions
  double boxlo_current[3], boxhi_current[3];      // orthogonal box global bounds
  double boxlo_current_bound[3], boxhi_current_bound[3];  // bounding box of tilted domain
  double xy_current, xz_current, yz_current;       // 3 tilt factors
  double h_current[6], h_current_inv[6];          // shape matrix in Voigt notation
  virtual void set_current_global_box();
  virtual void lamda2x_current(int, double **);
  virtual void x2lamda_current(int, double **);
  virtual void lamda2nodex_current(int, double ****);
  virtual void nodex2lamda_current(int, double ****);
  virtual void lamda2x_current(double *, double *);
  virtual void x2lamda_current(double *, double *);
  void remap_current(double *);
  void remap_current(double *, double ***, int, int);
  void unmap_reverse_current(double *, imageint);

 protected:
  double small[3];                  // fractions of box lengths

 private:
  template <typename T> static Region *region_creator(CAC *, int, char **);
};
...
CAC simulation may be inefficient as a result.
...
CAC simulations.  However they can cause problems when such atoms
...
This is a restriction because CAC can be confused about which image
of an atom in the bonded interaction is the correct one to use.
"Extent" in this context means the maximum end-to-end length of the
bond/angle/dihedral.  CAC computes this by taking the maximum bond
