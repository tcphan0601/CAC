...
FixDeformPressure::FixDeformPressure(CAC *cac, int narg, char **arg) :
  FixDeform(cac, narg, arg), id_temp(nullptr), id_press(nullptr), temperature(nullptr),
  pressure(nullptr)
{
  // set defaults
  ...
  while (i < leftover_iarg.size()) {
    ...
    if (strcmp(arg[iarg], "x") == 0 ||
      ...
      if (iarg+2 > narg) error->all(FLERR, "Illegal fix deform/pressure command");
      if (strcmp(arg[iarg + 1], "pressure") == 0) {
        if (iarg+4 > narg) error->all(FLERR, "Illegal fix deform/pressure command");
        set[index].style = PRESSURE;
        if (strstr(arg[iarg + 2], "v_") != arg[iarg + 2]) {
          set_extra[index].ptarget = universe->numeric(FLERR, arg[iarg + 2]);
        } else {
          strcpy(set_extra[index].pstr, &arg[iarg + 2][2]);
          set_extra[index].pvar_flag = 1;
        }
        set_extra[index].pgain = universe->numeric(FLERR, arg[iarg + 3]);
        i += 4;
        ...
        set_extra[index].pgain = universe->numeric(FLERR, arg[iarg + 3]);
        i += 4;
      } else error->all(FLERR, "Illegal fix deform/pressure command");
      ...
      if (strcmp(arg[iarg + 1], "pressure") == 0) {
        ...
        if (iarg+3 > narg) error->all(FLERR, "Illegal fix deform/pressure command");
        set[index].style = ERATERS;
        set[index].rate = universe->numeric(FLERR, arg[iarg + 2]);
        i += 3;
      } else error->all(FLERR, "Illegal fix deform/pressure command");
      ...
      if (strcmp(arg[iarg + 1], "volume") == 0) {
        ...
        if (strstr(arg[iarg + 2], "v_") != arg[iarg + 2]) {
          set_extra[6].ptarget = universe->numeric(FLERR, arg[iarg + 2]);
        } else {
          strcpy(set_extra[6].pstr, &arg[iarg + 2][2]);
          set_extra[6].pvar_flag = 1;
        }
        set_extra[6].pgain = universe->numeric(FLERR, arg[iarg + 3]);
        i += 4;
      } else error->all(FLERR, "Illegal fix deform/pressure command");
    } else break;
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (set[other1].style == NONE || set[other1].style == VOLUME)
      if (set[other2].style == NONE || set[other2].style == VOLUME)
        error->all(FLERR, "Fix deform/pressure volume setting is invalid");
  ...
  if ((!force_reneighbor) && flipflag && (set[3].style || set[4].style || set[5].style)) {
    force_reneighbor = 1;
    irregular = new IrregularComm(cac);
  }
...
void FixDeformPressure::init()
{
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (set_extra[i].pvar < 0)
      error->all(FLERR, "Variable name for fix deform/pressure does not exist");
    if (!input->variable->equalstyle(set_extra[i].pvar))
      error->all(FLERR, "Variable for fix deform/pressure is invalid style");
  }
  ...
  if (pressure_flag) {
    ...
    if (icompute < 0)
      error->all(FLERR, "Temperature ID for fix deform/pressure does not exist");
    temperature = modify->compute[icompute];

    icompute = modify->find_compute(id_press);
    if (icompute < 0)
      error->all(FLERR, "Pressure ID for fix deform/pressure does not exist");
  ...
  if (set[3].style && set[5].style) {
    if (flipflag && set[3].style == PRESSURE)
      error->all(FLERR, "Fix deform/press cannot use yz pressure with xy");
    if (flipflag && set[3].style == ERATERS)
      error->all(FLERR, "Fix deform/press cannot use yz erate/rescale with xy");
  }
}
...
void FixDeformPressure::options(int i, int narg, char **arg)
{
  ...
  while (i < (int) leftover_iarg.size()) {
    ...
    if (strcmp(arg[iarg], "couple") == 0) {
      ...
      else error->all(FLERR, "Illegal fix deform/pressure couple command");
      i += 2;
    } else if (strcmp(arg[iarg], "max/rate") == 0) {
      if (iarg + 2 > narg) error->all(FLERR,"Illegal fix deform/pressure command");
      max_h_rate = universe->numeric(FLERR, arg[iarg + 1]);
      if (max_h_rate <= 0.0)
        error->all(FLERR, "Maximum strain rate must be a positive, non-zero value");
      i += 2;
    } else if (strcmp(arg[iarg], "normalize/pressure") == 0) {
      if (iarg + 2 > narg) error->all(FLERR,"Illegal fix deform/pressure command");
      if (strcmp(arg[iarg + 1], "no") == 0) normalize_pressure_flag = 0;
      else if (strcmp(arg[iarg + 1], "yes") == 0) normalize_pressure_flag = 1;
      i += 2;
    } else if (strcmp(arg[iarg], "vol/balance/p") == 0) {
      if (iarg + 2 > narg) error->all(FLERR,"Illegal fix deform/pressure command");
      if (strcmp(arg[iarg + 1], "no") == 0) vol_balance_flag = 0;
      else if (strcmp(arg[iarg + 1], "yes") == 0) vol_balance_flag = 1;
      i += 2;
    } else error->all(FLERR, "Illegal fix deform/pressure command");
  }
}
...
int FixDeformPressure::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0], "temp") == 0) {
    ...
    icompute = modify->find_compute(arg[1]);
    if (icompute < 0) error->all(FLERR, "Could not find fix_modify temperature compute ID");
    temperature = modify->compute[icompute];

    if (temperature->tempflag == 0)
      error->all(FLERR, "Fix_modify temperature compute does not compute temperature");
    if (temperature->igroup != 0 && comm->me == 0)
      error->warning(FLERR, "Temperature compute for fix deform/pressure is not for group all");

    // reset id_temp of pressure to new temperature ID

    icompute = modify->find_compute(id_press);
    if (icompute < 0)
      error->all(FLERR, "Pressure compute ID for fix deform/press does not exist");
    ...
    icompute = modify->find_compute(arg[1]);
    if (icompute < 0) error->all(FLERR, "Could not find fix_modify pressure compute ID");
    pressure = modify->compute[icompute];
