...
void WriteData::command(int narg, char **arg)
{
  ...
  //pairflag = II;
  //coeffflag = 1;
  velocityflag = elementflag = atomflag = massflag = 1;
  max_nctypes = 0;
  int compresstypeflag = 0;
  int noinit = 0;
  ...
  while (iarg < narg) {
    ...
    //  coeffflag = 0;
     //  iarg++;
      ...
      atomflag = 0;
      iarg++;
    } else if (strcmp(arg[iarg], "nomass") == 0) {
      massflag = 0;
      iarg++;
    } else if (strcmp(arg[iarg], "compress/atom/type") == 0) {
      compresstypeflag = 1;
      iarg++;
  ...
  if (compresstypeflag) {
    int my_max_nctypes = 0;
    int *type = atom->type;
    int **ctype = element->ctype;
    int *etype = element->etype;
    int *apc = element->apc;
    for (int i = 0; i < atom->nlocal; i++)
      my_max_nctypes = MAX(my_max_nctypes, type[i]);
    for (int i = 0; i < element->nlocal; i++)
      for (int j = 0; j < apc[etype[i]]; j++)
        my_max_nctypes = MAX(my_max_nctypes, ctype[i][j]);
    MPI_Allreduce(&my_max_nctypes, &max_nctypes, 1, MPI_INT, MPI_MAX, world);
  }
...
void WriteData::header()
{
  ...
  if (atom->natoms && atomflag) fprintf(fp, BIGINT_FORMAT " atoms\n", atom->natoms);
