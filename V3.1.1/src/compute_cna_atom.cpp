...
void ComputeCNAAtom::compute_peratom()
{
  ...
  double rcutfccsq, rcutbccsq;
  double rscalefcc = (3.0 + sqrt(8.0)) / 576.0;
  double rscalebcc = (3.0 + sqrt(8.0)) / 784.0;
  ...
  if (adapt_cut_flag) {
    for (ii = 0; ii < inum; ii++) {
      ...
      // skip if atom I has less than 12 neighbors
      // or not in group
      ...
      *patternptr = OTHER;
      ...
      // average nearest neighbor distance to find rcut for CNA calculation (see OVITO doc)
      // NOTE: Eq. (7) in doi:10.1088/0965-0393/20/4/045021 is wrong. Here is the correct equation:
      //       rcutbcc = (1 + sqrt(2)) / 2 * [ 2/sqrt(3) * sum(r_j)_1^8 + sum(r_j)_9^14 ] / 14
      ...
      for (jj = 0; jj < num; jj++) {
        ...
        if (bccflag) {
          if (jj < 8) rcutbccsq += r * sqrt(4.0/3.0);
          else rcutbccsq += r;
        }
        ncommonfcc[jj] = 0;
        ncommonbcc[jj] = 0;
      ...
      if (bccflag) 
        rcutbccsq = rcutbccsq * rcutbccsq * rscalebcc;
