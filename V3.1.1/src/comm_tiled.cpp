/*  Original code from LAMMPS CAC comm_cac.cpp by Adrian Diaz - University of Florida 
 * Adopted and optimized by Thanh Phan - Iowa State University
 * */
...
void CommTiled::init_buffers()
{
  ...
  atom_sendnum = atom_recvnum = nullptr;
  atom_size_forward_recv = atom_firstrecv = nullptr;
  atom_size_reverse_send = atom_size_reverse_recv = nullptr;
  atom_forward_recv_offset = atom_reverse_recv_offset = nullptr;
  elem_sendnum = elem_recvnum = nullptr;
  elem_size_forward_recv = elem_firstrecv = nullptr;
  elem_size_reverse_send = elem_size_reverse_recv = nullptr;
  elem_forward_recv_offset = elem_reverse_recv_offset = nullptr;

  pbc_flag = nullptr;
  pbc = nullptr;
  minsendbox = nullptr;
  minsendbox_flag = nullptr;
  maxsendbox = nullptr;
  atom_maxsendlist = nullptr;
  atom_sendlist = nullptr;
  elem_maxsendlist = nullptr;
  elem_sendlist = nullptr;
  ...
  overlap_sendnum = nullptr;
  overlap_recvnum = nullptr;
  overlap_maxsendlist = nullptr;
  overlap_firstrecv = nullptr;
  overlap_sendlist = nullptr;

  maxforeign = nforeign = 0;
  foreign_overlap = nullptr;
  foreign_tag = nullptr;
  foreign_boxes = nullptr;
  ...
  memory->create(overlap_repeat, nprocs, 11, "comm:overlap_repeat");
...
/*  ----------------------------------------------------------------------
   ...
   wont be send as neighbor (if out of minsendbox and within maxsendbox, check 
   with foreign box to see if need to send as ghost)
...
{
  if (debug) debug_write_domain_box();
  int i, j, m, n, dim;

  // distance scaling factors for triclinic box 
  // = 1 if not triclinic 

  double length_scaling[3];
  if (triclinic) {
    ...
    length_scaling[0] = sqrt(h_inv[0] * h_inv[0] + h_inv[5] * h_inv[5] + h_inv[4] * h_inv[4]);
    length_scaling[1] = sqrt(h_inv[1] * h_inv[1] + h_inv[3] * h_inv[3]);
    length_scaling[2] = h_inv[2];
  } else {
    prd = domain->prd;
    boxlo = domain->boxlo;
    boxhi = domain->boxhi;
    ...
    length_scaling[0] = length_scaling[1] = length_scaling[2] = 1.0;
  }  
  ...
  if (layout != Comm::LAYOUT_TILED) {
    if (triclinic)
      error->all(FLERR, "comm_style tiled with triclinic box only work with non-uniform layout (use fix balance)");
  ...
  // check that cutoff < any periodic box length_scaling
  ...
  cutghost[0] = MAX(cut, cutghostuser[0]) * length_scaling[0];
  cutghost[1] = MAX(cut, cutghostuser[1]) * length_scaling[1];
  cutghost[2] = MAX(cut, cutghostuser[2]) * length_scaling[2];
  ...
  if ((periodicity[0] && cutghost[0] > prd[0]) ||
    ...
    error->all(FLERR, "Communication cutoff for comm_style tiled "
        "cannot exceed periodic box length_scaling");
  }
  ...
  // for non-tiled layouts, check if domain size is larger than element bound box

  if (layout != Comm::LAYOUT_TILED) {
    double subbox[3];
    double subbox_min[3];
    double max_elem_box[3];
    subbox[0] = subhi[0] - sublo[0];
    subbox[1] = subhi[1] - sublo[1];
    subbox[2] = subhi[2] - sublo[2];
    MPI_Allreduce(subbox, subbox_min, 3, MPI_DOUBLE, MPI_MIN, world);
    MPI_Allreduce(element->max_element_bound_box_size, max_elem_box, 3, MPI_DOUBLE, MPI_MAX, world);
    if (comm->me == 0) {
      if (subbox_min[0]/DMFACTOR < max_elem_box[0])
        error->warning(FLERR, "Proc domain size in x is smaller than element bound box, "
            "which could lead to missing neighbors. Consider reconfigurate the proc "
            "3D grid through processors command, reducing number of processors, "
            "or use comm_tiled with fix balance");
      if (subbox_min[1]/DMFACTOR < max_elem_box[1])
        error->warning(FLERR, "Proc domain size in y is smaller than element bound box, "
            "which could lead to missing neighbors. Consider reconfigurate the proc "
            "3D grid through processors command, reducing number of processors, "
            "or use comm_tiled with fix balance");
      if (subbox_min[2]/DMFACTOR < max_elem_box[2])
        error->warning(FLERR, "Proc domain size in z is smaller than element bound box, "
            "which could lead to missing neighbors. Consider reconfigurate the proc "
            "3D grid through processors command, reducing number of processors, "
            "or use comm_tiled with fix balance");
    }
  ...
  double *max_elem_box_size = element->max_element_bound_box_size;
  int limit_flag[3];

  for (dim = 0; dim < dimension; dim++) {
    MPI_Allreduce(&max_elem_box_size[dim], &max_cutghost[dim], 1, MPI_DOUBLE, MPI_MAX, world);
    max_cutghost[dim] += cutghost[dim];

    // limit max_cutghost to 3 * prd
    if (max_cutghost[dim] >= 3 * prd[dim]) {
      max_cutghost[dim] = 3 * prd[dim];
      limit_flag[dim] = 1;
    } else {
      limit_flag[dim] = 0;
    }
  ...
  // done by intersecting 26 ghost boxes (and their periodic images) with all proc sub-boxes it overlaps
  // sets noverlap, overlap
  // sets overlapother, overlelf, pbc_flag, pbc
  // resets nprocmax
  //***************************************************************************

  // setup the ghost box "one" grid
  // +/- BOX_EPSILON to account for rounding off error when max_cutghost is limit to prd

  double box_grid[3][4];
  for (dim = 0; dim < dimension; dim++) {
    box_grid[dim][0] = sublo[dim] - max_cutghost[dim] + BOX_EPSILON * limit_flag[dim] * length_scaling[dim];
    box_grid[dim][1] = sublo[dim];
    box_grid[dim][2] = subhi[dim];
    box_grid[dim][3] = subhi[dim] + max_cutghost[dim] - BOX_EPSILON * limit_flag[dim] * length_scaling[dim];
  }

  if (dimension == 2)
    box_grid[2][0] = box_grid[2][1] = box_grid[2][2] = box_grid[2][3] = sublo[2];


  // one = first ghost box in same periodic image
  // two = second ghost box (images of box "one") wrapped across periodic boundary
  // either may not exist
  // there can be multiple second ghost box (maximum of 343 for each "one" box)

  int indexme;
  double lo1[3], hi1[3], lo2[MAXN2BOX][3], hi2[MAXN2BOX][3];
  int current_pbc[3], current_pbc_flag[3];
  int pbc2[MAXN2BOX][3]; // pbc flag for second ghost box set
  int one, two;
  ...
  for (i = 0; i < nprocs; i++) {
    for (j = 0; j < 11; j++)
      overlap_repeat[i][j] = 0;
  }

  // loop over ghost box grids
  // NOTE: this does not works with 2D yet
  if (domain->dimension == 2) error->all(FLERR, "Comm_tiled does not work with 2D yet");
  ...
  for (int ix = 0; ix < 3; ix ++) {
    for (int iy = 0; iy < 3; iy ++) {
      for (int iz = 0; iz < 3; iz ++) {
        ...
        for (dim = 0; dim < dimension; dim++) {
          if (periodicity[dim]) {
            if (lo1[dim] < boxlo[dim] - 2 * prd[dim]) { 
              current_pbc[dim] = 3;
            } else if (lo1[dim] < boxlo[dim] - prd[dim]) {
              current_pbc[dim] = 2;
            } else if (lo1[dim] < boxlo[dim]) {
              current_pbc[dim] = 1;
            }
            if (hi1[dim] > boxhi[dim] + 2 * prd[dim]) {
              current_pbc[dim] = -3;
            } else if (hi1[dim] > boxhi[dim] + prd[dim]) {
              current_pbc[dim] = -2;
            } else if (hi1[dim] > boxhi[dim]) {
              current_pbc[dim] = -1;
            }
        ...
        // compute how many and which images of box "one" to test for overlap

        if (two) {
          for (int pbx = 0; pbx <= abs(current_pbc[0]); pbx++)
            for (int pby = 0; pby <= abs(current_pbc[1]); pby++)
              for (int pbz = 0; pbz <= abs(current_pbc[2]); pbz++) {
                if (pbx == 0 && pby == 0 && pbz == 0) continue;
                copy3(lo1, lo2[n2box]);
                copy3(hi1, hi2[n2box]);

                pbc2[n2box][0] = pbx * current_pbc_flag[0];
                pbc2[n2box][1] = pby * current_pbc_flag[1];
                pbc2[n2box][2] = pbz * current_pbc_flag[2];
                for (dim = 0; dim < dimension; dim++) {
                  lo2[n2box][dim] += prd[dim] * pbc2[n2box][dim];
                  hi2[n2box][dim] += prd[dim] * pbc2[n2box][dim];

                  // limit "two" box to domain box
                ...
                if (lo2[n2box][0] == hi2[n2box][0] ||
                    ...
                    lo2[n2box][2] == hi2[n2box][2]) continue;
                n2box++;
              }
        }
        ...
        if (one) {
          for (dim = 0; dim < dimension; dim++) {
            ...
            if (hi1[dim] == lo1[dim]) one = 0;
          }
        }

        // find overlap from ghost box "one" and "two"

        int noverlap_old = noverlap;
        if (one) (this->*box_drop)(0, lo1, hi1, nullptr, indexme, 1);

        if (two) 
          for (i = 0; i < n2box; i++) 
            (this->*box_drop)(0, lo2[i], hi2[i], pbc2[i], indexme, 2);
//        if (comm->me == 3){
//          printf("ixyz = %d %d %d overlap =",ix,iy,iz);
//        for (m = noverlap_old; m < noverlap;m++)
//          printf(" %d",overlap[m]);
//        printf("\n");
//            }
  ...
  if (noverlap > nprocmax) {
    int oldmax = nprocmax;
    while (nprocmax < noverlap) nprocmax += DELTA_PROCS;
  ...
  if (noverlap && noverlap-overlapself) overlapother = 1;
  else overlapother = 0;

  //*************************************************************************************
  // 2. Compute minsendbox for each of my overlap, 
  // this is the absolute send box (elements/atoms overlap this minsendbox is sent to other proc)
  // obox = other proc subdomain box
  // sbox = minimum box I need to send to other proc
  //*************************************************************************************
  ...
  if (mode == Comm::SINGLE) {
    for (i = 0; i < noverlap; i++) {
      minsendbox_flag[i] = 1;
      pbc_flag[i] = 0;
      pbc[i][0] = pbc[i][1] = pbc[i][2] =
        pbc[i][3] = pbc[i][4] = pbc[i][5] = 0;
      if (overlap_pbc[i][0] || overlap_pbc[i][1] || overlap_pbc[i][2]) {
        ...
        if (triclinic) {
          if (overlap_pbc[i][1]) pbc[i][5] = overlap_pbc[i][1];
          if (overlap_pbc[i][2]) pbc[i][3] = pbc[i][4] = overlap_pbc[i][2];
        } 
      }
      ...
      for (dim = 0; dim < dimension; dim++) {
        ...
        if (oboxlo[dim] <= sublo[dim]) {
          sbox[dim] = sublo[dim];
          if (sbox[3+dim] <= sbox[dim]) sbox[3+dim] = sbox[dim];
        } else {
          sbox[dim] = oboxlo[dim];
          if (sbox[dim] >= sbox[3+dim]) sbox[dim] = sbox[3+dim];
        }
      }

      // determine if minsendbox has zero thickness due to a lack of overlap with force cutoff radius

      for (dim = 0; dim < dimension; dim++) {
        if (sbox[dim] == sbox[dim+3]) minsendbox_flag[i] = 0;
      }
      memcpy(minsendbox[i], sbox, 6 * sizeof(double));
    }
  } else {
    error->all(FLERR,"Multi comm style not supported yet");
  }
  ...
  // send my expanded subbox and extend it by cutghost to other procs as my maxsendbox
  //*************************************************************************************
  ...
  //printf("me = %d nother = %d overlapother = %d count = ",me,nother,overlapother);
  //int count[8];
  ...
  for (m = 0; m < nother; m++)
    MPI_Irecv(&maxsendbox[m][0], 6, MPI_DOUBLE, overlap[m], 0, world, &requests[m]);
  for (m = 0; m < nother; m++)
  ...
  for (m = 0; m < noverlap; m++) {
    ...
    if (expand_subbox[0] > oboxhi[0] || expand_subbox[3] < oboxlo[0]) continue;
    if (expand_subbox[1] > oboxhi[1] || expand_subbox[4] < oboxlo[1]) continue;
    if (expand_subbox[2] > oboxhi[2] || expand_subbox[5] < oboxlo[2]) continue;
    ...
    n = 0;
    for (i = 0; i < nelocal; i++) {
      if (element_bound_box[i][0] > oboxhi[0] || element_bound_box[i][3] < oboxlo[0]) continue;  
      if (element_bound_box[i][1] > oboxhi[1] || element_bound_box[i][4] < oboxlo[1]) continue;  
      if (element_bound_box[i][2] > oboxhi[2] || element_bound_box[i][5] < oboxlo[2]) continue;  
  ...
  for (m = 0; m < nother; m++)
    MPI_Irecv(&overlap_recvnum[m], 1, MPI_INT, overlap[m], 1, world, &requests[m]);
  for (m = 0; m < nother; m++)
  ...
  if (max_send_size > maxsend) grow_send(max_send_size, 0);
  if (total_recv_size > maxrecv) grow_recv(total_recv_size);
  ...
  for (m = 0; m < nother; m++) 
    if (overlap_recvnum[m])
      MPI_Irecv(&buf_recv[overlap_firstrecv[m] * OVERLAP_SIZE], 
          overlap_recvnum[m] * OVERLAP_SIZE, MPI_DOUBLE, overlap[m], 2, world, &requests[m]);
  ...
  for (m = 0; m < nother; m++)
    if (overlap_sendnum[m]) {
      pack_element_bound_box(overlap_sendnum[m], m, overlap_sendlist[m], buf_send);
  ...
  for (m = 0; m < nother; m++) 
    if (overlap_recvnum[m]) 
      unpack_element_bound_box(overlap_recvnum[m], overlap_firstrecv[m], &buf_recv[overlap_firstrecv[m] * OVERLAP_SIZE], m);
  ...
  for (m = nother; m < noverlap; m++) 
    if (overlap_sendnum[m]) {
      pack_element_bound_box(overlap_sendnum[m], m, overlap_sendlist[m], buf_send);
      unpack_element_bound_box(overlap_recvnum[m], overlap_firstrecv[m], buf_send, m);
    }

  if (debug) debug_write_foreign_box(1);
...
{
  double cutx = (maxfbox[0] + element->max_element_bound_box_size[0]) / 2 + cutghost[0];
  double cuty = (maxfbox[1] + element->max_element_bound_box_size[1]) / 2 + cutghost[1];
  double cutz = (maxfbox[2] + element->max_element_bound_box_size[2]) / 2 + cutghost[2];

  int sx = static_cast<int> (cutx * bininvx);
  if (sx * binsizex < cutx) sx++;
  int sy = static_cast<int> (cuty * bininvy);
  if (sy * binsizey < cuty) sy++;
  int sz;
  if (dimension == 3) {
    sz = static_cast<int> (cutz * bininvz);
    if (sz * binsizez < cutz) sz++;
  } else sz = 0;
  ...
  if (dimension == 3) {
    for (int k = -sz; k <= sz; k++)
      for (int j = -sy; j <= sy; j++)
        for (int i = -sx; i <= sx; i++)
          if (bin_distance(i, 0) < cutx &&
              bin_distance(j, 1) < cuty &&
...
double CommTiled::bin_distance(int i, int dim)
{
  double distance;
  double binsize;
  if (dim == 0) binsize = binsizex;
  if (dim == 1) binsize = binsizey;
  if (dim == 2) binsize = binsizez;
...
void CommTiled::setup_bins() 
{
  ...
  tmp = MAX((maxfbox[0] + element->max_element_bound_box_size[0]) / 2.0 + cutghost[0],
            (maxfbox[1] + element->max_element_bound_box_size[1]) / 2.0 + cutghost[1]);
  tmp = MAX((maxfbox[2] + element->max_element_bound_box_size[2]) / 2.0 + cutghost[2], tmp);
  ...
  if (nforeign) {
    ...
    double binsizeinv = 2.0/binsize_optimal; // binsize_optimal is doubled here hence the 2.0
    ...
    if (bbin > MAXSMALLINT) error->one(FLERR, "Too many foreign bins");
    mbins = bbin;
...
int CommTiled::pack_element_bound_box(int n, int overlap_index, 
    int *list, double *buf)
{
  int i, j, m;
  double dx, dy, dz;
...
int CommTiled::unpack_element_bound_box(int n, int first, double *buf, int overlap_index)
{
  int i, j, m, last;
  ...
  for (i = first; i < last; i++) {
    ...
    if (nforeign == maxforeign) grow_foreign_box();
    ...
    // determine which overlap I need to send atoms/elements overlaping with this foreign box
    ...
    if (foreign_overlap[i] < 0) {
      error->one(FLERR, "Cannot find which overlap this foreign box is relevant to");
    }
...
void CommTiled::setup_exchange()
{
  ...
  // determine which procs I will send to and receive from in each exchange direction
  // done by intersecting epsilon box on each face with all proc sub-boxes it overlaps
  ...
  for (int idim = 0; idim < dimension; idim++) {
    ...
    if (periodicity[idim] || sublo[idim] != boxlo[idim]) {
      ...
      indexme = -1;
      (this->*box_drop)(idim, lo, hi, nullptr, indexme, 0);
    ...
    if (periodicity[idim] || subhi[idim] != boxhi[idim]) {
      ...
      // loop through second half of overlap list, if procs does not touch me, remove from list
      // compare with first half of overlap list to insure each proc appears exactly once
      // remove repeats
...
void CommTiled::forward_comm(int dummy)
{
  ...
  if (atom->natoms) {
    ...
    for (i = 0; i < nother; i++){
      MPI_Irecv(x[atom_firstrecv[i]], atom_size_forward_recv[i], 
          MPI_DOUBLE, overlap[i], 0, world, &requests[i]);
    }
    for (i = 0; i < nother; i++) {
      n = avec->pack_comm(atom_sendnum[i], atom_sendlist[i], 
          buf_send, pbc_flag[i], pbc[i]);
      MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
    }

    for (i = nother; i < noverlap; i++) {
      avec->pack_comm(atom_sendnum[i], atom_sendlist[i], 
          x[atom_firstrecv[i]], pbc_flag[i], 
          pbc[i]);
    }
    if (overlapother) MPI_Waitall(nother, requests, MPI_STATUS_IGNORE);
  ...
  if (element->nelements) {
    for (i = 0; i < nother; i++) {
      MPI_Irecv(&buf_recv[elem_size_forward * elem_forward_recv_offset[i]], 
          elem_size_forward_recv[i], 
          MPI_DOUBLE, overlap[i], 0, world, &requests[i]);
    }
    for (i = 0; i < nother; i++) {
      n = evec->pack_comm(elem_sendnum[i], elem_sendlist[i], 
          buf_send, pbc_flag[i], pbc[i]);
      MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
    }

    for (i = nother; i < noverlap; i++) {
      evec->pack_comm(elem_sendnum[i], elem_sendlist[i], 
          buf_send, pbc_flag[i], pbc[i]);
      evec->unpack_comm(elem_recvnum[i], elem_firstrecv[i], 
          buf_send);
    }

    for (i = 0; i < nother; i++) {
      MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
      evec->unpack_comm(elem_recvnum[irecv], elem_firstrecv[irecv], 
          &buf_recv[elem_size_forward * 
          elem_forward_recv_offset[irecv]]);
    }
...
/*  ----------------------------------------------------------------------
   reverse communication of forces on atoms/elements every timestep
    ...
    if (comm_f_only) {
      for (i = 0; i < nother; i++) {
        MPI_Irecv(&buf_recv[atom_size_reverse * atom_reverse_recv_offset[i]], 
            atom_size_reverse_recv[i], MPI_DOUBLE, 
            overlap[i], 0, world, &requests[i]);
      }
      for (i = 0; i < nother; i++)
        MPI_Send(f[atom_firstrecv[i]], atom_size_reverse_send[i], 
      ...
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        avec->unpack_reverse(atom_sendnum[irecv], atom_sendlist[irecv], 
      ...
      for (i = 0; i < nother; i++)
        MPI_Irecv(&buf_recv[atom_size_reverse * atom_reverse_recv_offset[i]], 
            ...
            overlap[i], 0, world, &requests[i]);
      for (i = 0; i < nother; i++) {
        n = avec->pack_reverse(atom_recvnum[i], atom_firstrecv[i], 
            buf_send);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
      }
      ...
      for (i = nother; i < noverlap; i++) {
        avec->pack_reverse(atom_recvnum[i], atom_firstrecv[i], 
            buf_send);
        avec->unpack_reverse(atom_sendnum[i], atom_sendlist[i], 
            buf_send);
      }
  ...
  if (element->nelements) {
    for (i = 0; i < nother; i++)
      MPI_Irecv(&buf_recv[elem_size_reverse * elem_reverse_recv_offset[i]], 
          elem_size_reverse_recv[i], MPI_DOUBLE, 
          overlap[i], 0, world, &requests[i]);
    for (i = 0; i < nother; i++) {
    ...
    for (i = nother; i < noverlap; i++) {
      evec->pack_reverse(elem_recvnum[i], elem_firstrecv[i], buf_send);
      evec->unpack_reverse(elem_sendnum[i], elem_sendlist[i], buf_send);
    }

    for (i = 0; i < nother; i++) {
      MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
      evec->unpack_reverse(elem_sendnum[irecv], elem_sendlist[irecv], 
...
/*  ----------------------------------------------------------------------
   ...
   for triclinic, atoms/elements must be in lamda coords (0-1) before exchange is called
   -------------------------------------------------------------------------  */
...
/*  ----------------------------------------------------------------------
   borders list nearby atoms/elements to send to neighboring procs at every timestep
   one list is created per overlap proc that will be made
...
void CommTiled::borders()
{
  ...
  double *hibox;                // max sendbox out of which atoms/elements are not needed as ghost
  double *coord;
  double **x;
  int nlocal, nscountall, nrcountall, nscount, nrcount;
  int *sendflag = new int[noverlap];    // flag per overlap if I will be send to that overlap
  AtomVec *avec = atom->avec;
  ElementVec *evec = element->evec;
  ...
  if (atom->natoms) {
      ...
      for (m = 0; m < noverlap; m++) {
        ...
        if (minsendbox_flag[m]) {
          ...
          if (coord[0] >= lobox[0] && coord[0] < lobox[3] &&
            ...
            if (atom_sendnum[m] == atom_maxsendlist[m]) grow_atom_list(m, atom_sendnum[m]);
      ...
      if (checkflag && nforeign) {
        ...
        for (k = 0; k < nstencil; k++) {
          ...
          for (j = fboxbinhead[ibin+stencil[k]]; j >= 0; j = fboxbins[j]) {
            ...
            if (sendflag[m] == 0) {
              if (coord[0] >= foreign_boxes[j][0] && coord[0] <= foreign_boxes[j][3] &&
                  coord[1] >= foreign_boxes[j][1] && coord[1] <= foreign_boxes[j][4] &&
                  coord[2] >= foreign_boxes[j][2] && coord[2] <= foreign_boxes[j][5]) {
                if (atom_sendnum[m] == atom_maxsendlist[m]) grow_atom_list(m, atom_sendnum[m]);
                atom_sendlist[m][atom_sendnum[m]++] = i;
                sendflag[m] = 1;
              }
    ...
    if (overlapother) {
      for (m = 0; m < nother; m++)
        MPI_Irecv(&atom_recvnum[m], 1, MPI_INT, 
            overlap[m], 0, world, &requests[m]);
      for (m = 0; m < nother; m++)
    ...
    for (m = 0; m < noverlap; m++) {
      ...
      if (m == 0) atom_reverse_recv_offset[0] = 0;
      else atom_reverse_recv_offset[m] =
    ...
    if (overlapother) {
      for (m = 0; m < nother; m++)
        MPI_Irecv(&buf_recv[atom_size_border * atom_forward_recv_offset[m]], 
            atom_recvnum[m] * atom_size_border, 
            MPI_DOUBLE, overlap[m], 0, world, &requests[m]);
      for (m = 0; m < nother; m++) {
        n = avec->pack_border(atom_sendnum[m], atom_sendlist[m], 
            buf_send, pbc_flag[m], pbc[m]);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[m], 0, world);
      }
    }
    if (overlapself) {
      for (m = nother; m < noverlap; m++) {
        avec->pack_border(atom_sendnum[m], atom_sendlist[m], 
            buf_send, pbc_flag[m], pbc[m]);
    ...
    if (overlapother) {
      MPI_Waitall(nother, requests, MPI_STATUS_IGNORE);
      for (m = 0; m < nother; m++)
        avec->unpack_border(atom_recvnum[m], atom_firstrecv[m], 
    ...
    if (noverlap)
      atom->nghost += atom_forward_recv_offset[noverlap-1] + atom_recvnum[noverlap-1];
  ...
  if (element->nelements) {
    ...
    for (i = 0; i < nlocal; i++) {
      ...
      for (m = 0; m < noverlap; m++) {
        ...
        if (coord[3] >= lobox[0] && coord[0] <= lobox[3] &&
          ...
          if (elem_sendnum[m] == elem_maxsendlist[m]) grow_elem_list(m, elem_sendnum[m]);
      ...
      if (checkflag && nforeign) {
        ...
        for (k = 0; k < nstencil; k++) {
          for (j = fboxbinhead[ibin+stencil[k]]; j >= 0; j = fboxbins[j]) {
            m = foreign_overlap[j];
            if (sendflag[m] == 0) {
              if (coord[3] >= foreign_boxes[j][0] && coord[0] <= foreign_boxes[j][3] &&
                  coord[4] >= foreign_boxes[j][1] && coord[1] <= foreign_boxes[j][4] &&
                  coord[5] >= foreign_boxes[j][2] && coord[2] <= foreign_boxes[j][5]) {
                if (elem_sendnum[m] == elem_maxsendlist[m]) grow_elem_list(m, elem_sendnum[m]);
                elem_sendlist[m][elem_sendnum[m]++] = i;
                sendflag[m] = 1;
              }
    ...
    if (overlapother) {
      for (m = 0; m < nother; m++)
        MPI_Irecv(&elem_recvnum[m], 1, MPI_INT, 
            overlap[m], 0, world, &requests[m]);
      for (m = 0; m < nother; m++)
    ...
    for (m = 0; m < noverlap; m++) {
      ...
      if (m == 0) elem_reverse_recv_offset[0] = 0;
      else elem_reverse_recv_offset[m] =
    ...
    if (overlapother) {
      for (m = 0; m < nother; m++)
        MPI_Irecv(&buf_recv[elem_size_border * elem_forward_recv_offset[m]], 
            elem_recvnum[m] * elem_size_border, 
            MPI_DOUBLE, overlap[m], 0, world, &requests[m]);
      for (m = 0; m < nother; m++) {
        n = evec->pack_border(elem_sendnum[m], elem_sendlist[m], 
            buf_send, pbc_flag[m], pbc[m]);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[m], 0, world);
      }
    }
    if (overlapself) {
      for (m = nother; m < noverlap; m++) {
        evec->pack_border(elem_sendnum[m], elem_sendlist[m], 
            buf_send, pbc_flag[m], pbc[m]);
        evec->unpack_border(elem_recvnum[m], elem_firstrecv[m], 
            buf_send);
      }
    }
    ...
    if (noverlap)
      element->nghost += elem_forward_recv_offset[noverlap-1] + elem_recvnum[noverlap-1];
...
void CommTiled::forward_comm_pair(Pair *pair)
{
  ...
  if (atom->natoms) {
    ...
    for (i = nother; i < noverlap; i++) {
      pair->pack_atom_forward_comm(atom_sendnum[i], atom_sendlist[i], 
          buf_send, pbc_flag[i], pbc[i]);
      pair->unpack_atom_forward_comm(atom_recvnum[i], atom_firstrecv[i], buf_send);
    }

    for (i = 0; i < nother; i++) {
      MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
      pair->unpack_atom_forward_comm(atom_recvnum[irecv], atom_firstrecv[irecv], 
          &buf_recv[nsize * 
  ...
  if (element->nelements) {
    ...
    for (i = nother; i < noverlap; i++) {
      pair->pack_elem_forward_comm(elem_sendnum[i], elem_sendlist[i], 
          buf_send, pbc_flag[i], pbc[i]);
    ...
    for (i = 0; i < nother; i++) {
      MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
      pair->unpack_elem_forward_comm(elem_recvnum[irecv], elem_firstrecv[irecv], 
    ...
    if (nsize) {
      for (i = 0; i < nother; i++)
        MPI_Irecv(&buf_recv[nsize * atom_reverse_recv_offset[i]], 
            ...
            overlap[i], 0, world, &requests[i]);
      for (i = 0; i < nother; i++) {
        n = pair->pack_atom_reverse_comm(atom_recvnum[i], atom_firstrecv[i], buf_send);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
      }

      for (i = nother; i < noverlap; i++) {
        pair->pack_atom_reverse_comm(atom_recvnum[i], atom_firstrecv[i], buf_send);
        pair->unpack_atom_reverse_comm(atom_sendnum[i], atom_sendlist[i], buf_send);
      }

      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        pair->unpack_atom_reverse_comm(atom_sendnum[irecv], atom_sendlist[irecv], 
    ...
    if (nsize) {
      for (i = 0; i < nother; i++)
        MPI_Irecv(&buf_recv[nsize * elem_reverse_recv_offset[i]], 
            nsize * elem_sendnum[i], MPI_DOUBLE, 
            overlap[i], 0, world, &requests[i]);
      for (i = 0; i < nother; i++) {
        n = pair->pack_elem_reverse_comm(elem_recvnum[i], elem_firstrecv[i], buf_send);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
      }

      for (i = nother; i < noverlap; i++) {
        pair->pack_elem_reverse_comm(elem_recvnum[i], elem_firstrecv[i], buf_send);
        pair->unpack_elem_reverse_comm(elem_sendnum[i], elem_sendlist[i], buf_send);
      }

      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        pair->unpack_elem_reverse_comm(elem_sendnum[irecv], elem_sendlist[irecv], 
    ...
    if (nsize) { 
      ...
      for (i = nother; i < noverlap; i++) {
        compute->pack_atom_forward_comm(atom_sendnum[i], atom_sendlist[i], 
            buf_send, pbc_flag[i], 
            pbc[i]);
        compute->unpack_atom_forward_comm(atom_recvnum[i], 
            atom_firstrecv[i], buf_send);
      }
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        compute->unpack_atom_forward_comm(atom_recvnum[irecv], atom_firstrecv[irecv], 
    ...
    if (nsize) { 
      ...
      for (i = nother; i < noverlap; i++) {
        compute->pack_elem_forward_comm(elem_sendnum[i], elem_sendlist[i], 
            buf_send, pbc_flag[i], 
            pbc[i]);
        compute->unpack_elem_forward_comm(elem_recvnum[i], 
            elem_firstrecv[i], buf_send);
      }
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        compute->unpack_elem_forward_comm(elem_recvnum[irecv], elem_firstrecv[irecv], 
            &buf_recv[nsize * elem_forward_recv_offset[irecv]]);
      }
    }
  }
    ...
    if (nsize) {
      for (i = 0; i < nother; i++)
        MPI_Irecv(&buf_recv[nsize * atom_reverse_recv_offset[i]], 
            ...
            overlap[i], 0, world, &requests[i]);
      for (i = 0; i < nother; i++) {
        n = compute->pack_atom_reverse_comm(atom_recvnum[i], atom_firstrecv[i], buf_send);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
      }

      for (i = nother; i < noverlap; i++) {
        compute->pack_atom_reverse_comm(atom_recvnum[i], atom_firstrecv[i], buf_send);
        compute->unpack_atom_reverse_comm(atom_sendnum[i], atom_sendlist[i], buf_send);
      }
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        compute->unpack_atom_reverse_comm(atom_sendnum[irecv], atom_sendlist[irecv], 
            &buf_recv[nsize * atom_reverse_recv_offset[irecv]]);
      }
    }
  }
    ...
    if (nsize) {
      for (i = 0; i < nother; i++)
        MPI_Irecv(&buf_recv[nsize * elem_reverse_recv_offset[i]], 
            ...
            overlap[i], 0, world, &requests[i]);
      for (i = 0; i < nother; i++) {
        n = compute->pack_elem_reverse_comm(elem_recvnum[i], elem_firstrecv[i], buf_send);
        MPI_Send(buf_send, n, MPI_DOUBLE, overlap[i], 0, world);
      }
      ...
      for (i = nother; i < noverlap; i++) {
        compute->pack_elem_reverse_comm(elem_recvnum[i], elem_firstrecv[i], 
            buf_send);
        compute->unpack_elem_reverse_comm(elem_sendnum[i], elem_sendlist[i], buf_send);
      }
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        compute->unpack_elem_reverse_comm(elem_sendnum[irecv], elem_sendlist[irecv], 
            &buf_recv[nsize * elem_reverse_recv_offset[irecv]]);
      }
    }
  }
    ...
    if (nsize) {
      ...
      for (i = nother; i < noverlap; i++) {
        dump->pack_atom_forward_comm(atom_sendnum[i], atom_sendlist[i], 
            buf_send, pbc_flag[i], pbc[i]);
        dump->unpack_atom_forward_comm(atom_recvnum[i], 
            atom_firstrecv[i], buf_send);
      }
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        dump->unpack_atom_forward_comm(atom_recvnum[irecv], atom_firstrecv[irecv], 
            &buf_recv[nsize * atom_forward_recv_offset[irecv]]);
      }
    }
  }
    ...
    if (nsize) {
      ...
      for (i = nother; i < noverlap; i++) {
        dump->pack_elem_forward_comm(elem_sendnum[i], elem_sendlist[i], 
            buf_send, pbc_flag[i], pbc[i]);
        dump->unpack_elem_forward_comm(elem_recvnum[i], 
            elem_firstrecv[i], buf_send);
      }
      for (i = 0; i < nother; i++) {
        MPI_Waitany(nother, requests, &irecv, MPI_STATUS_IGNORE);
        dump->unpack_elem_forward_comm(elem_recvnum[irecv], elem_firstrecv[irecv], 
            &buf_recv[nsize * elem_forward_recv_offset[irecv]]);
      }
    }
  }
...
void CommTiled::forward_comm_atom_array(int nsize, double **array)
{
  error->all(FLERR, "Forward comm atom array not yet supported by CommTiled");
...
/*  ----------------------------------------------------------------------
    forward communication of Nsize values in per-elem array
    -------------------------------------------------------------------------  */

void CommTiled::forward_comm_elem_array(int nsize, double **array)
{
  error->all(FLERR, "Forward comm elem array not yet supported by CommTiled");
  //  int i, j, k, m, iatom, last, irecv, nsend, nrecv;
  //
  //  // insure send/recv bufs are big enough for nsize
  //  // based on smaxone/rmaxall from most recent borders() invocation
...
void CommTiled::box_drop_brick(int idim, double *lo, double *hi, int *current_pbc, int &indexme, int flag)
{
    ...
    for (int x = xi[0]; x <= xf[0]; x++) {
      for (int y = xi[1]; y <= xf[1]; y++) {
        for (int z = xi[2]; z <= xf[2]; z++) {
          for (int dim = 0; dim < dimension; dim++) {
            int split_index;
            if (dim == 0) split_index = x;
            if (dim == 1) split_index = y;
            if (dim == 2) split_index = z;
            procbox_lo[dim] = boxlo[dim] + prd[dim] * split_array[dim][split_index];
            if (split_index < procgrid[dim]-1)
              procbox_hi[dim] = boxlo[dim] + prd[dim] * split_array[dim][split_index+1];
            else procbox_hi[dim] = boxhi[dim];
          }
          proc = grid2proc[x][y][z];

          // skip repeats from overlap

          int pbc_bit, pbc_index;
          if (flag == 1) {
            if (proc == me) return;
            pbc_bit = 171;
          } else {
            pbc_bit = ((current_pbc[0] + 3) + 
                   7 * (current_pbc[1] + 3) +
                  49 * (current_pbc[2] + 3));
          }
          pbc_index = pbc_bit / 32;
          pbc_bit %= 32;
          if (!(overlap_repeat[proc][pbc_index] & (1<<pbc_bit))) {
            overlap_repeat[proc][pbc_index] += 1<<pbc_bit;
          } else return;

          if (noverlap >= maxoverlap) grow_overlap_array();
...
void CommTiled::box_drop_tiled(int idim, double *lo, double *hi, int *current_pbc, int &indexme, int flag)
{
...
void CommTiled::box_drop_tiled_recurse(double *lo, double *hi, 
    int proclower, int procupper, int *current_pbc, 
    int &indexme, int flag)
{
  ...
  if (proclower == procupper) {
    ...
    if (flag) {
      ...
      pbc_index = pbc_bit / 32;
      pbc_bit %= 32;
      if (!(overlap_repeat[proclower][pbc_index] & (1<<pbc_bit))) {
        overlap_repeat[proclower][pbc_index] += 1<<pbc_bit;
      } else return;
  ...
  // use > and < criteria and add +/- EPSILON so does not include a box it only touches
  ...
  if (lo[idim] < cut)
    box_drop_tiled_recurse(lo, hi, proclower, procmid-1, current_pbc, indexme, flag);
  if (hi[idim] > cut)
    box_drop_tiled_recurse(lo, hi, procmid, procupper, current_pbc, indexme, flag);
}
...
void CommTiled::box_other_brick(int idim, int idir, 
    int proc, double *lo, double *hi, int overlap_counter)
{
...
int CommTiled::box_touch_tiled(int proc, int idim, int idir)
{
  ...
  if (idir == 0) {
    if (fabs(rcbinfo[proc].mysplit[idim][1] - rcbinfo[me].mysplit[idim][0]) < DOUBLE_EPSILON)
      return 1;
  ...
  // only touches if proc lo = my hi, or if proc lo = boxlo and my hi = boxhi
    ...
    if (fabs(rcbinfo[proc].mysplit[idim][0] - rcbinfo[me].mysplit[idim][1]) < DOUBLE_EPSILON)
      return 1;
...
int CommTiled::point_drop_tiled(int idim, double *x)
{
  ...
  if (idim == 0) {
    int done = 1;
    if (fabs(rcbinfo[proc].mysplit[1][0] - rcbinfo[me].mysplit[1][1]) < DOUBLE_EPSILON) {
      xnew[1] -= EPSILON * (subhi[1]-sublo[1]);
      done = 0;
    }
    if (fabs(rcbinfo[proc].mysplit[2][0] - rcbinfo[me].mysplit[2][1]) < DOUBLE_EPSILON) {
  ...
  if (flag) {
    ...
    for (int i = 0; i < nforeign; i++) {
      fprintf(fp, "%d 4 %g %g %g %d %d\n", n++
          ...
          , (foreign_boxes[i][2]+foreign_boxes[i][5])/2
          , foreign_tag[i]
          , foreign_overlap[i]);
