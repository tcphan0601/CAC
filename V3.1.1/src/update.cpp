...
void Update::set_units(const char *style)
{
  ...
  if (strcmp(style, "lj") == 0) {
  ...
  } else if ((strcmp(style, "metal") == 0) || (strcmp(style, "metalgpa") == 0)) {
    ...
    if (strcmp(style, "metalgpa") == 0) 
      force->nktv2p = 1.6021765e2;    // Stress Unit: GPa
    else if (strcmp(style, "metal") == 0) 
      force->nktv2p = 1.6021765e6;    // Stress Unit: Bar (same as LAMMPS metal)
...
void Update::create_integrate(int narg, char **arg, int trysuffix)
{
  ...
  if (sflag) {
    char estyle[256];
    if (sflag == 1) sprintf(estyle, "%s/%s", arg[0], cac->suffix);
    else sprintf(estyle, "%s/%s", arg[0], cac->suffix2);
  ...
  if (trysuffix && cac->suffix_enable) {
    if (cac->suffix) {
      ...
      if (integrate_map->find(estyle) != integrate_map->end()) {
        ...
        integrate = integrate_creator(cac, narg, arg);
        return;
      }
    }
...
/*  ----------------------------------------------------------------------
  ...
  template <typename T>
Integrate *Update::integrate_creator(CAC *cac, int narg, char ** arg)
{
...
/*  ----------------------------------------------------------------------
  ...
  template <typename T>
Min *Update::minimize_creator(CAC *cac)
{
  return new T(cac);
}
...
void Update::reset_timestep(bigint newstep)
{
  ...
  for (int i = 0; i < modify->ncompute; i++) {
    ...
    modify->compute[i]->invoked_scalar_local = -1;
    modify->compute[i]->invoked_vector_local = -1;
    modify->compute[i]->invoked_array_local = -1;
  }
