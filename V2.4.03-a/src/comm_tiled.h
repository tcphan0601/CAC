...
class CommTiled : public Comm {
 ...
 private:
  ...
  double **minsendbox;         // minimum bounding box of atoms to send per proc
  int *minsendbox_flag;        // 1 if minsendbox for this send proc is fitite
  double **maxsendbox;         // maximum bounding box of atoms to send per proc due to element extension

  // exchange overlaping box info
  ...
  int *overlap_repeat;         // flags for each proc to determine if overlap array has repeats in O(P)
  int current_pbc[3];          // grid offset for this proc need search 
  ...
  // debug flag and functions
  
  int debugflag;
  void debug_write_foreign_box();
};
