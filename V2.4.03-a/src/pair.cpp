...
/* ----------------------------------------------------------------------
   update elemscale when new element types are added
------------------------------------------------------------------------- */

void Pair::update_elemscale()
{
  delete [] elemscale;
  ...
  for (int i = 1; i <= netypes; i++)
    elemscale[i] = (static_cast<double>(npe[i]))/nintpl[i];
}
...
void Pair::node_ev_tally(int i, int inode, double evdwl, double ecoul, double fpair, double delx, double dely, double delz)
{
  if (inode < 0) return;
  double v[6];
  double iscale = elemscale[element->etype[i]];
  if (eflag_either) {
    if (eflag_global) {
    ...
    double *fk, double delxji, double delyji, double delzji, double delxki, double delyki, double delzki)
{
  ...
  double iscale = elemscale[element->etype[i]];
  double jscale = elemscale[element->etype[j]];
  double kscale = elemscale[element->etype[k]];
  if (eflag_either) {
    evdwlthird = THIRD * evdwl;
    ecoulthird = THIRD * ecoul;
    if (eflag_global) {
