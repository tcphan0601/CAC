/* Adopted from LAMMPS CAC comm_cac.cpp by Adrian Diaz - University of Florida */
...
void CommTiled::setup_borders() 
{
  ...
  double box_grid[3][4];
  for (dim = 0; dim < dimension; dim++) {
    box_grid[dim][0] = sublo[dim] - max_cutghost[dim];
    box_grid[dim][1] = sublo[dim];
    box_grid[dim][2] = subhi[dim];
    box_grid[dim][3] = subhi[dim] + max_cutghost[dim];
  }
  ...
  // setup borders for full corner and edge swaps
  ...
  // sets overlapother, overlelf, pbc_flag, pbc, minsendbox
  // resets nprocmax

  int noverlap1,indexme;
  double lo1[3],hi1[3],lo2[8][3],hi2[8][3];
  int current_pbc_set[8][3]; 
  int one,two;
  ...
  for (int ix = 0; ix < 3; ix ++)
    for (int iy = 0; iy < 3; iy ++)
      for (int iz = 0; iz < 3; iz ++) {
        ...
        int pbc_loop[3];
        pbc_loop[0] = pbc_loop[1] = pbc_loop[2] = 0;
        for (dim = 0; dim < dimension; dim++) {
          if (periodicity[dim]) {
          ...
          for (int pbx = 0; pbx <= pbc_loop[0]; pbx++)
            for (int pby = 0; pby <= pbc_loop[1]; pby++)
              for (int pbz = 0; pbz <= pbc_loop[2]; pbz++) {
                ...
                copy3(lo1,lo2[n2box]);
                copy3(hi1,hi2[n2box]);
                current_pbc_set[n2box][0] = current_pbc_set[n2box][1] = 
                  current_pbc_set[n2box][2] = 0;
                if (pbx) {
                  lo2[n2box][0] += prd[0]*current_pbc[0];
                  hi2[n2box][0] += prd[0]*current_pbc[0];
                  current_pbc_set[n2box][0] = current_pbc[0];
                }
                lo2[n2box][0] = MAX(lo2[n2box][0],boxlo[0]);
                hi2[n2box][0] = MIN(hi2[n2box][0],boxhi[0]);
                if (lo2[n2box][0] == hi2[n2box][0]) continue;

                if (pby) {
                  lo2[n2box][1] += prd[1]*current_pbc[1];
                  hi2[n2box][1] += prd[1]*current_pbc[1];
                  current_pbc_set[n2box][1] = current_pbc[1];
                }
                lo2[n2box][1] = MAX(lo2[n2box][1],boxlo[1]);
                hi2[n2box][1] = MIN(hi2[n2box][1],boxhi[1]);
                if (lo2[n2box][1] == hi2[n2box][1]) continue;

                if (pbz) {
                  lo2[n2box][2] += prd[2]*current_pbc[2];
                  hi2[n2box][2] += prd[2]*current_pbc[2];
                  current_pbc_set[n2box][2] = current_pbc[2];
                }
                lo2[n2box][2] = MAX(lo2[n2box][2],boxlo[2]);
                hi2[n2box][2] = MIN(hi2[n2box][2],boxhi[2]);
                if (lo2[n2box][2] == hi2[n2box][2]) continue;
        ...
        if (two) {
          for (i = 0; i < n2box; i++)
            for (dim = 0; dim < dimension; dim++) {
              lo2[i][dim] = MAX(lo2[i][dim],boxlo[dim]);
              hi2[i][dim] = MIN(hi2[i][dim],boxhi[dim]);
            }
        }
        if (one) (this->*box_drop)(0,lo1,hi1,indexme,1);
        noverlap1 = noverlap;
        if (two) {
          for (i = 0; i < n2box; i++) {
            current_pbc[0] = current_pbc_set[i][0];
            current_pbc[1] = current_pbc_set[i][1];
            current_pbc[2] = current_pbc_set[i][2];
            (this->*box_drop)(0,lo2[i],hi2[i],indexme,2);
          }
        }
  ...
  if (nprocs != 1) {
    for(i = 0; i < noverlap; i++) {
      if (overlap[i] == me) {
        ...
        if (layout != LAYOUT_TILED) {
          tmp[0] = proc2box[swapindex][0];
          tmp[1] = proc2box[swapindex][1];
          tmp[2] = proc2box[swapindex][2];
          tmp[3] = proc2box[swapindex][3];
          tmp[4] = proc2box[swapindex][4];
          tmp[5] = proc2box[swapindex][5];
          proc2box[swapindex][0] = proc2box[i][0];
          proc2box[swapindex][1] = proc2box[i][1];
          proc2box[swapindex][2] = proc2box[i][2];
          proc2box[swapindex][3] = proc2box[i][3];
          proc2box[swapindex][4] = proc2box[i][4];
          proc2box[swapindex][5] = proc2box[i][5];
          proc2box[i][0] = tmp[0];
          proc2box[i][1] = tmp[1];
          proc2box[i][2] = tmp[2];
          proc2box[i][3] = tmp[3];
  ...
  if (overlapself % 2) error->one(FLERR,"Does not find all the overlapself");
  ...
  // compute minsendbox for each of my sends, 
  // this is the absolute send box (elements or atoms overlap this minsendbox is sent to other proc)
  ...
  // communicating overlapping elements
  // for each overlap proc:
  //   obox = expanded subbox by cutghost and element bound box
  ...
  // setup bins/stencils for foreign boxes

  debug_write_foreign_box();
...
void CommTiled::create_stencil()
{
  ...
  double cutz = (maxfbox[1] + element->max_element_bound_box_size[1])/2 + cutghost[1];
  double cuty = (maxfbox[2] + element->max_element_bound_box_size[2])/2 + cutghost[2];
      ...
      for (int j = -sy; j <= sy; j++)
        for (int i = -sx; i <= sx; i++)
          if (bin_distance(i,0) < cutx &&
            ...
            if (me == 1) fprintf(screen,"ijk = %d %d %d distance = %g %g %g\n",i,j,k
                ,bin_distance(i,0)
                ,bin_distance(j,1)
                ,bin_distance(k,2)
                );
  ...
  fprintf(screen,"nstencil =%d s = %d %d %d cut = %g %g %g binsize = %g %g %g\n",nstencil,sx,sy,sz,cutx,cuty,cutz,binsizex,binsizey,binsizez);
}
...
void CommTiled::setup_bins() 
{
  ...
  tmp = MAX(maxfbox[0],maxfbox[1]);
  tmp = MAX(maxfbox[2],tmp) + cutghost[0];
  tmp /= 2;
...
{
  int i,j,m,last;
  int foreign_proc,foreign_pbc[3];
  m = 0;
  last = first + n;
...
void CommTiled::setup_exchange()
{
  ...
  for (int idim = 0; idim < dimension; idim++) {
    ...
    if (periodicity[idim] || sublo[idim] != boxlo[idim]) {
      ...
      indexme = -1;
      (this->*box_drop)(idim,lo,hi,indexme,0);
...
void CommTiled::borders()
{
  ...
  int i,j,k,m,n,check_flag,nother,ibin;
  double *lobox;
  double *hibox;
  double *coord;
  double **x;
  int nlocal,nscountall,nrcountall,nscount,nrcount;
  int *sendflag = new int[noverlap];
  AtomVec *avec = atom->avec;
  ElementVec *evec = element->evec;
  ...
  if (element->nelements) {
    ...
    for (i = 0; i < nlocal; i++) {
      ...
      if (check_flag) {
        ibin = coord2bin((coord[0]+coord[3])/2.0,
            (coord[1]+coord[4])/2.0,(coord[2]+coord[5])/2.0);
        for (k = 0; k < nstencil; k++) {
          for (j = fboxbinhead[ibin+stencil[k]]; j >= 0; j = fboxbins[j]) {
            m = foreign_overlap[j];
            if (sendflag[m] == 0) {
...
void CommTiled::grow_overlap_array()
{
  ...
  memory->grow(proc2box,maxoverlap,6,"comm:proc2box");
}
/* ----------------------------------------------------------------------
...
void CommTiled::box_drop_brick(int idim, double *lo, double *hi, int &indexme, int flag)
{
...
void CommTiled::box_drop_tiled(int idim, double *lo, double *hi, int &indexme, int flag)
{
  box_drop_tiled_recurse(lo,hi,0,nprocs-1,indexme,flag);
}
    ...
    int &indexme, int flag)
{
  ...
  if (lo[idim] < cut)
    box_drop_tiled_recurse(lo,hi,proclower,procmid-1,indexme,flag);
  if (hi[idim] > cut)
    box_drop_tiled_recurse(lo,hi,procmid,procupper,indexme,flag);
}
...
void CommTiled::debug_write_foreign_box()
{
  FILE *fp;
  ...
  fprintf(fp,BIGINT_FORMAT "\n",nforeign*9);
  fprintf(fp,"ITEM: BOX BOUNDS s p p\n");
