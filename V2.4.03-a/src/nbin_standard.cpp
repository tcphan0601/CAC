...
void NBinStandard::setup_bins(int style)
{
  ...
  if (triclinic == 0) {
    bsubboxlo[0] = domain->sublo[0] - max_cutghost[0];
    bsubboxlo[1] = domain->sublo[1] - max_cutghost[1];
    bsubboxlo[2] = domain->sublo[2] - max_cutghost[2];
    bsubboxhi[0] = domain->subhi[0] + max_cutghost[0];
    bsubboxhi[1] = domain->subhi[1] + max_cutghost[1];
    bsubboxhi[2] = domain->subhi[2] + max_cutghost[2];
  } else {
    double lo[3],hi[3];
    lo[0] = domain->sublo_lamda[0] - max_cutghost[0];
    lo[1] = domain->sublo_lamda[1] - max_cutghost[1];
    lo[2] = domain->sublo_lamda[2] - max_cutghost[2];
    hi[0] = domain->subhi_lamda[0] + max_cutghost[0];
    hi[1] = domain->subhi_lamda[1] + max_cutghost[1];
    hi[2] = domain->subhi_lamda[2] + max_cutghost[2];
  ...
  if (atom->natoms) {
    ...
    if (dimension == 3) natombinz = static_cast<int> (bbox[2]*binsizeinv);
    else natombinz = 1;

    if (natombinx == 0) natombinx = 1;
    if (natombiny == 0) natombiny = 1;
    if (natombinz == 0) natombinz = 1;
    ...
    if (dimension == 3) {
      matombinzlo--;
      matombinzhi++;
    } else matombinzlo = matombinzhi = 0;
    matombinz = matombinzhi - matombinzlo + 1;

    bigint bbin = ((bigint) matombinx) * ((bigint) matombiny) * ((bigint) matombinz) + 1;
    if (bbin > MAXSMALLINT) error->one(FLERR,"Too many neighbor atom bins");
    matombins = bbin;
  } else {
    matombins = 0; 
  }
  ...
  if (element->nelements) {
    ...
    if (binsizeflag) 
      binsizex_optimal = binsizey_optimal = binsizez_optimal = binsize_user;
    else if (style == BIN) {
      binsizex_optimal = 0.5*(cutneighmax + element->max_element_bound_box_size[0]);
      binsizey_optimal = 0.5*(cutneighmax + element->max_element_bound_box_size[1]);
      binsizez_optimal = 0.5*(cutneighmax + element->max_element_bound_box_size[2]);
    } else {
      binsizex_optimal = 0.5*(cutneighmin + element->max_element_bound_box_size[0]);
      binsizey_optimal = 0.5*(cutneighmin + element->max_element_bound_box_size[1]);
      binsizez_optimal = 0.5*(cutneighmin + element->max_element_bound_box_size[2]);
    }
    if (binsizex_optimal == 0.0) binsizex_optimal = bbox[0];
    if (binsizey_optimal == 0.0) binsizez_optimal = bbox[1];
    if (binsizez_optimal == 0.0) binsizey_optimal = bbox[2];
