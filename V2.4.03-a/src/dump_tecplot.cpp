...
enum{COORDINATE,STRESS,FORCE,DISPLACEMENT};
...
void DumpTecplot::init_style() 
{
  ...
  if (fileformat == ASCII) {
    ...
    title = new char[100];
    sprintf(title,"TIMESTEP " BIGINT_FORMAT,update->ntimestep);
    values = new char[500];
    values = strcpy(values,"x y z");
    if (stress_flag)
      strcat(values," sxx syy szz sxy sxz syz");
    if (force_flag)
      strcat(values," vx vy vz fx fy fz");
    if (displace_flag)
...
void DumpTecplot::write()
{
  ...
  if (fileformat == ASCII) {
    ...
    if (multifile && filewriter) {
      success = TECEND142();
      if (success == -1)
        error->all(FLERR,"Cannot close data file");
    }
