...
NPairFullBinIntg::NPairFullBinIntg(CAC *cac) : NPair(cac) {}
...
void NPairFullBinIntg::build(NeighList *list)
{

  int iintg,i,j,jj,jnum,k,l,n,ictype,jctype,ietype,jetype,ibin,isub,nintg;
  int *a2aneighptr,*a2eneighptr,*a2ianeighptr,*a2ia_indexneighptr;
  a2aneighptr = a2eneighptr = a2ianeighptr = a2ia_indexneighptr = NULL;
  int *i2aneighptr,*e2eneighptr,*i2ianeighptr,*i2ia_indexneighptr;
  i2aneighptr = e2eneighptr = i2ianeighptr = i2ia_indexneighptr = NULL;
  int *jlist;
  int na2a,na2e,na2ia,ni2a,ne2e,ni2ia;
  int iintpl,jintpl,inode;
  ...
  MyPage<int> *a2epage = list->a2epage;
  ...
  for (i = 0; i < nalocal; i++) {
    ...
    for (k = 0; k < nstencil; k++) {
      for (j = atombinhead[ibin+stencil[k]]; j >= 0; j = atombins[j]) {
        if (i == j) continue;
        jctype = atype[j];
        delx = ix - ax[j][0];
        dely = iy - ax[j][1];
    ...
    // loop over all elements in other bins in stencil including self if there are elements

    if (neall){
      na2e = 0;
      na2ia = 0;
      a2eneighptr = a2epage->vget();
      a2ianeighptr = a2iapage->vget();
      a2ia_indexneighptr = a2ia_indexpage->vget();
      for (k = 0; k < nstencil; k++) {

        for (j = elembinhead[ibin+stencil[k]]; j >= 0; j = elembins[j]) {
          ...
          if (rsq < cut_allsq) {
            ...
            for (isub = 0; isub < nsubelem; isub++) {
              ...
              for (inode = 0; inode < npe; inode++) {
                jx += shape_array_center_subelem[isub][inode]*nodex[j][inode][0];
                jy += shape_array_center_subelem[isub][inode]*nodex[j][inode][1];
                jz += shape_array_center_subelem[isub][inode]*nodex[j][inode][2];
              }
              delx = ix - jx; 
              dely = iy - jy; 
              delz = iz - jz; 
              ...
              if (rsq < cut_subelemsq)
                ...
                for (l = 0; l < natom_subelem[jetype][isub]; l++) {
                  ...
                  for (inode = 0; inode < npe; inode++) {
                    jx += shape_array[jetype][jintpl][inode]*nodex[j][inode][0];
                    jy += shape_array[jetype][jintpl][inode]*nodex[j][inode][1];
                    jz += shape_array[jetype][jintpl][inode]*nodex[j][inode][2];
                  }
                  delx = ix - jx; 
                  dely = iy - jy; 
                  delz = iz - jz; 
  ...
  for (i = 0; i < einum; i++){
    ...
    for (iintg = 0; iintg < nintg; iintg++){
      ...
      // loop over all atoms in all bins in stencil
