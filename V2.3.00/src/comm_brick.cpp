...
void CommBrick::setup()
{
  ...
  //   cutghost is in lambda coords = distance between those planes
  // for multi:
  ...
  if (triclinic == 0) {
    ...
    sublo = domain->sublo;
    subhi = domain->subhi;
    cutghost[0] = cutghost[1] = cutghost[2] = cut + element->max_size;
    ...
    //error->all(FLERR,"Tricilic box has not been considered yet");
    prd = domain->prd_lambda;
	...
	element_size_x[0] = element->max_size + domain->boxlo[0];
	element_size_x[1] = element->max_size + domain->boxlo[1];
	element_size_x[2] = element->max_size + domain->boxlo[2];

	domain->x2lambda(element_size_x, element_size_lamda);

    length0 = sqrt(h_inv[0]*h_inv[0] + h_inv[5]*h_inv[5] + h_inv[4]*h_inv[4]);
    cutghost[0] = cut * length0 + element_size_lamda[0];
    length1 = sqrt(h_inv[1]*h_inv[1] + h_inv[3]*h_inv[3]);
    cutghost[1] = cut * length1 + element_size_lamda[1];
    length2 = h_inv[2];
    cutghost[2] = cut * length2 + element_size_lamda[2];
  ...
  // for triclinic, slablo/hi and pbc_border will be used in lambda (0-1) coords
    ...
    if (element->nelements) {
      if (sendproc[iswap] != me) {
        if (elem_size_forward_recv[iswap]) 
          MPI_Irecv(buf_recv, elem_size_forward_recv[iswap],MPI_DOUBLE,recvproc[iswap],0,world,&request);
        n = evec->pack_comm(elem_sendnum[iswap],elem_sendlist[iswap],buf_send,pbc_flag[iswap],pbc[iswap]);
        if (n) MPI_Send(buf_send,n,MPI_DOUBLE,sendproc[iswap],0,world);
        ...
        evec->pack_comm(elem_sendnum[iswap],elem_sendlist[iswap],buf_send,pbc_flag[iswap],pbc[iswap]);
...
{
  error->all(FLERR,"Reverse comm has not been considered yet");
  /*
     int n;
     MPI_Request request;
     AtomVec *avec = atom->avec;
...
/* ----------------------------------------------------------------------
   ...
   for triclinic, atoms must be in lambda coords (0-1) before exchange is called
   ------------------------------------------------------------------------- */
...
void CommBrick::exchange()
{
  ...
  if (element->nelements) {
    ...
    for (int dim = 0; dim < dimension; dim++) {
      ...
      // also update number of local integration points
...
/* ----------------------------------------------------------------------
   ...
   for triclinic, atoms must be in lambda coords (0-1) before borders is called
   ------------------------------------------------------------------------- */
