...
void Comm::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"mode") == 0) {
      ...
      cutghostuser = universe->numeric(FLERR,arg[iarg+1]);
      if (cutghostuser < 0.0)
...
void Comm::set_proc_grid(int outflag)
{
  ...
  // set lambda box params after procs are assigned
  // only set once unless load-balancing occurs

  if (domain->triclinic) domain->set_lambda_box();
...
/* ----------------------------------------------------------------------
   ...
   x will be in box (orthogonal) or lambda coords (triclinic)
