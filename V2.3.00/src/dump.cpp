...
void Dump::write()
{
  ...
  bigint ntotal_atoms = natom_total + nintpl_total;
  ...
  // temporarily for dump cna/atom style
  // will remove when add variable class

  if (strcmp(style,"cna/atom") == 0){
    natom_me = count_cna_atoms();
 
    nelem_me = count_cna_intpl(); // nelem_me will be total number of interpolated atoms in this case
  
    bnatom_me = natom_me;
    bnelem_me = nelem_me;
    MPI_Allreduce(&bnatom_me,&natom_total,1,MPI_CAC_BIGINT,MPI_SUM,world);
    MPI_Allreduce(&bnelem_me,&nelem_total,1,MPI_CAC_BIGINT,MPI_SUM,world);
    write_header(natom_total+nelem_total);
  } else write_header(ntotal_atoms);
