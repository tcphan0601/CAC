...
void Verlet::setup(int flag)
{
  ...
  class Balance *balance = new Balance(cac);
  ...
  if (triclinic) domain->x2lambda(atom->nlocal, atom->x);
  if (triclinic) domain->x2lambda(element->nlocal, element->x);
  if (triclinic) domain->x2lambda(element->nlocal, element->npe, element->nodex);
  ...
  if (triclinic) domain->lambda2x(atom->nlocal + atom->nghost, atom->x);
  if (triclinic) domain->lambda2x(element->nlocal + element->nghost, element->x);
  if (triclinic) domain->lambda2x(element->nlocal + element->nghost, element->npe, element->nodex);
...
void Verlet::force_clear()
{
  ...
  int npe = element->npe;
  nbytes = sizeof(double) * nalocal;
  if (nbytes) {
    memset(&atom->f[0][0],0,3*nbytes);
  }
  nbytes = sizeof(double) * nelocal * npe;
  if (nbytes) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (nflag == 0) {
      ...
      if (domain->triclinic) {
		...
		domain->x2lambda(element->nlocal, element->npe, element->nodex);
      }
      domain->pbc();
      ...
      comm->borders();
      if (domain->triclinic) {
        domain->lambda2x(atom->nlocal+atom->nghost,atom->x);
        domain->lambda2x(element->nlocal+element->nghost,element->x);
		domain->lambda2x(element->nlocal, element->npe, element->nodex);
      }
      timer->stamp(Timer::COMM);
      if (n_pre_neighbor) {
        modify->pre_neighbor();
