...
void CommTiled::setup()
{
  ...
  double cut = MAX(neighbor->cutneighmax,cutghostuser) + element->max_size;
...
{
  error->all(FLERR,"Forward comm compute not yet supported by CommTiled");
  //  int i,irecv,n,nsend,nrecv;
  //
  //  int nsize = compute->comm_atom_forward;
  //
  //  for (int iswap = 0; iswap < nswap; iswap++) {
  //    nsend = nsendproc[iswap] - sendself[iswap];
  //    nrecv = nrecvproc[iswap] - sendself[iswap];
  ...
  //        MPI_Waitany(nrecv,requests,&irecv,MPI_STATUS_IGNORE);
  //        compute->
  //          unpack_atom_forward_comm(atom_recvnum[iswap][irecv],atom_firstrecv[iswap][irecv],
  //              &buf_recv[nsize*
  //              atom_forward_recv_offset[iswap][irecv]]);
  //      }
  //    }
