...
//  memory->create(cna,MAXNEAR,4,"cna:cna");
          ...
          for (ll = kk + 1; ll < ncommon; ll++) {
            ...
            if (commontype[ll] == ATOM) {
              ...
              for (lnode = 0; lnode < npe; lnode++) {
                lx += shape_array[letype][lintpl][lnode]*nodex[el][lnode][0];
                ly += shape_array[letype][lintpl][lnode]*nodex[el][lnode][1];
                lz += shape_array[letype][lintpl][lnode]*nodex[el][lnode][2];
              }
            }
            delx = kx - lx;
            dely = ky - ly;
