...
/* ----------------------------------------------------------------------
   update cut off for changed element size
   called in verlet
   ------------------------------------------------------------------------- */
void Neighbor::update_cut()
{
  cut_all = cutneighmax + element->max_size;
  cut_elem = cutneighmax + element->max_size/2.0;
  cut_subelem = cutneighmax + element->max_size/(2.0*element->esplit);
...
void Neighbor::print_pairwise_info()
{
  ...
  const double cutghost = MAX(cutneighmax,comm->cutghostuser) + elem_cut;
  ...
  else if (style == BIN) binsize = 0.5*(cutneighmax + elem_cut);
  else binsize = 0.5*(cutneighmin + elem_cut);
  ...
  for (m = 0; m < 2; m++) {
    ...
    if (out) {
      ...
      fprintf(out,"  element split %d x %d x %d\n",esplit,esplit,esplit);
