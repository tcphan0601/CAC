...
int Balance::shift()
{
  ...
  // all balancing done in lambda coords
  // only use element center coords so no need to convert
  // node coords to lambda coords
...
/* ----------------------------------------------------------------------
   ...
   atoms must be in lambda coords (0-1) before called
...
void Balance::dumpout(bigint tstep)
{
  ...
  if (triclinic == 0) {
    ...
    lo = domain->sublo_lambda;
    hi = domain->subhi_lambda;
  }
  ...
  if (outflag == 1) {
    ...
    if (triclinic == 0) {
      ...
      if (dimension == 2) {
        int m = 0;
        for (int i = 0; i < nprocs; i++) {
          domain->lambda_box_corners(&boxall[i][0],&boxall[i][3]);
