...
class Comm : protected Pointers {
 public:
  int style;     // comm pattern: 0 = 6-way stencil, 
                 //               1 = irregular tiling
  ...
  int debug;
  int skip_exchange;                // 0/1 flag for skipping exchange due to fix balance or balance commands
  int me,nprocs;                    // proc info
  int ghost_velocity;               // 1 if ghost atoms have velocity, 0 if not
  double cutghost[6];               // cutoffs used for acquiring ghost atoms/elements
  // 0 1 2: left sides
  // 3 4 5: right sides
  double max_cutghost[3];           // maximum cutoffs used for acquiring ghost atoms/elements
  ...
  virtual void setup_exchange() {};             // setup 3d exchange comm pattern
  virtual void setup_borders() = 0;              // setup 3d borders comm pattern
  ...
  virtual void forward_comm_atom_array(int, double **) = 0;
  virtual void forward_comm_elem_array(int, double **) = 0;
