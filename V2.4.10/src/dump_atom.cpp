...
DumpAtom::DumpAtom(CAC *cac, int narg, char **arg) : Dump(cac,narg,arg)
{
  ...
  if (dump_element_style == INTEGRATION && force_flag) force->pair->save_intg_force = 1;
...
{
  atom_size_one = 5 + 6*stress_flag + 3*(velocity_flag + force_flag) + 4*displace_flag + pe_flag + 6*element_flag;
  ...
  if (pe_flag) 
    strcat(columns," pe");
  if (element_flag) 
    strcat(columns," eid etype iintpl iintg inode comm");
      ...
      if (dump_element_style == INTERPOLATE) {
        for (iintpl = 0; iintpl < nintpl[ietype]; iintpl++) {
          ...
          if (element_flag) {
            ...
            buf[m++] = ietype;
            buf[m++] = iintpl;
            buf[m++] = ia2i[ietype][iintpl];
            if (ia2i[ietype][iintpl] < 0)
              buf[m++] = -1;
            else 
              buf[m++] = i2n[ietype][ia2i[ietype][iintpl]];
            buf[m++] = comm->me;
          }
        }
        ...
        for (iintg = 0; iintg < nintg[ietype]; iintg++) {
          ...
          if (force_flag) {
            for (j = 0; j < 3; j++) {
              buf[m++] = force->pair->intgf[i][iintg][j];
            }
          }
          if (stress_flag) {
            for (j = 0; j < 6; j++) {
        ...
        for (inode = 0; inode < npe[ietype]; inode++) {
          ...
          if (element_flag) {
            buf[m++] = tag[i];
            buf[m++] = ietype;
            buf[m++] = n2ia[ietype][inode];
            buf[m++] = n2i[ietype][inode];
            buf[m++] = inode;
            buf[m++] = comm->me;
          }
        }
