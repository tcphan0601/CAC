...
NPairFullBinIntplGroupNoInner::NPairFullBinIntplGroupNoInner(CAC *cac) : NPair(cac) {}

/* ----------------------------------------------------------------------
   binned neighbor list construction for all neighbors
   every neighbor pair appears in list of both atoms i and j
   build list for atoms, elements, and interpolated atoms
   exclude inner interpolated atoms, outside of group
   include neighbor of neighbor
------------------------------------------------------------------------- */

void NPairFullBinIntplGroupNoInner::build(NeighList *list)
{
  ...
  for (i = 0; i < nalocal; i++) {
    ...
    if (amask[i] & groupbit == 0) {
      firstneigha2a[ainum] = NULL;
      numneigha2a[ainum] = 0;
      ainum++;
      continue;
    }
    ...
    for (k = 0; k < natomstencil; k++) {
      if (ibin+atomstencil[k] < 0 || ibin+atomstencil[k] >= nb->matombins) {
        printf("bin = %d matombins = %d\n",ibin+atomstencil[k],nb->matombins);
      }
  ...
  for (ii = 0; ii < einum; ii++) {
    ...
    // loop over interpolated points in element i
    ...
    for (iintpl = 0; iintpl < nintpl[ietype]; iintpl++) {
      ...
      // skip if this intpl is inner or element not in group 

      if (!is_outer[ietype][iintpl] || emask[i] & groupbit == 0) {
        numneighia2a[iainum] = 0;
        firstneighia2a[iainum] = NULL;
        numneighia2ia[iainum] = 0;
        firstneighia2ia[iainum] = NULL;
        firstneighia2ia_index[iainum] = NULL;
        iainum++;
        continue;
      }
