...
Neighbor::~Neighbor()
{
  ...
  memory->destroy(innercutneighsq);
...
void Neighbor::init()
{
  ...
  if (cutneighsq == NULL) {
    ...
    memory->create(innercutneighsq,n+1,n+1,"neigh:innercutneighsq");
  ...
  double cutoff,innercutoff,delta,cut,innercut;
  cutneighmin = BIG;
  cutneighmax = 0.0;
    ...
    for (j = 1; j <= n; j++) {
      if (force->pair) {
        ...
        if (force->pair->double_neighbor_layers) 
          innercutoff = sqrt(force->pair->innercutsq[i][j]);
      } else cutoff = innercutoff = 0.0;
      if (cutoff > 0.0) delta = skin;
      else delta = 0.0;
      cut = cutoff + delta;
      innercut = innercutoff + delta;

      cutneigh[i][j] = cut;
      cutneighsq[i][j] = cut*cut;
      innercutneighsq[i][j] = innercut*innercut;
  ...
  if (dist_check) {
    ...
    if (nodemaxhold == 0) {
      ...
      memory->create(nodexhold,nodemaxhold,element->max_npe,3,"neigh:nodexhold");
    }
...
int Neighbor::init_pair()
{
  ...
  for (i = 0; i < nlist; i++)
    if (neigh_pair[i] && !lists[i]->copy) {
      ...
      if (lists[i]->nodelist) 
        lists[i]->grow_node(maxelem*max_npe,maxelem*max_npe);
      if (lists[i]->intglist) 
...
void Neighbor::morph_copy()
{
  ...
  for (i = 0; i < nrequest; i++) {
    ...
    for (j = 0; j < nrequest; j++) {
      ...
      if (irq->doublelist && !jrq->doublelist) continue;
...
int Neighbor::choose_bin(NeighRequest *rq)
{
  ...
  for (int i = 0; i < nbclass; i++) {
    ...
    //if (rq->custom_domain) {
    //  if (!(mask & NB_CUSTOMBOX)) continue;
    //} else if (mask & NB_CUSTOMBOX) continue;
...
int Neighbor::choose_pair(NeighRequest *rq)
{
  ...
  for (int i = 0; i < npclass; i++) {
    ...
    if (!rq->intglist != !(mask & NP_INTG)) continue;
    if (!rq->intpllist != !(mask & NP_INTPL)) continue;
    if (!rq->doublelist != !(mask & NP_DOUBLE)) continue;
    if (!rq->halffull != !(mask & NP_HALFFULL)) continue;
...
void Neighbor::build(int topoflag)
{
  ...
  for (i = 0; i < npair_perpetual; i++) {
    ...
    if (!lists[m]->copy) {
      ...
      if (lists[m]->nodelist) 
        lists[m]->grow_node(nelocal*max_npe,neall*max_npe);
      if (lists[m]->intglist) 
...
void Neighbor::build_one(class NeighList *mylist, int preflag)
{
  ...
  if (!mylist->copy) {
    ...
    if (mylist->nodelist) 
      mylist->grow_node(nelocal*max_npe,neall*max_npe);
    if (mylist->intglist) 
...
bigint Neighbor::memory_usage()
{
  ...
  bytes += memory->usage(nodexhold,nodemaxhold,element->max_npe,3);
