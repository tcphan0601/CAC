...
void ChangeBox::command(int narg, char **arg)
{
  ...
  if (comm->me == 0 && logfile) fprintf(logfile,"Changing box ...\n");
  ...
  domain->reset_box();
  IrregularComm *irregular = new IrregularComm(cac);
  irregular->migrate(1);
  delete irregular;
  if (domain->triclinic) {
    domain->lamda2x(atom->nlocal,atom->x);
  ...
  if (nelements != element->nelements && comm->me == 0) {
    char str[128];
    sprintf(str,"Lost elements via change_box: original " BIGINT_FORMAT
