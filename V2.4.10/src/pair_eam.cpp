...
void PairEAM::compute(int eflag, int vflag)
{
  ...
  if (element->nmax > nemax) {
    ...
    if (save_intg_force) {
      ...
      memory->create(intgf,nemax,element->max_nintg,3,"pair:intgf");
    }
  }
  ...
  if (edensity_style == 0) {
    ...
    for (ii = 0; ii < einum; ii++) {
      ...
      for (iintg = 0; iintg < nintg[ietype]; iintg++) {
        ...
        if (jnum) {
          ...
          for (jj = 0; jj < jnum; jj++) {
            ...
            if (rsq < cutforcesq) {
              ...
              p = MIN(p,1.0);
              coeff = rhor_spline[type2rhor[jctype][ictype]][m];
              rtmp += ((coeff[3]*p+coeff[4])*p+coeff[5])*p+coeff[6];
            }
          }
        }
        ...
        // distribute electron density at integration point to nodal values
        if (element->mass_style == Element::CONSISTENT) {
          ...
          for (node = 0; node < npe[ietype]; node++)
            ...
            noderho[i][node] += inversed_mass_matrix[node][j]*weighted_shape_array[ietype][iintg][j]*rtmp;

        } else {
          for (node = 0; node < npe[ietype]; node++)
            noderho[i][node] += weighted_shape_array[ietype][iintg][node]*rtmp;
        }
      }
    }
  }
  ...
  // debug 
  if (debug_mode && update->ntimestep % debug_nevery == 0) {
    debug_write_flag = 1;
    debug_open_dump();
    debug_intg_counter = 0;
  } else debug_write_flag = 0;
  // end debug
  ...
  for (ii = 0; ii < einum; ii++) {
    ...
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      ...
      if (element->debug_mode)
        if (element->debug_intg_inactive[ietype][element->debug_intg_type[ietype][iintg]])
          continue;
      ...
      if (jnum)
        if (!element->inner_rigid_flag || is_outer[ietype][iintpl]) {
      ...
      if (debug_write_flag)
        if (element->mask[i] & debug_groupbit)
          debug_write_intg_force(i,iintg,xtmp,ytmp,ztmp,fx,fy,fz);
      // end debug
