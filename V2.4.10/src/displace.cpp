...
void Displace::polar_ramp(char **arg)
{
  ...
  for (i = 0; i < nelocal; i++) 
    if (group_flag == Group::ELEMENT) {
      if (emask[i] & groupbit)
        for (j = 0; j < npe[etype[i]]; j++) polar_ramp_coord(coord_lo,coord_hi,d_lo,d_hi,nodex[i][j],coord_dim,d_dim,n,p);
    } else if (group_flag == Group::NODE) {
      for (j = 0; j < npe[etype[i]]; j++) 
        if (nodemask[i][j] & groupbit)
          polar_ramp_coord(coord_lo,coord_hi,d_lo,d_hi,nodex[i][j],coord_dim,d_dim,n,p);
    }
  element->evec->update_center_coord();
...
void Displace::options(int narg, char **arg)
{
  ...
  while (iarg < narg) 
    if (strcmp(arg[iarg],"units") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"inode/offset") == 0) {
      if (style != DISPFILE) error->all(FLERR,"Illegal displace command");
      if (iarg+2 > narg) error->all(FLERR,"Illegal displace command");
      inode_offset = universe->inumeric(FLERR,arg[iarg+1]);;
      iarg += 2;
...
void Displace::dislocations(char **arg)
{
  ...
  int *d_dim = new int[ndisl];
  int *slip_dim = new int[ndisl];
  int *dim1 = new int[ndisl];
  int *dim2 = new int[ndisl];
  int *direction = new int[ndisl];;
  double *center1 = new double[ndisl];
  double *center2 = new double[ndisl];;
  double *b = new double[ndisl];           // Burgers vector
  double *theta = new double[ndisl];       // rotation angle (in degrees) about dislocation line direction
  double *disl_angle = new double[ndisl];  // angle (in degrees) between Burgers vector and 
                        // dislocation line direction (toward slip direction)
  double *v = new double[ndisl];           // Poisson's ratio
  double *u = new double[ndisl];
  double *u1 = new double[ndisl];
  double *u2 = new double[ndisl];

  int i,j;
  int **dim_flag;
  memory->create(dim_flag,ndisl,3,"displace:dim_flag");
  for (int idisl = 0; idisl < ndisl; idisl++) {
    dim_flag[idisl][0] = dim_flag[idisl][1] = dim_flag[idisl][2] = 1;
    ...
    if (slip_dim[idisl] == d_dim[idisl]) 
      error->all(FLERR,"Illegal displace dislocation command:"
          "splip direction and dislocation line direction must be different");

    dim1[idisl] = slip_dim[idisl];
    dim2[idisl] = dim_flag[idisl][0] * 0 + dim_flag[idisl][1] * 1 + dim_flag[idisl][2] * 2;

    if (dim1[idisl] == (d_dim[idisl] + 1)%3) {
      center1[idisl] = universe->numeric(FLERR,arg[idisl*8+2]);
      center2[idisl] = universe->numeric(FLERR,arg[idisl*8+3]);
    } else {
      center1[idisl] = universe->numeric(FLERR,arg[idisl*8+3]);
      center2[idisl] = universe->numeric(FLERR,arg[idisl*8+2]);
    }

    b[idisl] = universe->numeric(FLERR,arg[idisl*8+5]);
    disl_angle[idisl] = universe->numeric(FLERR,arg[idisl*8+6]);
    v[idisl] = universe->numeric(FLERR,arg[idisl*8+7]);
    theta[idisl] = universe->numeric(FLERR,arg[idisl*8+8])/180*MY_PI;

    // check for errors

    //if (b[idisl] <= 0) error->all(FLERR,"Illegal displace dislocation command:"
    //    "Burger vector must be positive");
    if (v[idisl] >= 1 || v[idisl] < 0) 
      error->all(FLERR,"Illegal displace dislocation command:"
  ...
  if (nalocal) 
    for (i = 0; i < nalocal; i++) {
      for (int idisl = 0; idisl < ndisl; idisl++) {
        double bedge = b[idisl]*sin(disl_angle[idisl]/180*MY_PI);
        double bscrew = b[idisl]*cos(disl_angle[idisl]/180*MY_PI);
        x1 = direction[idisl]*(ax[i][dim1[idisl]] - center1[idisl]);
        x2 = direction[idisl]*(ax[i][dim2[idisl]] - center2[idisl]);
        u[idisl] = u1[idisl] = u2[idisl] = 0.0;
        edge_dislocation_field(u1[idisl],u2[idisl],x1,x2,bedge,v[idisl],theta[idisl]);
        screw_dislocation_field(u[idisl],x1,x2,bscrew,theta[idisl]);
      }
      for (int idisl = 0; idisl < ndisl; idisl++) {
        ax[i][dim1[idisl]] += u1[idisl];
        ax[i][dim2[idisl]] += u2[idisl];
        ax[i][d_dim[idisl]] += u[idisl];
      }
    }
  ...
  if (nelocal) {
    for (i = 0; i < nelocal; i++)
      ...
      for (int idisl = 0; idisl < ndisl; idisl++) {
        double bedge = b[idisl]*sin(disl_angle[idisl]/180*MY_PI);
        double bscrew = b[idisl]*cos(disl_angle[idisl]/180*MY_PI);
        x1 = direction[idisl]*(nodex[i][j][dim1[idisl]] - center1[idisl]);
        x2 = direction[idisl]*(nodex[i][j][dim2[idisl]] - center2[idisl]);
        u[idisl] = u1[idisl] = u2[idisl] = 0.0;
        edge_dislocation_field(u1[idisl],u2[idisl],x1,x2,bedge,v[idisl],theta[idisl]);
        screw_dislocation_field(u[idisl],x1,x2,bscrew,theta[idisl]);
      }
      for (int idisl = 0; idisl < ndisl; idisl++) {
        nodex[i][j][dim1[idisl]] += u1[idisl];
        nodex[i][j][dim2[idisl]] += u2[idisl];
        nodex[i][j][d_dim[idisl]] += u[idisl];
        }
      }
    element->evec->update_center_coord();
  }
...
void Displace::disp_file(char **arg)
{
  ...
  if (comm->me == 0) {
    fp = fopen(arg[0],"r");
    if (fp == NULL) error->one(FLERR,"Cannot open displacement file");
  }
  ...
  bigint ntotal = atom->natoms + element->nnodes;
  bigint nread = 0;
  ...
  while (nread < ntotal) {
    ...
    *next = '\n';
    ...
    for (ii = 0; ii < nchunk; ii++) {
      ...
      inode = atoi(values[1]) - 1 - inode_offset;
      dx = atof(values[2]);
      dy = atof(values[3]);
      dz = atof(values[4]);
