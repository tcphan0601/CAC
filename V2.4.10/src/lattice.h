...
class Lattice : protected Pointers {
 public:
  ...
  double basis_centroid[3];              // fractional coords of the centroid of all basis atoms
                                       // within unit cell (0 <= coord < 1)
  enum{NONE,SC,BCC,BCCT,BCCO,FCC,FCCT,FCCO,PRIFCC,PRIFCCT,PRIHCP,PRIFCCO,PRIBCC,PRIBCCT,PRIBCCO,HCP,DIAMOND,PRIDIAMOND,SQ,SQ2,HEX,CUSTOM};
 ...
 private:
  ...
  double *orientx;                      // lattice orientation vecs
  double *orienty;                      // orientx = what lattice dir lies
  double *orientz;                      //           along x dim in box
