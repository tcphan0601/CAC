...
PairMEAM::PairMEAM(CAC *cac) : Pair(cac)
{
  ...
  comm_elem_forward = comm_atom_forward * element->max_npe;;
  comm_atom_reverse = 30;
  comm_elem_reverse = comm_atom_reverse * element->max_npe;;
}
...
void PairMEAM::compute(int eflag, int vflag)
{
  ...
  int ietype,inode,iintg,iintg_local;
  ev_init(eflag,vflag);

  // pointers and parameters from neighbor list

  int ainum_half = listhalf->ainum;
  int einum_half = listhalf->einum;
  int niainum_half = listhalf->niainum;
  int *ailist_half = listhalf->ailist;
  int *eilist_half = listhalf->eilist;
  int *e2ilist_half = listhalf->e2ilist;
  int *numneigha2a_half = listhalf->numneigha2a;
  int *numneighi2ia_half = listhalf->numneighi2ia;

  int **firstneigha2a_half = listhalf->firstneigha2a;
  int **firstneighi2ia_half = listhalf->firstneighi2ia;
  int **firstneighi2ia_index_half = listhalf->firstneighi2ia_index;

  int *numneigha2a_full = listfull->numneigha2a;
  int *numneigha2ia_full = listfull->numneigha2ia;
  int *numneighi2a_full = listfull->numneighi2a;
  int *numneighi2ia_full = listfull->numneighi2ia;

  int **firstneigha2a_full = listfull->firstneigha2a;
  int **firstneigha2ia_full = listfull->firstneigha2ia;
  int **firstneigha2ia_index_full = listfull->firstneigha2ia_index;
  int **firstneighi2a_full = listfull->firstneighi2a;
  int **firstneighi2ia_full = listfull->firstneighi2ia;
  int **firstneighi2ia_index_full = listfull->firstneighi2ia_index;

  //int *numneigha2a_outer = listfull->numneigha2a_outer;
  //int *numneigha2ia_outer = listfull->numneigha2ia_outer;
  //int *numneighi2a_outer = listfull->numneighi2a_outer;
  //int *numneighi2ia_outer = listfull->numneighi2ia_outer;

  //int **firstneigha2a_outer = listfull->firstneigha2a_outer;
  //int **firstneigha2ia_outer = listfull->firstneigha2ia_outer;
  //int **firstneigha2ia_index_outer = listfull->firstneigha2ia_index_outer;
  //int **firstneighi2a_outer = listfull->firstneighi2a_outer;
  //int **firstneighi2ia_outer = listfull->firstneighi2ia_outer;
  //int **firstneighi2ia_index_outer = listfull->firstneighi2ia_index_outer;

  int **firstneighnia2a = listfull->firstneighnia2a;
  int **firstneighnia2ia = listfull->firstneighnia2ia;
  int **firstneighnia2ia_index = listfull->firstneighnia2ia_index;
  int *numneighnia2a = listfull->numneighnia2a;
  int *numneighnia2ia = listfull->numneighnia2ia;
  int **ia2nialist = listfull->ia2nialist;
  ...
  for (ii = 0; ii < ainum_half; ii++) 
    n += numneigha2a_half[ailist_half[ii]] + numneigha2ia_full[ailist_half[ii]];
  for (ii = 0; ii < einum_half; ii++) {
    i = eilist_half[ii];
    ietype = etype[i];
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      inode = i2n[ietype][iintg];
      iintg_local = e2ilist_half[ii] + iintg;
      n += numneighi2ia_half[iintg_local];
    }
  }   
  ...
  //            for interpolated atoms, simply interpolate the value from nodes
  ...
  for (ii = 0; ii < ainum_half; ii++) {
    i = ailist_half[ii];
    meam_inst->meam_dens_init(i,-1,-1,map,
        numneigha2a_half[i],firstneigha2a_half[i], numneigha2a_full[i],firstneigha2a_full[i],
        numneigha2ia_full[i],firstneigha2ia_full[i],firstneigha2ia_index_full[i],
        numneigha2ia_full[i],firstneigha2ia_full[i],firstneigha2ia_index_full[i],
        offset);
    offset += numneigha2a_half[i] + numneigha2ia_full[i];
  }
  ...
  for (ii = 0; ii < einum_half; ii++) {
    ...
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      inode = i2n[ietype][iintg];
      iintg_local = e2ilist_half[ii] + iintg;
      meam_inst->meam_dens_init(i,iintg,inode,map,
          0,NULL,numneighi2a_full[iintg_local],firstneighi2a_full[iintg_local],
          numneighi2ia_half[iintg_local],firstneighi2ia_half[iintg_local],
          firstneighi2ia_index_half[iintg_local],
          numneighi2ia_full[iintg_local],firstneighi2ia_full[iintg_local],
          firstneighi2ia_index_full[iintg_local],offset);
      offset += numneighi2ia_half[iintg_local];
    }
  ...
  for (ii = 0; ii < ainum_half; ii++) {
    i = ailist_half[ii];
    meam_inst->meam_force(i,-1,-1,eflag_either,eflag_global,eflag_atom,
        vflag_either,vflag_global,vflag_atom,virial,&eng_vdwl,eatom,enode,map,scale,
        numneigha2a_half[i],firstneigha2a_half[i], numneigha2a_full[i],firstneigha2a_full[i],
        numneigha2ia_full[i],firstneigha2ia_full[i],firstneigha2ia_index_full[i],
        numneigha2ia_full[i],firstneigha2ia_full[i],firstneigha2ia_index_full[i],
        numneighnia2ia,firstneighnia2ia,firstneighnia2ia_index,
        numneighnia2a,firstneighnia2a,ia2nialist,
        offset,vatomptr,vnodeptr,intgf);
    offset += numneigha2a_half[i] + numneigha2ia_full[i];
  }
  ...
  for (ii = 0; ii < einum_half; ii++) {
    ...
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      ...
      inode = i2n[ietype][iintg];
      iintg_local = e2ilist_half[ii] + iintg;
      meam_inst->meam_force(i,iintg,inode,eflag_either,eflag_global,eflag_atom,
          vflag_either,vflag_global,vflag_atom,virial,&eng_vdwl,eatom,enode,map,scale,
          0,NULL,numneighi2a_full[iintg_local],firstneighi2a_full[iintg_local],
          numneighi2ia_half[iintg_local],firstneighi2ia_half[iintg_local], firstneighi2ia_index_half[iintg_local],
          numneighi2ia_full[iintg_local],firstneighi2ia_full[iintg_local], firstneighi2ia_index_full[iintg_local],
          numneighnia2ia,firstneighnia2ia,firstneighnia2ia_index,
          numneighnia2a,firstneighnia2a,ia2nialist,
          offset,vatomptr,vnodeptr,intgf);
      offset += numneighi2ia_half[iintg_local];
  ...
  // compute nodal forces (nodef) from force columns (intgf)
  ...
  for (i = 0; i < neall; i++) {
    ...
    for (iintg = 0; iintg < nintg[ietype]; iintg++) {
      for (node = 0; node < npe[ietype]; node++) {
        force_columns[node][0] += weighted_shape_array[ietype][iintg][node]*intgf[i][iintg][0];
        force_columns[node][1] += weighted_shape_array[ietype][iintg][node]*intgf[i][iintg][1];
        force_columns[node][2] += weighted_shape_array[ietype][iintg][node]*intgf[i][iintg][2];
      }
    }
...
void PairMEAM::allocate()
{
  ...
  memory->create(innercutsq,n+1,n+1,"pair:cutsq");
    ...
    int /*pbc_flag*/, int * /*pbc*/)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    for (l = 0; l < npe[etype[j]]; l++) {
      buf[m++] = meam_inst->noderho0[j][l];
      buf[m++] = meam_inst->noderho1[j][l];
      buf[m++] = meam_inst->noderho2[j][l];
      buf[m++] = meam_inst->noderho3[j][l];
      buf[m++] = meam_inst->nodefrhop[j][l];
      buf[m++] = meam_inst->nodegamma[j][l];
      buf[m++] = meam_inst->nodedgamma1[j][l];
      buf[m++] = meam_inst->nodedgamma2[j][l];
      buf[m++] = meam_inst->nodedgamma3[j][l];
      buf[m++] = meam_inst->nodearho2b[j][l];
      for (k = 0; k < 3; k++) {
        buf[m++] = meam_inst->nodearho1[j][l][k];
        buf[m++] = meam_inst->nodearho3b[j][l][k];
        buf[m++] = meam_inst->nodet_ave[j][l][k];
        buf[m++] = meam_inst->nodetsq_ave[j][l][k];
      }
      for (k = 0; k < 6; k++) buf[m++] = meam_inst->nodearho2[j][l][k];
      for (k = 0; k < 10; k++) buf[m++] = meam_inst->nodearho3[j][l][k];
    }
  }
...
void PairMEAM::unpack_elem_forward_comm(int n, int first, double *buf)
{
  ...
  for (i = first; i < last; i++) {
    for (j = 0; j < npe[etype[i]]; j++) {
      meam_inst->noderho0[i][j] = buf[m++];
      meam_inst->noderho1[i][j] = buf[m++];
      meam_inst->noderho2[i][j] = buf[m++];
      meam_inst->noderho3[i][j] = buf[m++];
      meam_inst->nodefrhop[i][j] = buf[m++];
      meam_inst->nodegamma[i][j] = buf[m++];
      meam_inst->nodedgamma1[i][j] = buf[m++];
      meam_inst->nodedgamma2[i][j] = buf[m++];
      meam_inst->nodedgamma3[i][j] = buf[m++];
      meam_inst->nodearho2b[i][j] = buf[m++];
      for (k = 0; k < 3; k++) {
        meam_inst->nodearho1[i][j][k] = buf[m++];
        meam_inst->nodearho3b[i][j][k] = buf[m++];
        meam_inst->nodet_ave[i][j][k] = buf[m++];
        meam_inst->nodetsq_ave[i][j][k] = buf[m++];
      }
      for (k = 0; k < 6; k++) meam_inst->nodearho2[i][j][k] = buf[m++];
      for (k = 0; k < 10; k++) meam_inst->nodearho3[i][j][k] = buf[m++];
    }
  }
}
...
int PairMEAM::pack_elem_reverse_comm(int n, int first, double *buf)
{
  ...
  for (i = first; i < last; i++) {
    for (j = 0; j < npe[etype[i]]; j++) {
      buf[m++] = meam_inst->noderho0[i][j];
      buf[m++] = meam_inst->nodearho2b[i][j];
      for (k = 0; k < 3; k++) {
        buf[m++] = meam_inst->nodearho1[i][j][k];
        buf[m++] = meam_inst->nodearho3b[i][j][k];
        buf[m++] = meam_inst->nodet_ave[i][j][k];
        buf[m++] = meam_inst->nodetsq_ave[i][j][k];
      }
      for (k = 0; k < 6; k++) buf[m++] = meam_inst->nodearho2[i][j][k];
      for (k = 0; k < 10; k++) buf[m++] = meam_inst->nodearho3[i][j][k];
    }
  }
  return m;
}
...
void PairMEAM::unpack_elem_reverse_comm(int n, int *list, double *buf)
{
  ...
  for (i = 0; i < n; i++) {
    j = list[i];
    for (l = 0; l < npe[etype[j]]; l++) {
      meam_inst->noderho0[j][l] += buf[m++];
      meam_inst->nodearho2b[j][l] += buf[m++];
      for (k = 0; k < 3; k++) {
        meam_inst->nodearho1[j][l][k] += buf[m++];
        meam_inst->nodearho3b[j][l][k] += buf[m++];
        meam_inst->nodet_ave[j][l][k] += buf[m++];
        meam_inst->nodetsq_ave[j][l][k] += buf[m++];
      }
      for (k = 0; k < 6; k++) meam_inst->nodearho2[j][l][k] += buf[m++];
      for (k = 0; k < 10; k++) meam_inst->nodearho3[j][l][k] += buf[m++];
    }
  }
}
...
{
  double bytes = 11 * meam_inst->namax * sizeof(double);
  bytes += (3 + 6 + 10 + 3 + 3 + 3) * meam_inst->namax * sizeof(double);
