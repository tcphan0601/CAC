...
Pair::~Pair()
{
  ...
  if (save_intg_force) memory->destroy(intgf);
}
...
void Pair::init()
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (double_neighbor_layers) {
        innercut = init_one_inner(i,j);
        innercutsq[i][j] = innercutsq[j][i] = innercut*innercut;
        cutforce = MAX(cutforce,innercut);
      } else cutforce = MAX(cutforce,cut);
      if (tail_flag) {
        etail += etail_ij;
        ptail += ptail_ij;
        if (i != j) {
