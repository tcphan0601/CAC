...
void FixMove::init()
{
  ...
  if (displaceflag) memory->create(nodedisplace,maxelem,8,3,"move:nodedisplace");
  else nodedisplace = NULL;
  if (velocityflag) memory->create(nodevelocity,maxelem,8,3,"move:nodevelocity");
  ...
  if (mstyle == LINEAR) {
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < npe[etype[i]]; j++) {
        if (element_group_style == ELEMENT) {
          ...
          if (!(nodemask[i][j] & groupbit)) continue;
        }
        ...
        if (vxflag) {
          nodev[i][j][0] = vx;
          nodex[i][j][0] = nodexoriginal[i][j][0] + vx*delta;
        } else {
          dtfm = dtf / mass[ctype[i]];
        ...
        if (vyflag) {
          nodev[i][j][1] = vy;
          nodex[i][j][1] = nodexoriginal[i][j][1] + vy*delta;
        } else {
          dtfm = dtf / mass[ctype[i]];
        ...
        if (vzflag) {
          nodev[i][j][2] = vz;
          nodex[i][j][2] = nodexoriginal[i][j][2] + vz*delta;
        } else {
          dtfm = dtf / mass[ctype[i]];
        ...
        domain->remap_near(nodex[i][j],xold);
      }
    }
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < npe[etype[i]]; j++) {
        ...
        if (axflag) {
          nodev[i][j][0] = ax*omega_rotate*cosine;
          nodex[i][j][0] = nodexoriginal[i][j][0] + ax*sine;
        } else {
          dtfm = dtf / mass[ctype[i]];
          nodev[i][j][0] += dtfm * nodef[i][j][0];
          nodex[i][j][0] += dtv * nodev[i][j][0];
        ...
        if (ayflag) {
          nodev[i][j][1] = ay*omega_rotate*cosine;
          nodex[i][j][1] = nodexoriginal[i][j][1] + ay*sine;
        } else {
          dtfm = dtf / mass[ctype[i]];
          nodev[i][j][1] += dtfm * nodef[i][j][1];
          nodex[i][j][1] += dtv * nodev[i][j][1];
        ...
        if (azflag) {
          nodev[i][j][2] = az*omega_rotate*cosine;
          nodex[i][j][2] = nodexoriginal[i][j][2] + az*sine;
        } else {
          dtfm = dtf / mass[ctype[i]];
          nodev[i][j][2] += dtfm * nodef[i][j][2];
          nodex[i][j][2] += dtv * nodev[i][j][2];
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < npe[etype[i]]; j++) {
        ...
        d[0] = nodexoriginal[i][j][0] - point[0];
        d[1] = nodexoriginal[i][j][1] - point[1];
        d[2] = nodexoriginal[i][j][2] - point[2];
        ddotr = d[0]*runit[0] + d[1]*runit[1] + d[2]*runit[2];
        c[0] = ddotr*runit[0];
        c[1] = ddotr*runit[1];
        c[2] = ddotr*runit[2];
        ...
        nodex[i][j][0] = point[0] + c[0] + disp[0];
        nodex[i][j][1] = point[1] + c[1] + disp[1];
        nodex[i][j][2] = point[2] + c[2] + disp[2];
        nodev[i][j][0] = omega_rotate * (runit[1]*disp[2] - runit[2]*disp[1]);
        nodev[i][j][1] = omega_rotate * (runit[2]*disp[0] - runit[0]*disp[2]);
        nodev[i][j][2] = omega_rotate * (runit[0]*disp[1] - runit[1]*disp[0]);
        domain->remap_near(nodex[i][j],xold);
      }
    }
    ...
    if ((displaceflag || velocityflag)) {
      ...
      if (element->nmax > maxelem) {
        ...
        if (displaceflag) {
          memory->destroy(nodedisplace);
          memory->create(nodedisplace,maxelem,8,3,"move:nodedisplace");
        }
        if (velocityflag) {
          memory->destroy(nodevelocity);
          memory->create(nodevelocity,maxelem,8,3,"move:nodevelocity");
        }
      }
    }
    ...
    for (int i = 0; i < nelocal; i++) {
      for (int j = 0; j < npe[etype[i]]; j++) {
        ...
        if (xvarstr && vxvarstr) {
          if (vxvarstyle == EQUAL) nodev[i][j][0] = vx;
          else nodev[i][j][0] = nodevelocity[i][j][0];
          if (xvarstyle == EQUAL) nodex[i][j][0] = nodexoriginal[i][j][0] + dx;
          else nodex[i][j][0] = nodexoriginal[i][j][0] + nodedisplace[i][j][0];
        } else if (xvarstr) {
          if (xvarstyle == EQUAL) nodex[i][j][0] = nodexoriginal[i][j][0] + dx;
          else nodex[i][j][0] = nodexoriginal[i][j][0] + nodedisplace[i][j][0];
        } else if (vxvarstr) {
        ...
        if (yvarstr && vyvarstr) {
          if (vyvarstyle == EQUAL) nodev[i][j][1] = vy;
          else nodev[i][j][1] = nodevelocity[i][j][1];
          if (yvarstyle == EQUAL) nodex[i][j][1] = nodexoriginal[i][j][1] + dy;
          else nodex[i][j][1] = nodexoriginal[i][j][1] + nodedisplace[i][j][1];
        } else if (yvarstr) {
          if (yvarstyle == EQUAL) nodex[i][j][1] = nodexoriginal[i][j][1] + dy;
          else nodex[i][j][1] = nodexoriginal[i][j][1] + nodedisplace[i][j][1];
        } else if (vyvarstr) {
        ...
        if (zvarstr && vzvarstr) {
          if (vzvarstyle == EQUAL) nodev[i][j][2] = vz;
          else nodev[i][j][2] = nodevelocity[i][j][2];
          if (zvarstyle == EQUAL) nodex[i][j][2] = nodexoriginal[i][j][2] + dz;
          else nodex[i][j][2] = nodexoriginal[i][j][2] + nodedisplace[i][j][2];
        } else if (zvarstr) {
          if (zvarstyle == EQUAL) nodex[i][j][2] = nodexoriginal[i][j][2] + dz;
          else nodex[i][j][2] = nodexoriginal[i][j][2] + nodedisplace[i][j][2];
        } else if (vzvarstr) {
...
{
  double bytes = (atom->nmax*3 * sizeof(double) + element->nmax*8*3 * sizeof(double))
...
{
  memory->grow(nodexoriginal,nmax,8,3,"move:nodexoriginal");
...
{
  for (int k = 0; k < 8; k++) {
    nodexoriginal[j][k][0] = nodexoriginal[i][k][0];
    nodexoriginal[j][k][1] = nodexoriginal[i][k][1];
    nodexoriginal[j][k][2] = nodexoriginal[i][k][2];
  }
}
...
void FixMove::set_elem_arrays(int i)
{
  ...
  for (int j = 0; j < npe[etype[i]]; j++) {
    ...
    if (update->ntimestep == time_origin) {
      domain->unmap(nodex[i][j],image[i],nodexoriginal[i][j]);
      continue;
    }
    ...
    if (mstyle == LINEAR) {
      if (vxflag) nodexoriginal[i][j][0] -= vx * delta;
      if (vyflag) nodexoriginal[i][j][1] -= vy * delta;
      if (vzflag) nodexoriginal[i][j][2] -= vz * delta;
    } else if (mstyle == WIGGLE) {
      double arg = omega_rotate * delta;
      double sine = sin(arg);
      if (axflag) nodexoriginal[i][j][0] -= ax*sine;
      if (ayflag) nodexoriginal[i][j][1] -= ay*sine;
      if (azflag) nodexoriginal[i][j][2] -= az*sine;
    } else if (mstyle == ROTATE) {
      double a[3],b[3],c[3],d[3],disp[3],ddotr;
      double arg = - omega_rotate * delta;
      double sine = sin(arg);
      double cosine = cos(arg);
      d[0] = nodex[i][j][0] - point[0];
      d[1] = nodex[i][j][1] - point[1];
      d[2] = nodex[i][j][2] - point[2];
      ddotr = d[0]*runit[0] + d[1]*runit[1] + d[2]*runit[2];
      c[0] = ddotr*runit[0];
      c[1] = ddotr*runit[1];
      c[2] = ddotr*runit[2];
      ...
      nodexoriginal[i][j][0] = point[0] + c[0] + disp[0];
      nodexoriginal[i][j][1] = point[1] + c[1] + disp[1];
      nodexoriginal[i][j][2] = point[2] + c[2] + disp[2];
    }
  }
}
...
int FixMove::pack_elem_exchange(int i, double *buf)
{
  ...
  for (int j = 0; j < element->npe[element->etype[i]]; j++) {
    buf[n++] = nodexoriginal[i][j][0];
    buf[n++] = nodexoriginal[i][j][1];
    buf[n++] = nodexoriginal[i][j][2];
  }
  return n;
}
...
{
  int n = 0;
  for (int j = 0; j < element->npe[element->etype[nlocal]]; j++) {
    nodexoriginal[nlocal][j][0] = buf[n++];
    nodexoriginal[nlocal][j][1] = buf[n++];
    nodexoriginal[nlocal][j][2] = buf[n++];
  }
  return n;
}
