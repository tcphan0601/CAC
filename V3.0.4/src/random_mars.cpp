...
RanMars::RanMars(CAC *cac, int seed) : Pointers(cac), 
  u(NULL)
{
...
void RanMars::select_subset(bigint ntarget, int nmine, int *mark, int *next)
{
  ...
  bigint bnmine = nmine;
  bigint bnall;
  MPI_Allreduce(&bnmine, &bnall, 1, MPI_CAC_BIGINT, MPI_SUM, world);
  activeall[0] = bnall;

  for (int i = 0; i < nmine; i++) mark[i] = 0;
  for (int i = 0; i < nmine; i++) next[i] = i+1;
  if (nmine) next[nmine-1] = -1;
  else {
    mark[0] = 0;
    next[0] = -1;
  }
  ...
  while (nmark != ntarget) {
    ...
    bsum[1] = active[0];
    bsum[2] = active[1];
    MPI_Allreduce(&bsum, &bsumall, 3, MPI_CAC_BIGINT, MPI_SUM, world);
    nflipall = bsumall[0];
